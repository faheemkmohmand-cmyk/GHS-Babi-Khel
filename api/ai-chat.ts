...
- ALWAYS answer as a short list of bullet points using "- " at the start of each line. Never write a plain, unbroken paragraph — even a one-fact answer should be 1–3 bullets, not a sentence blob.
- Keep each bullet SHORT: one idea per line, ideally under 15 words.
- If the answer naturally has a lead-in sentence, keep it to a single short line, then break the rest into bullets.
- Add a single relevant emoji at the start of key bullets (📚 🎓 📅 ✅ 🏆 💳 📞) — but don't overuse emojis (max ~3 per answer).
- When the visitor's question maps to a specific page, end your answer with one short bullet like: "👉 You can do this on the Results page (/results)." — give the path so they can navigate.
...
- Keep the total answer concise — typically 3 to 6 bullets. Don't write essays and don't write long paragraphs.`;
...
// mode:"notes"). The student is reading a SPECIFIC chapter and needs
// chapter-aware help: explain a concept, summarize key points, give practice
// questions, walk through an example. The chapter title, subject name, and a
// short plain-text snippet of the chapter content are injected here so the
// model can answer accurately without the student having to copy-paste the
// chapter into the chat.
//
// The snippet is capped at SNIPPET_MAX_CHARS_CHARS on the server (in addition
// to the client-side cap in NoteAiAssistant) so a malicious or buggy client
// can't bloat the prompt with megabytes of text.
...
export default async function handler(req: any, res: any) {
  ...
  //   - "notes"   → chapter-aware study buddy prompt (built fresh per request
  //                 using subject + chapterTitle + chapterSnippet sent by the
  //                 NoteAiAssistant panel)
  //   - anything else → existing homepage assistant prompt (default, fully
  //                 backwards-compatible with the original /api/ai-chat callers)
  const systemPrompt =
    mode === "notes"
  ...
  const payload = {
    ...
    // GLM-4.5-Flash supports an internal "thinking" reasoning pass before
    // it starts writing the visible answer. That pass is hidden from the
    // SSE token stream, so from the browser's point of view it just looks
    // like a long stall before the first token arrives (the "..." dots
    // sit there for several seconds). We don't want or need that reasoning
    // pass for short website Q&A, so we explicitly turn it off — this
    // makes the first visible token arrive much faster.
  ...
  try {
    ...
    // ── Pipe upstream SSE → client SSE, CHARACTER BY CHARACTER ──────────────
    // Per site-owner request (2026-07-22, revised): forward every raw delta
    // the moment it arrives from Z.AI — NO buffering, NO waiting for a word
    // boundary. Z.AI's GLM-4.5-Flash typically streams a few characters per
    // delta chunk, so forwarding immediately gives the closest thing to true
    // letter-by-letter typing in the browser (like DeepSeek). If a delta
    // chunk itself contains multiple characters, the frontend is responsible
    // for revealing them one at a time on a short interval (see
    // AIAssistantWidget.tsx) so the animation never looks chunky even if a
    // single upstream delta contains a whole word or more.
