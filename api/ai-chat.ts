...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // ── Pipe upstream SSE → client SSE, WORD BY WORD ────────────────────────
    // The model's stream often emits partial-word tokens (e.g. "Hel", "lo",
    // " wor", "ld") — which made the chat bubble show half-words flickering
    // as the text grew. Per site-owner request (2026-07-22), we now BUFFER
    // the upstream tokens and only FLUSH a chunk to the client when we hit
    // a word boundary (whitespace or punctuation). This way each visible
    // update in the browser is a COMPLETE word, never a half-word.
    //
    //   upstream tokens →  wordBuf  →  flush on word boundary  →  client
    //   "Hel" "lo"       →  "Hello" →  flush "Hello "          →  client sees "Hello"
    //   " wor" "ld"      →  "world" →  flush "world"           →  client sees "world"
    //
    // The newline character is also a flush trigger, so the start of a new
    // line is always emitted immediately (lets the bubble grow line-by-line
    // instead of waiting for the next word).
    const reader = upstream.body.getReader();
    const decoder = new TextDecoder("utf-8");
    let wordBuf = "";

    // Flush complete words from `wordBuf`. A "complete word" is any run of
    // non-whitespace characters followed by either whitespace or end-of-buffer
    // at a natural boundary. We keep the trailing whitespace attached to the
    // word so the client doesn't have to re-add spaces.
    const flushWords = (force: boolean) => {
      // If forcing (stream ended), flush everything that's left as one final
      // word.
      if (force) {
        if (wordBuf) {
          send({ token: wordBuf });
      ...
      // Walk through wordBuf looking for word boundaries. A boundary is the
      // position immediately AFTER a whitespace or punctuation character.
      // We emit everything up to and including the boundary as one chunk,
      // leaving the rest in wordBuf for next time.
      while (wordBuf.length > 0) {
        // Find the next whitespace or punctuation that ends a word.
        // Match a run of non-ws chars followed by either ws or punctuation,
        // or a standalone whitespace run.
        const m = wordBuf.match(/^(\S+?[\s.,!?;:。，、！？：；]+|\s+|\S+)/);
        if (!m) {
          // No match — shouldn't happen, but flush everything to be safe.
          send({ token: wordBuf });
          wordBuf = "";
          return;
        }
        const chunk = m[1];
        // Only flush if the chunk represents a COMPLETE word (i.e. it ends
        // with whitespace or punctuation). If it's a bare non-ws run with
        // no trailing boundary, keep it in the buffer and wait for more
        // upstream tokens.
        if (/[\s.,!?;:。，、！？：；]$/.test(chunk)) {
          send({ token: chunk });
          wordBuf = wordBuf.slice(chunk.length);
        } else {
          // Bare word with no trailing boundary — wait for more tokens.
          return;
        }
      }
    };
    ...
    while (true) {
      ...
      while ((idx = lineBuf.indexOf("\n\n")) !== -1) {
        ...
        if (dataStr === "[DONE]") {
          // Flush any remaining buffered word before closing.
          flushWords(true);
          send({ done: true });
          return res.end();
        }
    ...
    // Stream ended without explicit [DONE] — flush any remaining buffered
    // word and close gracefully.
    flushWords(true);
    send({ done: true });
    return res.end();
  } catch (err: any) {
