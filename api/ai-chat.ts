...
//
//   Everything is bounded by a global budget (GLOBAL_BUDGET_MS) that stays
//   under the function's maxDuration (60s, set below) and under the
...
// Allow this serverless function to run long enough for the retry budget.
...
export default async function handler(req: any, res: any) {
  ...
  // Returns false once the client is gone, so the attempt loop stops
  // burning retries for a visitor who already left.
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (remaining < MIN_ATTEMPT_RESERVE_MS) {
      console.warn(`ai-chat: stopping before attempt ${attempt + 1}, only ${remaining}ms left in budget`);
      break;
    }
    ...
    try {
      ...
      if (clientClosed) {
        // Client gone — stop everything, no point retrying.
        return res.end();
      }
      continue;
    }
