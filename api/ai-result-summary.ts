...
- IDENTIFY EACH FAILED SUBJECT BY NAME. If one or more individual subjects are below the pass mark for their own total (even if the overall result is a pass), name that subject specifically in line 3 as a focus area — do not skip over it or generalize it away. It is fine and expected to say a specific subject/paper was failed (e.g. "Math-I was not cleared this time") — only avoid labelling the STUDENT as "a failure" as a person.
- If the student's OVERALL result did not pass (is_pass = false / percentage below the pass mark), still be encouraging in tone — but be factually direct that they did not pass, and name the specific subjects that pulled the result down. Give 2 lines of concrete next-step advice + 1 motivational close.
  ...
  if (r.subject_marks && typeof r.subject_marks === "object") {
    for (const [sub, m] of Object.entries(r.subject_marks)) {
      if (m && typeof m === "object" && typeof m.obtained === "number" && typeof m.total === "number") {
        ...
        const status = m.obtained < passMark ? "FAILED (below pass mark)" : "passed";
