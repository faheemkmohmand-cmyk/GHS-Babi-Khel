...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Same word-buffering logic as api/ai-chat.ts: collect upstream tokens
    // into `wordBuf`, flush only at word boundaries so each visible update
    // in the browser is a COMPLETE word (no half-words flickering).
    ...
    // Stream ended without explicit [DONE] — flush any remaining word.
    flushWords(true);
    send({ done: true });
    return res.end();
  } catch (err: any) {
