...
const BROWSER_HEADERS = [
  ...
  "--http2",                // negotiate HTTP/2 — faster against Cloudflare
                             // (multiplexed, usually quicker TLS handshake)
                             // than curl's HTTP/1.1 default; falls back
...
export default async function handler(req, res) {
  ...
  // ── Caching for FOUND results only ──────────────────────────────────
  // A published board result never changes once announced, so re-scraping
  // BISE live on every repeat lookup (e.g. a student refreshing, or the
  // Grand Reveal / Scratch & Shine stages both rendering the same roll)
  // only adds latency for no benefit. Cache successful lookups at the
  // edge for 24h with stale-while-revalidate so a repeat hit is instant
  // instead of a fresh curl round-trip. "Not found" and error responses
  // stay `no-store` above — a result can be published seconds later, so
  // those must always go live.
