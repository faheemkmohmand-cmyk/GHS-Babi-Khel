...
// Numeric fallback for fail detection — used only when BISE's HTML gives
// no color/class signal for a cell. BISE's theory papers generally require
// 33% to pass; without knowing each subject's total marks from this table
// alone, we treat a raw theory value under 33 as a conservative fail signal
// (matches your sample: STAT-T-I theory 25/? and M-I theory 13/? were both
// shown red by BISE). This never overrides an actual red-styled cell — it
// only fills in when style detection finds nothing.
function isFailingMark(rawValue) {
  const n = parseFloat(String(rawValue).trim());
...
function parseBiseHtml(html, roll) {
  ...
  if (subjectsBlockMatch) {
    ...
    while ((m = rowRe.exec(tbody)) !== null) {
      ...
      const theoryFail = FAIL_STYLE_RE.test(theoryAttrs) || isFailingMark(theory);
