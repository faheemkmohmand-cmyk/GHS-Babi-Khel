...
async function scrapeBisepCurrentExam() {
  ...
  try {
    ...
    const subMatch = html.match(/<div\s+class="sub-header"[^>]*>\s*([\s\S]*?)\s*<\/div>/i);
    ...
    if (liveMatch) {
      const rawTitle = liveMatch[1].trim();
    ...
    // Pending state — best-effort
    const examNameMatch = raw.match(/\b(SSC|HSSC)\s+ANNUAL-(I|II)(?:\s+EXAMINATION)?\s+(\d{4})\b/i);
    const examTitle = examNameMatch
      ? `${examNameMatch[1].toUpperCase()} Annual-${examNameMatch[2].toUpperCase()} ${examNameMatch[3]}`
      : null;

    const pendingDateMatch = raw.match(
      /(?:ANNOUNCED\s+ON|EXPECTED\s+(?:ON|BY)|DECLARED\s+ON|PUBLISHED\s+ON)\s*:?\s*([0-9]{1,2}(?:st|nd|rd|th)?\s+\w+\s+\d{4})/i
    );
    const pendingMonthMatch = raw.match(
    ...
    return {
      ...
      exam_year: examNameMatch ? parseInt(examNameMatch[3], 10) : null,
