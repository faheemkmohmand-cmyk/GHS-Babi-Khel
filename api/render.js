...
async function fetchMeritLists(limit = 10) {
  ...
  return rows.map((m) => {
    ...
    const entries = Array.isArray(m.entries)
      ? [...m.entries].sort((a, b) => (Number(b.percentage) || 0) - (Number(a.percentage) || 0))
      : [];
    // If the school withheld students at publish time ("Hide top 3" / "Hide
    // top 3 of each class"), the snapshot's best entries are NOT the real
    // toppers and the stored highest_percentage is the best VISIBLE score —
    // exactly why the public page shows a dash for Highest % and never
    // crowns the visible top 3. Mirror that behaviour here so AI consumers
    // are never told the wrong achievers. total_students / passing_count /
    // average_percentage already include the hidden students, so they stay.
    ...
    return {
      ...
      total_students: m.total_students ?? (entries.length || null),
