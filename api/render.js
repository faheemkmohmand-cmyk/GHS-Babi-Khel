          ...
          meritLists
            .map((m) => {
              ...
              const top = (m.top_positions || [])
                ...
                .join("; ");
              return `<article><h3>${esc(m.title)}${m.exam ? ` — ${esc(m.exam)}` : ""}${m.year ? ` ${esc(String(m.year))}` : ""}${m.status === "scheduled" ? " (scheduled)" : ""}</h3>${stats.length ? `<p class="meta">${esc(stats.join(" · "))}</p>` : ""}${top ? `<p>Top positions: ${esc(top)}</p>` : ""}${m.notes ? `<p class="meta">${esc(m.notes)}</p>` : ""}</article>`;
            })
            .join("")
        );
      } else {
