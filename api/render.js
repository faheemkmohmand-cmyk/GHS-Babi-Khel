...
const ROUTE_SECTIONS = {
  "/":                ["school", "admission", "notices", "news", "events"],
...
function liveSections(route, data) {
  const parts = [];
  const { admission, notices, news, events, exams, teachers, library, onlineClasses, gallery, notes, duty, school } = data;
  ...
  switch (true) {
    ...
    case route === "/admission": {
      ...
      parts.push(`<h2>Documents available on the Admission page</h2>
<ul>
...
<p class="meta">These documents are generated on the page itself (open it in a browser and use the download buttons); the online application form and the application-status tracker are on the same page.</p>`);
      break;
    }
    case route === "/notices": {
