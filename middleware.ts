...
export const config = {
  runtime: "edge",
  matcher: [
    // Page routes (unchanged) — everything except static assets and most
    // /api/ routes skips this middleware.
    ...
    // SECURITY FIX: /api/bisep-proxy and /api/og were previously exempt
    // from all rate limiting and bot blocking (the negative lookahead
    // above excludes "api/"). Both endpoints shell out to `curl` and make
    // an outbound request to a third party (BISE Peshawar) on every hit,
    // so an unthrottled attacker could cheaply exhaust Vercel function
    // time/cost or get our server IP flagged by BISE's Cloudflare — a
    // resource-exhaustion / DoS vector. Explicitly route these two
    // through the same middleware so the existing 'api' rate limit tier
    // (60 req/min per fingerprint) and bot blocklist apply to them too.
    "/api/bisep-proxy",
    "/api/og",
  ],
};
...
// Known bad bots / scrapers to block (word-boundary matched, not substring)
const BLOCKED_USER_AGENTS = [
  "bot", "spider", "crawler", "scraper", "curl", "wget",
  "python-requests", "httpclient", "go-http",
  "nikto", "sqlmap", "dirbuster", "nmap", "masscan",
  "zgrab", "gobuster", "wfuzz", "hydra",
...
const RATE_LIMITS = {
  ...
  // third-party server (BISE Peshawar) on every request — each hit is
  // far more expensive than a normal API call, so it needs a tighter cap
  // to prevent both Vercel cost/time exhaustion and getting our server
  // IP flagged by BISE's Cloudflare for excessive traffic.
...
function getClientInfo(request: Request): {
  ...
  // Check if should be blocked (word-boundary match to avoid false positives
  // like "bot" matching inside unrelated UA substrings)
