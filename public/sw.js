...
async function networkFirstNavigation(request, event) {
  ...
  // from the precached build assets + IndexedDB data. (Previously this case
  // 302-redirected everything to the homepage, which threw away the user's
  // intended destination even when that page could render perfectly well
  // offline.) offline.html remains the last resort when nothing is cached.
  if (typeof navigator !== "undefined" && navigator.onLine === false) {
  ...
  // ── Online: freshness first, but NEVER hold the user hostage ──────────
  // Race the network against a deadline. Fast network → fresh HTML wins
  // (unchanged behaviour). Slow network → cached copy is served instantly
  // and the network fetch continues in the background to refresh the cache.
  ...
  let res = await Promise.race([networkPromise, deadlinePromise]);

  if (!res) {
    // ── Deadline hit: SLOW network. Serve the best cached copy now. ────
    const cachedCopy =
    ...
    if (cachedCopy) {
      // Background-refresh: when the fresh response finally lands, store it
      // for next time. waitUntil keeps the SW alive until this settles.
      if (event && typeof event.waitUntil === "function") {
        event.waitUntil(
          networkPromise
            .then((fresh) => cacheNavigationResponse(fresh))
            .catch(() => {})
        );
      }
      return cachedCopy;
    }
    // No cached copy of this route yet — all we can do is wait out the
    // network fetch (the browser spinner already shows; this is the honest
    // first-visit case).
    try {
      res = await networkPromise;
    } catch (err) {
      res = null;
      // Network failed AND nothing cached — offline.html is the floor.
      const offlineFallback = await cache.match(OFFLINE_FALLBACK);
      if (offlineFallback) return offlineFallback;
      throw err;
    }
  ...
  if (res && res.ok) {
    ...
    // the background so the response is never delayed by cache writes.
    if (event && typeof event.waitUntil === "function") {
  ...
  // Non-OK response (5xx, offline edge after the race, etc.) — fall back to
  // whatever we have cached, in decreasing order of specificity.
  const cachedCopy =
    (await cache.match(request, { ignoreSearch: true })) ||
    (await cache.match(SHELL_URL));
