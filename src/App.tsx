...
import HujraAmbiance from "./components/shared/HujraAmbiance";
import AcademicUniverse from "./components/background/AcademicUniverse";
...
// Broadcasts the lifecycle of a route-chunk download (start / end) so the
// RouteProgressBar can show a slim YouTube-style top bar during in-app
// navigation. This matters because of v7_startTransition below: since the
// current page STAYS VISIBLE while the next page's chunk downloads (instead
// of being replaced by the white full-screen fallback), the progress bar is
// the visual signal that a navigation is happening — without it, a slow
// chunk would look like the tap "did nothing".
...
function lazyWithRetry(factory: () => Promise<any>) {
  return lazy(() => {
    // Mark the chunk download as in-flight for the progress bar. The events
    // are ref-counted inside RouteProgressBar, so concurrent loads (fast
    // double-taps, redirect chains) never corrupt the bar's state.
...
// ── HARD RECOVERY for the stale-chunk dead end ────────────────────────────
// A service worker that survived a deploy can keep pairing a cached HTML
// shell (or a PARTIALLY-failed asset precache) with hashed files that no
// longer exist on the server. Every reload boots the same poisoned shell,
// hits the same 404s and burns through the reload cap — so the crash panel
// looks permanent. Clearing the browser's "cached images and files" does
// NOT remove a service worker or its Cache Storage, so users stay stuck no
// matter how often they clear/force-stop. This routine removes the actual
// culprit: unregister every service worker + delete every ghs-* cache,
// then reload once. The fresh document loads straight from the network and
// re-registers the worker (src/main.tsx), which re-precache's the CURRENT
// build — offline support returns automatically afterwards.
async function hardServiceWorkerRecovery(): Promise<boolean> {
  try {
    ...
    /* storage blocked — still attempt recovery once per document */
  }
  try {
  ...
  while (Date.now() - startedAt < MAX_RECOVERY_MS) {
    ...
    if (failedUrl) {
      ...
      if (state === "stale") {
        ...
        // in-document recovery exists through ordinary reloads WHEN an old
        // service worker is the thing serving the stale shell — so if a
        // worker controls this page, do ONE hard recovery (unregister +
        // clear its caches + reload) instead of burning the reload cap on
        // reloads that can never succeed. Without a controlling worker,
        // keep the historic behavior: one guarded reload to fetch the
        // fresh index.html with the new hashes. Skipped if the user has
        // already moved to another page (never yank someone mid-task).
        if (location.pathname === intendedPath) {
          if (typeof navigator !== "undefined" && navigator.serviceWorker?.controller) {
            if (await hardServiceWorkerRecovery()) {
              console.warn("[routeChunk] Stale chunk with an active service worker — unregistering worker, clearing ghs-* caches, reloading once");
              window.location.reload();
              await sleep(30_000); // page is reloading; this is just a safety net
              continue;
            }
        ...
        <LazyMotion features={domAnimation} strict>
          ...
          >
            ...
            {/* ⌘K / Ctrl+K command palette — global, works from any route.
                ...
                navigation. */}
            <CommandPalette />
            {/* Lantern Mode · Hujra Reading Experience — renders nothing
                outside /notes* /news* /notices*, and is CSS-gated to the
                lantern theme, so mounting it globally is always safe. */}
            <HujraAmbiance />
            {/* Academic Universe — the ambient 3D math-bubble background
                (WebGL, fixed, z-index -1, behind all content). Self-gating:
                only mounts on public pages, only after idle, skips on
                Data-Saver/2G/no-WebGL/prerender, pauses for open dialogs
                and hidden tabs. See components/background/AcademicUniverse.tsx */}
            <AcademicUniverse />
            <RouteErrorBoundary>
