...
export default function ReportCardModal({ open, onClose }: Props) {
  ...
  // Roll entries — persisted across sessions
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LS_ROLLS_KEY, JSON.stringify(rollEntries));
    } catch {
  ...
  const downloadExcel = () => {
    if (!results) return;
    const stats = computeStats(results);
    generateResultExcel(results, stats, selection);
  };
  ...
  return (
    ...
    >
      <div
        className="bg-card w-full max-w-4xl min-h-screen sm:min-h-0 sm:max-h-[92vh] sm:rounded-2xl shadow-2xl border border-border flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Header ── */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-primary text-primary-foreground sm:rounded-t-2xl sticky top-0 z-10">
          <div className="flex items-center gap-2.5 min-w-0">
            <FileText className="w-5 h-5 shrink-0" />
            <div className="min-w-0">
              <h2 className="font-bold text-base sm:text-lg leading-tight truncate">
                Report Card — Bulk Result Generator
              </h2>
