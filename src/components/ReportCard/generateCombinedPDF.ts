...
// PROBLEM 6 FIX (lazy PDF): jsPDF is imported as a TYPE only here (the
// helpers' `doc: jsPDF` annotations) and as a VALUE via dynamic import
// inside generateCombinedReportPDF — so the ~142 KB PDF library only
// downloads when a user actually generates a combined report.
...
function drawPage1(
  ...
  // ── Combined Pass Rate donut + stats panel ──
  //   Twin ring: outer ring = Class 9th (sky = pass, red = fail portion),
  //   inner ring = Class 10th (violet = pass, red = fail portion).
  ...
  drawDonutRing(doc, donutCx, donutCy, 55, 42, classA.stats.passPercentage, C.classA, C.fail);
  drawDonutRing(doc, donutCx, donutCy, 38, 25, classB.stats.passPercentage, C.classB, C.fail);
  ...
  const legendRows: Array<{ label: string; value: string; sub: string; color: RGB }> = [
    ...
    {
      label: "Failed",
      value: `${100 - combined.passPct}%`,
      sub: `${combined.failed}/${combined.found} failed`,
      color: C.fail,
    },
  ];
...
export async function generateCombinedReportPDF(opts: CombinedPDFOpts): Promise<void> {
  // Lazy-load the PDF library only at download time (Problem 6 fix).
