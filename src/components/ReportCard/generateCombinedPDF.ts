 ...
 *  `color`; the rest stays as a soft gray background. */
...
function drawPage1(
  ...
  for (let i = 0; i < legendRows.length; i++) {
    ...
    // Value (right-aligned at end of panel)
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    setText(doc, r.color);
    doc.text(r.value, MARGIN + (PAGE_W - MARGIN * 2) - 100, ry + 3);
    // Sub-text (right of value)
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.5);
    setText(doc, C.muted);
    doc.text(r.sub, MARGIN + (PAGE_W - MARGIN * 2) - 92, ry + 3);
  }
