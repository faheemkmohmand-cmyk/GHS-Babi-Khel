...
function drawPage1(
  ...
  //   Single ring = the actual combined pass/fail split (purple = passed,
  //   red = failed) — i.e. exactly the number shown in the center label
  //   and the panel title. Per-class numbers (9th / 10th) are shown as
  //   plain stat rows on the right, not as extra rings — two unrelated
  //   per-class percentages drawn as concentric rings around a "combined"
  //   center label was misleading (a nearly-full inner ring made the 20%
  //   combined failure rate look negligible, and it didn't depict the
  //   combined split at all).
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  setText(doc, C.ink);
  ...
  // Donut geometry — single ring depicting combined pass% (purple) vs
  // fail% (red), r=55→34.
  const donutCx = MARGIN + 80;
  const donutCy = y + donutPanelH / 2;
  drawDonutRing(doc, donutCx, donutCy, 55, 34, combined.passPct, C.ink, C.fail);
