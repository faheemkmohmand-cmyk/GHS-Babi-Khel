...
// PROBLEM 6 FIX (lazy Excel): ExcelJS (~938 KB raw / ~270 KB compressed —
// the single heaviest dependency in the app) is now imported DYNAMICALLY
// inside generateResultExcel. It only downloads when a user actually clicks
// the Excel download button. Previously the static import dragged it into
// the Results route chunk, and the app's idle route-prefetcher made every
// homepage visitor download it in the background.
...
export async function generateResultExcel(
  ...
  // Lazy-load ExcelJS only at download time (Problem 6 fix).
