...
import { getMaxMarksForSubjects } from "./subjectMaxMarks";
...
export async function generateResultExcel(
  ...
  header.push("Total Marks", "Max Marks", "Percentage", "Grade", "Status", "Failed Papers");
  ...
  // Column widths
  const colWidths: number[] = [10, 20, 20];
  for (let i = 0; i < subjectCols.length; i++) colWidths.push(9, 9);
  colWidths.push(10, 10, 11, 8, 14, 34);
  ws2.columns = colWidths.map((w) => ({ width: w }));

  let dataRowIdx = 0;
  for (const r of results) {
    let rowValues: (string | number)[];
    // Max marks computed from THIS student's actual subjects (75 marks each,
    // except Quran / Islamiyat / Pak Study which are 50) — not a flat
    // 100-per-subject guess. For the standard 9-subject exam this is 600.
    const maxMarks = r.found
      ? getMaxMarksForSubjects(r.subjects.map((s) => s.name))
      : 0;

    if (!r.found) {
      rowValues = [r.roll, "—", "—"];
      for (let i = 0; i < subjectCols.length * 2; i++) rowValues.push("—");
      rowValues.push("—", "—", "—", "—", "Not Found", r.error || "");
    } else {
      rowValues = [r.roll, r.name || "—", r.fatherName || "—"];
      for (const subj of subjectCols) {
      ...
      const pct = maxMarks > 0 ? Math.round((r.totalMarks / maxMarks) * 1000) / 10 : 0;
      const failedPapers = r.subjects
        .filter((s) => s.isFail)
        .map((s) => {
          const parts: string[] = [];
          if (s.theoryFail) parts.push("Th");
          if (s.practicalFail) parts.push("Pr");
          return `${s.name} (${parts.join("+")})`;
        })
        .join(", ");
      rowValues.push(
        r.totalMarks > 0 ? r.totalMarks : "—",
        maxMarks,
        r.totalMarks > 0 ? `${pct}%` : "—",
        r.grade || (r.isFail ? "F" : "—"),
        r.isFail ? `FAIL (${r.failedSubjectCount} sub)` : "PASS",
        failedPapers || "—"
      );
    }

    const row = ws2.addRow(rowValues);
    const statusColIdx = 3 + subjectCols.length * 2 + 4; // 1-indexed status column
    row.eachCell((cell, colNum) => {
      cell.border = thinBorder;
    ...
    if (statusText === "PASS") {
      statusCell.font = { bold: true, size: 9.5, color: { argb: COLOR_PASS_TEXT } };
      statusCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLOR_PASS_BG } };
    } else if (statusText.startsWith("FAIL") || statusText === "Not Found") {
      statusCell.font = { bold: true, size: 9.5, color: { argb: COLOR_FAIL_TEXT } };
