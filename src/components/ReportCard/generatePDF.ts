...
// are now imported DYNAMICALLY inside generateResultPDF, so they only
// download when a user actually clicks a PDF download button. Previously
// they were statically imported here, which dragged the whole PDF library
// into the Results route chunk — and the app's idle route-prefetcher meant
...
export async function generateResultPDF(
  ...
  // Rank badges for the top 3 ranked students (only those with an actual
  // score — not-found rows never get one). Plain text, not emoji: jsPDF's
  // default "helvetica" font has no glyphs for 🥇🥈🥉, so they were
  // rendering as garbled characters and throwing off the Name column.
  ...
  const rows = rankedResults.map((r) => {
    ...
    const status = r.isFail
      ...
      : "PASS";
    const badge = r.totalMarks > 0 && rankIdx < rankBadges.length ? rankBadges[rankIdx++] : "";
    return [
      r.roll,
      badge ? `${badge}  ${r.name || "—"}` : (r.name || "—"),
      r.fatherName || "—",
