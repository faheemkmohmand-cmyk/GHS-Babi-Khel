...
function sampleTextPoints(
  ...
  const step = Math.max(1.6, Math.round(size / 11));
...
const ParticleManifesto = () => {
  ...
  useEffect(() => {
    ...
    function buildTargetsForCurrentLine(reflow: boolean) {
      ...
      const fontPx = isMobile() ? Math.round(Math.min(w * 0.09, 34)) : Math.round(Math.min(w * 0.058, 72));
      const floorPx = isMobile() ? 15 : 34;
      const pts = sampleTextPoints(LINES[lineIndex], w, h, fontPx, floorPx);

      // Cap particle count for perf; sample down evenly if oversized.
      const CAP = isMobile() ? 2600 : 5200;
      let sampled = pts;
      if (pts.length > CAP) {
