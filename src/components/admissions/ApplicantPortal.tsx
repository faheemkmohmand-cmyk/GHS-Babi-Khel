...
export default function ApplicantPortal() {
  ...
  const refresh = useCallback(async () => {
    if (!session) return;
    try {
      // Re-fetch the admission record by reference_no via track_admission RPC.
      // (We can't re-verify without a code, and we don't need to — the user is
      // already logged in. We just want the latest status + timeline.)
      ...
      if (trackData && trackData.length > 0) {
        // Fetch timeline directly (RLS allows anon read via the lookup RPC,
        // but direct table reads are admin-only — so we gracefully handle
        // an RLS error by just keeping the existing timeline).
        let timelineData = session.timeline;
        try {
          ...
          if (!te && td) timelineData = td as any;
        } catch {
          // RLS blocked the read — keep the existing timeline we got at login.
        }
        setSession({
          ...session,
          admission: { ...session.admission, ...trackData[0] },
