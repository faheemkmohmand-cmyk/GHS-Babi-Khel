...
export default function ApplicantPortal() {
  ...
  // Real booking info, fetched from interview_bookings — this is the
  // source of truth for "has the applicant actually booked a slot?",
  // NOT the note text on the interview_scheduled timeline entry (which
  // may just be a free-text admin note with no real booking behind it).
  ...
  // (Switched from sessionStorage: on mobile browsers, navigating back to
  // this page or backgrounding the tab can create a new session-storage
  // context, wiping the login. localStorage persists across those so the
  // applicant isn't forced to re-register every time they revisit.)
  ...
  // Fetch the applicant's real booking (if any). This is what actually
  // decides whether the slot picker is hidden — not note text.
  const loadBooking = useCallback(async (admissionId: string | undefined) => {
    if (!admissionId) {
      setRealBooking(null);
    ...
    try {
      ...
      if (data && (data as any).interview_slots) {
        ...
        setRealBooking({
          label: `${dateLabel} at ${timeLabel}${slot.location ? " — " + slot.location : ""}`,
        });
      } else {
        setRealBooking(null);
      }
    } catch {
      // If the query fails (e.g. RLS not yet migrated), fail safe by
      // showing the slot picker rather than silently hiding it.
      setRealBooking(null);
    } finally {
      setBookingChecked(true);
    }
  ...
  useEffect(() => {
    loadBooking(session?.admission?.id);
  }, [session?.admission?.id, loadBooking]);
  ...
  // the applicant is directly accepted with nothing further required, so it
  // must not trigger an admit card download.
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      {canBookInterview && bookingChecked && (
        <InterviewSlotBooking
          ...
          onBooked={async () => { await refresh(); await loadBooking(session.admission.id); }}
        />
      )}
