 ...
 * Shows:
 *   - Voucher details (amount, due date, status)
 *   - Payment methods if configured by admin (Bank, EasyPaisa, JazzCash)
 *   - Office submission instructions if no online payment methods available
 *
 * Props:
 *   - admissionId: The admission record ID to look up voucher
 *   - admissionType: 'fresh' or 'migration' to determine which fee type to show
 *   - studentName: For display purposes
 *   - applyingClass: To match with voucher class
 */
...
export default function FeeChallan({ 
  ...
  useEffect(() => {
    const fetchVoucher = async () => {
      setLoading(true);
      try {
        // Look for a voucher that matches this student's class and was auto-generated
        // We search by checking if notes contains the reference number or admission ID
        const { data, error } = await supabasePublic
          ...
          .limit(10);  // Increased limit to find the right one

        if (error) {
          console.error("FeeChallan: Error fetching vouchers:", error);
          throw error;
        }

        console.log("FeeChallan: Fetched vouchers for class", applyingClass, ":", data?.length, "vouchers");
        console.log("FeeChallan: Looking for referenceNo:", referenceNo, "or admissionId:", admissionId);

        // Find the matching voucher (by checking if it's related to this admission)
        // The voucher notes contain the reference_no of the admission
        const matched = (data || []).find((v: VoucherData) => {
          // Primary match: check notes for reference number or admission ID
          const notesMatch = v.notes?.includes(referenceNo) || v.notes?.includes(admissionId);
          
          // Secondary match: check fee_items for admission/migration type
          // This helps when notes don't contain the reference (e.g., manually created)
          const feeTypeMatch = v.fee_items?.some((item: any) => 
            item.fee_type === "admission" || item.fee_type === "migration"
          );
          
          return notesMatch || (feeTypeMatch && v.class === applyingClass);
        });

        if (matched) {
          console.log("FeeChallan: Found matching voucher:", matched.voucher_number);
          setVoucher(matched as VoucherData);
        } else {
          console.log("FeeChallan: No matching voucher found");
        }
      } catch (err) {
        console.error("FeeChallan: Failed to fetch voucher:", err);
      } finally {
        setLoading(false);
      }
    };

    if (admissionId && applyingClass) {
      fetchVoucher();
    } else {
      setLoading(false);
    }
