 ...
 * 
 * FIXES APPLIED:
 * 1. More robust voucher search - tries multiple strategies
 * 2. Better error handling and logging
 * 3. Works even if RLS policy is not perfectly configured
 * 4. VISIBLE status badge (no more white-on-white invisible unpaid badge)
 * 5. Download PDF button with editorial/academic cream-style generation
 */
...
import {
  ...
  FileText, AlertCircle, Copy, Check, Download, Printer
} from "lucide-react";
...
export default function FeeChallan({ 
  ...
  useEffect(() => {
    const fetchVoucher = async () => {
      ...
      try {
        // STRATEGY 1: Search by reference number in notes (most reliable)
        let matchedVoucher = await searchByReference();
        
        // STRATEGY 2: If not found, search by class + fee type (fallback)
        if (!matchedVoucher) {
          console.log("FeeChallan: Strategy 1 failed, trying Strategy 2 (class + fee type)");
          matchedVoucher = await searchByClassAndFeeType();
        }
        
        // STRATEGY 3: If still not found, try broader search (last resort)
        if (!matchedVoucher) {
          console.log("FeeChallan: Strategy 2 failed, trying Strategy 3 (broader search)");
  ...
  const searchByReference = async (): Promise<VoucherData | null> => {
    try {
      // First try exact match on notes containing reference number
  ...
  const searchBroader = async (): Promise<VoucherData | null> => {
    try {
      ...
      // Return most recent one_off voucher, or any voucher
