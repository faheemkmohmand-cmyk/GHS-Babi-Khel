...
function makeSpec(textureCount: number, dnaIndex: number, dnaAvailable: () => boolean, fromBottom = false): BubbleSpec {
  ...
  return {
    ...
    // A touch livelier than before (was 0.25–0.55) so the idle drift
    // reads as gentle motion, not a near-static frame.
    speed: rand(0.34, 0.7),
...
function Bubble({
  ...
  useFrame((state, dt) => {
    ...
    // Gentle idle drift — a slow Lissajous bob, unique per bubble. Slightly
    // more amplitude + rotation speed than earlier revisions so the scene
    ...
    const bobY = Math.sin(t * spec.speed + spec.phase) * 0.46 * w.scale * motion;
    const bobX = Math.cos(t * spec.speed * 0.77 + spec.phase * 1.7) * 0.28 * w.scale * motion;
    ...
    mesh.rotation.x += dt * 0.08 * spec.spinX;
    mesh.rotation.y += dt * 0.1 * spec.spinY;
