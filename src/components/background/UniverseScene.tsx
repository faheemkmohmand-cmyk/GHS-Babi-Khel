...
const bubbleCount = () => (isCoarse() ? 6 : 10);
...
const SHELL_FRAG = /* glsl */ `
  ...
  void main() {
    ...
    // Tighter rim exponent (3.0) concentrates the sheen right at the edge,
    // which reads as clear glass instead of a filled disc.
    ...
    // As the rim builds up, the cream body fades OUT so the edge glows with
    // the blue↔orange iridescence (premium glass) instead of a flat white ring.
    vec3 col = uBody * (1.0 - fres * 0.8) + irid * fres * (1.05 + uDark * 0.6);
    // TRANSPARENT bubbles — alpha comes ONLY from the fresnel rim, so the
    // centre of every bubble is fully see-through (no cream disc over the
    // page); just a slim iridescent edge + the floating formula inside.
    float a = fres * 0.75 * uFade;
...
/** Picks a texture index for a fresh/respawned bubble. DNA (dnaIndex) is
 *  only handed out when `dnaAvailable()` says no other bubble currently
 *  holds it — this is what keeps "a single DNA helix" true across the
 *  whole scene instead of several bubbles independently rolling DNA. */
function pickTexIndex(textureCount: number, dnaIndex: number, dnaAvailable: () => boolean): number {
  let i = Math.floor(rand(0, textureCount));
  // Re-roll (bounded) if we hit DNA while it's already taken elsewhere.
  let guard = 0;
  while (i === dnaIndex && !dnaAvailable() && guard++ < 6) {
    i = Math.floor(rand(0, textureCount));
  }
  return i;
}

function makeSpec(textureCount: number, dnaIndex: number, dnaAvailable: () => boolean, fromBottom = false): BubbleSpec {
  // Edge bias: keep |ux| ≥ 0.5 so the central reading column stays calm —
  // slightly wider than before, which also reduces glyph clumping near
  // the corners on narrow mobile viewports.
  const side = Math.random() < 0.5 ? -1 : 1;
  const ux = side * rand(0.5, 1.04);
  const z = rand(-8, 3);
  ...
  return {
    ...
    // Livelier idle drift (was 0.34–0.7) — the bob now reads as clearly
    // animated motion instead of a near-static frame on mobile.
    speed: rand(0.6, 1.15),
    spinX: Math.random() < 0.5 ? -1 : 1,
    spinY: Math.random() < 0.5 ? -1 : 1,
    texIndex: pickTexIndex(textureCount, dnaIndex, dnaAvailable),
...
function Bubble({
  ...
  /** Scene-wide "who currently holds the DNA glyph" — at most one bubble
   *  id at a time. Checked before handing DNA to a fresh/respawned bubble,
   *  and released when this bubble stops showing it (pop → respawn, or
   *  unmount), so "a single DNA helix" holds for the whole scene, not
   *  just at first paint. */
  dnaHolderRef: React.MutableRefObject<number | null>;
}) {
  ...
  // Claim DNA ownership if this bubble was spawned holding it; release on
  // unmount so a fresh bubble elsewhere is free to pick it up again.
  useEffect(() => {
    if (specRef.current.texIndex === formulas.light.dnaIndex) {
      dnaHolderRef.current = id;
    }
    return () => {
      if (dnaHolderRef.current === id) dnaHolderRef.current = null;
    };
  ...
  const applyWorld = (t: number) => {
    ...
    if (modeRef.current === "popping") {
      ...
      if (p >= 1) {
        ...
        // Release DNA ownership first (if this bubble held it) so the
        // dedup check below can correctly see it as available again.
        if (dnaHolderRef.current === id) dnaHolderRef.current = null;
        specRef.current = makeSpec(
          formulas.light.textures.length,
          formulas.light.dnaIndex,
          () => dnaHolderRef.current === null,
          true,
        );
        const ns = specRef.current;
        if (ns.texIndex === formulas.light.dnaIndex) dnaHolderRef.current = id;
  ...
  useFrame((state, dt) => {
    ...
    // Gentle idle drift — a slow Lissajous bob, unique per bubble. Larger
    // amplitude + faster frequency than earlier revisions so the scene
    // reads as calmly ALIVE rather than a near-static painting on mobile.
    const live = modeRef.current === "live";
    const motion = live ? 1 : 0.4; // keep a whisper of life while entering
    const bobY = Math.sin(t * spec.speed + spec.phase) * 0.6 * w.scale * motion;
    const bobX = Math.cos(t * spec.speed * 0.77 + spec.phase * 1.7) * 0.36 * w.scale * motion;
    ...
    // Continuous gentle RISE — real bubbles float upward, they don't just
    // bob in place. When a bubble drifts past the top of the viewport it
    // respawns below with a fresh glyph (same machinery as the pop-respawn
    // path), so the parade never stops. Live bubbles only — entering ones
    // already float up, popping ones are dissolving.
    if (live) {
      const rise = 0.06 + spec.speed * 0.055; // normalized screen units / s
      specRef.current.uy += rise * dt;
      if (specRef.current.uy > 1.35) {
        if (dnaHolderRef.current === id) dnaHolderRef.current = null;
        const ns = makeSpec(
          formulas.light.textures.length,
          formulas.light.dnaIndex,
    ...
    // Faster tumble + a whisper of scroll-driven rotation.
    mesh.rotation.x += dt * 0.2 * spec.spinX;
    mesh.rotation.y += dt * 0.26 * spec.spinY;
    mesh.rotation.z = scroll * 0.00012 * spec.parallax;
    // Formula sprite spins lazily the other way.
    sprite.material.rotation += dt * 0.09 * -spec.spinY;
  });
...
function SceneContents({ staticMode, paused }: { staticMode: boolean; paused: boolean }) {
  ...
  // Sequential dedup at first spawn too: a local counter mimics
  // dnaHolderRef during the initial batch so at most ONE of the specs
  // built here starts out on the DNA glyph; dnaHolderRef itself is set
  // from the first Bubble's mount effect once rendered.
  const specs = useMemo(() => {
    if (!formulas) return [];
    let dnaTaken = false;
    return Array.from({ length: bubbleCount() }, () => {
      const spec = makeSpec(formulas.light.textures.length, formulas.light.dnaIndex, () => !dnaTaken);
      if (spec.texIndex === formulas.light.dnaIndex) dnaTaken = true;
      return spec;
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formulas]);
  const dnaHolderRef = useRef<number | null>(null);
