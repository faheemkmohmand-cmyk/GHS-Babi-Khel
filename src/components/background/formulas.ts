...
/* ════════════════════════════════════════════════════════════════════════
   ...
   The set: π · ∫ · E=mc² · √x · ∑ · a²+b²=c² · πr² · λ · Δ
           + science glyphs drawn with strokes: an atom, a molecule, a
             chemistry flask, a physics damped-wave trace, a geometry
             medallion (circle/triangle/square) and exactly ONE DNA double
             helix glyph — spawn selection (see UniverseScene) makes sure
             at most one bubble shows DNA at any time, so the background
             never clumps into "DNA everywhere" like earlier revisions.
...
/** Molecule: three atoms joined by bonds — a simple chemistry glyph. */
function drawMolecule(ctx: Ctx, ink: string) {
  const cx = W / 2;
  const cy = H / 2;
...
/** Flask: a simple chemistry beaker/flask outline with a liquid fill line. */
function drawFlask(ctx: Ctx, ink: string) {
  const cx = W / 2;
  const neckTop = H / 2 - 40;
  const neckW = 12;
  const bodyTop = H / 2 - 8;
  const bodyR = 34;
  ctx.strokeStyle = ink;
  ...
  ctx.lineTo(cx - bodyR, bodyTop + bodyR * 1.15);
  ctx.arc(cx, bodyTop + bodyR * 0.55, bodyR, Math.PI * 0.74, Math.PI * 0.26, true);
  ctx.lineTo(cx + neckW, bodyTop);
  ctx.lineTo(cx + neckW, neckTop);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx - neckW - 6, neckTop);
  ctx.lineTo(cx + neckW + 6, neckTop);
  ctx.stroke();
  // Liquid fill — a faint tangerine/azure wash near the base.
  ctx.globalAlpha = 0.35;
  ctx.fillStyle = ink;
  ctx.beginPath();
  ctx.moveTo(cx - bodyR + 6, bodyTop + bodyR * 0.72);
  ctx.arc(cx, bodyTop + bodyR * 0.55, bodyR - 5, Math.PI * 0.62, Math.PI * 0.38, true);
  ctx.closePath();
  ctx.fill();
  ctx.globalAlpha = 1;
  // Rising bubbles inside the flask.
  ctx.fillStyle = ink;
  for (const [dx, dy, r] of [[-8, 14, 3], [6, 20, 2.5], [0, 8, 2]] as const) {
    ctx.beginPath();
    ctx.arc(cx + dx, bodyTop + bodyR * 0.55 + dy, r, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.shadowBlur = 0;
}

/** Pendulum / wave: a simple physics motion glyph — arc + sine trail. */
function drawWave(ctx: Ctx, ink: string) {
  const midY = H / 2;
  ctx.strokeStyle = ink;
  ctx.lineWidth = 3.5;
  ctx.shadowColor = ink;
  ...
  for (let x = 30; x <= W - 30; x += 3) {
    const y = midY + Math.sin(((x - 30) / 56) * Math.PI) * 30 * Math.exp(-(x - 30) / 170);
...
export interface FormulaSet {
  ...
  dispose: () => void;
  /** Index of the single DNA glyph in `textures` — spawn selection uses
   *  this to keep at most ONE DNA helix on screen at a time (see
   *  UniverseScene's makeSpec/texture-pick logic). */
  dnaIndex: number;
}
 ...
 *  Mix: math (π ∫ √ ∑ a²+b² πr² λ Δ), physics (E=mc², wave), chemistry
 *  (atom, molecule, flask) and ONE geometry + ONE DNA glyph — a broad,
 *  school-appropriate spread instead of a formula-heavy or DNA-heavy set. */
export function createFormulaTextures(theme: FormulaTheme): FormulaSet {
  ...
  const painters: Array<(ctx: Ctx) => void> = [
    ...
    (c) => drawDna(c, ink.azure), // ← single DNA glyph — see dnaIndex below
    (c) => drawGeometry(c, ink.azure),
    (c) => drawMolecule(c, ink.tangerine),
    (c) => drawFlask(c, ink.azure),
    (c) => drawWave(c, ink.tangerine),
  ];
  const dnaIndex = 10; // must track drawDna's position in `painters` above
