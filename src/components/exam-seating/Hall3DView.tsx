...
import {
  ...
  encodeSeatingQRData,
  listPlanPaperTimes,
  listLivePlanPaperTimesToday,
  type SeatingPlanFull,
} from "@/hooks/useExamSeating";
import { useAllExamSchedule } from "@/hooks/useNewFeatures";
import { useExamSessions } from "@/hooks/useExamAttendance";
...
// Shared MCQ timer + siren logic. The actual siren sound + state shape now
// live in @/lib/mcqSiren so they can be reused by the GLOBAL controller
// (McqSirenGlobalController) that fires the siren on EVERY page, not just
// inside this 3D Hall view. This view still fires its own SIREN_ON message
// to the iframe for the in-scene red lightning VISUAL effect, and keeps its
// own siren dedup ref so it doesn't double-fire alongside the global one
// — but the SOUND + red flash overlay are now owned by the global controller.
import {
  MCQ_TIMER_KEY,
  SIREN_DURATION_MS,
  readMcqTimer,
...
// ─── Exam Instructions Whiteboard — admin-editable, stored in localStorage ───
// The admin edits these in the PlanEditor (AdminExamSeating.tsx) and they're
// saved to localStorage so they persist across sessions. The 3D Hall reads
// them and paints them onto the LEFT whiteboard below the logo.
...
// NOTE: The MCQ timer constants (MCQ_TIMER_KEY, SIREN_DURATION_MS), the
// McqTimerState interface, readMcqTimer(), formatMcqTime(), and ALL the
// siren sound code (startSirenSound, stopSirenSound, unlockSirenAudio,
// ensureSirenAudioCtx, makeDistortionCurve) now live in @/lib/mcqSiren so
// they can be shared with McqSirenGlobalController (the app-wide siren
// controller that fires on EVERY page, not just inside this 3D Hall view).
//
// This view STILL handles its OWN siren-firing logic for the in-scene 3D
// visual effect (it sends SIREN_ON to the iframe so the 3D scene shows the
// red lightning overlay), and it keeps its own sirenFiredForRef dedup so
// the in-scene effect doesn't double-fire alongside the global controller.
//
// The SOUND + a SEPARATE full-document red flash overlay are now owned by
// McqSirenGlobalController, which is mounted once in App.tsx and runs on
// every page. So even if the admin leaves this 3D Hall view, the siren
// will still fire when MCQ time ends.
...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  // ── LIVE DATE SHEET (bug fix — mirrors AdminExamConsole) ──────────────────
  // The 3D Hall's countdown was built from the seating plan's frozen
  // class_paper_times snapshot, so it could show a paper "in progress" on a
  // day the Exam Date Sheet has no paper at all, and stayed stuck on
  // whichever subject/date the plan was created with even after the Date
  // Sheet changed. Reading the Date Sheet live fixes both, and keeps the 3D
  // Hall perfectly in sync with the Live Console (same source of truth).
  const { data: dateSheetEntries = [] } = useAllExamSchedule();
  const { data: examSessions = [] } = useExamSessions();
  const examSession = useMemo(
    () => examSessions.find(s => s.id === plan?.session_id) ?? null,
    [examSessions, plan?.session_id]
  );

  // ── Build chair assignments (sequential fill across rooms) ─────────────────
  //
  // The 3D hall (Hall_3D.html) has 10 rows × 3 sections of 4 cols (120 chairs).
  // Its getChairPos(idx) walks chair index in COLUMN-MAJOR order per section:
  //   section 0 → col 0 (all 10 rows), col 1 (all 10 rows), col 2, col 3 →
  //   section 1 → col 0 (all 10 rows) → ...
  // (See ROWS=10, COLS=[4,4,4] and the nested loop order in getChairPos.)
  //
  // Previously chairIdx was a flat sequential counter (0,1,2,3...) applied to
  // assignments sorted row-major (R1·S1, R1·S2, R1·S3, R2·S1...). Because the
  // 3D side interprets index 0,1,2,3... as "down column 0, rows 0-3", that
  // mismatch made every room's seating render as one straight column instead
  // of filling left-to-right like the admin seating grid.
  //
  // Fix: build the same column-major index sequence the 3D hall uses, then
  // hand chairs out from that sequence in ROW-MAJOR order (matching the 2D
  // admin grid: R1·S1, R1·S2, R1·S3, R2·S1...). This makes the 3D layout
  // visually match the admin panel's seating grid.
  const HALL_ROWS = 10;
  const HALL_SECTION_COLS = [4, 4, 4]; // must match COLS in Hall_3D.html

  const assignmentsWithChairs = useMemo<ChairAssignment[]>(() => {
    if (!plan) return [];

    // Precompute the column-major chair index for every (row, col) position,
    // where `col` is a flat 0-indexed column across all sections (0..11).
    // This mirrors getChairPos's traversal exactly.
    const totalCols = HALL_SECTION_COLS.reduce((a, b) => a + b, 0);
    const chairIdxByRowCol: number[][] = Array.from({ length: HALL_ROWS }, () => new Array(totalCols).fill(-1));
    {
      let idx = 0;
      let flatCol = 0;
      for (const sectionCols of HALL_SECTION_COLS) {
        for (let c = 0; c < sectionCols; c++) {
          for (let r = 0; r < HALL_ROWS; r++) {
            chairIdxByRowCol[r][flatCol] = idx;
            idx++;
          }
          flatCol++;
        }
    ...
    // Track which flat 3D chair indices are already used across rooms, since
    // rooms are placed sequentially into the same 120-chair hall.
    let roomColOffset = 0;

    for (const room of plan.rooms) {
      // Sort assignments row-major (R1·S1, R1·S2, R1·S3, R2·S1...) to match
      // the admin seating grid's fill order.
      const sorted = [...room.assignments].sort((a, b) =>
        a.row_idx !== b.row_idx ? a.row_idx - b.row_idx : a.col_idx - b.col_idx
      );

      for (const a of sorted) {
        // Map this assignment's local (row_idx, col_idx) into the 3D hall's
        // flat column space, offset so each room occupies its own columns.
        const flatRow = a.row_idx;
        const flatCol = roomColOffset + a.col_idx;
        if (flatRow >= HALL_ROWS || flatCol >= totalCols) continue; // exceeds 3D hall capacity
        const chairIdx = chairIdxByRowCol[flatRow][flatCol];
        if (chairIdx < 0 || chairIdx >= TOTAL_3D_CHAIRS) continue;
  ...
  // DECLARED EARLY (before any effect that uses it) to avoid temporal dead
  // zone errors — `const` can't be referenced before its declaration line.
  ...
  // Wrapped in defensive try/catch so QR generation failures never crash the
  // 3D Hall — stickers just don't appear, but the hall still works.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("[Hall3DView] Sticker QR generation failed:", err);
      }
    })();
    return () => { cancelled = true; };
  }, [plan, assignmentsWithChairs]);

  // Send stickers WITH QR codes to the iframe when QR generation is complete.
  // NOTE: The iframe creates TEXT-ONLY stickers immediately when it receives
  // the SETUP message (with assignment data). This effect sends QR code data
  // separately so the iframe can recreate the stickers WITH QR codes added.
  // If QR generation fails or is slow, the text-only stickers from SETUP
  // remain visible — stickers NEVER disappear.
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[Hall3DView] Send stickers failed:", err);
    }
  }, [iframeReady, stickerQrs, assignmentsWithChairs, sendMessage]);

  // ── Exam Instructions Whiteboard ───────────────────────────────────────────
  // Loads admin-edited instructions from localStorage and sends them to the
  // iframe. The LEFT whiteboard (below the logo) paints them. Also listens
  // for cross-tab storage events so edits in the admin panel update live.
  const [instructions, setInstructions] = useState<string[]>(() => loadInstructions());
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key === INSTRUCTIONS_KEY) setInstructions(loadInstructions());
    };
  ...
  useEffect(() => {
    ...
    sendMessage({ type: "UPDATE_INSTRUCTIONS", instructions });
  }, [iframeReady, instructions, sendMessage]);
  ...
  // ── MULTI-PAPER TIMELINE (per-class timing) ──────────────────────────────
  // Mirrors AdminExamConsole: when classes mixed into this hall are sitting
  // different papers at different times today (e.g. Islamiyat 9-11, Urdu
  // 9-12), count down whichever paper is currently in progress with the
  // soonest end, then auto-advance to the next one once it finishes.
  const paperTimeline = useMemo(() => {
    if (!plan) return [] as { classes: string[]; subject: string | null; startMs: number; endMs: number }[];
    // LIVE fix: only classes with an ACTUAL Date Sheet paper today produce
    // an entry — see listLivePlanPaperTimesToday in useExamSeating.ts.
    const groups = listLivePlanPaperTimesToday(plan, examSession, dateSheetEntries);
    const n = new Date();
    const out: { classes: string[]; subject: string | null; startMs: number; endMs: number }[] = [];
    for (const g of groups) {
      if (!g.startTime || !g.endTime) continue;
      const [sh, sm] = g.startTime.split(":").map(Number);
      const [eh, em] = g.endTime.split(":").map(Number);
      const startMs = new Date(n.getFullYear(), n.getMonth(), n.getDate(), sh, sm, 0, 0).getTime();
      const endMs = new Date(n.getFullYear(), n.getMonth(), n.getDate(), eh, em, 59, 999).getTime();
      out.push({ classes: g.classes, subject: g.subject, startMs, endMs });
    }
    out.sort((a, b) => a.endMs - b.endMs);
    return out;
  }, [plan, examSession, dateSheetEntries]);

  const activeTimelineEntry = useMemo(() => {
    if (paperTimeline.length === 0) return null;
    const inProgress = paperTimeline.find(e => now >= e.startMs && now <= e.endMs);
    if (inProgress) return inProgress;
    const upcoming = paperTimeline.find(e => now < e.startMs);
    if (upcoming) return upcoming;
    return paperTimeline[paperTimeline.length - 1];
  }, [paperTimeline, now]);

  // ── NEXT PAPER DAY (mirrors AdminExamConsole) ────────────────────────────
  // For multi-day recurring plans (exam_date_from → exam_date_to), once
  // today's paper(s) end, the countdown should switch to "time remaining to
  // next paper" (tomorrow's start time) instead of freezing on "ended".
  // Without this, the 3D Hall disagreed with the Console — Console correctly
  // showed the days-until-tomorrow countdown while the Hall kept showing
  // "paper ended" because it only ever looked at TODAY's timeline.
  // ── LIVE fix: scan the real Date Sheet for the next actual future date
  // that has a paper for any class in this plan, instead of guessing
  // "tomorrow, same time-of-day as when the plan was created". Mirrors
  // AdminExamConsole's nextPaperInfo exactly, so the Hall and Console never
  // disagree.
  const nextPaperInfo = useMemo(() => {
    if (!plan || !examSession) return null;
    const d = new Date();
    const today = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
    const termOrdinal = (s: string): number | null => {
      if (/1st|first|annual-i\b/i.test(s)) return 1;
      if (/2nd|second|annual-ii\b/i.test(s)) return 2;
      return null;
    };
    const termMatches = (examType: string, term: string): boolean => {
      const a = (examType || "").trim().toLowerCase();
      const b = (term || "").trim().toLowerCase();
      if (!a || !b) return false;
      if (a === b || b.includes(a) || a.includes(b)) return true;
      const oa = termOrdinal(a), ob = termOrdinal(b);
      return oa !== null && oa === ob;
    };
    let bestDate: string | null = null;
    for (const cls of plan.classes || []) {
      const entries = dateSheetEntries.filter(
        (e: any) => e.class === cls && e.year === examSession.exam_year && termMatches(e.exam_type, examSession.exam_term) && e.exam_date > today && e.start_time
      );
      for (const e of entries) {
        if (!bestDate || e.exam_date < bestDate) bestDate = e.exam_date;
      }
    }
    if (!bestDate) return null;
    type Row = { cls: string; subject: string; startTime: string; endTime: string | null };
    const rows: Row[] = [];
    for (const cls of plan.classes || []) {
      const entry = dateSheetEntries.find(
        (e: any) => e.class === cls && e.year === examSession.exam_year && termMatches(e.exam_type, examSession.exam_term) && e.exam_date === bestDate && e.start_time
      );
      if (entry) rows.push({ cls, subject: entry.subject, startTime: entry.start_time, endTime: entry.end_time ?? null });
    }
    if (rows.length === 0) return null;
    rows.sort((a, b) => a.startTime.localeCompare(b.startTime));
    const earliest = rows[0];
    return { dateStr: bestDate, startTime: earliest.startTime, endTime: earliest.endTime, subject: earliest.subject, rows };
  }, [plan, examSession, dateSheetEntries]);

  const nextPaperDay = useMemo(() => {
    if (!nextPaperInfo) return null;
    const [y, m, d] = nextPaperInfo.dateStr.split("-").map(Number);
  ...
  const nextPaperStartTime = useMemo(() => {
    if (!nextPaperInfo) return null;
    const [y, m, d] = nextPaperInfo.dateStr.split("-").map(Number);
    const [hh, mm] = nextPaperInfo.startTime.split(":").map(Number);
    return new Date(y, m - 1, d, hh, mm, 0, 0).getTime();
  }, [nextPaperInfo]);

  const paperPhase = useMemo(() => {
    if (paperTimeline.length >= 2 && activeTimelineEntry) {
      if (now < activeTimelineEntry.startMs) return "before" as const;
      if (now <= activeTimelineEntry.endMs) return "in_progress" as const;
      // Today's last timeline paper ended — check if a next paper day exists.
      if (nextPaperStartTime && now < nextPaperStartTime) return "before_next" as const;
    ...
    if (now <= end) return "in_progress" as const;
    if (nextPaperStartTime && now < nextPaperStartTime) return "before_next" as const;
    return "ended" as const;
  }, [plan, now, paperTimeline, activeTimelineEntry, nextPaperStartTime]);

  const countdownText = useMemo(() => {
    if (paperTimeline.length >= 2 && activeTimelineEntry) {
      const label = activeTimelineEntry.classes.length > 0 || activeTimelineEntry.subject
        ? ` (${activeTimelineEntry.classes.map(c => `Cls ${c}`).join(", ")}${activeTimelineEntry.subject ? " · " + activeTimelineEntry.subject : ""})`
        : "";
      if (paperPhase === "before") return `Paper starts in ${formatDuration(activeTimelineEntry.startMs - now)}${label}`;
      if (paperPhase === "in_progress") return `Paper ends in ${formatDuration(activeTimelineEntry.endMs - now)}${label}`;
      if (paperPhase === "before_next" && nextPaperStartTime) return `Time remaining to next paper: ${formatDuration(nextPaperStartTime - now)}`;
      return `Paper ended ${formatDuration(now - activeTimelineEntry.endMs)} ago${label}`;
    }
  ...
  }, [plan, now, paperPhase, paperTimeline, activeTimelineEntry, nextPaperStartTime]);

  // ── MCQ Timer (read from localStorage, shared with AdminExamConsole) ──────
  // Re-reads every second (driven by the `now` tick above) so the countdown
  // stays smooth. Also listens for cross-tab `storage` events so a
  // Start/Stop/Reset in the Console tab is reflected here instantly.
  const [mcqTimer, setMcqTimer] = useState<McqTimerState | null>(() => readMcqTimer());
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key === MCQ_TIMER_KEY) setMcqTimer(readMcqTimer());
    };
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, []);
  // Re-read on every 1s tick — handles the same-tab case (Console + Hall in
  // the same tab via the PlanEditor's 3D Hall button) where `storage` events
  // DON'T fire. Cheap operation, runs once per second.
  useEffect(() => {
    setMcqTimer(readMcqTimer());
  }, [now]);

  // Derived remaining ms (clamped to ≥ 0).
  const mcqRemainingMs = useMemo(() => {
    if (!mcqTimer) return 0;
    if (mcqTimer.status === "finished") return 0;
    return Math.max(0, mcqTimer.endTime - Date.now());
  }, [mcqTimer, now]);
  const mcqDisplay = mcqTimer ? formatMcqTime(mcqRemainingMs) : null;

  // ── Send UPDATE_TIMER to the iframe every second the timer is active ──────
  // The iframe paints the timer onto the big in-scene HD screen alongside
  // the paper countdown + tally. We send a message every second so the
  // screen's timer ticks in lockstep with this top bar.
  useEffect(() => {
    if (!iframeReady) return;
    sendMessage({
      type: "UPDATE_TIMER",
      timer: mcqTimer
        ? {
            status: mcqTimer.status,
            remainingMs: mcqRemainingMs,
            display: mcqDisplay,
            totalMs: mcqTimer.totalMs,
          }
        : null,
    });
  }, [iframeReady, mcqTimer, mcqRemainingMs, mcqDisplay, sendMessage]);

  // ── Siren: fire when the timer transitions running → finished ─────────────
  // Two ways the timer can "finish":
  //   (a) The Console tab already wrote status="finished" to localStorage
  //       (because its own tick detected endTime being reached). In that
  //       case we receive it via the `storage` event or the 1s re-read.
  //   (b) The 3D Hall tab's own tick sees endTime reached first (because
  //       the Console tab is closed or its tick is slower). In that case
  //       we promote the timer to "finished" ourselves and write it back
  //       to localStorage so the Console tab (if open) also flips.
  //
  // IMPORTANT (post-refactor):
  // The SIREN SOUND + the full-document RED FLASH OVERLAY are now owned by
  // McqSirenGlobalController (mounted once in App.tsx), so they fire on
  // EVERY page — not just inside this 3D Hall view. This view ONLY handles
  // the in-scene 3D visual effect (SIREN_ON/SIREN_OFF messages to the
  // iframe so the 3D scene shows its own red lightning overlay).
  //
  // We do NOT call startSirenSound()/stopSirenSound() here anymore — the
  // global controller owns the AudioContext lifecycle. We also do NOT
  // delete the timer from localStorage here — the global controller's
  // dedup (sirenFiredForRef) handles that, so the siren fires exactly once
  // per finishedAt across the whole app.
  //
  // We keep our OWN sirenFiredForRef here so we don't double-send SIREN_ON
  // to the iframe (which would restart the iframe's CSS animation). And we
  // keep our OWN sirenTimeoutRef to send SIREN_OFF to the iframe after 15s
  // (the iframe doesn't know about the global controller's timeout).
  //
  // CRITICAL DESIGN (fixes "siren stops after 2 seconds" + "re-fires on reopen"):
  // The in-scene red lightning must play for the FULL 15 seconds. The
  // `mcqTimer` state object changes reference every second (the `now` tick
  // re-reads localStorage and setMcqTimer creates a new object). If we put
  // the SIREN_OFF call in this effect's cleanup, it would run on every
  // re-render (~1s) and KILL the in-scene lightning after ~1-2 seconds.
  //
  // Solution: manage the in-scene lightning lifecycle via REFS, not effect
  // cleanup. The effect itself has NO cleanup function, so it never stops
  // the lightning prematurely. The lightning is stopped ONLY by:
  //   1. The 15s timeout (normal completion), or
  //   2. The unmount-only cleanup effect (user closes the 3D Hall)
  const sirenFiredForRef = useRef<number | null>(null);
  const sirenTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // ── Unlock the siren AudioContext on first user interaction ──────────────
  // NOTE: This unlock listener is now also registered by the global
  // controller, but we keep it here too as a belt-and-suspenders measure
  // — it's idempotent (calling unlockSirenAudio() multiple times is safe).
  // Mobile autoplay policy requires a user gesture to start audio.
  useEffect(() => {
    const unlock = () => unlockSirenAudio();
    document.addEventListener("pointerdown", unlock, { passive: true });
    document.addEventListener("keydown", unlock, { passive: true });
    return () => {
      document.removeEventListener("pointerdown", unlock);
      document.removeEventListener("keydown", unlock);
      // Unmount-only cleanup: clear the in-scene SIREN_OFF timeout + send
      // SIREN_OFF to the iframe so the in-scene lightning stops when the
      // user closes the 3D Hall. We do NOT call stopSirenSound() here
      // anymore — the global controller owns the sound lifecycle and the
      // sound must continue playing even if the user closes the 3D Hall.
      if (sirenTimeoutRef.current) {
        clearTimeout(sirenTimeoutRef.current);
        sirenTimeoutRef.current = null;
      }
      sendMessage({ type: "SIREN_OFF" });
    };
  }, [sendMessage]);
  ...
  useEffect(() => {
    ...
    // Detect transition: timer was running, now endTime is reached.
    if (mcqTimer && mcqTimer.status === "running" && Date.now() >= mcqTimer.endTime) {
      const finished: McqTimerState = {
        ...mcqTimer,
        status: "finished",
        finishedAt: mcqTimer.endTime,
      };
      try {
        localStorage.setItem(MCQ_TIMER_KEY, JSON.stringify(finished));
      } catch { /* ignore */ }
      setMcqTimer(finished);
      // fall through — the next clause picks up the siren now that status is "finished".
    }

    // Fire the in-scene siren VISUAL when we observe a fresh "finished"
    // state we haven't yet fired for. finishedAt is the unique key.
    if (mcqTimer?.status === "finished") {
      const finishedAt = mcqTimer.finishedAt ?? mcqTimer.endTime;
      if (sirenFiredForRef.current !== finishedAt) {
        sirenFiredForRef.current = finishedAt;

        // Send SIREN_ON to the iframe for the in-scene red lightning VISUAL.
        // The SOUND + full-document red flash overlay are handled by the
        // global controller (McqSirenGlobalController), which fires on
        // every page — not just here.
        sendMessage({ type: "SIREN_ON", durationMs: SIREN_DURATION_MS });

        // Arm the 15s auto-off for the in-scene visual. Stored in a ref so
        // it survives effect re-runs (the effect re-runs every second when
        // mcqTimer changes, but we do NOT stop the visual in a cleanup —
        // only this timeout stops it).
        if (sirenTimeoutRef.current) clearTimeout(sirenTimeoutRef.current);
        sirenTimeoutRef.current = setTimeout(() => {
          sendMessage({ type: "SIREN_OFF" });
          sirenTimeoutRef.current = null;
        }, SIREN_DURATION_MS);
        // NOTE: intentionally NO return cleanup here. The in-scene lightning
        // must keep playing for the full 15s even as this effect re-runs.
        // The unmount cleanup (in the unlock effect above) handles stopping
        // on close.
      }
    }

    // If the timer was reset (no state at all), clear the siren memory so
    // a fresh timer can fire the siren again.
    if (!mcqTimer) {
      sirenFiredForRef.current = null;
    }
  }, [iframeReady, mcqTimer, sendMessage]);
  ...
  useEffect(() => {
    ...
    // LIVE fix: was plan.paper_subject (frozen). Now the live subject(s)
    // for today, or "Upcoming: ..." when nothing is scheduled today — same
    // logic as the on-screen React label above, kept in sync with the 3D
    // scene's own screen texture.
    const liveSubject = paperTimeline.length > 0
      ? Array.from(new Set(paperTimeline.map(t => t.subject).filter(Boolean))).join(", ")
      : nextPaperInfo
        ? `Upcoming: ${Array.from(new Set(nextPaperInfo.rows.map(r => r.subject))).join(", ")}`
        : null;
    const screenData = {
      planTitle: plan.title,
      paperSubject: liveSubject,
      examDate: plan.exam_date
  ...
  }, [iframeReady, plan, paperPhase, countdownText, tally, perRoomTally, examStaff, sendMessage, paperTimeline, nextPaperInfo]);
  ...
  useEffect(() => {
    ...
    if (mode === "invigilator" && selectedInvigilator) {
      ...
      // Find 3D chair indices for this room's columns in the duty range.
      // Reuse assignmentsWithChairs — the single source of truth for how
      // (roomId, row_idx, col_idx) maps to the 3D hall's chairIdx — instead
      // of re-deriving the mapping here with separate (and now-stale) logic.
      const chairIndices: number[] = assignmentsWithChairs
        .filter(a =>
          a.roomId === room.id &&
          // duty.col_start / col_end are 1-indexed; colIdx is 0-indexed
          a.colIdx >= (duty as any).col_start - 1 &&
          a.colIdx <= (duty as any).col_end - 1
        )
        .map(a => a.chairIdx);
  ...
  }, [mode, selectedInvigilator, iframeReady, plan, assignmentsWithChairs, sendMessage]);
    ...
    const handler = (e: MessageEvent) => {
      ...
      switch (msg.type) {
        ...
        case "STICKERS_READY":
          ...
          break;
        case "CHAIR_CLICKED":
          // msg.assignment is the chair's student data, OR null when the
          // user double-tapped the same chair again to dismiss (toggle off).
          // Setting clickedAssignment to null hides the details panel.
          setClickedAssignment(msg.assignment ?? null);
          break;
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 min-w-0 pointer-events-auto">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-white font-bold text-xs sm:text-base leading-tight flex items-start gap-1.5">
                  <Building2 className="w-4 h-4 shrink-0 mt-0.5" />
                  <span className="break-words line-clamp-2">{plan.title}</span>
                </h2>
                <div className="flex items-center gap-2 text-[10px] sm:text-xs text-white/70 flex-wrap mt-0.5">
                  {/* LIVE fix: was plan.paper_subject — the frozen field.
                      Now shows today's live subject(s), falling back to the
                      upcoming paper's subject when nothing is today. */}
                ...
                {/* Compact class + subject chips — small, mobile-friendly.
                    Classes mixed into this hall can be sitting different
                    papers (synced from the Exam Date Sheet); timing itself
                    is shown via the countdown pill on the right. */}
                <div className="flex items-center gap-1 flex-wrap mt-1">
                  {/* LIVE fix: was listPlanPaperTimes(plan) — the frozen
                      snapshot. Now reads the Date Sheet live so this never
                      gets stuck on an old date's subjects. */}
                  {listLivePlanPaperTimesToday(plan, examSession, dateSheetEntries).map((t, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-white/10 text-white/90 border border-white/20"
                    >
                      <span className="font-bold">{t.classes.map(c => `Cls ${c}`).join(", ")}</span>
                      {t.subject && <span className="text-white/60">· {t.subject}</span>}
                    </span>
                  ))}
                  {listLivePlanPaperTimesToday(plan, examSession, dateSheetEntries).length === 0 && (
                    <span className="text-[9px] sm:text-[10px] text-white/50 italic">No paper today</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pointer-events-auto">
              {/* ── MCQ Timer pill ─────────────────────────────────────────────
                  NOTE: The MCQ timer pill is rendered INSIDE the iframe
                  (Hall_3D.html's #mcq-timer element) so it shows in the
                  same place whether you're looking at the top bar OR the
                  in-scene HD screen. We do NOT render a second pill here
                  in React — that would create a duplicate "00:00" display
                  (one in the React top bar, one in the iframe). The iframe
                  pill is updated every second via the UPDATE_TIMER message
                  sent from the effect below. */}
              {countdownText && (
                <div className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold backdrop-blur-md border ${
                  paperPhase === "in_progress"
                    ...
                    : (paperPhase === "before" || paperPhase === "before_next")
                    ...
                    return (
                      <div key={room.id} className="space-y-1">
                        ...
                        {invigs.map((name: string, ii: number) => {
                          const duty = duties[ii] ?? { col_start: 1, col_end: room.cols };
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[9px] text-white/40 shrink-0">C{(duty as any).col_start ?? 1}–{(duty as any).col_end ?? room.cols}</span>
                            </button>
                          );
                        })}
                      </div>
