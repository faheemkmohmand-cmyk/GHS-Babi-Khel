...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  const nextPaperInfo = useMemo(() => {
    ...
    let best: { dateStr: string; startTime: string } | null = null;
    for (const cls of plan.classes || []) {
      ...
      for (const e of entries) {
        if (!best || e.exam_date < best.dateStr || (e.exam_date === best.dateStr && e.start_time < best.startTime)) {
          best = { dateStr: e.exam_date, startTime: e.start_time };
        }
      }
    }
    return best;
