 ...
 *
 * v5 fixes:
 *  - Auto-generation is now bulletproof: wraps htmlToTree in try/catch,
 *    falls back to a sensible default tree if parsing fails.
 *  - Read-only by default: the Edit button is hidden inside an "Edit" toggle
 *    so the map is clearly a *view* of the chapter, not a blank canvas.
 *  - Better empty state: when content is empty, shows a clear message
 *    instead of "Start adding nodes" (which implied manual editing).
 ...
 *  - Editable (opt-in): drag, add/delete nodes, inline rename, color picker.
...
function htmlToTree(html: string, fallbackTitle: string): Tree {
  ...
  // Safely parse HTML — wrapped in try/catch so a malformed HTML payload
  // (e.g. a chapter whose content was corrupted in the database) can never
  // take down the whole ConceptMap block.
  let doc: Document | null = null;
  try {
    if (html && typeof html === "string" && html.trim()) {
      // DOMParser may not exist in very old environments, or may throw on
      // truly broken input. Feature-detect and fall back gracefully.
  ...
  if (!doc) {
    // No content — show a clear message that this is auto-generated but
    // has nothing to map yet. Don't say "Start adding nodes" (which
    // implied manual editing and confused users).
    const id = newId();
    nodes[id] = {
  ...
  try {
    ...
    // ignore parsing errors — we'll show whatever nodes we managed to add
  }
  ...
  // If nothing was added, show a clear message — NOT "Start adding nodes"
  // (which implied manual editing). The map is auto-generated; if there's
  // no structure to extract, say so.
...
export default function ConceptMap({
  ...
  // Wrapped in try/catch so a parsing failure can never crash the component —
  // we fall back to a single-node tree with a clear error message.
  const safeHtmlToTree = (html: string, title: string): Tree => {
    try {
      return htmlToTree(html, title);
    } catch {
      // Ultimate fallback — should never happen because htmlToTree itself
      // is defensive, but just in case.
      const rootId = "n1";
      const fallbackId = "n2";
      return {
        rootId,
  ...
  return (
    <div className="rounded-2xl border border-border bg-card overflow-hidden">
      ...
      <div className="p-2 border-t border-border bg-secondary/20 text-[10px] text-muted-foreground text-center">
        {editMode
          ? "Editing mode · Double-click a node to rename · Use the toolbar above to add/delete"
          : "Auto-generated from chapter content · Drag to pan · Wheel to zoom · Click Edit to customise"}
      </div>
    </div>
  );
}
