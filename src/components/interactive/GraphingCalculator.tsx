...
export default function GraphingCalculator({
  ...
  const [exprs, setExprs] = useState<Expr[]>(() => {
    try {
      const saved = localStorage.getItem("graph:exprs");
      if (saved) {
        const parsed = JSON.parse(saved);
        // Validate: must be a non-empty array of objects with at least `raw` and `color`
        if (Array.isArray(parsed) && parsed.length > 0 && parsed.every((e: any) => e && typeof e.raw === "string")) {
          // Ensure all required fields exist (defensive — old saved data might be missing fields)
          return parsed.map((e: any, i: number) => ({
            id: e.id || `e${Date.now()}_${i}`,
            raw: e.raw || "",
            body: e.body || "",
            kind: e.kind || "fn",
            color: e.color || CURVE_COLORS[i % CURVE_COLORS.length],
            visible: e.visible !== false,
            sliderVars: Array.isArray(e.sliderVars) ? e.sliderVars : [],
            sliderValues: e.sliderValues && typeof e.sliderValues === "object" ? e.sliderValues : {},
            sliderPlaying: false,
          }));
        }
      }
  ...
  useEffect(() => {
    const measure = () => {
      ...
      setCanvasSize({ w: Math.floor(r.width), h: Math.max(360, Math.floor(r.width * 0.62)) });
    };
    measure();
  ...
  // persist
  useEffect(() => {
    try { localStorage.setItem("graph:exprs", JSON.stringify(exprs)); } catch { /* ignore */ }
  }, [exprs]);
  ...
  const draw = useCallback(() => {
    ...
    for (let mx = Math.ceil(xMin / subStep) * subStep; mx <= xMax; mx += subStep) {
      const [px] = toPx(mx, 0);
      ctx.moveTo(px, 0); ctx.lineTo(px, canvasSize.h);
    }
    for (let my = Math.ceil(yMin / subStep) * subStep; my <= yMax; my += subStep) {
      const [, py] = toPx(0, my);
    ...
    ctx.beginPath();
    for (let mx = Math.ceil(xMin / step) * step; mx <= xMax; mx += step) {
    ...
    for (let my = Math.ceil(yMin / step) * step; my <= yMax; my += step) {
  ...
  const onPointerDown = (e: ReactPointerEvent<HTMLCanvasElement>) => {
    ...
    // pinch detection
    const evs = (e.nativeEvent as PointerEvent);
    if (evs.isPrimary === false || (e as any).pointerType === "touch") {
      // try to find another active pointer — simplified two-finger pinch:
      // we track distance from primary pointer position
    }
  };
