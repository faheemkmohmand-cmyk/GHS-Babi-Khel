      ...
      importer()
        ...
        .catch((err) => {
          ...
          if (attempt >= retries) {
            ...
            // Short, fixed-ish backoff — these failures are almost always
            // a momentary mobile network hiccup (WiFi <-> 4G handoff),
            // now further reduced by the service worker's own retry
            // (see public/sw.js networkFirstAsset), so this is a second
...
class BlockErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error) {
    // Log the REAL underlying error to the console so it can be inspected
    // (previously this was swallowed entirely — the on-screen message was
    // always the same generic text no matter what actually failed, making
    // it impossible to tell a real bug apart from a network hiccup).
  ...
  componentDidUpdate(_prevProps: ErrorBoundaryProps, prevState: ErrorBoundaryState) {
    // Auto-retry almost immediately on the first error — most failures at
    // this point are a momentary mobile network hiccup that's already
    // resolved by the time React re-renders (see public/sw.js for the
    // deeper fix that prevents most of these from ever reaching here).
