...
export default function PhETEmbed({
  ...
  //
  // Why direct (not via /api/phet-proxy):
  //  1. PhET's old `_en-iframe.html` endpoint was retired and now 404s — the proxy
  //     was therefore always returning "PhET returned 404 for sim: ..." and the
  //     iframe showed nothing.
  //  2. PhET's `_en.html` ships with NO `X-Frame-Options` and NO `frame-ancestors`
  //     CSP, and `access-control-allow-origin: *`, so it is embeddable cross-origin.
  //  3. The site CSP (vercel.json) already whitelists `https://phet.colorado.edu`
  //     and `https://*.phet.colorado.edu` under `frame-src`, so direct embedding
  //     works without any header changes.
  //  4. Bypassing the proxy also avoids the global `X-Frame-Options: DENY` rule
  //     in vercel.json (which was overriding the proxy's SAMEORIGIN and causing
  //     the "ghsbabikhel.indevs.in refused to connect" error in the iframe).
  const simUrl = simId
