...
function solveLinear(input: string): SolveResult {
  ...
  steps.push({
    ...
    explanation: `Move x-terms to left, constants to right (${sign(-c)}x from both sides, ${sign(-b)} from both sides)`,
...
function solveQuadratic(input: string): SolveResult {
  ...
  if (a !== 1) {
    steps.push({
      math: `${input}  →  divide by ${formatNum(a)}: x^2 ${sign(b / a)} x ${sign(c / a)} = 0`,
  ...
  if (factors) {
    const [p, q] = factors;
    steps.push({
      math: `x^2 ${sign(b)} x ${sign(c)} = (x ${sign(p)})(x ${sign(q)})`,
      explanation: `Factor: find two numbers that add to ${formatNum(targetSum)} and multiply to ${formatNum(targetProd)}`,
      rule: `${p} + ${q} = ${formatNum(targetSum)}, ${p} × ${q} = ${formatNum(targetProd)}. So the factors are (x ${sign(p)})(x ${sign(q)}).`,
    });
    steps.push({
      math: `x ${sign(p)} = 0  →  x = ${formatNum(-p)}`,
      explanation: "Set each factor to zero",
      rule: "Zero product property: if a × b = 0, then a = 0 or b = 0.",
    });
    steps.push({
      math: `x ${sign(q)} = 0  →  x = ${formatNum(-q)}`,
...
function solveSimultaneous(input: string): SolveResult {
  ...
  steps.push({
    math: `Multiply eq1 by ${formatNum(m1)} → ${formatNum(m1 * e1.x)}x ${sign(newE1y)} y = ${formatNum(newE1c)}`,
  ...
  steps.push({
    math: `Multiply eq2 by ${formatNum(m2)} → ${formatNum(m2 * e2.x)}x ${sign(newE2y)} y = ${formatNum(newE2c)}`,
  ...
  steps.push({
    math: `Substitute y=${formatNum(yVal)} into eq1: ${formatNum(e1.x)} x ${sign(e1.y * yVal)} = ${formatNum(e1.c)} → x = ${formatNum(xVal)}`,
...
function solveInequality(input: string): SolveResult {
  ...
  const steps: Step[] = [
    ...
    { math: `${formatNum(a)}x ${sign(b)} ${op} 0`,
