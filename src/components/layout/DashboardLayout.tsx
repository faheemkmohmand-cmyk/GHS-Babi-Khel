import { useState, useEffect, useRef, memo, useMemo } from "react";
import { Link, useNavigate } from "react-router-dom";
import { LogOut, Menu, X, ExternalLink, Moon, Sun, Search, Shield } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import NotificationBell from "@/components/shared/NotificationBell";
import { useDarkMode } from "@/hooks/useDarkMode";
import { safeCloudinaryUrl } from "@/lib/cloudinaryValidator";

// ── Emoji icon component (legacy, still used by items without a custom icon) ──
const EmojiIcon = ({ emoji, size = "w-5 h-5" }: { emoji: string; size?: string }) => (
  <span className={`${size} flex items-center justify-center text-base leading-none select-none`} aria-hidden>
    {emoji}
  </span>
);

// ── Realistic SVG icons ────────────────────────────────────────────────────
// Timetable — a wall/desk calendar page with a clock overlay, evoking a
// weekly class schedule rather than a single date (distinct from the plain
// 📅 emoji used elsewhere, and distinct from Exam Schedule below).
const TimetableIcon = ({ className = "w-5 h-5" }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    {/* Calendar body */}
    <rect x="3" y="4.5" width="18" height="16" rx="2.2" fill="#EFF6FF" stroke="#2563EB" strokeWidth="1.4" />
    {/* Header bar */}
    <path d="M3 8.2C3 6.15 3 5.1 3.66 4.45 4.31 3.8 5.35 3.8 7.4 3.8h9.2c2.05 0 3.09 0 3.74.65.65.65.65 1.7.65 3.75v.3H3v-.3Z" fill="#2563EB" />
    {/* Binder rings */}
    <rect x="6.6" y="2.3" width="1.6" height="3" rx="0.8" fill="#1E3A8A" />
    <rect x="15.8" y="2.3" width="1.6" height="3" rx="0.8" fill="#1E3A8A" />
    {/* Weekly grid of class slots */}
    <rect x="5.5" y="11" width="3.6" height="2.1" rx="0.5" fill="#93C5FD" />
    <rect x="10.2" y="11" width="3.6" height="2.1" rx="0.5" fill="#BFDBFE" />
    <rect x="14.9" y="11" width="3.6" height="2.1" rx="0.5" fill="#93C5FD" />
    <rect x="5.5" y="14" width="3.6" height="2.1" rx="0.5" fill="#BFDBFE" />
    <rect x="10.2" y="14" width="3.6" height="2.1" rx="0.5" fill="#93C5FD" />
    <rect x="14.9" y="14" width="3.6" height="2.1" rx="0.5" fill="#BFDBFE" />
    <rect x="5.5" y="17" width="3.6" height="1.6" rx="0.5" fill="#DBEAFE" />
    <rect x="10.2" y="17" width="3.6" height="1.6" rx="0.5" fill="#DBEAFE" />
  </svg>
);

// Exam Schedule — a calendar page with a checkmark badge, signalling a
// confirmed/scheduled exam date rather than a routine weekly slot.
const ExamScheduleIcon = ({ className = "w-5 h-5" }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    {/* Calendar body */}
    <rect x="3" y="4.5" width="16" height="15.5" rx="2.2" fill="#FFF7ED" stroke="#EA580C" strokeWidth="1.4" />
    {/* Header bar */}
    <path d="M3 8C3 6.05 3 5.08 3.62 4.46 4.24 3.85 5.2 3.85 7.16 3.85h7.68c1.96 0 2.92 0 3.54.61.62.62.62 1.59.62 3.54v.3H3V8Z" fill="#EA580C" />
    {/* Binder rings */}
    <rect x="6.4" y="2.3" width="1.5" height="2.9" rx="0.75" fill="#9A3412" />
    <rect x="14.1" y="2.3" width="1.5" height="2.9" rx="0.75" fill="#9A3412" />
    {/* Date lines on the page */}
    <rect x="5.6" y="10.6" width="6.8" height="1.15" rx="0.4" fill="#FDBA74" />
    <rect x="5.6" y="13.1" width="5" height="1.15" rx="0.4" fill="#FED7AA" />
    <rect x="5.6" y="15.6" width="5.8" height="1.15" rx="0.4" fill="#FED7AA" />
    {/* Confirmed badge */}
    <circle cx="18" cy="17" r="4.6" fill="#16A34A" stroke="white" strokeWidth="1.3" />
    <path d="M15.8 17.1L17.2 18.5L20.2 15.3" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

// ── Nav structure with sections ───────────────────────────────────────────────
interface NavItem {
  id: string;
  label: string;
  emoji: string;
  // Optional realistic SVG icon component — when present, this renders
  // instead of the emoji (used for items that need a more distinctive,
  // non-generic look, e.g. Timetable vs Exam Schedule which previously
  // shared one "Schedule" entry and one 📅 emoji).
  icon?: (props: { className?: string }) => JSX.Element;
}
interface NavSection {
  heading: string;
  items: NavItem[];
}

const navSections: NavSection[] = [
  {
    heading: "OVERVIEW",
    items: [
      { id: "overview",   label: "Overview",      emoji: "🏠" },
      { id: "profile",    label: "My Profile",    emoji: "👤" },
    ],
  },
  {
    heading: "STUDENTS",
    items: [
      { id: "seating",     label: "Exam Seating",     emoji: "🪑" },
      { id: "credentials", label: "Credentials",      emoji: "🪪" },
      { id: "fees",        label: "Fee Status",       emoji: "💰" },
    ],
  },
  {
    heading: "SCHOOL",
    items: [
      { id: "timetable",      label: "Timetable",       emoji: "📅", icon: TimetableIcon },
      { id: "exam-schedule",  label: "Exam Schedule",   emoji: "📝", icon: ExamScheduleIcon },
      { id: "teachers",       label: "Teachers",        emoji: "👨‍🏫" },
    ],
  },
];

// Flat list for bottom bar / searching
const allNavItems: NavItem[] = navSections.flatMap(s => s.items);

// Deep search index
const searchIndex: { label: string; sublabel?: string; tabId: string }[] = [
  ...allNavItems.map(item => ({ label: item.label, tabId: item.id })),
  { label: "ID Cards",          sublabel: "Credentials",      tabId: "credentials" },
  { label: "Monitor Pass",      sublabel: "Credentials",      tabId: "credentials" },
  { label: "Student ID",        sublabel: "Credentials",      tabId: "credentials" },
  { label: "Seating",            sublabel: "Exam Seating",      tabId: "seating" },
  { label: "Exam Seating",       sublabel: "Exam Seating",      tabId: "seating" },
  { label: "My Seat",            sublabel: "Exam Seating",      tabId: "seating" },
  { label: "Exam Hall",          sublabel: "Exam Seating",      tabId: "seating" },
  { label: "Desk",               sublabel: "Exam Seating",      tabId: "seating" },
  { label: "Room",               sublabel: "Exam Seating",      tabId: "seating" },
  { label: "Fee Vouchers",      sublabel: "Fee Status",       tabId: "fees" },
  { label: "Fee Structure",     sublabel: "Fee Status",       tabId: "fees" },
  { label: "Payment Status",    sublabel: "Fee Status",       tabId: "fees" },
  { label: "Timetable",         sublabel: "Timetable",        tabId: "timetable" },
  { label: "Class Schedule",    sublabel: "Timetable",        tabId: "timetable" },
  { label: "Exam Schedule",     sublabel: "Exam Schedule",    tabId: "exam-schedule" },
  { label: "Exam Dates",        sublabel: "Exam Schedule",    tabId: "exam-schedule" },
];

// Case-insensitive match against a label or any of its known synonyms
// in searchIndex, so e.g. typing "fee" finds "Fee Status".
const matchesQuery = (item: NavItem, query: string) => {
  const q = query.trim().toLowerCase();
  if (!q) return true;
  if (item.label.toLowerCase().includes(q)) return true;
  return searchIndex.some(
    (e) => e.tabId === item.id && e.label.toLowerCase().includes(q)
  );
};

// ── PERFORMANCE: NavBtn + SectionedNav are defined OUTSIDE the parent ────────
// Previously these were defined INSIDE <DashboardLayout>, which meant every
// render of the parent (e.g. typing in the search box, opening the mobile
// sidebar, the auth profile hook re-rendering) created NEW component
// identities for NavBtn and SectionedNav. React then unmounted + remounted
// every single nav button on every render → focus loss, layout thrash,
// visible jank, and slow search filtering. Moving them outside + wrapping
// in memo() means React can now skip re-rendering buttons whose props
// haven't changed. Pure refactor — the DOM output is byte-identical.
type NavBtnProps = {
  item: NavItem;
  activeTab: string;
  onTabChange: (tab: string) => void;
  isMobile?: boolean;
  onItemClick?: () => void;
};
const NavBtn = memo(({ item, activeTab, onTabChange, isMobile = false, onItemClick }: NavBtnProps) => {
  const isActive = activeTab === item.id;
  return (
    <button
      key={item.id}
      data-nav-id={item.id}
      onClick={() => { onTabChange(item.id); onItemClick?.(); }}
      className={`w-full flex items-center gap-3 px-3 ${isMobile ? "py-2.5" : "py-2"} rounded-lg text-sm font-medium transition-colors ${
        isActive
          ? "bg-orange-600 text-white"
          : "hover:bg-secondary text-foreground"
      }`}
    >
      {item.icon ? <item.icon className="w-5 h-5 shrink-0" /> : <EmojiIcon emoji={item.emoji} size="w-5 h-5" />}
      <span className={isActive ? "text-white" : "text-muted-foreground"}>{item.label}</span>
    </button>
  );
});
NavBtn.displayName = "NavBtn";

type SectionedNavProps = {
  activeTab: string;
  onTabChange: (tab: string) => void;
  isMobile?: boolean;
  onItemClick?: () => void;
  query?: string;
};
// ── PERFORMANCE: SectionedNav memoizes the filtered sections array so the
// .map().filter() work only re-runs when `query` actually changes — not on
// every parent render.
const SectionedNav = memo(({ activeTab, onTabChange, isMobile = false, onItemClick, query = "" }: SectionedNavProps) => {
  const filteredSections = useMemo(() =>
    navSections
      .map(section => ({
        ...section,
        items: section.items.filter(item => matchesQuery(item, query)),
      }))
      .filter(section => section.items.length > 0),
    [query]
  );

  if (query.trim() && filteredSections.length === 0) {
    return (
      <p className="text-sm text-muted-foreground text-center py-6">
        No matches for "{query.trim()}"
      </p>
    );
  }

  return (
    <div className="space-y-4">
      {filteredSections.map(section => (
        <div key={section.heading}>
          <p className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase px-3 mb-1.5">
            {section.heading}
          </p>
          <div className="space-y-0.5">
            {section.items.map(item => <NavBtn key={item.id} item={item} activeTab={activeTab} onTabChange={onTabChange} isMobile={isMobile} onItemClick={onItemClick} />)}
          </div>
        </div>
      ))}
    </div>
  );
});
SectionedNav.displayName = "SectionedNav";

interface DashboardLayoutProps {
  activeTab: string;
  onTabChange: (tab: string) => void;
  children: React.ReactNode;
}

const DashboardLayout = ({ activeTab, onTabChange, children }: DashboardLayoutProps) => {
  const { profile, signOut } = useAuth();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [desktopQuery, setDesktopQuery] = useState("");
  const [mobileQuery, setMobileQuery] = useState("");
  const navigate = useNavigate();
  const { isDark, toggle } = useDarkMode();
  const mobileNavRef = useRef<HTMLElement>(null);

  // Image error states for handling broken Cloudinary URLs
  const [sidebarImgError, setSidebarImgError] = useState(false);
  const [headerImgError, setHeaderImgError] = useState(false);

  // Validate avatar URL to detect broken Cloudinary links early
  const validatedAvatarUrl = safeCloudinaryUrl(profile?.avatar_url);

  // The dashboard scrolls at the window/document level (the sidebar is
  // sticky, not the content pane), so switching features must reset the
  // window scroll position — otherwise the new feature renders while the
  // page stays scrolled wherever the previous feature left it.
  useEffect(() => {
    window.scrollTo({ top: 0, left: 0, behavior: "auto" });
  }, [activeTab]);

  // When the mobile "Menu" slide-out opens, it always renders scrolled to
  // the top of the list — so if the active feature is further down (e.g.
  // "Online Classes" or below), it's hidden off-screen and looks like
  // nothing is selected. Scroll the highlighted item into view instead.
  useEffect(() => {
    if (!sidebarOpen) return;
    const el = mobileNavRef.current?.querySelector(`[data-nav-id="${activeTab}"]`);
    if (el) {
      // Run after the panel has actually painted/opened.
      requestAnimationFrame(() => {
        el.scrollIntoView({ block: "center" });
      });
    }
  }, [sidebarOpen, activeTab]);

  const handleSignOut = async () => { await signOut(); navigate("/"); };
  // Wrap onTabChange so we also close the mobile sidebar whenever a nav
  // button is tapped. This mirrors the previous handleTabChange behavior
  // but is passed down to the memoized NavBtn via props.
  const handleTabChange = (tabId: string) => { onTabChange(tabId); setSidebarOpen(false); };

  const initials = profile?.full_name
    ?.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase() || "U";

  // Bottom bar: first 4 items from all nav + Website + More
  const bottomBarItems = allNavItems.slice(0, 4);

  return (
    <div className="min-h-screen bg-background flex">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-64 bg-card border-r border-border shrink-0 sticky top-0 h-screen">
        <div className="p-4 border-b border-border">
          <Link to="/" className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg overflow-hidden shrink-0 ring-1 ring-border">
              <img src="/icon-512.png" alt="GHS Babi Khel logo" className="w-full h-full object-cover" />
            </div>
            <div>
              <span className="font-heading font-bold text-foreground text-sm leading-tight block">GHS Babi Khel</span>
              <p className="text-[10px] text-muted-foreground font-medium leading-tight">User Panel</p>
            </div>
          </Link>
        </div>

        <div className="p-4 border-b border-border">
          <div className="flex items-center gap-3">
            {/* Sidebar avatar with error handling */}
            {validatedAvatarUrl && !sidebarImgError ? (
              <img 
                src={validatedAvatarUrl} 
                alt={`${profile.full_name}'s avatar`} 
                className="w-10 h-10 rounded-full object-cover" 
                loading="lazy" 
                onError={() => setSidebarImgError(true)}
              />
            ) : (
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold">
                {initials}
              </div>
            )}
            <div className="min-w-0">
              <p className="text-sm font-semibold text-foreground truncate">{profile?.full_name || "User"}</p>
              <span className="inline-block text-xs font-medium capitalize bg-primary/10 text-primary px-2 py-0.5 rounded-full mt-0.5">
                {profile?.role || "user"}
              </span>
            </div>
          </div>
        </div>

        {/* Desktop Search — filters the menu below in place, never opens a popup */}
        <div className="px-3 py-2 border-b border-border">
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
            <input
              type="text"
              value={desktopQuery}
              onChange={(e) => setDesktopQuery(e.target.value)}
              placeholder="Search anything..."
              className="w-full pl-8 pr-2 py-1.5 text-xs rounded-lg bg-secondary border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/40 transition-colors"
            />
          </div>
        </div>

        <nav className="flex-1 p-3 overflow-y-auto">
          <SectionedNav activeTab={activeTab} onTabChange={handleTabChange} query={desktopQuery} />
        </nav>

        <div className="p-3 border-t border-border space-y-1">
          <Link to="/" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors">
            <ExternalLink className="w-4 h-4" /> Main Website
          </Link>
          {profile?.role === "admin" && (
            <Link to="/admin" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-warning hover:bg-warning/10 transition-colors">
              <Shield className="w-4 h-4" /> Admin Panel
            </Link>
          )}
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors">
            <LogOut className="w-4 h-4" /> Sign Out
          </button>
        </div>
      </aside>

      {/* Main */}
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-40 h-14 bg-card border-b border-border flex items-center px-4 gap-3">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-secondary text-foreground">
            <Menu className="w-5 h-5" />
          </button>
          <h1 className="font-heading font-semibold text-foreground capitalize">
            {allNavItems.find(item => item.id === activeTab)?.label || "Dashboard"}
          </h1>
          <div className="ml-auto flex items-center gap-2">
            <NotificationBell />
            <button onClick={toggle} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground transition-colors" title={isDark ? "Switch to light mode" : "Switch to dark mode"}>
              {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>
            {/* Header avatar with error handling */}
            <button onClick={() => onTabChange("profile")} className="p-1">
              {validatedAvatarUrl && !headerImgError ? (
                <img 
                  src={validatedAvatarUrl} 
                  alt={`${profile.full_name}'s avatar`} 
                  className="w-8 h-8 rounded-full object-cover" 
                  onError={() => setHeaderImgError(true)}
                />
              ) : (
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold">
                  {initials}
                </div>
              )}
            </button>
          </div>
        </header>

        <main className="flex-1 p-4 md:p-6 pb-20 lg:pb-6">{children}</main>
      </div>

      {/* Mobile bottom bar */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-card border-t border-border">
        <div className="flex items-center justify-around py-1">
          {bottomBarItems.map(item => (
            <button
              key={item.id}
              onClick={() => handleTabChange(item.id)}
              className={`flex flex-col items-center gap-0.5 p-2 min-w-[3rem] ${
                item.id === "merit-list"
                  ? "text-foreground"
                  : activeTab === item.id ? "text-accent" : "text-muted-foreground"
              }`}
            >
              {item.icon ? <item.icon className="w-5 h-5" /> : <span className="text-lg leading-none">{item.emoji}</span>}
              <span className="text-[10px] font-medium">{item.label}</span>
            </button>
          ))}
          <Link to="/" className="flex flex-col items-center gap-0.5 p-2 min-w-[3rem] text-foreground">
            <ExternalLink className="w-5 h-5" />
            <span className="text-[10px] font-medium">Website</span>
          </Link>
          <button onClick={() => setSidebarOpen(true)} className="flex flex-col items-center gap-0.5 p-2 min-w-[3rem] text-muted-foreground">
            <Menu className="w-5 h-5" />
            <span className="text-[10px] font-medium">More</span>
          </button>
        </div>
      </div>

      {/* Mobile sidebar overlay */}
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          <div className="absolute inset-0 bg-foreground/50" onClick={() => setSidebarOpen(false)} />
          <div className="relative w-72 bg-card h-full shadow-elevated flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 ring-1 ring-border">
                  <img src="/icon-512.png" alt="GHS Babi Khel logo" className="w-full h-full object-cover" />
                </div>
                <div>
                  <span className="font-heading font-bold text-foreground text-sm leading-tight block">GHS Babi Khel</span>
                  <p className="text-[10px] text-muted-foreground font-medium leading-tight">User Panel</p>
                </div>
              </div>
              <button onClick={() => setSidebarOpen(false)} className="p-2 rounded-lg hover:bg-secondary">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="px-3 py-2 border-b border-border">
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
                <input
                  type="text"
                  value={mobileQuery}
                  onChange={(e) => setMobileQuery(e.target.value)}
                  placeholder="Search anything..."
                  className="w-full pl-8 pr-2 py-1.5 text-xs rounded-lg bg-secondary border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/40 transition-colors"
                />
              </div>
            </div>
            <nav ref={mobileNavRef} className="flex-1 p-3 overflow-y-auto">
              <SectionedNav activeTab={activeTab} onTabChange={handleTabChange} isMobile onItemClick={() => setSidebarOpen(false)} query={mobileQuery} />
            </nav>
            <div className="p-3 border-t border-border space-y-1">
              <Link to="/" onClick={() => setSidebarOpen(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-secondary">
                <ExternalLink className="w-4 h-4" /> Main Website
              </Link>
              <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-destructive hover:bg-destructive/10">
                <LogOut className="w-4 h-4" /> Sign Out
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default DashboardLayout;
