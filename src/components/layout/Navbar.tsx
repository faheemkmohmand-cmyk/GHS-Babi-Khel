...
/* ── Mobile accordion link row (indented inside its group) ────────────────── */
function MobileLinkRow({ link, active, onNavigate }: { link: NavLinkItem; active: boolean; onNavigate: () => void }) {
  ...
  return (
    ...
    >
      <span
        className={`w-7 h-7 rounded-lg bg-gradient-to-br ${link.tile} text-white flex items-center justify-center shrink-0 ${active ? "opacity-95" : "opacity-90"}`}
        aria-hidden="true"
      >
...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* ════════════ MOBILE MENU — grouped accordion sections ════════════ */}
      <AnimatePresence>
        {open && (
          <motion.div
            id="mobile-menu"
          ...
          >
            <div className="p-3 pb-6">
              ...
              >
                <Home
                  className={`w-[18px] h-[18px] shrink-0 ${location.pathname === "/" ? "opacity-90" : "text-muted-foreground"}`}
                  aria-hidden="true"
                />
                Home
              </Link>
                ...
                return (
                  <div key={section.id} className="mb-1">
                    <button
                      ...
                      aria-expanded={isOpen}
                      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold no-underline transition-colors ${
                        sectionActive
                    ...
                    >
                      <span
                        className={`w-8 h-8 rounded-lg bg-gradient-to-br ${section.tile} text-white flex items-center justify-center shrink-0 shadow-sm`}
                        aria-hidden="true"
                      >
                        <SectionIcon className="w-4 h-4" />
                      </span>
                      <span className="flex-1 text-left">{section.label}</span>
                      <span className="shrink-0 text-[10px] font-bold rounded-full bg-secondary px-2 py-0.5 text-muted-foreground">
                        {linkCount}
                      </span>
                      <ChevronDown
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <div className="pl-4 pr-1 pt-1 pb-1 space-y-0.5">
                            ...
                            {/* Results group: Roll No. Slip spotlight row with
                                the live countdown badge */}
                            {section.id === "results" && (
                              <MobileLinkRow
                                link={rollSlipItem}
                                active={location.pathname === rollSlipItem.to}
                                onNavigate={closeAllMenus}
                              />
                            )}
                          </div>
              ...
              >
                <ClipboardList
                  className={`w-[18px] h-[18px] shrink-0 ${location.pathname === "/admission" ? "opacity-90" : "text-muted-foreground"}`}
                  aria-hidden="true"
                />
                Admission
              </Link>
