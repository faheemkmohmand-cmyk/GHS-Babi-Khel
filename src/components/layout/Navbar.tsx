  ...
  return (
    ...
    >
      ...
      {/* ── Mobile bottom dock (X-style, global) ───────────────────────────
          5 evenly-spaced icon buttons fixed to the bottom of the viewport
          on screens < lg. Mirrors the X (Twitter) bottom bar: icons only,
          no labels, hides on scroll down, reveals on scroll up. Hidden on
          the chapter page because that page already has its own dock. */}
      {!hideGlobalBottomBar && (
        <div
        ...
        >
          <div className="flex items-stretch justify-around">
            ...
            {/* Search — opens the existing search drawer (same as nav Search icon) */}
            <button
              type="button"
              aria-label="Search"
              onClick={() => { closeAllMenus(); setSearchOpen((s) => !s); }}
