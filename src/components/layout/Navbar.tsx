...
const NAV_SECTIONS: NavSection[] = [
  {
    ...
    tile: "from-sky-600 to-indigo-700",
    links: [
      { to: "/about",   label: "About",   icon: Landmark,   desc: "History, mission & facilities",      tile: "from-sky-600 to-indigo-700" },
      { to: "/gallery", label: "Gallery", icon: Images,     desc: "Photos & videos of school life",     tile: "from-pink-600 to-rose-700" },
      { to: "/contact", label: "Contact", icon: Mail,       desc: "Address, phone & location map",      tile: "from-emerald-500 to-teal-600" },
      { to: "/faq",     label: "FAQs",    icon: HelpCircle, desc: "Answers to common questions",        tile: "from-amber-500 to-orange-700" },
    ],
  },
  {
    id: "news",
    ...
    tile: "from-orange-600 to-red-700",
    links: [
      { to: "/news",     label: "News",     icon: Newspaper,   desc: "Stories & latest updates",     tile: "from-orange-600 to-red-700" },
      { to: "/notices",  label: "Notices",  icon: Megaphone,   desc: "Official announcements",       tile: "from-violet-600 to-purple-800" },
      { to: "/calendar", label: "Calendar", icon: CalendarDays, desc: "Events & academic dates",     tile: "from-cyan-600 to-sky-700" },
    ],
  },
  {
    id: "results",
    ...
    tile: "from-gold to-amber-600",
    links: [
      { to: "/results",   label: "Results",   icon: Trophy, desc: "Exam results & DMCs",           tile: "from-gold to-amber-600" },
      { to: "/merit-list", label: "Merit List", icon: Medal, desc: "Toppers & position holders",   tile: "from-fuchsia-600 to-pink-700" },
    ],
  },
  {
    id: "academics",
    ...
    tile: "from-violet-600 to-purple-800",
    links: [
      { to: "/online-classes", label: "Online Classes", icon: Video,   desc: "Live & recorded lectures",         tile: "from-red-600 to-rose-700" },
      { to: "/notes",          label: "Notes",          icon: BookOpen, desc: "Study notes by class & subject",   tile: "from-green-600 to-emerald-700" },
      { to: "/library",        label: "Library",        icon: Library,  desc: "Books & reading resources",        tile: "from-indigo-600 to-blue-800" },
    ],
  },
];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={`w-9 h-9 shrink-0 rounded-xl bg-gradient-to-br ${link.tile} text-white flex items-center justify-center shadow-sm ring-1 ring-inset ring-white/25 transition-transform duration-200 group-hover:scale-110 group-hover:-rotate-3`}
          aria-hidden="true"
        >
...
/* ── Mobile accordion link row — mirrors the desktop MegaRow language:
      jewel tile, label + description, glowing gold active bar.
...
const MobileLinkRow = memo(function MobileLinkRow({ link, active, onNavigate }: { link: NavLinkItem; active: boolean; onNavigate: () => void }) {
  ...
  return (
    <Link
      to={link.to}
      onClick={onNavigate}
      className={`relative flex items-center gap-3 px-2.5 py-2.5 rounded-xl text-sm no-underline transition-colors duration-150 ${
        active ? "bg-gold/[0.10]" : "hover:bg-secondary active:bg-secondary/80"
      }`}
    >
      {active && (
        <span
          className="absolute left-0 top-2 bottom-2 w-1 rounded-full bg-gold shadow-[0_0_8px_hsl(var(--gold)/0.9)]"
          aria-hidden="true"
        />
      )}
      <span
        className={`w-8 h-8 rounded-lg bg-gradient-to-br ${link.tile} text-white flex items-center justify-center shrink-0 shadow-sm ring-1 ring-inset ring-white/25`}
        aria-hidden="true"
      >
...
const MobileDirectRow = memo(function MobileDirectRow({ to, label, icon: Icon, tile, active, onNavigate }: {
  to: string;
  label: string;
  icon: LucideIcon;
  tile: string;
  ...
  return (
    <Link
      to={to}
      onClick={onNavigate}
      className={`relative flex items-center gap-3 px-3 py-3 rounded-2xl text-sm font-bold no-underline transition-colors duration-150 ${
        active ? "bg-gold/[0.08] text-primary" : "text-foreground hover:bg-secondary active:bg-secondary/80"
      }`}
    >
      {active && (
        <span
      ...
      <span
        className={`w-10 h-10 rounded-[13px] bg-gradient-to-br ${tile} text-white flex items-center justify-center shrink-0 shadow-sm ${active ? "ring-2 ring-gold/45" : "ring-1 ring-inset ring-white/25"}`}
        aria-hidden="true"
      >
        <Icon className="w-[18px] h-[18px]" />
      </span>
...
/* ── Mobile accordion group — one of the four drop-downs.
  ...
  return (
    <div>
      <motion.button
        ...
        className={`relative w-full flex items-center gap-3 px-3 py-3 text-left no-underline touch-manipulation select-none ${
          sectionActive ? "bg-gold/[0.08]" : "active:bg-secondary/70"
        }`}
      >
        {sectionActive && (
          <span
        ...
        <span
          className={`w-10 h-10 rounded-[13px] bg-gradient-to-br ${section.tile} text-white flex items-center justify-center shrink-0 shadow-sm ${sectionActive ? "ring-2 ring-gold/45" : "ring-1 ring-inset ring-white/25"}`}
          aria-hidden="true"
        >
        ...
        <motion.span
          ...
          className={`shrink-0 w-7 h-7 rounded-full border flex items-center justify-center ${
            isOpen ? "border-gold/60 text-gold bg-gold/10" : "border-border text-muted-foreground"
          }`}
        >
        ...
        {isOpen && (
          <motion.div
            ...
            animate={{
              ...
              opacity: 1,
              transition: {
                height: { duration: 0.34, ease: [0.32, 0.72, 0, 1] },
            ...
            exit={{
              ...
              opacity: 0,
              transition: {
                height: { duration: 0.24, ease: [0.55, 0, 0.55, 0.2] },
          ...
          >
            <div ref={panelRef} className="px-2 pt-0.5 pb-2.5">
              <div className="rounded-2xl border border-border/50 bg-background/80 p-1.5 shadow-sm">
                ...
                {section.id === "results" && (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-600 text-white flex items-center justify-center shrink-0 shadow-sm ring-1 ring-inset ring-white/25"
                        aria-hidden="true"
                      >
                        <Hash className="w-4 h-4" />
                      </span>
                      <span className="flex-1 min-w-0">
                        <span className={`block text-[13px] font-semibold leading-tight ${pathname === rollSlipItem.to ? "text-primary" : "text-foreground"}`}>
                          {rollSlipItem.label}
                        </span>
                        <span className="block text-[10.5px] text-muted-foreground leading-tight mt-0.5 truncate">
                          {rollSlipItem.desc}
                      ...
                      >
                        <div className="relative rounded-3xl border border-border bg-card/95 backdrop-blur-xl shadow-elevated overflow-hidden">
                          ...
                          <div className="relative px-5 pt-4 pb-3 flex items-center gap-3">
                            <span className={`w-9 h-9 rounded-xl bg-gradient-to-br ${section.tile} text-white flex items-center justify-center shadow-card ring-1 ring-inset ring-white/25`} aria-hidden="true">
                              <SectionIcon className="w-[18px] h-[18px]" />
                            </span>
        ...
        {open && (
          ...
          >
            <div className="p-3 pb-5 space-y-2.5" style={{ WebkitTapHighlightColor: "transparent" }}>
              ...
              {/* ── Grouped accordion sections — one editorial panel with
                     hairline dividers. Each group is a memoised component:
                     ...
                     — silky, never "hanging". ──────────────────────────── */}
              <div className="rounded-[22px] border border-border/60 bg-secondary/40 overflow-hidden divide-y divide-border/40">
                {NAV_SECTIONS.map((section) => (
              ...
              <MobileDirectRow
                ...
                label="Admission"
                icon={ClipboardList}
                tile="from-amber-500 to-orange-700"
