...
/* ── Nav model: every link carries its own icon so the mobile menu and
   the desktop "More" dropdown render rich, scannable rows. ── */
      ...
      <Link
        ...
        className={`lg:hidden flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold no-underline transition-colors ${
          live
            ...
            : "bg-gradient-to-r from-gold/20 via-gold/10 to-gold/20 text-foreground border-b border-gold/30"
        }`}
      >
        <Timer className={`w-3.5 h-3.5 shrink-0 ${live ? "" : "text-gold"}`} />
        <span className="truncate">
          {live ? (
            <>Roll No. Slips are <span className="underline underline-offset-2">LIVE</span> — tap to view</>
          ) : (
            <>Roll No. Slip in <span className="font-mono tabular-nums font-bold">{compactCountdown(diff)}</span></>
          )}
        </span>
        <span className={`w-1.5 h-1.5 rounded-full shrink-0 animate-pulse ${live ? "bg-white" : "bg-gold"}`} aria-hidden="true" />
      </Link>
    );
  }
          ...
          {moreLinks.length > 0 && (
            <div className="relative" ref={moreRef}>
              <button
                ...
                data-active={moreLinks.some((l) => l.to === location.pathname) || undefined}
                className={desktopLinkClass(moreLinks.some((l) => l.to === location.pathname))}
              >
                More
              ...
              <AnimatePresence>
                {moreOpen && (
                  <motion.div
                    ...
                    className="absolute top-full left-0 mt-2 min-w-[200px] rounded-2xl border border-border bg-card shadow-elevated py-2 z-50 overflow-hidden"
                  >
                    {/* gold hairline accent on the dropdown's top edge */}
                    <div className="absolute top-0 inset-x-5 h-px bg-gradient-to-r from-transparent via-gold/60 to-transparent" />
                    {moreLinks.map((link) => {
                      const active = location.pathname === link.to;
                      return (
                        <Link
                          ...
                          className={`group flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium transition-colors ${
                            active
                              ? "text-primary bg-accent/10"
                              : "text-foreground/85 hover:bg-secondary hover:text-foreground"
                          }`}
                        >
                          {/* dot placeholder keeps rows aligned */}
                          <span
                            className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors ${
                              active ? "bg-gold shadow-[0_0_6px_hsl(var(--gold)/0.9)]" : "bg-transparent"
                            }`}
                            aria-hidden="true"
                          />
                          <link.icon
                            className={`w-4 h-4 shrink-0 transition-colors ${
                              active ? "text-gold" : "text-muted-foreground group-hover:text-primary"
                            }`}
                            aria-hidden="true"
                          />
                          {link.label}
                          {link.to === "/roll-no-slip" && <RollSlipCountdown variant="menu-badge" />}
                        </Link>
                      );
                    })}
                  </motion.div>
        ...
        {open && (
          ...
          >
            <div className="p-3 pb-6">
              ...
              {/* ── Nav links (icon rows, gradient active pill) ── */}
              {navLinks.map((link) => {
                const active = location.pathname === link.to;
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-2.5 rounded-xl mb-1 text-sm font-medium no-underline transition-colors ${
                      active
                        ? "bg-gradient-to-r from-primary to-primary-glow text-primary-foreground shadow-card"
                        : "bg-transparent text-muted-foreground hover:bg-secondary hover:text-foreground"
                    }`}
                  >
                    <link.icon
                      className={`w-[18px] h-[18px] shrink-0 ${active ? "opacity-90" : "text-muted-foreground"}`}
                      aria-hidden="true"
                    />
                    {link.label}
                    {link.to === "/roll-no-slip" && <RollSlipCountdown variant="menu-badge" />}
                  ...
                  <div className="flex flex-col gap-1.5">
                    {isAdmin && (
                      <Link
                        to="/admin"
                        onClick={() => setOpen(false)}
                        className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-blue-500 text-white font-semibold text-sm no-underline"
                      >
                    ...
                    <Link
                      to="/dashboard"
                      onClick={() => setOpen(false)}
                      className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-orange-600 text-white font-semibold text-sm no-underline"
                    >
