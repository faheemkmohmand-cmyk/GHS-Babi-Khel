...
const NAV_SECTIONS: NavSection[] = [
  {
    ...
    tint: "bg-sky-100 text-sky-700 dark:bg-sky-400/10 dark:text-sky-300",
    links: [
      { to: "/about",   label: "About",   icon: Landmark,   desc: "History, mission & facilities",      tint: "bg-sky-100 text-sky-700 dark:bg-sky-400/10 dark:text-sky-300" },
      { to: "/gallery", label: "Gallery", icon: Images,     desc: "Photos & videos of school life",     tint: "bg-pink-100 text-pink-700 dark:bg-pink-400/10 dark:text-pink-300" },
      { to: "/contact", label: "Contact", icon: Mail,       desc: "Address, phone & location map",      tint: "bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300" },
      { to: "/faq",     label: "FAQs",    icon: HelpCircle, desc: "Answers to common questions",        tint: "bg-amber-100 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300" },
    ],
  },
  {
    id: "news",
    ...
    tint: "bg-orange-100 text-orange-700 dark:bg-orange-400/10 dark:text-orange-300",
    links: [
      { to: "/news",     label: "News",     icon: Newspaper,   desc: "Stories & latest updates",     tint: "bg-orange-100 text-orange-700 dark:bg-orange-400/10 dark:text-orange-300" },
      { to: "/notices",  label: "Notices",  icon: Megaphone,   desc: "Official announcements",       tint: "bg-rose-100 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300" },
      { to: "/calendar", label: "Calendar", icon: CalendarDays, desc: "Events & academic dates",     tint: "bg-cyan-100 text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-300" },
    ],
  },
  {
    id: "results",
    ...
    tint: "bg-amber-100 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300",
    links: [
      { to: "/results",   label: "Results",   icon: Trophy, desc: "Exam results & DMCs",           tint: "bg-amber-100 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300" },
      { to: "/merit-list", label: "Merit List", icon: Medal, desc: "Toppers & position holders",   tint: "bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-400/10 dark:text-fuchsia-300" },
    ],
  },
  {
    id: "academics",
    ...
    tint: "bg-violet-100 text-violet-700 dark:bg-violet-400/10 dark:text-violet-300",
    links: [
      { to: "/online-classes", label: "Online Classes", icon: Video,   desc: "Live & recorded lectures",         tint: "bg-indigo-100 text-indigo-700 dark:bg-indigo-400/10 dark:text-indigo-300" },
      { to: "/notes",          label: "Notes",          icon: BookOpen, desc: "Study notes by class & subject",   tint: "bg-green-100 text-green-700 dark:bg-green-400/10 dark:text-green-300" },
      { to: "/library",        label: "Library",        icon: Library,  desc: "Books & reading resources",        tint: "bg-blue-100 text-blue-700 dark:bg-blue-400/10 dark:text-blue-300" },
    ],
  },
];
...
const rollSlipItem: NavLinkItem = {
  ...
  desc: "Find, download & share your slip", tint: "bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300",
};
    ...
    >
      <Link
        ...
        className={`group relative flex items-center gap-2.5 rounded-xl px-2.5 py-2 no-underline transition-colors duration-150 ${
          active ? "bg-gold/[0.08]" : "hover:bg-secondary"
        }`}
      >
        {active && (
          <span
            className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-full bg-gold shadow-[0_0_8px_hsl(var(--gold)/0.9)]"
            aria-hidden="true"
          />
        )}
        <span
        ...
        <ArrowRight
          className="w-3.5 h-3.5 shrink-0 -translate-x-1 opacity-0 text-gold transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100"
          aria-hidden="true"
        />
      </Link>
    </motion.div>
  ...
  return (
    <Link
      ...
      className={`relative flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-sm no-underline transition-colors duration-150 ${
        active ? "bg-gold/[0.10]" : "hover:bg-secondary active:bg-secondary/80"
      }`}
    >
      {active && (
        <span
          className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-full bg-gold shadow-[0_0_8px_hsl(var(--gold)/0.9)]"
          aria-hidden="true"
        />
      )}
      <span
      ...
      <span className="flex-1 min-w-0">
        <span className={`block text-[12.5px] font-semibold leading-tight ${active ? "text-primary" : "text-foreground"}`}>
          {link.label}
        </span>
  ...
  return (
    <Link
      ...
      className={`relative flex items-center gap-3 px-3 py-2.5 rounded-2xl border bg-secondary/40 text-sm font-bold no-underline transition-colors duration-150 ${
        active ? "border-gold/40 bg-gold/[0.06] text-primary" : "border-border/50 text-foreground active:bg-secondary/70"
      }`}
    >
      {active && (
        <span
          className="absolute left-0 top-2 bottom-2 w-[3px] rounded-full bg-gold shadow-[0_0_8px_hsl(var(--gold)/0.9)]"
          aria-hidden="true"
        />
      )}
      <span
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden transition-colors duration-200 ${
        sectionActive ? "border-gold/35 bg-gold/[0.05]" : "border-border/50 bg-secondary/40"
      }`}
    >
      <motion.button
        type="button"
      ...
      >
        {sectionActive && (
          <span
            className="absolute left-0 top-2 bottom-2 w-[3px] rounded-full bg-gradient-to-b from-gold to-gold/30 shadow-[0_0_8px_hsl(var(--gold)/0.7)]"
            aria-hidden="true"
          />
        )}
        <span
        ...
        <span className="flex-1 min-w-0">
          <span className={`block text-[13.5px] font-bold leading-tight ${sectionActive ? "text-primary" : "text-foreground"}`}>
            {section.label}
          </span>
          <span className="block text-[10.5px] text-muted-foreground leading-tight mt-px truncate">
            {section.tagline}
        ...
        <motion.span
          ...
          className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-colors duration-200 ${
            isOpen ? "text-gold bg-gold/10" : "text-muted-foreground/60"
          }`}
        >
                ...
                {section.id === "results" && (
                  ...
                  >
                    <Link
                      ...
                      onClick={onNavigate}
                      className={`relative flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-sm no-underline transition-colors duration-150 ${
                        pathname === rollSlipItem.to ? "bg-gold/[0.10]" : "hover:bg-secondary active:bg-secondary/80"
                      }`}
                    >
                      ...
                      <span className="flex-1 min-w-0">
                        <span className={`block text-[12.5px] font-semibold leading-tight ${pathname === rollSlipItem.to ? "text-primary" : "text-foreground"}`}>
                          {rollSlipItem.label}
                        </span>
                        <span className="block text-[10px] text-muted-foreground leading-tight mt-px truncate">
                          {rollSlipItem.desc}
                  ...
                  <AnimatePresence>
                    {isOpen && (
                      <motion.div
                        ...
                        className="w-[min(92vw,320px)] max-w-[calc(100vw-2rem)]"
                      >
                        <div className="relative rounded-2xl border border-border bg-card/95 backdrop-blur-xl shadow-elevated overflow-hidden">
                          ...
                          <div className="absolute top-0 inset-x-5 h-px bg-gradient-to-r from-transparent via-gold/60 to-transparent" aria-hidden="true" />
                          <div className="orb orb-gold w-28 h-28 -top-12 -right-8 opacity-20 pointer-events-none" aria-hidden="true" />

                          {/* micro eyebrow — small gold icon + tagline, one whisper line */}
                          ...
                          {section.id === "results" && (
                            ...
                            >
                              <Link
                                ...
                                className="group relative flex items-center gap-2.5 overflow-hidden rounded-xl bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-600 text-white px-3 py-2.5 no-underline shadow-card hover:shadow-elevated transition-all duration-200"
                              >
                                <span
                                  ...
                                  aria-hidden="true"
                                />
                                <span className="relative w-9 h-9 rounded-[10px] bg-white/20 flex items-center justify-center shrink-0" aria-hidden="true">
                                  <Hash className="w-[18px] h-[18px]" />
                                </span>
                                <span className="relative flex-1 min-w-0">
                                  <span className="block text-[12.5px] font-bold leading-tight">{rollSlipItem.label}</span>
                                  <span className="block text-[10.5px] text-white/85 leading-tight mt-px truncate">{rollSlipItem.desc}</span>
                                </span>
                                <RollSlipCountdown variant="menu-badge" />
                                <ArrowRight className="relative w-3.5 h-3.5 shrink-0 transition-transform duration-200 group-hover:translate-x-1" aria-hidden="true" />
                              </Link>
                            </motion.div>
                          )}
                        </div>
          ...
          >
            <div className="p-3 pb-5 space-y-2.5" style={{ WebkitTapHighlightColor: "transparent" }}>
              ...
              <MobileDirectRow
                ...
                label="Home"
                icon={Home}
                tint="bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300"
