        ...
        {open && (
          ...
          >
            <div className="p-3 pb-4 space-y-2.5">
              ...
              <div className="rounded-3xl border border-border/70 bg-secondary/30 p-1.5 space-y-0.5">
                {NAV_SECTIONS.map((section) => {
                  const SectionIcon = section.icon;
                  const sectionActive =
                    section.links.some((l) => l.to === location.pathname) ||
                  ...
                  return (
                    <div key={section.id}>
                      <button
                        ...
                        aria-expanded={isOpen}
                        className={`w-full flex items-center gap-3 px-2.5 py-2.5 rounded-2xl text-sm no-underline transition-colors ${
                          ...
                          >
                            <div className="pl-2.5 pr-1 pt-0.5 pb-1.5">
                              ...
                              <div className="rounded-2xl bg-card border border-border/60 p-1.5 space-y-0.5 shadow-sm">
                                ...
                                {/* Results group: Roll No. Slip emerald spotlight
                                    with the live countdown badge — the same
                                    showpiece as the desktop panel */}
                                {section.id === "results" && (
                                  <Link
                                    to={rollSlipItem.to}
                                    onClick={closeAllMenus}
                                    className="group relative flex items-center gap-3 overflow-hidden rounded-xl bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-600 text-white px-3 py-2.5 mt-1 no-underline shadow-card"
                                  >
                                    <span
                                      className="absolute -right-5 -top-7 w-20 h-20 rounded-full bg-white/10 transition-transform duration-300 group-hover:scale-125"
                                      aria-hidden="true"
                                    />
                                    <span className="relative w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center shrink-0" aria-hidden="true">
                                      <Hash className="w-4 h-4" />
                                    </span>
                                    <span className="relative flex-1 min-w-0">
                                      <span className="block text-sm font-bold leading-tight">{rollSlipItem.label}</span>
                                      <span className="block text-[11px] text-white/85 leading-tight mt-0.5 truncate">{rollSlipItem.desc}</span>
                                    </span>
                                    <RollSlipCountdown variant="menu-badge" />
                                  </Link>
                                )}
