...
const CreateAnnotationPopup = ({
  ...
  // Computed position — starts at the raw selection position, then gets
  // clamped against the popup's *actual* rendered size once it's mounted.
  // The previous version clamped against a hardcoded 350px height guess,
  // which is shorter than the real popup (header + text preview + colors +
  // textarea + visibility buttons + save button), so on phones the bottom
  // of the popup — including the Save button — was getting pushed off
  // screen / barely visible.
  const [style, setStyle] = useState<{ left: number; top: number; maxHeight: number }>({
  ...
  useEffect(() => {
    ...
    // Prefer placing below the selection; if it doesn't fit below, place
    // above it instead; if it doesn't fit either way, just clamp to the
    // viewport and let the popup scroll internally.
    let top = position.y;
    ...
    // Re-run once on mount only — position is fixed for the popup's lifetime.
  ...
  return (
    <motion.div
      ...
      style={{ left: style.left, top: style.top, maxHeight: style.maxHeight }}
    >
