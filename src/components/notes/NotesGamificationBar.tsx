...
export default function NotesGamificationBar({
  ...
  // Full mode - expanded bar with all stats
  return (
    <div className={`space-y-3 ${className}`}>
      {/* Main Bar */}
      <div className="
        p-4 rounded-2xl bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 
        dark:from-indigo-950/40 dark:via-purple-950/40 dark:to-pink-950/40
        border border-indigo-100 dark:border-indigo-900/30 shadow-lg
      ">
        {/* Header Row */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-bold text-sm">Study Session</p>
              <p className="text-xs text-muted-foreground">{formatDuration(sessionDuration)} elapsed</p>
            </div>
          </div>
          
          <button
            onClick={() => setShowDetails(!showDetails)}
            className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-white/50 dark:bg-white/10 
                       hover:bg-white/80 dark:hover:bg-white/20 transition-colors text-sm font-medium"
          >
        ...
        <div className="grid grid-cols-4 gap-2">
          {/* Streak */}
          <StatPill
            icon={<Flame size={16} className="text-orange-500 fill-orange-500" />}
          ...
          {/* Session Points */}
          <StatPill
            icon={<Star size={16} className="text-green-500 fill-green-500" />}
        ...
        <div className="mt-3 pt-3 border-t border-white/20 dark:border-white/10">
          <div className="flex items-center gap-3">
            <ProgressRing progress={progressToNextRank} size={36} strokeWidth={3} />
            <div className="flex-1">
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-muted-foreground">Progress to next milestone</span>
                <span className="font-medium">{currentPoints} / {nextMilestone} pts</span>
              </div>
              <div className="h-2 bg-white/30 dark:bg-black/20 rounded-full overflow-hidden">
                <div 
            ...
            {onShowLeaderboard && (
              <button
                onClick={onShowLeaderboard}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground 
        ...
        {/* Expanded Details */}
        {showDetails && (
          <div className="mt-3 pt-3 border-t border-white/20 dark:border-white/10 space-y-3 animate-in slide-in-from-top-2">
            {/* Badges Earned This Session placeholder */}
            <div className="flex items-center gap-2 p-2 rounded-lg bg-white/30 dark:bg-black/10">
              ...
              <div className="flex -space-x-1">
                {(gamification?.badges || []).slice(0, 4).map((badge: string, i: number) => (
                  <div key={i} className="w-6 h-6 rounded-full bg-yellow-100 dark:bg-yellow-900/50 
                                      flex items-center justify-center text-xs border-2 border-background">
                    🏆
                  </div>
                ))}
              </div>
            ...
            <div className="flex gap-2">
              <button 
                onClick={() => triggerBadgeNotification('test', 'Test Badge', '🎉')}
                className="flex-1 flex items-center justify-center gap-1 py-2 rounded-lg bg-white/50 
