            ...
            {hasBar ? (
              ...
              {s.barPct != null && (
                <div className="flex-1 min-w-[28px] bg-secondary rounded-full h-1 overflow-hidden">
                  <motion.div
                    initial={{ scaleX: 0 }}
                    animate={visible ? { scaleX: Math.max(0, Math.min(100, s.barPct)) / 100 } : {}}
                    transition={{ delay: animate ? 0.85 + i * 0.045 : 0, duration: 0.55, ease: "easeOut" }}
                    style={{ transformOrigin: "left center", willChange: "transform" }}
                    className={`h-full w-full rounded-full ${s.barFail ? "bg-gradient-to-r from-red-500 to-red-400" : "bg-gradient-to-r from-blue-600 to-sky-400"}`}
                  />
                </div>
              )}
