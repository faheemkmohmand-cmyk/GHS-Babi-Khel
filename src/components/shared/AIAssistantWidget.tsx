...
const AIAssistantWidget = () => {
  ...
  const runStream = useCallback(
    async (base: ChatMessage[], retryDepth = 0) => {
      ...
      // `charsPerTick`: 1 → ~100 chars/s — visibly typed, Claude-style.
  ...
  return (
    <>
      ...
      <m.button
        ...
        whileTap={{ scale: 0.94 }}
        className="fixed right-5 sm:right-6 z-[60] w-[48px] h-[48px] flex items-center justify-center text-orange-600 bottom-[calc(4.75rem+env(safe-area-inset-bottom,0px))] lg:bottom-[calc(1.5rem+env(safe-area-inset-bottom,0px))]"
      >
      ...
      <AnimatePresence>
        {open && (
          <m.div
            ...
            className="fixed right-3 sm:right-6 z-[60] w-[calc(100vw-1.5rem)] max-w-md h-[34rem] max-h-[78vh] bg-[#FAFAF8] dark:bg-[#1A1918] border border-black/[0.08] dark:border-white/[0.08] rounded-2xl shadow-2xl flex flex-col overflow-hidden bottom-[calc(7rem+env(safe-area-inset-bottom,0px))] lg:bottom-24"
          >
