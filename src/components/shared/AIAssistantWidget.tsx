...
import { getInstantAnswer } from "./aiInstantAnswers";
...
interface ChatMessage {
  ...
  streaming?: boolean;
  // `instant` = true when the answer came from the LOCAL knowledge base
  // (zero network requests). Rendered with a small ⚡ badge.
  instant?: boolean;
}

const API_ENDPOINT = "/api/ai-chat";
// Hard client-side cap. The server's own budget is 9s and it ALWAYS ends its
// stream (done or error) within that, so 25s here is a generous last-resort
// safety net for catastrophic cases (dead network, platform outage). The old
// 60s value meant a broken endpoint looked like an infinite hang.
...
const AIAssistantWidget = () => {
  ...
  const runStream = useCallback(
    async (base: ChatMessage[], instantAnswer?: string) => {
      const nextMessages: ChatMessage[] = [
        ...base,
        // Pre-add an empty assistant message that we'll fill as tokens arrive.
        { role: "assistant", content: "", streaming: true, instant: !!instantAnswer },
      ];
      ...
      const CHAR_INTERVAL_MS = 10; // one tick per reveal step
      ...
      // `charsPerTick`: 1 for streamed LLM answers (~100 chars/s — visibly
      // typed), 2 for instant local answers (~200 chars/s — snappy but still
      // animated; the whole answer is already in memory so we just reveal it
      // a touch faster).
      const startRevealTimer = (charsPerTick = 1) => {
        if (revealTimer) return;
        revealTimer = setInterval(() => {
          if (revealQueue.length === 0) return;
          const chunk = revealQueue.slice(0, Math.max(1, charsPerTick));
      ...
      try {
        // ── INSTANT PATH (Layer 1) ──────────────────────────────────
        // The local knowledge base matched this question: answer with ZERO
        // network requests. The match itself takes microseconds (see
        // aiInstantAnswers.ts) — this is the "nano-second answer" tier.
        // No server, no model, no outage can affect it.
        if (instantAnswer) {
          // First token is already available — skip the "Thinking…" state.
          waitingRef.current = false;
          setWaitingFirstToken(false);
          revealQueue = instantAnswer;
          startRevealTimer(2);
          // Let the reveal queue drain naturally (same contract as streams).
        ...
        // ── STREAMED PATH (Layer 2) ─ /api/ai-chat (Node runtime) ───
        ...
        if (err?.name === "AbortError") {
          if (abortKindRef.current === "panel") {
            ...
            setError(
              "The AI didn't respond in time. Send your question again — common questions are answered instantly."
            );
          }
        } else {
  ...
  const sendMessage = useCallback(
    (text: string) => {
      ...
      // Layer 1: known question → instant local answer (0 network).
      // Layer 2: anything else → streamed answer from /api/ai-chat.
      const instant = getInstantAnswer(trimmed);
      void runStream(base, instant?.answer);
    },
  ...
  const saveEdit = useCallback(() => {
    ...
    setEditingIdx(null);
    setEditDraft("");
    const instant = getInstantAnswer(draft);
    void runStream(base, instant?.answer);
                ...
                >
                  {msg.role === "user" ? (
                    ...
                    <div className="max-w-[92%] rounded-2xl rounded-bl-sm px-3.5 py-2.5 text-sm leading-relaxed bg-white dark:bg-white/[0.04] border border-black/[0.08] dark:border-white/[0.08] text-[#3D3B37] dark:text-[#E4E2DD] min-w-[52px]">
                      {waitingFirstToken && !msg.content ? (
                        ...
                        <span className="whitespace-pre-wrap break-words">
                          {msg.instant && (
                            <span className="mr-1.5 inline-flex items-center gap-0.5 align-middle text-[10px] font-semibold uppercase tracking-wide text-orange-600/90 dark:text-orange-400/90">
                              ⚡ Instant
                            </span>
                          )}
                          {msg.content}
                    ...
                    <div className="max-w-[92%]">
                      {msg.instant && (
                        <span className="mb-1 ml-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-orange-600/90 dark:text-orange-400/90">
                          ⚡ Instant answer
                        </span>
                      )}
                      <div
