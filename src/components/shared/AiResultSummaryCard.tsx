...
//   the summary LIVE, appending each chunk to the screen the instant it
//   arrives (no artificial per-character delay) — feels fast and
//   professional rather than a slow typewriter.
//
...
const AiResultSummaryCard = ({ result, fetchKey }: AiResultSummaryCardProps) => {
  ...
  useEffect(() => {
    ...
    // ── Instant reveal — no artificial per-character delay ─────────────────
    // Each streamed chunk from the server is appended to the visible text
    // the moment it arrives, so the summary appears as fast as the network
    // delivers it (feels instant/professional rather than a slow typewriter).
    const appendText = (text: string) => {
      if (cancelled) return;
      setState((s) => ({ ...s, started: true, modelText: s.modelText + text }));
    };
      ...
      try {
        ...
        for await (const evt of parseSseStream(res.body, controller.signal)) {
          ...
          if (typeof evt?.token === "string" && evt.token.length > 0) {
            appendText(evt.token);
          } else if (evt?.error && typeof evt.error === "string") {
            // Show error only if nothing rendered at all.
            setState((s) => ({
              ...s,
              streaming: false,
