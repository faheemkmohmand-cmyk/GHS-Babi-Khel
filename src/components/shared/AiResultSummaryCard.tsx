  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // ── Consume the SSE stream, word by word ───────────────────────────
        // The server emits `{ token: "word " }` events — each one is a
        // complete word (with trailing whitespace/punctuation). We append
        // each chunk to modelText; the bubble re-renders with each update
        // so the visitor sees words appear one-by-one.
