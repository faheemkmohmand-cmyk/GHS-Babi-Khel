...
// ─── SAFE FIELD HELPERS ───────────────────────────────────────────────────
// A single honor_roll row with a NULL student_name or month used to throw
// right here ("Cannot read properties of null (reading '0')" /
// ".slice of undefined") and take the whole homepage down into the route
// error screen — but only for visitors whose RLS role can actually see
// honor_roll rows (signed-in users), which is why it looked like a random
// crash "after a few seconds" while anonymous testing always passed.
const safeName = (entry: any): string =>
  typeof entry?.student_name === "string" && entry.student_name.trim().length > 0
    ? entry.student_name
    : "Student";
const safeInitial = (entry: any): string => safeName(entry).trim()[0] ?? "S";
const safeMonthLabel = (entry: any): string =>
  MONTHS[(Number(entry?.month) || 1) - 1] ?? "";
          ...
          <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full p-1.5 bg-white/20 backdrop-blur-sm shadow-xl">
            {entry.photo_url ? (
              ...
              <div className="w-full h-full rounded-full bg-gradient-to-br from-white/30 to-white/10 flex items-center justify-center backdrop-blur-sm border-2 border-white/20">
                <span className="text-5xl font-bold">{safeInitial(entry)}</span>
              </div>
            )}
          </div>
          {/* Floating Trophy */}
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex items-center justify-center md:justify-start gap-3 mb-3 flex-wrap">
            ...
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-white/15 backdrop-blur-sm">
              ...
              {safeMonthLabel(entry)} {entry.year}
            </span>
          </div>
          ...
          return (
            <div key={entry.id} className="flex flex-col items-center gap-2 flex-1 max-w-[120px]">
              ...
              <div className="flex flex-col items-center">
                ...
                <p className={`text-xs font-bold mt-1.5 ${textColor} text-center truncate w-full px-1`}>{safeName(entry)}</p>
      ...
      <div className={`${classColor.bg} rounded-xl p-4 h-full`}>
        ...
        <div className="flex justify-center -mt-5 mb-3">
          <div className={`px-4 py-1.5 bg-gradient-to-r ${classColor.gradient} text-white text-xs font-bold rounded-full shadow-md flex items-center gap-1.5`}>
            ...
            {safeMonthLabel(entry).slice(0, 3)} {entry.year}
          </div>
        </div>
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div className="flex-1 min-w-0 pt-1">
            <h4 className="font-bold text-foreground text-sm truncate">{safeName(entry)}</h4>
...
export function HonorRollHomeSection() {
  // Fetch ALL published honor roll entries (not just current month)
  // This ensures any added honor roll student shows up on homepage
  const { data: fetchedEntries = [], isLoading } = useHonorRoll(); // No year/month filter = get all published

  // Drop rows that cannot be rendered safely (missing/null student_name,
  // non-numeric month) instead of letting one bad row crash the homepage.
  // Admin-entered rows occasionally miss these fields — one such row used
  // to throw the ENTIRE route into the error screen for signed-in visitors.
  const allEntries = (fetchedEntries as any[]).filter(
    (e) =>
      e &&
      typeof e.student_name === "string" &&
      e.student_name.trim().length > 0 &&
      Number.isFinite(Number(e.month))
  );

  const [isVisible, setIsVisible] = useState(false);

  // Current date for stats footer display
  const now = new Date();
  ...
  // 🔥 FIX: If NO honor roll entries exist at all, DON'T show the section
  // Return null = section is completely hidden/skipped on homepage
