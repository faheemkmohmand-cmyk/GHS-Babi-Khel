...
const NewsTicker = () => {
  ...
  const items = useMemo<HeadlineItem[]>(() => {
    ...
    // same source /results uses — not a generic placeholder).
    if (hasSchoolResults) {
      list.push({
        id: "results-live",
    ...
    } else if (bisepMeta?.is_live && bisepMeta.exam_title) {
      list.push({
        id: "results-bisep",
        text: `BISE Peshawar ${bisepMeta.exam_title} Results Are Live — Search Your Roll Number`,
        to: "/results",
      });
    } else if (bisepMeta?.exam_title) {
      list.push({
        id: "results-bisep-upcoming",
        text: `BISE Peshawar ${bisepMeta.exam_title} — Check Result Status`,
        to: "/results",
      });
    }
