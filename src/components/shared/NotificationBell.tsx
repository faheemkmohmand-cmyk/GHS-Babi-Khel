...
import {
  ...
  Mail, MessageSquareText, Flag, LayoutGrid,
} from "lucide-react";
...
const NotificationBell = ({ variant = "header" }: NotificationBellProps = {}) => {
  ...
  return (
    <div
      ref={ref}
      className={variant === "bottom-bar" ? "relative flex-1 flex" : "relative"}
    >
      <button
        onClick={() => setOpen(!open)}
        className={
          variant === "bottom-bar"
            ? "flex-1 flex flex-col items-center justify-center py-2.5 relative text-muted-foreground"
            : "p-2 rounded-lg hover:bg-secondary text-muted-foreground relative transition-colors"
        }
        aria-label={`Notifications${unreadCount > 0 ? `, ${unreadCount} unread` : ""}`}
      >
        <Bell
          className={`w-5 h-5 ${pulse ? "animate-bounce text-primary" : ""} transition-colors`}
          fill={variant === "bottom-bar" && open ? "currentColor" : "none"}
        />
        {unreadCount > 0 && (
          <>
      ...
      <AnimatePresence>
        {open && (
          <m.div
            ...
            className={
              variant === "bottom-bar"
                ? // Bottom-dock variant: the panel lives inside the dock, which
                  // carries a transform (hide-on-scroll translate), so "fixed"
                  // coordinates here are relative to the DOCK box — which spans
                  // the full viewport width at the bottom of the screen. Anchor
                  // the panel just ABOVE the dock (bottom: 100% + 8px gap) the
                  // same way X (Twitter) opens its sheet upward from the bar.
                  "fixed left-2 right-2 bottom-[calc(100%+8px)] bg-card border border-border rounded-xl shadow-elevated z-50 overflow-hidden"
                : "fixed inset-x-3 top-16 sm:absolute sm:inset-x-auto sm:right-0 sm:top-full sm:mt-2 sm:w-96 bg-card border border-border rounded-xl shadow-elevated z-50 overflow-hidden"
            }
          >
            {/* ── Header ── */}
            ...
            {latest.length > 0 && (
              <button
                onClick={() => {
                  setOpen(false);
                  // FIXED: was hardcoded to "/dashboard" (= Overview tab) for
                  // every user, admin or student. Students now go to their
                  // dedicated Notifications tab. Admins don't have a
                  // notifications tab in AdminDashboard's tabMap, so they go
                  // to Admin Overview (unchanged behavior for admins only).
                  navigate(profile?.role === "admin" ? "/admin" : "/dashboard?tab=notifications");
                }}
