...
export const Captcha: React.FC<CaptchaProps> = ({
  ...
  // propagated yet. Previously handleOptionClick called
  // setUserAnswer(option) then setTimeout(verifyAnswer, 100) — but that
  // captured the verifyAnswer closure with the OLD userAnswer value
  // (React state updates are async), so a fast tap could verify against
  // an empty/stale answer and show "Please complete the security
  // verification" even though the correct option was clicked.
    ...
    setTimeout(() => {
      ...
      if (isCorrect && !isSuspicious) {
        ...
        if (newAttempts >= 3) {
          ...
          onError?.('Too many incorrect attempts. New challenge generated.');
          generateNewChallenge();
        } else if (isSuspicious) {
          onError?.('Suspicious activity detected. Please try again.');
          generateNewChallenge();
        } else {
          onError?.(`Incorrect. ${3 - newAttempts} attempts remaining.`);
          setUserAnswer('');
  ...
  }, [challenge, userAnswer, attempts, token, onVerify, onError, checkBotBehavior, generateNewChallenge]);
  ...
  return (
    <div 
      ref={containerRef}
      className={`captcha-container bg-card border-2 border-border rounded-xl p-4 shadow-sm ${className}`}
          ...
          {challenge.options ? (
            <div className="grid grid-cols-2 gap-2">
              {challenge.options.map((option, index) => (
                <button
                  ...
                  disabled={isLoading}
                  className="p-3 border-2 border-border rounded-lg hover:border-primary hover:bg-primary/5 
                           transition-all text-sm font-mono font-semibold disabled:opacity-50"
                >
                  {option}
                </button>
              ))}
            ...
            <div className="space-y-2">
              <input
                ...
                autoCapitalize="off"
                spellCheck={false}
                className="w-full px-4 py-3 border-2 border-border rounded-lg text-center text-lg font-mono
