 ...
 *
 * This function keeps ONE row per (student_id, subject, exam_date) group,
 * preferring:
...
export function dedupeAttendanceRows(rows: ExamAttendanceRecord[]): ExamAttendanceRecord[] {
  ...
  for (const r of rows) {
    const key = `${r.session_id}|${r.student_id}|${r.subject}|${(r.exam_date || "").slice(0, 10)}`;
