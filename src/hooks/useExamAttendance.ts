...
export function useExamAttendanceOverview(sessionId: string | undefined, cls: string | undefined) {
  return useQuery<ExamAttendanceRecord[]>({
    queryKey: ["exam-attendance-overview", cls],
    queryFn: async () => {
      if (!cls) return [];
      const { data, error } = await supabase
        .from("exam_attendance")
...
export function useDeleteAttendanceCell() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, vars) => {
      ...
      qc.invalidateQueries({ queryKey: ["exam-attendance-overview", vars.cls] });
...
export function useDeleteStudentAttendance() {
  ...
  return useMutation({
    ...
    onError: (err: any) => {
      const msg = err?.message || "Failed to delete student records";
      toast.error(msg);
...
export function useUpdateExamAttendance() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      // NO window guard — corrections are allowed after paper ends.
