 ...
 *
 * FILTERING (rev. 3): we now filter by `session_id` (strict UUID match) AND
 * subject partial match (case-insensitive). This fixes the bug where the Live
 * Console would show non-zero Present/Absent/Leave counts BEFORE the admin
 * had taken any attendance — caused by stale rows from previous sessions
 * bleeding in, or by rows from a different paper earlier the same day.
 *
 * Subject matching is intentionally loose: the seating plan's `paper_subject`
 * is free-text (e.g. "Mathematics — Paper 1"), while the attendance row's
 * `subject` is from a fixed dropdown (e.g. "Mathematics"). We treat a row as
 * a match if EITHER string contains the other (case-insensitive). This way:
 *   - "Mathematics" row ↔ "Mathematics — Paper 1" plan     → MATCH ✓
 *   - "Mathematics" row ↔ "Physics — Paper 1" plan         → no match ✗
 *   - "English"      row ↔ "English Literature" plan       → MATCH ✓
 *
 * If `sessionId` or `paperSubject` is not provided, we fall back to the
 * legacy behavior (filter by `exam_date` only) so the hook still works for
...
export function useLiveAttendance(
  ...
  const query = useQuery<{
    ...
    queryFn: async () => {
      if (!examDate) return [];
      // Build the base query — always filter by exam_date.
      let q = supabase
        .from("exam_attendance")
        .select("*")
        .eq("exam_date", examDate);
      // Add session_id filter if provided (strict UUID match).
      if (sessionId) q = q.eq("session_id", sessionId);
      const { data, error } = await q;
      if (error) throw error;
      let rows = data ?? [];

      // Loose subject match (client-side) — only if paperSubject is provided.
      // Match if EITHER string contains the other (case-insensitive, trimmed).
      if (paperSubject) {
