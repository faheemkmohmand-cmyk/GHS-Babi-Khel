 ...
 *
 * IMPORTANT: We intentionally do NOT filter by `session_id` or `subject`.
 * The Exam Attendance admin tab records attendance under a specific session
 * and with a dropdown subject (e.g. "Mathematics"), while the seating plan
 * may reference a different session and stores a free-text `paper_subject`
 * (e.g. "Mathematics — Paper 1"). Filtering by either would miss attendance
 * rows where the naming doesn't match.
 *
 * Instead, we fetch ALL attendance rows for the exam_date, and the console
 * matches them to seating assignments by `student_id`. This way, ANY
 * attendance taken on that day (regardless of session or subject) is
 * correctly detected by the Live Console — exactly matching the admin's
 * mental model: "if I marked a student present/absent today, the console
 * should reflect it today."
 *
...
export function useLiveAttendance(
  ...
  const qc = useQueryClient();
  const queryKey = ["live-attendance", examDate];
