 ...
 */
/**
 * Fisher-Yates shuffle, in place. Uses crypto randomness when available so
 * seating regeneration is not predictable/replayable across runs.
 */
...
export function generateSeating(
  ...
  // Shuffle each class's queue randomly (Fisher-Yates) so that every time
  // seating is generated -- a new plan for a new paper, or an "Update" on a
  // recurring All-Papers plan -- the resulting desk arrangement is different
  // from last time, even for the exact same set of students. The anti-cheat
  // adjacency rule below still guarantees no two same-class students ever
  // sit next to each other, regardless of the shuffle order.
