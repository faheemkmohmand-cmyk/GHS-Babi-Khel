  ...
  return useQuery<SeatingPlanFull | null>({
    ...
    queryFn: async () => {
      ...
      const roomsWithAssigns: RoomWithAssignments[] = (rooms ?? []).map(r => ({
        ...r,
        // Normalize `invigilators` so the UI always has an array to render,
        // even on a DB that hasn't run migration 020 yet (column missing →
        // undefined) or has legacy rows with only the singular column set.
        invigilators: Array.isArray((r as any).invigilators) && (r as any).invigilators.length > 0
...
export function useUpdateSeatingPlanStaff() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const { error } = await supabase.from("exam_seating_plans").update(payload).eq("id", params.planId);
      if (error) {
...
export function useUpsertRoom() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      // Try with the new `invigilators` array column first; if the DB
      // hasn't been migrated yet, retry with just the legacy column so the
      // room create/edit flow keeps working (single invigilator only) until
      ...
      let res = await runUpsert(payloadWithList);
      ...
      // Normalize the returned row so the UI always has an `invigilators`
      // array to render, even on a pre-migration DB that only returned the
