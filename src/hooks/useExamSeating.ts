...
// ─── ANTI-CHEAT SEATING ALGORITHM (rev. 2 — Problem 3 + 4 fix) ──────────────
// Goal: place students into rooms such that:
//   1. Students are seated in EXAM ROLL NUMBER order (434728, 434729, 434730, …)
//      — the primary sort key, as the admin requested. Since roll numbers are
//      assigned sequentially across classes (class 6 → 434728, class 7 → 434729,
//      class 8 → 434730, …), this naturally interleaves classes.
//   2. No two students of the same class are orthogonally adjacent
//      (front/back/left/right). Diagonal adjacency is OK.
//      → After placing by roll-no order, if a same-class adjacency WOULD occur,
//        the algorithm swaps forward to the next eligible student of a
//        different class. This is the anti-cheat layer.
//   3. "Update Seating" produces a DIFFERENT arrangement each time (random
//      rotation of the sorted list), while "Auto-Generate" (first time)
//      produces the canonical roll-no arrangement.
//
// Strategy:
//  1. Sort ALL students by exam_roll_no ascending (across all classes).
//  2. If a rotationOffset > 0 is provided (Update Seating mode), rotate the
//     sorted list by that many positions so the starting point shifts —
//     producing a visibly different desk arrangement.
//  3. Build a flat list of seatable desk positions across all rooms, in snake
//     (boustrophedon) row-major order.
//  4. Walk the desk list, placing one student at a time from the (sorted,
//     rotated) list. After each placement, if the just-placed student would
//     create a same-class adjacency with an already-filled neighbour, swap
//     forward to the next eligible student of a different class.
...
export interface GenerateSeatingOptions {
  /**
   * If > 0, rotate the sorted-by-roll-no list by this many positions before
   * placing. Used by "Update Seating" to produce a different arrangement
   * each time. 0 = canonical roll-no order (first Auto-Generate).
   * Default: 0.
   */
 ...
 * @param options    optional: rotationOffset for Update Seating mode.
 *
 * rev. 2 (Problem 3 + 4 fix):
 *   - Students are now sorted by exam_roll_no ASCENDING before placement
 *     (was: shuffled randomly per class then interleaved). This gives the
 *     admin the deterministic roll-no order they asked for.
 ...
 *   - rotationOffset (Problem 4) lets "Update Seating" produce a different
 *     arrangement each time by shifting the starting point in the sorted list.
 */
export function generateSeating(
  ...
  // ── STEP 1: Sort ALL students by exam_roll_no ascending ──────────────
  // This is the primary order the admin wants: 434728, 434729, 434730, …
  // Since exam roll numbers are assigned sequentially across classes
  // (class 6 → 434728, class 7 → 434729, class 8 → 434730), sorting by
  // exam_roll_no naturally interleaves classes.
  //
  // Numeric sort (parse as number) so "434728" < "434729" numerically, not
  // lexicographically (which would also work for same-length strings, but
  // numeric is safer if lengths ever differ).
  const sortedStudents = [...students].sort((a, b) => {
    const na = parseInt(a.exam_roll_no, 10) || 0;
    const nb = parseInt(b.exam_roll_no, 10) || 0;
    if (na !== nb) return na - nb;
    // Fallback: lexicographic comparison if numbers are equal or unparseable.
    return a.exam_roll_no.localeCompare(b.exam_roll_no);
  });

  // ── STEP 2: Apply rotation offset (Update Seating mode) ──────────────
  // Rotating the sorted list by N positions shifts which student starts at
  // desk 1, producing a visibly different arrangement. The anti-cheat
  // swap-forward below still prevents same-class adjacency.
  let orderedStudents = sortedStudents;
  const rotationOffset = options?.rotationOffset ?? 0;
  if (rotationOffset > 0 && sortedStudents.length > 1) {
    const n = sortedStudents.length;
    const offset = ((rotationOffset % n) + n) % n; // safe modulo
    if (offset > 0) {
      orderedStudents = [
        ...sortedStudents.slice(offset),
        ...sortedStudents.slice(0, offset),
      ];
    }
  }
 ...
 *
 * ── Problem 4 fix (rev. 2): isUpdate parameter ────────────────────────────
 * When `isUpdate` is true (Update Seating button on an already-seated plan),
 * a RANDOM rotation offset is applied to the sorted-by-roll-no list so the
 * resulting desk arrangement is visibly DIFFERENT from the previous one.
 * When `isUpdate` is false (first Auto-Generate), rotationOffset = 0 so the
 * canonical roll-no order is used.
 */
export function useGenerateSeating() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      // Compute rotation offset: 0 for first Auto-Generate, random 1..N-1 for
      // Update Seating. This guarantees Update always produces a different
      // arrangement (the offset is never 0 in Update mode).
      let rotationOffset = 0;
      if (params.isUpdate && params.students.length > 1) {
        const n = params.students.length;
        // Random integer in [1, n-1] — never 0, never n (which == 0 mod n).
        if (typeof crypto !== "undefined" && crypto.getRandomValues) {
          const buf = new Uint32Array(1);
          crypto.getRandomValues(buf);
          rotationOffset = 1 + (buf[0] % (n - 1));
        } else {
          rotationOffset = 1 + Math.floor(Math.random() * (n - 1));
        }
      }

      const result = generateSeating(params.students, params.rooms, { rotationOffset });
