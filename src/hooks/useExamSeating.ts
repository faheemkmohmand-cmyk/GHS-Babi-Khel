...
export function generateSeating(
  ...
  while (stillConflicted.length > 0 && guard < assignments.length * 2) {
    ...
    for (const b of assignments) {
      ...
      // Simulate swap: does it clear a's conflict without creating one for b?
      placedMap.set(deskKey(a.desk.roomId, a.desk.row, a.desk.col), b);
      placedMap.set(deskKey(b.desk.roomId, b.desk.row, b.desk.col), a);
      const aWouldConflict = neighboursOf(b).some(n => n && n !== b && n.class === b.class); // b now sits at a's old desk
      const bWouldConflict = neighboursOf(a).some(n => n && n !== a && n.class === a.class); // a now sits at b's old desk

      if (!aWouldConflict && !bWouldConflict) {
        // Commit: physically swap desks between the two assignment records.
        const tmpDesk = a.desk;
        a.desk = b.desk;
        b.desk = tmpDesk;
      ...
      // Revert simulation and try the next candidate.
      placedMap.set(deskKey(a.desk.roomId, a.desk.row, a.desk.col), a);
      placedMap.set(deskKey(b.desk.roomId, b.desk.row, b.desk.col), b);
    }
