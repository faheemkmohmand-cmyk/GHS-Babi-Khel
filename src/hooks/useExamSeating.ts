...
export interface SeatingRoom {
  ...
  invigilators: string[];
  // ── PER-INVIGILATOR ROW DUTY (added by migration 021) ──
  // Splits the room's rows across invigilators[] so each one is
  // responsible for a specific, non-overlapping band of rows (1-indexed,
  // inclusive), matching the actual duty of "supervise students inside
  // the exam hall" for a defined area rather than the whole room at once.
  // Index-aligned with `invigilators` — duties[i] is invigilators[i]'s
  // row range. Empty/missing array (pre-migration DB, or a room that
  // hasn't been re-saved yet) means "not yet split" and the UI falls
  // back to auto-splitting rows evenly for display purposes only.
  invigilator_duties: { row_start: number; row_end: number }[];
  notes: string | null;
}
...
/**
 * Splits a room's rows (1..totalRows) as evenly as possible across N
 ...
 * rows changes (e.g. adding a 2nd invigilator to an existing room).
 * Returns [] if there are no invigilators to split across.
 */
export function autoSplitRowDuties(
  totalRows: number,
  invigilatorCount: number
): { row_start: number; row_end: number }[] {
  if (invigilatorCount <= 0 || totalRows <= 0) return [];
  const base = Math.floor(totalRows / invigilatorCount);
  const extra = totalRows % invigilatorCount; // first `extra` invigilators get one more row
  const out: { row_start: number; row_end: number }[] = [];
  let cursor = 1;
  for (let i = 0; i < invigilatorCount; i++) {
    const span = base + (i < extra ? 1 : 0);
    if (span <= 0) { out.push({ row_start: cursor, row_end: cursor - 1 }); continue; } // empty range (more invigilators than rows)
    out.push({ row_start: cursor, row_end: cursor + span - 1 });
    cursor += span;
  }
  return out;
}
...
/**
 * Resolves the effective duty ranges for a room: uses explicitly-saved
 * `invigilator_duties` if present and the right length, otherwise falls
 * back to an even auto-split — so the UI always has something sensible to
 * show even before the admin has customized anything, and even on a DB
 * that hasn't run migration 021 yet.
 */
export function resolveRowDuties(room: Pick<SeatingRoom, "rows" | "invigilators" | "invigilator_duties">) {
  const n = room.invigilators?.length ?? 0;
  if (room.invigilator_duties?.length === n && n > 0) return room.invigilator_duties;
  return autoSplitRowDuties(room.rows, n);
}
  ...
  return useQuery<SeatingPlanFull | null>({
    ...
    queryFn: async () => {
      ...
      const roomsWithAssigns: RoomWithAssignments[] = (rooms ?? []).map(r => {
        ...
        // Normalize `invigilator_duties` the same way: use saved ranges if
        // present and matching the invigilator count, otherwise fall back
        // to an even auto-split (also covers a DB that hasn't run
        // migration 021 yet, where the column is simply absent).
        const savedDuties = (r as any).invigilator_duties;
        const invigilator_duties = Array.isArray(savedDuties) && savedDuties.length === invigilators.length
          ? savedDuties
          : autoSplitRowDuties(r.rows, invigilators.length);
        return {
          ...r,
          invigilators,
          invigilator_duties,
...
export function useUpsertRoom() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      // Per-invigilator row duty (added by migration 021): if the caller
      // didn't pass explicit ranges, or passed a mismatched count (e.g.
      // an invigilator was just added/removed), auto-split evenly so the
      // saved data always stays in sync with the current invigilator list.
      const explicitDuties = params.room?.invigilator_duties ?? [];
      const invigilatorDuties = explicitDuties.length === invigilatorsList.length && invigilatorsList.length > 0
        ? explicitDuties
        : autoSplitRowDuties(rows, invigilatorsList.length);
      ...
      if (!Array.isArray(row.invigilator_duties)) {
        row.invigilator_duties = autoSplitRowDuties(row.rows, row.invigilators.length);
      }
