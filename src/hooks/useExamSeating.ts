...
/**
 * Live attendance for a specific session + exam date.
 *
 * IMPORTANT: We intentionally do NOT filter by `subject`. The Exam Attendance
 * admin tab records attendance using a dropdown of subject names (e.g.
 * "Mathematics"), while the seating plan stores a free-text `paper_subject`
 * (e.g. "Mathematics — Paper 1"). Filtering by subject would miss attendance
 * rows where the naming doesn't match. Instead, we fetch ALL attendance rows
 * for the session + exam_date, and the console matches them to seating
 * assignments by `student_id`. This way, any attendance taken in the Exam
 * Attendance tab is correctly detected by the Live Console.
 *
...
export function useLiveAttendance(
  ...
  const qc = useQueryClient();
  const queryKey = ["live-attendance", sessionId, examDate];
