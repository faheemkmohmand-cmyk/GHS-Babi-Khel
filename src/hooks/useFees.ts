...
export function useFeePayments(filters?: {
  ...
  return useQuery<FeePayment[]>({
    ...
    queryFn: async () => {
      ...
      if (filters?.studentId) query = query.eq("student_id", filters.studentId);
      // FIX: was using .eq("payment_date", "gte:...") which never matched anything.
      // Now correctly uses .gte() for the start-of-month and .lte() for end-of-month.
    ...
    staleTime: 0, // Always fetch fresh — payments must appear instantly after recording
    refetchOnWindowFocus: true,
    refetchOnMount: true,
  });
}
