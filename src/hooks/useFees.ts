...
/**
 * useMyFeeStructures — student-facing hook that uses the SECURITY DEFINER
 * RPC `get_my_fee_structures()`. This bypasses fee_structures RLS issues
 * by resolving the student's class server-side and returning only the
 * active fee structures for that class.
 *
 * Use this in the student dashboard (FeeTab) instead of useFeeStructures
 * to guarantee students always see the latest admin-set amounts.
 */
export function useMyFeeStructures() {
  return useQuery<{ structures: FeeStructure[]; class: string | null; error: string | null }>({
    queryKey: ["my-fee-structures"],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_my_fee_structures");
      if (error) throw error;
      const result = (data || { success: false, structures: [], class: null, error: "RPC failed" }) as any;
      return {
...
export function useStudentVouchers(classLevel?: string | null, studentId?: string | null) {
  // Uses the SECURITY DEFINER RPC `get_my_vouchers()` to resolve the student
  // server-side and return only the calling student's own vouchers.
  // This bypasses:
  //   - Client-side filtering by myStudentId (which fails when useMyStudentRecord
  //     returns null due to students table RLS issues)
  //   - fee_vouchers RLS edge cases (the RPC runs as the caller but resolves
  //     the student_id internally, so RLS just needs to allow authenticated reads)
  return useQuery<{ vouchers: FeeVoucher[]; studentId: string | null; class: string | null; error: string | null }>({
    queryKey: ["student-vouchers", classLevel, studentId],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_my_vouchers");
      if (error) throw error;
      const result = (data || { success: false, vouchers: [], student_id: null, class: null, error: "RPC failed" }) as any;
      return {
