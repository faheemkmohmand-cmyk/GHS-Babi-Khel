...
export function useJoinHouse() {
  ...
  return useMutation({
    mutationFn: async ({ houseId, userId }: { houseId: string; userId: string }) => {
      ...
      if (error) throw error;
      // ── Reset this user's house contribution counter ──────────────────────
      // house_points tracks "points earned while a member of the CURRENT
      // house". Without this reset, a student's entire lifetime point total
      // would silently transfer to whichever house they join next, and their
      // old house would lose credit for points earned while they were there.
      // Resetting to 0 on every join makes each house's total accurately
      // reflect only the contributions of its current members, earned since
      // they joined — old members (and the points they earned) stay behind
      // with the house they left, since useHouses only sums CURRENT members.
      try {
