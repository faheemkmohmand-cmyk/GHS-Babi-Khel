...
export function useLeaderboard() {
  return useQuery<(Gamification & { full_name: string })[]>({
    ...
    queryFn: async () => {
      // Show every student who has any recorded activity (>= 0 points), ranked
      // from the very first point earned — no artificial minimum threshold.
...
export function useWeeklyLeaderboard() {
  return useQuery<(Gamification & { full_name: string })[]>({
    ...
    queryFn: async () => {
      // Important: do NOT filter on weekly_points > 0. A brand-new student who
      // just earned their very first point should appear immediately, ranked
      // alongside everyone else, instead of waiting for a "new week" to begin.
      const { data } = await supabase
        .from("student_gamification")
        .select("*, profiles(full_name)")
...
export async function awardPoints(userId: string, points: number, badgeId?: string) {
  ...
  const weeklyPts = (lastReset === weekStartStr) ? (current?.weekly_points || 0) + points : points;

  // House points contribution
  const housePts = (current?.house_points || 0) + points;

  const { error: upsertError } = await supabase.from("student_gamification").upsert(
