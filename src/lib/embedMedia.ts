...
/** True when this URL is a Facebook share/short link that must be resolved
 *  to its canonical permalink before it can be embedded. */
...
const resolveMemory = new Map<string, string>();
...
function cacheGet(key: string): string | undefined {
  ...
  try {
    ...
    if (typeof store[key] === "string") {
      resolveMemory.set(key, store[key]);
      return store[key];
    }
...
/**
 * Return the best embeddable form of a gallery media URL:
 *  - Facebook share links → canonical permalink (via /api/resolve-fb,
 *    cached per session; falls back to the original URL on any failure so
 *    rendering never blocks or breaks).
 *  - Everything else → the URL itself, unchanged.
 */
export async function resolveMediaUrl(rawUrl: string): Promise<string> {
  const url = normalizeMediaUrl(rawUrl);
  if (!url || !needsCanonicalResolve(url)) return url;
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (j?.ok && typeof j.canonical === "string" && j.canonical) {
          cacheSet(url, j.canonical);
          return j.canonical as string;
        }
      }
    } catch { /* offline / function cold-start hiccup */ }
    return url; // graceful fallback — old behaviour
  })();
