...
const Admission = () => {
  ...
  const handleFilePick = async (docType: string, file: File) => {
    ...
    // We need an admission ID to store the doc under — create a temp one if not yet created
    if (!admissionIdRef.current) {
      try {
        const result = await submitAdmission({
          full_name:       form.full_name.trim() || "Draft",
          father_name:     form.father_name.trim() || "Draft",
          date_of_birth:   form.date_of_birth || null,
          b_form_no:       form.b_form_no.trim() || "000",
          contact_number:  form.contact_number.trim() || "000",
          whatsapp_number: null,
          home_address:    null,
          gender:          form.gender || null,
          applying_class:  form.applying_class || "6",
          admission_type:  form.admission_type,
          previous_school: null, previous_class: null,
          previous_marks:  null, year_of_passing: null,
        });
        admissionIdRef.current = result.id;
      } catch {
        // Can't create record yet — just upload to Cloudinary temp folder
      }
    }

    try {
      const folder = admissionIdRef.current
        ? `admissions/${admissionIdRef.current}`
        : `admissions/temp`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      let admissionId = admissionIdRef.current;
      let refNo = "";

      if (admissionId) {
        // Admission record already created during file upload — just update it with real data
