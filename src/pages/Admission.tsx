...
const DOWNLOAD_ITEMS = [
  {
    ...
    pdfSections: [
      ...
      { heading: "Documents (where applicable)", body: `• B-Form (NADRA) — required for all applicants\n• Recent passport-size photograph — required for all applicants\n• Previous class result card / marksheet — required for all applicants\n• School Leaving Certificate — required for migration/transfer cases (Class 9 & 10 migration, and school-to-school transfer for Class 6–8)\n• Father's/Guardian's CNIC copy — required for all applicants\n• For Class 9/10 migration from another BISE-affiliated school: DMC, migration certificate from the previous Board, and enrollment/reference letter from the previous school's head — submitted by the schools to BISE Peshawar as part of the portal migration, per current BISEP notification requirements` },
  ...
  },
  {
    ...
    pdfSections: [
      ...
      { heading: "BISE Peshawar Charges (Class 9, current session)", body: `Per BISE Peshawar's official Class 9 enrollment notification for the current session:\n• Normal fee (enrollment + sports): Rs. 725 — payable by the standard due date\n• With late fee: Rs. 1,013\n• With double late fee: Rs. 1,300 (after the late-fee deadline)\nThese figures are set by BISE Peshawar and can change each academic session — always confirm the current amount and deadlines with the school office or BISEP before paying, as fees are paid through the board's designated channels, not to the school directly.` },
      { heading: "Migration / Related BISEP Charges", body: `BISE Peshawar's schedule also lists charges for related corrections/changes during enrollment (e.g., picture correction, group change, subject change). Migration itself is processed school-to-school on the BISEP portal; any applicable fee is set by BISEP for that session. Ask the school office for the current amounts before making any payment — this prospectus does not fix these figures, since they are set by the board and may change.` },
  ...
  },
  {
    ...
    pdfSections: [
      ...
      { heading: "Document Requirements", body: `• B-Form (NADRA) is required for all applicants.\n• Passport size photograph should be recent.\n• Result card from the previous class should be from a recognized board/school.\n• Father's/Guardian's CNIC copy should be valid and legible.\n• Migration/transfer cases require a School Leaving Certificate from the previous school; Class 9/10 migration from another Board additionally requires the documents listed under Migration Rules above.` },
...
/**
 * Apply Online Icon - Realistic Laptop with Application Form
 * Shows a modern laptop device with a form/document on screen
 */
function ApplyOnlineIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Laptop Base */}
      <rect x="2" y="14" width="20" height="2" rx="1" fill="currentColor" opacity="0.9"/>
      <path d="M4 16L6 18H18L20 16" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round"/>
      
      {/* Laptop Screen */}
      <rect x="3" y="5" width="18" height="10" rx="1.5" fill="white" fillOpacity="0.25" stroke="currentColor" strokeWidth="1.3"/>
      
      {/* Screen Content - Form Lines */}
      <rect x="5.5" y="7" width="8" height="1.2" rx="0.4" fill="currentColor" opacity="0.7"/>
      <rect x="5.5" y="9.2" width="13" height="0.9" rx="0.35" fill="currentColor" opacity="0.4"/>
      <rect x="5.5" y="10.6" width="11" height="0.9" rx="0.35" fill="currentColor" opacity="0.4"/>
      <rect x="5.5" y="12" width="6" height="0.9" rx="0.35" fill="currentColor" opacity="0.4"/>
      
      {/* Checkmark Circle */}
      <circle cx="17" cy="11" r="2.2" fill="currentColor" fillOpacity="0.2" stroke="currentColor" strokeWidth="1.1"/>
      <path d="M15.8 11L16.7 12L18.2 10" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
      
      {/* Camera/Webcam */}
      <circle cx="12" cy="4.3" r="0.6" fill="currentColor" opacity="0.5"/>
    </svg>
  );
}

/**
 * Track Application Icon - Realistic Search with Status Indicator
 * Shows magnifying glass with tracking/list elements
 */
function TrackApplicationIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Background Circle */}
      <circle cx="10" cy="10" r="7" fill="white" fillOpacity="0.2" stroke="currentColor" strokeWidth="1.4"/>
      
      {/* Search Glass Handle */}
      <line x1="15.2" y1="15.2" x2="19.5" y2="19.5" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"/>
      
      {/* Inner Elements - Document/List representation */}
      <rect x="6.5" y="7" width="7" height="1" rx="0.3" fill="currentColor" opacity="0.6"/>
      <rect x="6.5" y="9" width="5" height="1" rx="0.3" fill="currentColor" opacity="0.4"/>
      <rect x="6.5" y="11" width="6" height="1" rx="0.3" fill="currentColor" opacity="0.4"/>
      
      {/* Status Dot */}
      <circle cx="14.5" cy="8" r="1.8" fill="currentColor" fillOpacity="0.25" stroke="currentColor" strokeWidth="1"/>
      <path d="M14.5 7V8.2L15.2 8.7" stroke="currentColor" strokeWidth="0.9" strokeLinecap="round" strokeLinejoin="round"/>
      
      {/* Magnifier Reflection */}
      <ellipse cx="7.5" cy="6.5" rx="2" ry="1.2" fill="white" fillOpacity="0.3" transform="rotate(-30 7.5 6.5)"/>
    </svg>
  );
}

/**
 * Apply Now / Graduation Cap Icon - Enhanced Realistic Version
 * Detailed graduation cap with tassel
 */
function ApplyNowIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Cap Top (Diamond shape) — solid fill so it reads clearly even on white/currentColor text */}
      <path d="M12 3L2 8.5L12 14L22 8.5L12 3Z" fill="currentColor" stroke="currentColor" strokeWidth="0.6" strokeLinejoin="round"/>
      {/* Diamond top facet shading for a 3D look */}
      <path d="M12 3L22 8.5L12 14L12 3Z" fill="black" fillOpacity="0.14"/>
      <path d="M12 3L2 8.5L12 14L12 3Z" fill="white" fillOpacity="0.18"/>

      {/* Cap Bottom/Skull part */}
      <path d="M5 10V13.6C5 15.9 8 17.8 12 17.8C16 17.8 19 15.9 19 13.6V10"
        fill="currentColor" fillOpacity="0.85" stroke="currentColor" strokeWidth="0.6" strokeLinejoin="round"/>

      {/* Tassel String */}
      <path d="M22 8.5V13.6C22 14.4 21.2 14.9 20.7 14.9C20.2 14.9 19.4 14.4 19.4 13.6V8.9"
        stroke="currentColor" strokeWidth="1.1" fill="none" strokeLinecap="round"/>

      {/* Tassel End */}
      <circle cx="20.7" cy="15.3" r="1.15" fill="currentColor"/>

      {/* Button on top of cap */}
      <circle cx="12" cy="8.5" r="1" fill="white" fillOpacity="0.6"/>

      {/* Highlight/Reflection line on the crown */}
      <path d="M6.3 7.7L12 5" stroke="white" strokeWidth="0.7" strokeLinecap="round" opacity="0.4"/>
    </svg>
  );
}

/**
 * Printable Admission Form Icon - Realistic Document with Pen
 * Shows a paper form with fold corner, ruled lines, and a pen — signals "print & fill by hand"
 */
function PrintableFormIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Paper sheet with folded corner */}
      <path d="M6 2.5H14.5L18.5 6.5V20.5C18.5 21.05 18.05 21.5 17.5 21.5H6C5.45 21.5 5 21.05 5 20.5V3.5C5 2.95 5.45 2.5 6 2.5Z"
        fill="white" fillOpacity="0.3" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round" />
      <path d="M14.5 2.5V5.5C14.5 6.05 14.95 6.5 15.5 6.5H18.5" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />

      {/* Form lines */}
      <rect x="7" y="9.3" width="7.5" height="1" rx="0.35" fill="currentColor" opacity="0.55" />
      <rect x="7" y="11.6" width="9.5" height="0.9" rx="0.3" fill="currentColor" opacity="0.35" />
      <rect x="7" y="13.7" width="6" height="0.9" rx="0.3" fill="currentColor" opacity="0.35" />
      <rect x="7" y="15.8" width="8" height="0.9" rx="0.3" fill="currentColor" opacity="0.35" />

      {/* Pen, diagonally over bottom-right of the page */}
      <g transform="rotate(45 18 17)">
        <rect x="16.6" y="12.5" width="2.2" height="9" rx="1.1" fill="currentColor" opacity="0.9" />
        <path d="M16.6 12.9C16.6 12 17.1 11.3 17.7 11.3C18.3 11.3 18.8 12 18.8 12.9" fill="currentColor" />
      </g>
    </svg>
  );
}
...
function EligibilityChecker({ onApply }: { onApply: (cls: string, type: AdmissionType) => void }) {
  ...
  const checkEligibility = () => {
    ...
    // Age check — only Class 9 has a verified, published minimum-age rule from
    // BISE Peshawar's official enrollment notification ("a candidate who does
    // not attain above 12 years of age shall not be eligible for 9th class
    // enrollment"). For other classes there is no single verified age table
    // from an official source, so we do not invent numeric ranges — we only
    // flag the one rule we can confirm and otherwise defer to the school.
    if (dob) {
      const ageMs = Date.now() - new Date(dob).getTime();
      const age = Math.floor(ageMs / (365.25 * 24 * 60 * 60 * 1000));
      if (cls === "9" && age < 12) {
        reasons.push(`Minimum age for Class 9 enrollment (per BISE Peshawar) is 12 years. Recorded age is ${age} years.`);
      }
    } else {
    ...
    <div className="bg-card border border-border rounded-2xl shadow-card p-4">
      ...
      <div className="space-y-2.5">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground mt-1">Used to check the published Class 9 minimum-age rule where applicable</p>
        </div>
  ...
  const generateOfflineForm = useCallback(() => {
    ...
    try {
      ...
      const docChecklist = [
        ...
        "Previous Result Card / Marksheet — Required for all applicants",
        "School Leaving Certificate (SLC) — Required for migration/transfer cases",
        "Father's/Guardian's CNIC Copy — Required for all applicants",
        "DMC & Migration Certificate from previous Board — Class 9/10 migration from another Board only, submitted by schools",
      ...
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Admission Application Form</title>
        ...
        <div class="note" style="margin-bottom:14px;">The four fields below are required for Migration/Transfer. For Fresh Admission they are optional — fill in if applicable.</div>
        ...
        ${line("Parent/Guardian Full Name", 100, true)}
        ${line("Relationship to Student", 50, true)}${line("CNIC Number", 50, true)}
        ${line("Contact Number", 50, true)}${line("Occupation (optional)", 50)}
  ...
  const fieldError = (id: string, raw: string, isMigrationCtx: boolean = isMigration): string | null => {
    ...
    switch (id) {
      ...
      case "previous_school": {
        // Required for migration/transfer (we need to know the sending school). For
        // fresh admission it's optional — e.g. a first-time Class 6 entrant may not
        // have a "previous school" in the sense migration cases do.
        if (!v) return isMigrationCtx ? "Required for migration/transfer — enter your current school's name" : null;
        if (v.length < 3) return "Enter a valid school name";
        if (!SCHOOL_RE.test(v)) return "Enter a valid school name — no stray symbols";
        return null;
      }
      case "previous_class": {
        if (!v) return isMigrationCtx ? "Required for migration/transfer" : null;
        ...
        {view === "home" && (
          <div className="container mx-auto px-4 pt-0 max-w-3xl">
            ...
            <motion.div
              ...
              className="relative rounded-b-3xl overflow-hidden mb-8 -mx-4 sm:-mx-8 px-4 sm:px-8 py-12 sm:py-16"
              style={{
            ...
            >
              ...
              {/* Offline form download — highlighted at top of Downloads */}
              <div className={`mb-2.5 ${!isEffectivelyOpen ? "opacity-50 pointer-events-none select-none" : ""}`}>
                <button
                ...
                >
                  <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center shrink-0">
                    {downloading === "offline_form" ? <Loader2 className="w-4 h-4 text-white animate-spin" /> : <PrintableFormIcon className="w-5 h-5 text-white" />}
                  </div>
                  <div>
            ...
            <section aria-label="Admission information" className="sr-only">
              ...
              <div>
                ...
                <p>B-Form (NADRA); passport size photo; previous result card; School Leaving Certificate (for migration/transfer); father's/guardian's CNIC copy. Class 9/10 migration from another Board may additionally require DMC and migration certificate, submitted by the schools as part of BISEP's process.</p>
              </div>
              <div>
                    ...
                    {step === 2 && (
                      <>
                        ...
                        <div className="space-y-3">
                          ...
                          {!isMigration && (
                            <div className="bg-muted/50 border border-border rounded-xl p-3 text-xs text-muted-foreground flex gap-2">
                              <Info className="w-4 h-4 shrink-0 mt-0.5" />
                              <span>Previous school details below are optional for fresh admission — fill them in if you have them (e.g. moving up from an earlier class).</span>
                            </div>
                          )}
                          {[
                            { id: "previous_school", label: "Previous School Name", placeholder: "School name", req: isMigration, icon: GraduationCap, tip: isMigration ? "Required — name of the school you are transferring from" : "Optional for fresh admission — fill in if applicable" },
                            { id: "previous_class", label: "Previous Class", placeholder: "e.g. Class 8", req: isMigration, icon: BookOpen, tip: isMigration ? "Required — the class you last completed" : "Optional for fresh admission" },
                            { id: "previous_marks", label: "Previous Marks / Grade", placeholder: "e.g. 450/600 or A", req: isMigration, icon: TrendingUp, tip: isMigration ? "Required — your marks or grade from the last exam" : "Optional for fresh admission" },
                            { id: "year_of_passing", label: "Year of Passing", placeholder: `${new Date().getFullYear()}`, req: isMigration, icon: Calendar, tip: isMigration ? "Required — year you completed the previous class" : "Optional for fresh admission" },
                          ].map(f => (
                            <div key={f.id}>
                              <Label className="text-xs font-semibold mb-1 flex items-center">
                                {f.label} {f.req && <span className="text-red-500 ml-0.5">*</span>}
