...
function EligibilityChecker({ onApply }: { onApply: (cls: string, type: AdmissionType) => void }) {
  ...
  const checkEligibility = () => {
    ...
    // Age check
    const ageMap: Record<string, number> = { "6": 10, "7": 11, "8": 12, "9": 13, "10": 14 };
    if (dob) {
      const age = Math.floor((Date.now() - new Date(dob).getTime()) / (365.25 * 24 * 60 * 60 * 1000));
      const minAge = ageMap[cls] || 10;
      if (age < minAge) reasons.push(`Minimum age for Class ${cls} is ${minAge} years. Your age is ${age} years.`);
    }

    // Marks check
    if (marks) {
      const pct = parseFloat(marks);
      if (!isNaN(pct) && pct < 33) reasons.push(`Previous marks below 33% — minimum passing marks required.`);
    }
...
const Admission = () => {
  ...
  const generateAndDownload = useCallback(async (item: typeof DOWNLOAD_ITEMS[number]) => {
    ...
    try {
      ...
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${item.pdfTitle}</title>
        ...
        <div class="header"><div class="school-name">${schoolName}</div><div class="tagline">${tagline}</div><div class="address">${address}</div></div>
        <div class="doc-title">${item.pdfTitle}</div>${sectionsHtml}
    ...
    try {
      ...
      const checkbox = (labelText: string) => `
        <span style="display:inline-flex;align-items:center;gap:6px;margin:0 18px 8px 0;font-size:11.5px;color:#334155;">
          <span style="display:inline-block;width:13px;height:13px;border:1.4px solid #64748b;border-radius:3px;"></span>${labelText}
        </span>`;
      ...
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Offline Admission Form</title>
        ...
        ${line("Full Name *", 100)}${line("Father Name *", 100)}
        ...
        <div style="margin:4px 0 14px;"><div style="font-size:10.5px;color:#475569;margin-bottom:6px;">Gender *</div>${checkbox("Male")}${checkbox("Female")}</div>
        <div class="section-title">Academic Information</div>
        <div style="margin:4px 0 12px;"><div style="font-size:10.5px;color:#475569;margin-bottom:6px;">Applying for Class *</div>${["6","7","8","9","10"].map(c => checkbox(`Class ${c}`)).join("")}</div>
        <div style="margin:4px 0 14px;"><div style="font-size:10.5px;color:#475569;margin-bottom:6px;">Admission Type *</div>${checkbox("Fresh Admission")}${checkbox("Migration / Transfer")}</div>
        ${line("Previous School Name *", 100)}${line("Previous Class *", 50)}${line("Previous Marks / Grade *", 50)}${line("Year of Passing *", 50)}
        ...
        <div class="officebox"><div class="t">For Office Use Only</div>${line("Reference No.", 50)}${line("Received By", 50)}${line("Date Received", 50)}${line("Status", 50)}</div>
  ...
  const handleSubmit = async () => {
    ...
    if (!isEffectivelyOpen) {
      toast.error(nextOpeningNote || `Admissions for Session ${displaySessionYear} open soon. Please check back.`);
      return;
    }
        ...
        {view === "home" && (
          <div className="container mx-auto px-4 pt-0 max-w-3xl">
            ...
            >
              ...
              {/* School Stats */}
              <div className="relative mt-8 grid grid-cols-3 gap-4 max-w-md mx-auto">
                <AnimatedCounter end={450} label="Students Enrolled" icon={Users} />
                <AnimatedCounter end={92} label="Pass Rate %" icon={TrendingUp} />
                <AnimatedCounter end={35} label="Years of Excellence" icon={Calendar} />
              </div>
            </motion.div>

            {/* ── ACTION CARDS (Apply / Track / Download) ── */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
              {[
                ...
                },
                {
                  ...
                  desc: "Get admission forms, rules & documents",
                  icon: Download,
                  gradient: "from-purple-500 to-violet-600",
                  shadowColor: "shadow-purple-500/20",
                  action: () => {}, // scroll to downloads
                },
              ].map((card, i) => (
                <motion.button key={i}
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={card.action}
                  className={`text-left p-5 rounded-2xl border border-border bg-card hover:shadow-xl ${card.shadowColor} transition-all group`}
                >
            ...
            <motion.div
              ...
              className="bg-amber-50 dark:bg-amber-950/10 border border-amber-200 dark:border-amber-900 rounded-2xl p-5 mb-6"
            >
              <div className="flex items-center gap-2 mb-3">
                <MapPin className="w-5 h-5 text-amber-600" />
                <h3 className="font-bold text-amber-800 dark:text-amber-300 text-sm">Walk-in Admission</h3>
                <span className="text-[10px] bg-amber-600/15 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-bold ml-auto">Recommended</span>
              </div>
              <p className="text-xs text-amber-700 dark:text-amber-400 mb-4">
                Visit the school office with your documents for direct admission. No internet or computer needed!
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                <div className="flex items-start gap-2">
                  <Clock className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">Office Hours</p>
                    <p className="text-xs text-amber-700 dark:text-amber-400">Monday to Saturday — 8:00 AM to 12:00 PM</p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <Phone className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">Call Before Visiting</p>
                    <p className="text-xs text-amber-700 dark:text-amber-400">0346-9898295</p>
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-2 mb-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-amber-800 dark:text-amber-300 mb-1.5">What to Bring:</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {[
                      ...
                      "Signed Letter (Class 10 migration)",
                    ].map((doc, i) => (
                      <p key={i} className="text-[11px] text-amber-700 dark:text-amber-400 flex items-start gap-1">
                        <CheckCircle2 className="w-3 h-3 shrink-0 mt-0.5 text-amber-500" /> {doc}
                      </p>
                    ))}
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-2 bg-amber-100/50 dark:bg-amber-900/20 rounded-xl p-3">
                <Lightbulb className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                <p className="text-xs text-amber-800 dark:text-amber-300">
                  <span className="font-bold">Pro tip:</span> Apply online first to check eligibility. If approved, then download forms and visit the office — saves you a trip if not eligible!
                </p>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="p-4 bg-card border border-border rounded-2xl mb-6"
            >
              ...
              <p className="text-[10px] text-muted-foreground mt-2 text-center">
                Tap to download • Admin-uploaded files used when available
              </p>
            </motion.div>
          ...
          <div className="container mx-auto px-4 pt-10 max-w-md text-center">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
              ...
              <div className="flex flex-col gap-3">
                ...
                <Button variant="outline" onClick={() => { resetForm(); setView("home"); }}
                  className="gap-2 rounded-xl">
