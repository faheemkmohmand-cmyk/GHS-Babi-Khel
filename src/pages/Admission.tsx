...
const Admission = () => {
  ...
  const generateFilledForm = useCallback(() => {
    ...
    try {
      ...
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Admission Application Form</title>
        ...
        ${line("Relationship to Student", "Father/Guardian", 50, true)}${line("Father's CNIC Number", form.father_cnic, 50, true)}
        ${line("Contact Number", form.contact_number, 50, true)}${line("Occupation", form.occupation, 50)}
  ...
  const generateEmptyForm = useCallback(() => {
    ...
    try {
      ...
      const line = (label: string, value?: string | null, w = 70, req = false) =>
        `<div style="display:flex;margin-bottom:6px;"><div style="font-size:10px;font-weight:600;color:#0f4c3a;width:${w}%;flex-shrink:0;">${esc(label)}${req ? ' <span style="color:#c96b3b;">*</span>' : ''}</div><div style="font-size:10.5px;color:#1e293b;border-bottom:1px solid #cbd5e1;flex-grow:1;padding-bottom:2px;">${esc(value || "")}</div></div>`;
      ...
      const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
        ...
        ${line("Contact Number (WhatsApp)", "", 50, true)}${line("WhatsApp (if different)", "")}
        ...
        ${line("Relationship to Student", "Father/Guardian", 50, true)}${line("Father's CNIC Number", "", 50, true)}
        ${line("Contact Number", "", 50, true)}${line("Occupation", "")}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[9px] text-muted-foreground font-medium mt-1 flex items-center gap-1">
                        <FileDown className="w-3 h-3" /> No pre-filling required
                      </p>
                    </div>
                  </div>
                </button>
