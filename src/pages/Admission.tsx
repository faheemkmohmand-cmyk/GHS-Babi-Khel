      ...
      if (false as boolean) {
        ...
        const { data: admData, error: admError } = await supabase
          ...
          .insert({
            ...
            previous_marks:  form.previous_marks.trim() || null,
            year_of_passing: form.year_of_passing.trim() || null,
            reference_no:    "",   // trigger will overwrite with OHS-YYYY-XXXX
          })
        ...
        refNo = admData!.reference_no;

        // Save already-uploaded Cloudinary URLs to admission_documents
        ...
        if (docs.length > 0) {
          const { error: docError } = await supabase.from("admission_documents").insert(docs);
