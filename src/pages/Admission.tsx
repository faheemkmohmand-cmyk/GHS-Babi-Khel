...
import {
  ...
  FileText, FileDown, CalendarClock, Info,
} from "lucide-react";
          ...
          <div className="container mx-auto px-4 pt-8 max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              ...
              {isEffectivelyOpen ? (
                ...
                <div className="inline-flex items-center gap-2 bg-amber-100 text-amber-800 text-sm font-bold px-4 py-1.5 rounded-full mb-3">
              ...
              {/* Closed/paused notice — now visually distinct (amber) instead
                  of a barely-noticeable gray line, so it's clear applying is
                  not currently possible. */}
              {nextOpeningNote && (
                <p className="text-sm font-medium text-amber-700 mt-1 flex items-center justify-center gap-1.5">
            ...
            {/* ── Apply category cards ──
                Always rendered (previously gated on isEffectivelyOpen).
                When admissions are paused in the DB, we still show the cards so
                prospective parents can read about each program — they just get
                a small "applications open <date>" hint on top. Clicking a card
                still opens the apply form; if the form is paused, the submit
                handler surfaces a friendly toast. This keeps the page rich for
                Google (the card titles "Class 6–8 Admission", "Class 9
                Admission", "Class 9 Migration", "Class 10 Migration" are all
                indexed as visible text) and useful for parents. */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.1 } }}
              className="space-y-3 mb-6">
              {!isEffectivelyOpen && nextOpeningNote && (
                <div className="flex items-start gap-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-xl p-2.5 mb-1">
                  <Info className="w-3.5 h-3.5 shrink-0 mt-0.5 text-amber-600" />
                  <span><strong>Admissions closed.</strong> {nextOpeningNote} You can preview the form below — submissions will be enabled on the opening date.</span>
                </div>
              )}
              {[
                ...
                <motion.button key={i}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {!isEffectivelyOpen && (
                        <span className="text-[10px] font-semibold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded">Preview only</span>
                      )}
                    </div>
