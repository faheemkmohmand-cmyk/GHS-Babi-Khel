...
const Admission = () => {
  ...
  const startApply = (cls?: string, type?: AdmissionType) => {
    if (!isEffectivelyOpen) {
      toast.error(nextOpeningNote || `Admissions for Session ${displaySessionYear} are currently closed.`);
      return;
    }
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-red-700/80 dark:text-red-400/80 mt-1">
                    {nextOpeningNote || `Applications for Session ${displaySessionYear} are not being accepted right now.`} Online application, tracking, eligibility check, and downloads will be available again once admissions reopen.
                  </p>
                </div>
              </motion.div>
            )}
