    ...
    pdfSections: [
      {
        ...
        body: `Use this template to write your school transfer application. Fill in the blanks with your details, print it, and get it signed by both principals. This letter is required for Class 9 and Class 10 migration.`,
      },
      {
        heading: "School Transfer Application",
...
function TrackResult({ result }: { result: any }) {
  ...
  // ── Interview booking (source of truth: interview_bookings table) ──────
  ...
  const loadBooking = useCallback(async () => {
    ...
    try {
      ...
      if (data && (data as any).interview_slots) {
        ...
        setRealBooking({
          label: `${dateLabel} at ${formatSlotTime(slot.start_time)}${slot.location ? " — " + slot.location : ""}`,
        });
      } else {
        setRealBooking(null);
      }
  ...
  useEffect(() => {
    loadTimeline();
    loadBooking();
  }, [loadTimeline, loadBooking]);

  const canBookInterview = ["documents_verified", "under_review", "interview_scheduled"].includes(status);
  // Admit card is for the INTERVIEW, not final approval — "approved" means
  // the applicant is directly accepted with nothing further required.
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      {/* Full status timeline — same history previously shown in Applicant Portal */}
      {!timelineLoading && timeline.length > 0 && (
        <ApplicationTracker timeline={timeline} currentStatus={status} />
      )}

      {/* Interview slot booking — applicant picks/cancels their own slot */}
        ...
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              All interview slots for your class are currently full. We'll automatically promote you when a seat opens up — no action needed from you. You'll see the status change here.
            </p>
          </div>
        </div>
      )}
...
const Admission = () => {
  ...
  // ── SEO-FIRST ADMISSION STATUS ──────────────────────────────────────────────
  // Previously: if the admin toggled admissions "off" (or the last_date had
  // passed) the entire visible page flipped to a red "Admissions Currently
  // Closed" badge and the apply buttons disappeared. Because Googlebot fully
  // renders this SPA, that "closed" text became the snippet Google showed in
  // search results — pushing the school DOWN in rankings whenever someone
  // searched "GHS Babi Khel admission".
  //
  // Fix: the visible page ALWAYS presents "Admissions Open" as the headline
  // status (the page itself is the school's permanent admission landing
  // page — admissions info is always relevant). The actual `isEffectivelyOpen`
  // flag still gates live form submission: when admissions are technically
  // closed in the DB, the apply buttons route to a friendly "next session
  // opens on …" notice instead of blocking the page outright. This keeps the
  // SEO content rich and on-brand while preserving the admin's ability to
  ...
  // Always shown as "open" in the visible hero — Google indexes this text.
  // The session year falls back to the upcoming academic year if the admin
  // hasn't set one (or the previous session has closed).
  const displaySessionYear =
    settings?.session_year || String(new Date().getFullYear() + 1);

  // Friendly "next opening" date shown only when admissions are currently
  // paused — never as a "Closed" warning, just informational.
  const nextOpeningNote = (() => {
    if (isEffectivelyOpen) return null;
    const today = new Date(new Date().toDateString());
    if (settings?.open_date && new Date(settings.open_date) > today) {
      try {
        return `Next session opens ${new Date(settings.open_date).toLocaleDateString("en-PK", { day: "numeric", month: "long", year: "numeric" })}.`;
      } catch { return null; }
    }
    return `Applications for Session ${displaySessionYear} open soon — prepare your documents now.`;
  })();
  ...
  // ── Generate the printable Offline Admission Form ──────────────────────────
  // A single PDF a parent can download, print, fill by hand, and submit at
  // the school office in person — no computer/internet needed after
  // printing. Mirrors every field collected by the online 3-step form
  // (Student Info, Academic Info, Documents) so office staff receive the
  // same data either way. Uses the same print-to-PDF approach as the other
  // downloads for a consistent look and because it works without any
  ...
  const generateOfflineForm = useCallback(() => {
    ...
    try {
      ...
      const line = (labelText: string, widthPct = 100) => `
        <div style="display:inline-block;width:${widthPct}%;vertical-align:top;margin-bottom:14px;padding-right:10px;">
          <div style="font-size:10.5px;color:#475569;margin-bottom:3px;">${labelText}</div>
          <div style="border-bottom:1.2px solid #94a3b8;height:20px;"></div>
        </div>`;

      const checkbox = (labelText: string) => `
        <span style="display:inline-flex;align-items:center;gap:6px;margin:0 18px 8px 0;font-size:11.5px;color:#334155;">
          <span style="display:inline-block;width:13px;height:13px;border:1.4px solid #64748b;border-radius:3px;"></span>
          ${labelText}
        </span>`;
      ...
      const docChecklist = [
        ...
        "Father's CNIC Copy",
        "Migration Certificate — migration cases only",
        "Signed Letter from both principals — Class 10 migration only",
      ].map(d => `<li style="margin-bottom:5px;">${d}</li>`).join("");

      const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Offline Admission Form</title>
        <style>
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .school-name { font-size: 19px; font-weight: 800; color: #1e3a5f; letter-spacing: 0.5px; }
          .tagline { font-size: 10.5px; color: #64748b; margin-top: 2px; }
          .address { font-size: 9.5px; color: #94a3b8; margin-top: 2px; }
          .doc-title { font-size: 16px; font-weight: 700; color: #0f172a; text-align: center; margin: 4px 0 6px; padding: 7px; background: #f1f5f9; border-radius: 6px; }
          .doc-sub { font-size: 10px; color: #64748b; text-align: center; margin-bottom: 16px; }
          .section-title { font-size: 12.5px; font-weight: 700; color: #1e3a5f; margin: 16px 0 8px; padding-bottom: 4px; border-bottom: 2px solid #e2e8f0; }
          .note { font-size: 10px; color: #475569; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 5px; padding: 8px 10px; margin-top: 4px; }
          .footer { text-align: center; border-top: 2px solid #e2e8f0; padding-top: 8px; margin-top: 22px; font-size: 8.5px; color: #94a3b8; }
          .footer span { color: #64748b; }
          .officebox { border: 1.4px dashed #94a3b8; border-radius: 6px; padding: 10px 12px; margin-top: 14px; }
          .officebox .t { font-size: 10.5px; font-weight: 700; color: #1e3a5f; margin-bottom: 6px; }
        </style>
      </head><body>
        <div class="header">
          <div class="school-name">${schoolName}</div>
        ...
        <div class="doc-sub">Fill by hand and submit in person at the school office — no online submission required.</div>

        <div class="section-title">Student Information</div>
        ${line("Full Name *", 100)}
        ${line("Father Name *", 100)}
        ${line("B-Form Number *", 50)}${line("Date of Birth", 50)}
        ${line("Contact Number (WhatsApp) *", 50)}${line("WhatsApp (if different)", 50)}
        ${line("Home Address (Village / Mohalla)", 100)}
        <div style="margin:4px 0 14px;">
          <div style="font-size:10.5px;color:#475569;margin-bottom:6px;">Gender *</div>
          ${checkbox("Male")}${checkbox("Female")}
        </div>

        <div class="section-title">Academic Information</div>
        <div style="margin:4px 0 12px;">
          <div style="font-size:10.5px;color:#475569;margin-bottom:6px;">Applying for Class *</div>
          ${["6","7","8","9","10"].map(c => checkbox(`Class ${c}`)).join("")}
        </div>
        <div style="margin:4px 0 14px;">
          <div style="font-size:10.5px;color:#475569;margin-bottom:6px;">Admission Type *</div>
          ${checkbox("Fresh Admission")}${checkbox("Migration / Transfer")}
        </div>
        ${line("Previous School Name *", 100)}
        ${line("Previous Class *", 50)}${line("Previous Marks / Grade *", 50)}
        ${line("Year of Passing *", 50)}

        <div class="section-title">Documents to Attach</div>
        <ul style="font-size:11px;color:#334155;padding-left:18px;">${docChecklist}</ul>
        ...
        <div class="footer">
          <span>${schoolName}</span> &bull; Generated on ${date} &bull; Submit this form in person at the school office &bull; ${window.location.origin}
        </div>
      </body></html>`;
      ...
      if (printWin) {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = "Admission Application Form.html";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
  ...
  const handleSubmit = async () => {
    ...
    // ── Pause-gate (SEO-friendly) ───────────────────────────────────────────
    // The page always shows "Admissions Open" for SEO, but the admin can
    // still pause live submissions via admission_settings.is_open=false.
    // If paused, we surface a friendly toast and bail out — no DB write.
    if (!isEffectivelyOpen) {
      toast.error(
        nextOpeningNote || `Admissions for Session ${displaySessionYear} open soon. Please check back.`
      );
      return;
    }
        ...
        {view === "home" && (
          <div className="container mx-auto px-4 pt-8 max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="w-16 h-16 rounded-2xl bg-transparent border border-blue-200 flex items-center justify-center mx-auto mb-4 shadow-lg">
                <svg
                  className="w-9 h-9"
                  viewBox="0 0 48 48"
                  fill="none"
                ...
                >
                  <defs>
                    <linearGradient id="admissionIconGrad" x1="0" y1="0" x2="48" y2="48" gradientUnits="userSpaceOnUse">
                      <stop offset="0%" stopColor="#F5F8FF" />
                      <stop offset="100%" stopColor="#C7D6F5" />
                    </linearGradient>
                  </defs>
                  {/* Clipboard body */}
                  <rect x="8" y="6" width="32" height="38" rx="4" fill="url(#admissionIconGrad)" stroke="#94A9D9" strokeWidth="1" />
                  {/* Clip top */}
                  <rect x="17" y="3" width="14" height="7" rx="2.5" fill="#FFD75E" stroke="#C99A1E" strokeWidth="1" />
                  {/* Ruled lines */}
                  <line x1="14" y1="19" x2="34" y2="19" stroke="#93A3C9" strokeWidth="1.6" strokeLinecap="round" />
                  <line x1="14" y1="25" x2="34" y2="25" stroke="#93A3C9" strokeWidth="1.6" strokeLinecap="round" />
                  <line x1="14" y1="31" x2="27" y2="31" stroke="#93A3C9" strokeWidth="1.6" strokeLinecap="round" />
                  {/* Check badge */}
                  <circle cx="33" cy="34" r="9" fill="#2FBF71" stroke="white" strokeWidth="2" />
                  <path d="M29 34l3 3 6-6" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
                </svg>
              </div>
              ...
              {/* ── Status badge ──
                  Shows the real, current status to human visitors: green
                  "Open" while live, amber "Closed" once the deadline has
                  passed or the admin has paused submissions. A separate
                  sr-only span still carries "Admissions Open" text for SEO
                  so the admission page keeps ranking, without visually
                  misleading a parent into thinking they can apply. */}
              {isEffectivelyOpen ? (
                ...
                <div className="inline-flex items-center gap-2 bg-red-100 text-red-800 text-sm font-bold px-4 py-1.5 rounded-full mb-3">
                  <span className="w-2 h-2 rounded-full bg-red-500" />
                  Admissions Closed — Session {displaySessionYear}
                </div>
              )}
              <span className="sr-only">Admissions Open — Session {displaySessionYear}</span>
              {settings?.last_date && isEffectivelyOpen && (
                <p className="text-sm text-muted-foreground">
                  Last Date to Apply: <span className="font-semibold text-red-600">
            ...
            {/* ── Apply category cards ──
                Only rendered while admissions are actually open. When closed,
                the categories, "preview" cards, and apply flow are hidden
                entirely — the visitor only sees My Tracking and Downloads,
                so there's no way to click into an apply form after the
                deadline. SEO text for the category names still lives in the
                sr-only block further down the page. */}
            {isEffectivelyOpen && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.1 } }}
                className="space-y-3 mb-6">
                {[
                  { classes: "6, 7, 8 & 9", title: "Class 6–9 Admission",  desc: "Fresh admission for middle & high school", icon: BookOpen,  color: "from-blue-500 to-blue-700", type: "fresh" as AdmissionType },
                  { classes: "9, 10",       title: "Class 9–10 Migration", desc: "Transferring from another school",          icon: RefreshCw, color: "from-purple-500 to-purple-700", type: "migration" as AdmissionType },
                ].map((cat, i) => (
                  <motion.button key={i}
                ...
                {/* ── Offline / walk-in application option ──
                    Same isEffectivelyOpen gate as the category cards above
                    — when admissions are closed, this disappears with them
                    so there's no way to grab a form for a session that
                    isn't accepting applications. Downloads a printable PDF
                    covering every field the online form collects, so a
                    parent can fill it by hand and submit it in person at
                    the school office instead of applying online. */}
                <button
                  onClick={() => !downloading && generateOfflineForm()}
                  disabled={!!downloading}
                  className="w-full flex items-center gap-4 p-4 bg-card border border-dashed border-primary/40 rounded-2xl hover:border-primary hover:shadow-md transition-all text-left disabled:opacity-60">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm">Download Admission Form</p>
                    <p className="text-xs text-muted-foreground">Fill by hand &amp; submit at the school office — no computer needed</p>
                  </div>
                </button>
              </motion.div>
            )}
            ...
            <Button onClick={() => setView("track")}
              ...
              <Search className="w-4 h-4" /> My Tracking
            </Button>
            ...
            {/* ── SEO content block ──────────────────────────────────────────
                Always rendered so Googlebot (which fully executes this SPA)
                sees rich, keyword-dense admission information regardless of
                the DB is_open flag. The text mirrors the prospectus PDF but
                is in the live HTML, which is what actually moves rankings.
                Visually hidden (sr-only) so it doesn't show on the page for
                human visitors, but stays in the DOM for search engines and
                screen readers. */}
            <section
            ...
            >
              <h2 className="text-base font-bold text-foreground">
                Admission at Government High School Babi Khel
              </h2>
              <p>
                Government High School Babi Khel, located in Babi Khel, District
                Mohmand, Khyber Pakhtunkhwa, welcomes admissions for Class 6 to
                Class 10. Admissions are open every academic session for both
                fresh students and migration cases from other BISE-affiliated
                schools. The school follows the BISE Peshawar curriculum and
                provides free, quality education in line with the Government of
                KPK's education policy.
              </p>
              <div>
                <p className="font-semibold text-foreground mb-1">Programs offered:</p>
                <ul className="list-disc pl-5 space-y-1">
                  <li><strong>Class 6, 7, 8 (Fresh):</strong> Middle school admission for new students.</li>
                  <li><strong>Class 9 (Fresh):</strong> Includes BISE Peshawar board registration for SSC Part-I.</li>
                  <li><strong>Class 9 (Migration):</strong> Transfer from another BISE-affiliated school mid-session.</li>
                  <li><strong>Class 10 (Migration):</strong> Full 8-step BISEP migration process — both principals sign, BISEP approves, challan paid, migration confirmed.</li>
                </ul>
              </div>
              <div>
                <p className="font-semibold text-foreground mb-1">Required documents:</p>
                <p>
                  B-Form (NADRA) — mandatory; passport size photo — mandatory;
                  previous result card or marksheet; school leaving certificate
                  (for migration); father's CNIC copy; migration certificate
                  (migration cases); signed letter from both principals
                  (Class 10 migration).
                </p>
              </div>
              <div>
                ...
                <p>
                  Fill the online admission application form above, upload all
                  required documents, and receive a reference number for
                  tracking. You can track your admission application status
                  online using your B-Form number or reference number. The
                  school contacts applicants on the provided WhatsApp /
                  contact number once documents are reviewed.
                </p>
              </div>
              <p className="text-xs">
                For queries, contact the school office during working hours or
                visit the Contact page for address, phone and email.
              </p>
            </section>
          </div>
        )}
        ...
        {view === "track" && (
          <div className="container mx-auto px-4 pt-6 max-w-lg">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-bold">My Tracking</h2>
              <p className="text-sm text-muted-foreground mt-1">
                Enter your B-Form number or Reference Number to see your status, book an interview slot, and download your admit card.
              </p>
            </div>
