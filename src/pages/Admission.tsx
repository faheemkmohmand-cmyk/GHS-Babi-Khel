...
/**
 * Apply Online Icon - Realistic Laptop with Form
 * Professional blue laptop with green success indicator
 */
function ApplyOnlineIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Laptop screen outline - professional blue */}
      <rect x="2.5" y="3.5" width="19" height="13" rx="2" stroke="#3B82F6" strokeWidth="1.8"/>
      {/* Screen background - subtle blue tint */}
      <rect x="3.3" y="4.3" width="17.4" height="11.4" rx="1.2" fill="#EFF6FF"/>
      {/* Screen content - form lines (realistic gray text lines) */}
      <rect x="4.5" y="6" width="7.5" height="1.2" rx="0.4" fill="#64748B"/>
      <rect x="4.5" y="8.3" width="11" height="1.1" rx="0.35" fill="#94A3B8"/>
      <rect x="4.5" y="10.3" width="9" height="1.1" rx="0.35" fill="#CBD5E1"/>
      {/* Checkmark circle - green success */}
      <circle cx="17.3" cy="10" r="2.3" fill="#10B981"/>
      <path d="M16.2 10L17.1 10.9L18.5 9.1" stroke="white" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
      {/* Laptop base - dark gray for realism */}
      <path d="M1.5 16.5H22.5L21.4 19C21.1 19.6 20.5 20 19.8 20H4.2C3.5 20 2.9 19.6 2.6 19L1.5 16.5Z" fill="#475569" stroke="#334155" strokeWidth="1.8" strokeLinejoin="round"/>
    </svg>
  );
}

/**
 * Track Application Icon - Realistic Search with Status
 * Professional blue magnifier with green status indicator
 */
function TrackApplicationIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Magnifying glass - professional blue */}
      <circle cx="10" cy="10" r="7" stroke="#3B82F6" strokeWidth="2.2"/>
      {/* Glass handle - darker blue */}
      <line x1="15.2" y1="15.2" x2="20.5" y2="20.5" stroke="#1E40AF" strokeWidth="2.4" strokeLinecap="round"/>
      {/* Inner document/list representation - realistic gray */}
      <rect x="6.7" y="7.2" width="6.2" height="1.2" rx="0.3" fill="#64748B"/>
      <rect x="6.7" y="9.4" width="4.8" height="1.1" rx="0.3" fill="#94A3B8"/>
      <rect x="6.7" y="11.5" width="5.3" height="1.1" rx="0.3" fill="#CBD5E1"/>
      {/* Status indicator - green check */}
      <circle cx="14" cy="6.2" r="2.2" fill="#10B981"/>
...
/**
 * Apply Now / Graduation Cap Icon - Realistic Version
 * Dark graduation cap with golden tassel accent only
 */
function ApplyNowIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} xmlns="http://www.w3.org/2000/svg">
      {/* Cap top (Diamond) - dark navy/blue */}
      <path d="M12 2L1 9L12 16L23 9L12 2Z" fill="#1E3A5F" stroke="#1E3A5F" strokeWidth="1.4" strokeLinejoin="round"/>
      {/* Cap highlight - subtle lighter shade */}
      <path d="M12 2L5 7L12 12L19 7L12 2Z" fill="#2D4A6F"/>
      {/* Cap base/skull part - dark */}
      <path d="M4 10V14.5C4 17 7.5 19 12 19C16.5 19 20 17 20 14.5V10"
        fill="#1E3A5F" stroke="#1E3A5F" strokeWidth="1.4" strokeLinejoin="round"/>
      {/* Tassel string - golden */}
      <path d="M21 9V14.5C21 15.3 20.2 15.8 19.7 15.8C19.2 15.8 18.5 15.3 18.5 14.5V9.5"
        stroke="#D97706" strokeWidth="1.4" fill="none" strokeLinecap="round"/>
      {/* Tassel end - golden accent */}
      <circle cx="19.7" cy="16.2" r="1.3" fill="#D97706"/>
      {/* Button on top of cap - white/gold */}
      <circle cx="12" cy="8" r="1.1" fill="#FBBF24"/>
    </svg>
  );
}
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-card p-3">
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-[9px] bg-amber-500/15 text-amber-700 px-1.5 py-0.5 rounded-full font-bold ml-auto">Quick Check</span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          {!['9', '10'].includes(cls) && cls && (
            <p className="text-[9px] text-muted-foreground mt-1">Note: For Class {cls}, only Fresh Admission is available. School-level transfers are handled directly between schools.</p>
          )}
        </div>
        <div>
            ...
            >
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2.5 ${!isEffectivelyOpen ? "opacity-50 pointer-events-none select-none" : ""}`}>
                ...
                <button
                  ...
                  className="text-left p-3 rounded-xl border-2 border-[#C96B3B] bg-transparent hover:bg-[#C96B3B]/10 transition-all group relative overflow-hidden disabled:opacity-60 sm:col-span-2"
                >
                  ...
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-lg bg-transparent border-2 border-[#3B82F6] flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                      ...
                      <>
                        ...
                        <div className="space-y-3">
                          ...
                          <button
                            ...
                            disabled={!!downloading}
                            className="w-full flex items-center gap-3 p-3 border-2 border-[#C96B3B] border-dashed bg-transparent hover:bg-[#C96B3B]/10 transition-all text-left disabled:opacity-60"
                          >
                            <div className="w-9 h-9 rounded-lg border-2 border-[#C96B3B] bg-transparent flex items-center justify-center shrink-0">
                              {downloading === "filled_form" ? <Loader2 className="w-4 h-4 text-[#C96B3B] animate-spin" /> : <Download className="w-5 h-5 text-[#C96B3B]" />}
                            </div>
                            <div>
                              <p className="font-semibold text-xs text-[#C96B3B]">Download Admission Form (Printable)</p>
