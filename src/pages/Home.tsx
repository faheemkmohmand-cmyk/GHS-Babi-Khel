...
const Home = () => {
  ...
  return (
    ...
    <PageLayout>
      ...
      {/* ══ 2.5. BISE PESHAWAR LIVE RESULT BANNER ══
          Shows the current BISEP exam title (e.g. "HSSC Annual-II 2025")
          pulled live from cloud.bisep.edu.pk every hour via
          /api/bisep-proxy?mode=current. Three states:
            • LIVE       — green pill, "Result is LIVE — Check Now" CTA
            • COUNTDOWN  — amber pill with live ticking countdown
            • SILENT     — banner not rendered (proxy failed or BISEP
                            returned something we couldn't parse; the
                            page falls back to the static hero CTAs)
          Rendered ABOVE the stats bar so it's the first thing visitors
          see after the hero — same priority BISEP gives it on their own
          homepage. */}
      <BisepResultBanner />
