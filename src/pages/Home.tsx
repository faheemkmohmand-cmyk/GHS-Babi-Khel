        ...
        <m.div style={{ y: heroContentY, opacity: heroOpacity }} className="container mx-auto px-4 relative z-10 pt-0 pb-20 md:pt-2 md:pb-24">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl">
              {/* ── Hexagon banner badge (replaces old full-bleed photo) ── */}
              {settings?.banner_url && !bannerFailed && (
                <m.div variants={stagger.child} className="mb-6 flex justify-center lg:justify-start">
                  <div
                    className="relative w-40 h-40 md:w-48 md:h-48 shrink-0"
                    style={{ clipPath: "polygon(25% 3%, 75% 3%, 100% 50%, 75% 97%, 25% 97%, 0% 50%)" }}
                  >
                    {/* thin gold hexagon border, slightly larger than the image so it frames it evenly */}
                    <div
                      className="absolute -inset-[3px] bg-gold"
                      style={{ clipPath: "polygon(25% 3%, 75% 3%, 100% 50%, 75% 97%, 25% 97%, 0% 50%)" }}
                    />
                    <img
