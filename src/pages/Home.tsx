...
const HERO_SEGMENTS: HeroSegment[] = [
  { text: "Where " },
  { text: "bright", gold: true },
  { text: " ", breakAfter: true },
  { text: "minds", gold: true },
...
function HeroTypewriterHeading({ className, textColor }: { className: string; textColor: string }) {
  ...
  const reduced = typeof window !== "undefined" &&
    window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
  return (
    <m.h1 variants={stagger.child} className={`${className} ${textColor} drop-shadow-lg`}>
      {segments.map((seg, i) => (
        <span key={i}>
          <span className={seg.gold ? "text-gold" : undefined}>{seg.text}</span>
          {seg.breakAfter && <br />}
        </span>
      ))}
      {!reduced && (
        <span className="inline-block w-[3px] h-[0.85em] -mb-[0.1em] ml-0.5 bg-gold align-middle animate-pulse" aria-hidden="true" />
      )}
    </m.h1>
  );
}
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl relative">
              ...
              <HeroTypewriterHeading
                className="mt-6 text-5xl md:text-6xl lg:text-7xl font-display font-semibold leading-[1.05]"
                ...
                {isAdmissionEffectivelyOpen ? (
                  <>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                      ...
                      {/* "My Tracking": if the homepage's own tracking section
                          is present, smooth-scroll to it; otherwise jump to
                          the Admission page directly INTO its tracking view
                          via router state — an instant SPA swap (zero page
