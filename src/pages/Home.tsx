...
/** Fetch a genuinely new word from the live API each day (a random-word
 *  service + dictionaryapi.dev for the definition), instead of cycling
 *  through the small local dict.json every ~5 weeks. Result is cached in
 *  localStorage per calendar day so it doesn't refetch on every visit and
 *  still works if the user goes offline later that day. Falls back to the
 *  local dictionary (day-of-year indexed) only if the live fetch fails. */
async function getTodayEntry(): Promise<DictEntry | null> {
  ...
  try {
    const rw = await fetch("https://random-word-api.herokuapp.com/word", {
      signal: AbortSignal.timeout(5000),
    });
    if (rw.ok) {
      const words: string[] = await rw.json();
      const word = words?.[0]?.toLowerCase().trim();
  ...
  } catch { /* network/API failure — fall through to local fallback */ }

  // 3) Offline / API failure fallback: local curated dictionary, indexed by
  //    day-of-year so it's at least deterministic and different from
  //    yesterday's fallback (though it will repeat every ~5 weeks).
