  ...
  return (
    ...
    <PageLayout>
      ...
      <section id="hero-section" className="relative min-h-[88vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[hsl(45,35%,92%)] via-[hsl(40,30%,97%)] to-[hsl(43,55%,88%)]" />
        ...
        <m.div style={{ y: heroContentY, opacity: heroOpacity }} className="container mx-auto px-4 relative z-10 pt-0 pb-20 md:pt-2 md:pb-24">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl relative">
              ...
              {/* ── Two-arrow diagonal banner — sits BEHIND the heading/description ──
                  Two SEPARATE full corner-to-corner triangles cut from the same
                  photo, with a genuine diagonal GAP between them (verified with
                  a plotted mockup before implementing — a single shared diagonal
                  line between two clip-paths produces zero visible gap, so each
                  triangle instead uses its own cut line offset a few percent
                  from the true center diagonal):
                    • Piece A (top-right triangle): corners (gap%,0)→(100%,0)→
                      (100%,100%-gap%)
                    • Piece B (bottom-left triangle): corners (0,gap%)→(0,100%)→
                      (100%-gap%,100%)
                  Both pieces reference the same background image at the same
                  size/position so the two halves line up as one coherent photo,
                  just split into two separated triangles with visible empty
                  space between them, matching the two-sided-arrow reference.
                  The wrapper is anchored to the right edge on mobile so the
                  image actually stays inside the viewport (was off-screen when
                  centered with a right-only clip). */}
              {settings?.banner_url && !bannerFailed && (
                <m.div
                  variants={stagger.child}
                  className="absolute right-0 md:right-auto md:left-1/2 md:-translate-x-1/2 top-16 md:top-20 -z-10 flex justify-end md:justify-center pointer-events-none"
                >
                  <div className="relative w-[20rem] h-[20rem] md:w-[31rem] md:h-[31rem] shrink-0 opacity-90">
                    {(() => {
                      const bgUrl = optimizedCloudinaryUrl(settings.banner_url, { width: 900 });
                      ...
                      // Two PARALLEL diagonal cuts (not both through the center
                      // point — that leaves zero gap since both edges sit on the
                      // same line). Each cut line is offset from the true center
                      // diagonal by `gap`, so a clean diagonal band of empty
                      // space separates the two triangles, matching the
                      // reference: two full corner-to-corner triangles with a
                      // visible gap between their facing edges.
                      //   Piece A cut line: (gap, 0) → (100%, 100%-gap)
                      //   Piece B cut line: (0, gap) → (100%-gap, 100%)
                      const gap = 8; // percent
                      const clipA = `polygon(${gap}% 0%, 100% 0%, 100% ${100 - gap}%)`;
                      const clipB = `polygon(0% ${gap}%, 0% 100%, ${100 - gap}% 100%)`;
                      return (
                        <>
                          {/* Piece A — top-right triangle */}
                          <div className="absolute inset-0" style={{ clipPath: clipA }}>
                            <div className="absolute -inset-[3px] bg-gold-soft" style={{ clipPath: clipA }} />
                            <div className="absolute inset-0" style={{ ...sharedBg, clipPath: clipA }} />
                            <div className="absolute inset-0 bg-white/55" style={{ clipPath: clipA }} />
                          </div>
                          {/* Piece B — bottom-left triangle */}
                          <div className="absolute inset-0" style={{ clipPath: clipB }}>
                            <div className="absolute -inset-[3px] bg-gold-soft" style={{ clipPath: clipB }} />
                            <div className="absolute inset-0" style={{ ...sharedBg, clipPath: clipB }} />
                            <div className="absolute inset-0 bg-white/55" style={{ clipPath: clipB }} />
                          </div>
                          {/* Accessible fallback img (visually hidden, keeps alt text / SEO) */}
                          <img
                            src={bgUrl!}
                            alt="School campus"
                            className="sr-only"
              ...
              <m.h1 variants={stagger.child} className={`mt-6 text-4xl md:text-6xl lg:text-7xl font-display font-semibold leading-[1.05] ${heroTextColor} drop-shadow-lg`}>
                Where <span className="text-gold">bright minds</span> find their light.
              </m.h1>
