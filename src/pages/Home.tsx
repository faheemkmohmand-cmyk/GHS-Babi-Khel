          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl relative">
              ...
              {/* ── Two-arrow diagonal banner — sits BEHIND the heading/description ──
                  ONE diagonal line splits the square into two exact triangles
                  that share the same seam — no gap, no overlap, no stripe:
                    • Piece A (top-right triangle): polygon(0,0 → 100%,0 → 100%,100%)
                    • Piece B (bottom-left triangle): polygon(0,0 → 100%,100% → 0,100%)
                  Both triangles reference the same background image at the same
                  size/position so the two halves line up as one coherent photo
                  cut cleanly along the diagonal, tips meeting exactly at the
                ...
                >
                  <div className="relative w-[20rem] h-[20rem] md:w-[31rem] md:h-[31rem] shrink-0 opacity-90">
                    {(() => {
                      ...
                      const clipA = "polygon(0% 0%, 100% 0%, 100% 100%)";
                      const clipB = "polygon(0% 0%, 100% 100%, 0% 100%)";
                      return (
                        <>
                          ...
                          {/* Gold seam line along the diagonal cut — this is what
                              actually makes the two triangles read as two
                              distinct touching arrow-halves instead of an
                              invisible seam (identical image content on both
                              sides means the clip-path alone is invisible
                              without a drawn line marking the split). */}
                          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                            <line x1="0" y1="0" x2="100" y2="100" stroke="#c9a24b" strokeWidth="1.2" vectorEffect="non-scaling-stroke" />
                          </svg>
                          {/* outer gold frame around the whole banner */}
                          <div className="absolute inset-0 border-2 border-gold/50 pointer-events-none" />
