...
const Home = () => {
  ...
  const heroOverlay = !hasBannerPhoto
    ? "bg-transparent"
    : isDark
    ? "bg-gradient-to-b from-black/15 via-black/10 to-black/35"
    : "bg-gradient-to-b from-black/30 via-black/20 to-black/40";
  const heroBadgeBg   = hasBannerPhoto ? "bg-white/20 border border-white/30 text-white" : "bg-primary/10 border border-gold/40 text-primary";
  const heroTextColor = hasBannerPhoto ? "text-white" : "text-primary";
  const heroSubColor  = hasBannerPhoto ? (isDark ? "text-white/90" : "text-white/95") : "text-foreground/80";
  const heroDescColor = hasBannerPhoto ? (isDark ? "text-white/80" : "text-white/90") : "text-muted-foreground";
  const heroCursorColor = hasBannerPhoto ? "bg-white/80" : "bg-primary/80";
  const heroStatCard = hasBannerPhoto
    ? (isDark ? "bg-white/15 backdrop-blur-md border border-white/20" : "bg-black/20 backdrop-blur-md border border-white/20")
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl">
              ...
              <m.h1 variants={stagger.child} className={`mt-6 text-4xl md:text-6xl lg:text-7xl font-display font-semibold leading-[1.05] ${heroTextColor} drop-shadow-lg`}>
                Where bright minds find their light.
              </m.h1>
