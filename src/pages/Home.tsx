...
// Calls the same safe server endpoint used on /results (api/auto-publish-
// results.js) the instant a homepage visitor's countdown reaches zero,
// instead of waiting for the Vercel Cron's next scheduled tick. Safe to call
// from any browser: it can only flip rows whose `publish_at` has already
// passed. This directly fixes "Publishing now… then nothing happens" — the
// countdown itself now performs the publish instead of just displaying it.
  ...
  return (
    <section className="container mx-auto px-4 pt-6">
      <m.div
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-2xl shadow-lg overflow-hidden"
      >
        <div className="flex items-center justify-between gap-4 p-4 sm:p-5 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
            ...
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-wider opacity-80 font-semibold">
                School Result is LIVE
              </p>
              <p className="text-base sm:text-lg font-bold truncate">
                Search your result now
              </p>
            </div>
          </div>
          <Link
            to="/results"
            className="inline-flex items-center gap-2 bg-white text-green-700 font-bold px-5 py-2.5 rounded-xl shadow-sm hover:bg-white/90 transition-colors text-sm shrink-0"
          >
...
// Decides between: our school's LIVE result, our school's countdown, or
// (only when neither applies) the BISE Peshawar banner. This is the single
// component rendered on the homepage — it replaces the old bare
// `<BisepResultBanner />` which never checked school state at all.
function HomeResultsBanner() {
  ...
  if (hasSchoolResults.data === true) return <SchoolResultLiveBanner />;
  if (soonestSchedule) return <SchoolResultCountdownBanner schedule={soonestSchedule} />;

  // No school result, no schedule — fall back to BISE Peshawar.
...
const Home = () => {
  ...
  const heroOverlay = isDark
    ? "bg-gradient-to-b from-black/15 via-black/10 to-black/35"
      ...
      <section id="hero-section" className="relative min-h-[88vh] flex items-center overflow-hidden">
        ...
        {settings?.banner_url && !bannerFailed ? (
          <img
            ...
            srcSet={[
              `${optimizedCloudinaryUrl(settings.banner_url, { width: 640 })} 640w`,
              `${optimizedCloudinaryUrl(settings.banner_url, { width: 1024 })} 1024w`,
              `${optimizedCloudinaryUrl(settings.banner_url, { width: 1600 })} 1600w`,
              `${optimizedCloudinaryUrl(settings.banner_url, { width: 1920 })} 1920w`,
            ].join(", ")}
            sizes="100vw"
            alt="School campus"
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <ScrollReveal direction="left">
              <div className="text-foreground">
                <span className="inline-block bg-card text-foreground border border-border text-xs font-semibold uppercase tracking-widest px-3 py-1 rounded-full mb-4">About Us</span>
