        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {newsLoading ? Array.from({ length: 3 }).map((_, i) => (
              ...
              <m.article key={item.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                className="bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-elevated transition-all duration-300 group border border-border/60">
                <div className="h-48 overflow-hidden bg-secondary">
                  {item.image_url
                    ...
                    : (
                      <div className="w-full h-full relative overflow-hidden bg-[linear-gradient(160deg,hsl(45_38%_95%)_0%,hsl(43_35%_90%)_55%,hsl(40_30%_86%)_100%)] flex items-center justify-center">
                        <div className="absolute -right-8 -top-8 w-28 h-28 rounded-full border border-gold/25" />
                        <div className="absolute inset-2.5 border border-primary/10 rounded-lg" />
                        <span className="font-display italic font-semibold text-primary text-5xl" style={{ fontFamily: "var(--font-display)" }}>
                ...
                <div className="p-5">
                  ...
                  {item.content && (
                    <p className="text-sm text-muted-foreground mt-2 line-clamp-2" dir={detectTextLanguage(item.content) === "ur" ? "rtl" : "ltr"}>
                      {item.content}
                    </p>
                  )}
