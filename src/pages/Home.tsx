 ...
 *  Strategy: get a batch of candidate words from Datamuse, then try each
 *  against the Free Dictionary API until one returns a real definition.
 *  Cached in localStorage per calendar day. If both APIs fail, returns
 *  null so the UI can show the "Could not load today's word — Retry"
 *  error state. */
async function getTodayEntry(): Promise<DictEntry | null> {
  ...
  // 2) Try real online words. Throws on any network/API failure so the
  //    UI shows the connection-error state instead of silent fallback.
  const candidates = await fetchCandidateWords();
  for (const raw of candidates) {
    const word = raw.toLowerCase().trim();
    if (!word) continue;
    const entry = await fetchDictEntry(word);
    if (entry && entry.meanings?.length) {
      try { localStorage.setItem(WOTD_CACHE_KEY, JSON.stringify({ date: key, entry })); } catch { /* ignore */ }
      return entry;
    }
  }
  // 3) All candidates failed the dictionary lookup — surface as error.
  return null;
}
