        ...
        <m.div style={{ y: heroContentY, opacity: heroOpacity }} className="container mx-auto px-4 relative z-10 py-20 md:py-24">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl">
              <m.div variants={stagger.child} className="flex items-center gap-3 mb-5">
                <HexagonLogoFrame size={44}>
                  {settings?.logo_url && !heroLogoFailed ? (
                    <img
                ...
                <div>
                  <span className={`font-display italic font-medium text-xl leading-tight block tracking-tight ${heroTextColor} drop-shadow`}>
                    {settings?.school_name || "GHS Babi Khel"}
                  </span>
                  <span className="text-[11px] font-semibold uppercase tracking-[0.25em] text-gold leading-none">
                    High School · Mohmand
                  </span>
                </div>
              </m.div>
              <m.h1 variants={stagger.child} className={`mt-6 text-4xl md:text-6xl lg:text-7xl font-display font-semibold leading-[0.95] min-h-[1em] ${heroTextColor} drop-shadow-lg`}>
                {displayedSchoolName}
                <span className={`inline-block w-1 h-[0.85em] ${heroCursorColor} ml-1 align-middle`} style={{ animation: "blink 1s step-end infinite" }} />
              </m.h1>
              <m.h2 variants={stagger.child} className={`mt-4 text-xl md:text-2xl font-display font-medium italic min-h-[2rem] drop-shadow ${heroSubColor}`}>
                <span className="not-italic font-semibold text-white">{displayed}</span>
