...
// Live BISEP exam title + countdown — polled hourly via /api/bisep-proxy?mode=current
...
/* ─── Word of the Day — fully online, no offline fallback ──────────────────
 * Pulls a fresh random English word from the Datamuse API each day, then
 * looks up its definition + example + audio from the Free Dictionary API.
 * Replaces the dead random-word-api.herokuapp.com (broken since Heroku
 * killed its free tier in Nov 2022).
 *
 * If the network is unreachable, the section shows the existing
 * "Could not load today's word — Retry" error UI (no silent offline
 * substitute). Cached in localStorage per calendar day so repeat visits
 * the same day don't re-hit the APIs.
 * ──────────────────────────────────────────────────────────────────────── */

/** Returns today's date key, e.g. "2026-07-31", used to cache the daily
 *  word so repeat visits on the same day don't re-hit the API, and so the
 *  word is stable across reloads within a day. */
function todayKey(): string {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

const WOTD_CACHE_KEY = "wotd_cache_v1"; // { date: string, entry: DictEntry }

/** Get a list of candidate words for today from the Datamuse API.
 *  Returns up to 15 candidate words, deterministically shuffled by today's
 *  date so the chosen word is stable within a day (avoids the word changing
 *  on every Retry click). Throws if Datamuse is unreachable — the caller
 *  surfaces that as the connection-error UI. */
async function fetchCandidateWords(): Promise<string[]> {
  ...
  const arr = data
    .map((d: any) => d.word)
    .filter((w: any) => typeof w === "string" && /^[a-z]+$/.test(w));
  // Seeded Fisher–Yates shuffle so the chosen word is stable within a day.
  const key = todayKey();
  let seed = 0;
...
/** Small built-in fallback word list, used ONLY if both Datamuse and the
 *  Free Dictionary API are unreachable (common on some mobile carriers that
 *  throttle/block dictionaryapi.dev). Keeps the section from dead-ending on
 *  a flaky connection while the rest of the site loads fine. Rotates by
 *  today's date so it still feels like a "word of the day" if it's ever hit. */
const FALLBACK_WORDS = [
  "diligent", "curious", "resilient", "candid", "meticulous",
  "eloquent", "gracious", "vigilant", "amiable", "prudent",
  "tenacious", "sincere", "humble", "genuine", "steadfast",
];

function fallbackEntryFor(key: string): DictEntry {
  let seed = 0;
  for (let i = 0; i < key.length; i++) seed = (seed * 31 + key.charCodeAt(i)) | 0;
  const word = FALLBACK_WORDS[Math.abs(seed) % FALLBACK_WORDS.length];
  return {
    word,
    phonetics: [],
    meanings: [
      {
        ...
        definitions: [{ definition: `A word to learn today: "${word}".`, synonyms: [] }],
      },
    ],
  } as DictEntry;
}

/** Look up a single word's definition using the Free Dictionary API. */
async function fetchDictEntry(word: string): Promise<DictEntry | null> {
  ...
  const data = await res.json();
  return Array.isArray(data) && data[0] ? (data[0] as DictEntry) : null;
}

/** Fetch a fresh word-of-the-day entry from the online APIs.
 *  Strategy: get a batch of candidate words from Datamuse, then look them
 *  up against the Free Dictionary API IN PARALLEL (not one-by-one — a
 *  sequential loop with a 6s timeout per word could take up to 90s to fail
 *  on a slow connection before ever showing an error). Returns the first
 *  candidate that has a real definition. Cached in localStorage per
 *  calendar day. If the online APIs are fully unreachable, falls back to a
 *  small built-in word list rather than showing an error. */
async function getTodayEntry(): Promise<DictEntry | null> {
  const key = todayKey();

  // 1) Cache hit for today?
  try {
    const cachedRaw = localStorage.getItem(WOTD_CACHE_KEY);
    if (cachedRaw) {
      const cached = JSON.parse(cachedRaw) as { date: string; entry: DictEntry };
      if (cached.date === key && cached.entry) return cached.entry;
    }
  } catch { /* localStorage unavailable — fall through */ }

  // 2) Try real online words, looked up in parallel.
  try {
    const candidates = await fetchCandidateWords();
    const lookups = candidates.map((raw) => {
      const word = raw.toLowerCase().trim();
      return word ? fetchDictEntry(word).catch(() => null) : Promise.resolve(null);
    });
    const settled = await Promise.all(lookups);
    const entry = settled.find((e) => e && e.meanings?.length) || null;
    if (entry) {
      try { localStorage.setItem(WOTD_CACHE_KEY, JSON.stringify({ date: key, entry })); } catch { /* ignore */ }
      return entry;
    }
  } catch { /* Datamuse unreachable — fall through to built-in fallback */ }

  // 3) Online APIs unreachable or returned nothing usable — use the small
  //    built-in fallback so the section still shows a word today.
  const fallback = fallbackEntryFor(key);
  try { localStorage.setItem(WOTD_CACHE_KEY, JSON.stringify({ date: key, entry: fallback })); } catch { /* ignore */ }
  return fallback;
}

/** Look up a word's definition using the live Free Dictionary API. */
...
function BisepResultBanner() {
  const { data, isLoading } = useBisepCurrentExam();

  // While the first fetch is in flight, render nothing — the homepage
  // already has hero CTAs that link to /results, so an empty banner
  // would just be visual noise. Once data arrives, the banner appears.
  if (isLoading || !data || !data.ok) return null;

  const title = data.exam_title;
  if (!title) return null; // BISEP homepage didn't yield a parseable exam name

  // ── LIVE state ──
  // Removed per request: this green "BISE Peshawar Result is LIVE" banner
  // used to show on the homepage even while the school's OWN result or
  // countdown was active/pending — which was confusing (two different
  // results being advertised at once) and is no longer wanted. BISE
  // Peshawar's countdown/pending states below are kept as a fallback for
  // when the school has neither a published result nor a schedule, but the
  // "is LIVE" banner itself is intentionally not rendered.

  // ── COUNTDOWN state ──
  // BISEP has announced a future publish date. Show a ticking countdown.
    ...
    return (
      <section className="container mx-auto px-4 pt-6">
        <m.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-2xl shadow-lg overflow-hidden"
        >
          <div className="flex items-center justify-between gap-4 p-4 sm:p-5 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-wider opacity-80 font-semibold">
                  BISE Peshawar {title} Result
                </p>
                <p className="text-base sm:text-lg font-bold">
                  {data.countdown_text ? `${data.countdown_text} · ` : ""}
                  <BisepCountdownText targetDate={data.countdown_date} />
                </p>
              </div>
            </div>
            <a
              href="https://cloud.bisep.edu.pk/"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 bg-white/15 hover:bg-white/25 text-white font-semibold px-4 py-2 rounded-xl transition-colors text-xs sm:text-sm shrink-0"
            >
              Visit BISE Peshawar <ExternalLink className="w-3.5 h-3.5" />
            </a>
          </div>
        </m.div>
      </section>
  ...
  // ── PENDING but no date ──
  // BISEP says something is pending but we couldn't parse a date.
    ...
    return (
      <section className="container mx-auto px-4 pt-6">
        <m.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-700/40 text-amber-900 dark:text-amber-200 rounded-2xl p-4 sm:p-5 flex items-center gap-3"
        >
          <Clock className="w-5 h-5 shrink-0" />
          <p className="text-sm font-medium">
            BISE Peshawar {title} · {data.countdown_text}
          </p>
        </m.div>
      </section>
    );
  ...
  // Nothing useful to render — stay silent.
  return null;
}
