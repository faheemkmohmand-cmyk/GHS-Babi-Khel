        ...
        <m.div style={{ y: heroContentY, opacity: heroOpacity }} className="container mx-auto px-4 relative z-10 pt-0 pb-20 md:pt-2 md:pb-24">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl relative">
              ...
              {/* ── Diagonal-split banner — sits BEHIND the heading/description ──
                  Single diagonal cut from top-left to bottom-right, offset
                  so the image reads as two staggered halves (like a
                  two-sided arrow): top-right triangle sits higher/further
                  right, bottom-left triangle sits lower/further left, with
                  the diagonal seam running through the middle.
                    • path: (50%,0) → (100%,0) → (100%,100%) → (50%,100%) → (0%,50%)
              ...
              {settings?.banner_url && !bannerFailed && (
                ...
                >
                  <div
                    className="relative w-[20rem] h-[20rem] md:w-[31rem] md:h-[31rem] shrink-0 opacity-90"
                    style={{ clipPath: "polygon(50% 0%, 100% 0%, 100% 100%, 50% 100%, 0% 50%)" }}
                  >
                    {/* thin gold-cream diagonal border, slightly larger than the image so it frames it evenly */}
                    <div
                      className="absolute -inset-[3px] bg-gold-soft"
                      style={{ clipPath: "polygon(50% 0%, 100% 0%, 100% 100%, 50% 100%, 0% 50%)" }}
                    />
                    <img
                      src={optimizedCloudinaryUrl(settings.banner_url, { width: 900 })!}
                      srcSet={[
                        `${optimizedCloudinaryUrl(settings.banner_url, { width: 480 })} 480w`,
                        `${optimizedCloudinaryUrl(settings.banner_url, { width: 700 })} 700w`,
                        `${optimizedCloudinaryUrl(settings.banner_url, { width: 900 })} 900w`,
                      ].join(", ")}
                      sizes="496px"
                      alt="School campus"
                    ...
                    />
                    {/* soft veil so overlaid text stays readable against the photo */}
                    <div
