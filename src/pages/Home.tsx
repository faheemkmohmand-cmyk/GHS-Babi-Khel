...
function HeroTypewriterHeading({ className, textColor }: { className: string; textColor: string }) {
  ...
  const reduced = typeof window !== "undefined" &&
    window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
  return (
    <m.h1 variants={stagger.child} className={`${className} ${textColor}`}>
      {segments.map((seg, i) => (
        <span key={i}>
          <span>{seg.text}</span>
          {seg.breakAfter && <br />}
      ...
      {!reduced && (
        <span className="inline-block w-[3px] h-[0.85em] -mb-[0.1em] ml-0.5 bg-azure align-middle animate-pulse" aria-hidden="true" />
      )}
    </m.h1>
  );
}
...
const Home = () => {
  ...
  return (
    ...
    <PageLayout>
      ...
      {/* ══ 9b. SPONSORED — Adsterra Native Banner ══
          In-content ad slot directly below the Latest Notices
          (announcements) grid — the page's primary scroll path between
          announcements and news. Loads async, fails silently offline. */}
      <AdsterraNativeBanner className="container mx-auto px-4 py-6" />
