...
import HonorRollHomeSection from "@/components/shared/HonorRollHomeSection";
...
const TextStat = ({ value, label }: { value: string; label: string }) => (
  <div className="text-center px-4 py-3">
    <div className="text-3xl md:text-4xl font-heading font-extrabold bg-gradient-to-r from-primary to-primary-glow bg-clip-text text-transparent">{value}</div>
    <div className="text-xs uppercase tracking-widest text-muted-foreground mt-1 font-medium">{label}</div>
  </div>
);
...
// of publish time for near-instant response.
function useHomeAutoPublishWatcher(schedules: ScheduledGroup[]) {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      const now = Date.now();
      const dueNow = schedules.some(s => new Date(s.publish_at).getTime() <= now);
      if (!dueNow) return;
      ...
      if (publishedSomething) {
        ...
        // Clear interval after successful publish
    ...
    const setupAdaptiveInterval = () => {
      const getInterval = () => {
        const minDiff = Math.min(...schedules.map(s => new Date(s.publish_at).getTime() - Date.now()));
...
// Single homepage countdown card for OUR school's scheduled result.
// REDESIGNED: Big, bold RED countdown matching reference design (same as Results.tsx)
// When the countdown finishes, useHomeAutoPublishWatcher (above) actively triggers the
// publish via the server endpoint — at that point useHasPublishedSchoolResults()
// becomes true on its next refetch and this card is replaced by nothing automatically.
function SchoolResultCountdownBanner({ schedule }: { schedule: ScheduledGroup }) {
  ...
  useEffect(() => {
    const calc = () => {
      const diff = new Date(schedule.publish_at).getTime() - Date.now();
  ...
  const announceDate = new Date(schedule.publish_at);
    ...
    return (
      <section className="container mx-auto px-4 pt-6">
        <m.div
          ...
          className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-6 sm:p-8 text-center border border-gray-200 dark:border-gray-700"
        >
          <div className="animate-pulse">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
              Results will appear in seconds! <Link to="/results" className="text-blue-600 hover:underline">Go to Results Page</Link>
            </p>
          </div>
        </m.div>
      </section>
    ...
    <section className="container mx-auto px-4 pt-6">
      ...
      >
        ...
        <h3 className="text-lg sm:text-xl font-bold text-blue-600 dark:text-blue-400 mb-4">
          Result – {schedule.exam_type} {schedule.year}
        </h3>
...
//
// UPDATED: Now shows the beautiful RED countdown when a schedule is active!
function HomeResultsBanner() {
  ...
  // ACTIVE SCHEDULE — Show the beautiful RED countdown!
  if (soonestSchedule) {
    return <SchoolResultCountdownBanner schedule={soonestSchedule} />;
  }

  // No school result and no schedule — fall back to BISE Peshawar (currently returns null).
  ...
  return (
    ...
    <PageLayout>
      ...
      <section id="hero-section" className="relative min-h-[88vh] flex items-center overflow-hidden">
        ...
        {/* ── Background: warm gold gradient (light) / deep emerald-black
             gradient (dark) — dark: variant keeps hero text tokens readable ── */}
        <div className="absolute inset-0 bg-gradient-to-br from-[hsl(43,55%,86%)] via-[hsl(42,50%,90%)] to-[hsl(38,65%,78%)] dark:from-[hsl(165,30%,9%)] dark:via-[hsl(170,25%,12%)] dark:to-[hsl(40,28%,10%)]" />
              ...
              <m.div variants={stagger.child} className="mt-10 flex flex-wrap gap-4">
                <Link to="/results">
                  <m.button whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.97 }}
                    className="inline-flex items-center gap-2 bg-white text-primary font-semibold px-7 py-3.5 rounded-xl border border-gold shadow-md hover:shadow-lg transition-all duration-200 dark:bg-gold dark:text-[hsl(165,30%,12%)] dark:border-transparent dark:shadow-[0_8px_24px_-8px_hsl(43_65%_62%/0.45)]">
                    View Results <ArrowRight className="w-4 h-4" />
                  </m.button>
                </Link>
                <Link to="/auth/signin">
                  <m.button whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.97 }}
                    className="inline-flex items-center gap-2 bg-white text-primary font-semibold px-7 py-3.5 rounded-xl border border-gold shadow-md hover:shadow-lg transition-all duration-200 dark:bg-white/10 dark:text-on-hero dark:border-gold/40">
                    Student Portal
                  </m.button>
                </Link>
              </m.div>
            ...
            {features.map((f, idx) => (
              <ScrollReveal key={f.title} delay={idx * 0.08}>
                <m.div variants={stagger.child} whileHover={{ y: -8, scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  ...
                  <div className="w-12 h-12 rounded-xl bg-transparent border border-gold/50 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
      ...
      {/* ══ 8b. HONOR ROLL - HALL OF FAME ══ */}
      <HonorRollHomeSection />
          ...
          {newsLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 3 }).map((_, i) => (
                <Skeleton key={i} className="h-[22rem] rounded-md" />
              ))}
            </div>
          ) : news.length === 0 ? (
              ...
              <div className="relative z-10">
                {isAdmissionEffectivelyOpen ? (
                  <>
                    ...
                    <div className="inline-flex items-center gap-2.5 bg-gradient-to-r from-[#f8f4f0] via-white to-[#fef7f2] text-[#8B4513] border border-[#D4A574]/40 text-sm font-semibold tracking-wide uppercase px-5 py-2.5 rounded-full mb-5 shadow-sm">
                      <span className="relative flex h-2.5 w-2.5">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#C96B3B] opacity-75"></span>
                      ...
                      <span className="text-[#A0522D]/70 font-light">— Session {admSettings.session_year}</span>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                      <Link to="/admission">
                        <m.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                          className="w-full sm:w-auto px-10 py-5 bg-transparent text-[#C96B3B] border-2 border-[#C96B3B] rounded-2xl font-semibold hover:bg-[#C96B3B]/10 transition-all flex items-center justify-center gap-2 text-lg">
                          <Send className="w-5 h-5" /> Apply Online
                        </m.button>
                      </Link>
                      <Link to="/admission" onClick={(e) => { e.preventDefault(); const el = document.querySelector('[data-track-section]'); if (el) { el.scrollIntoView({ behavior: 'smooth' }); } else { window.location.href = '/admission#track'; }}}>
                        <m.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                          className="w-full sm:w-auto px-10 py-5 bg-transparent text-[#C96B3B] border-2 border-[#C96B3B] rounded-2xl font-semibold hover:bg-[#C96B3B]/10 transition-all flex items-center justify-center gap-2 text-lg">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-8 max-w-2xl mx-auto">
                      {[
                        { label: "Class 6–10", icon: BookOpen },
              ...
              <div className="relative">
                <div className="aspect-square rounded-3xl bg-card border border-gold/40 p-2 shadow-xl">
                  <div className="w-full h-full rounded-2xl bg-muted flex items-center justify-center overflow-hidden">
                    ...
                    <div className={`w-full h-full flex items-center justify-center ${settings?.principal_photo_url ? "hidden" : ""}`}>
