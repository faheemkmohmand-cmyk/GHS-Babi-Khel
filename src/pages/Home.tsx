...
function useSchoolScheduledPublish() {
  return useQuery<ScheduledGroup | null>({
    ...
    queryFn: async () => {
      ...
      // Return only the soonest-publishing schedule — homepage shows a
      // single countdown, not a list.
      return Array.from(byPublishAt.values())
        .sort((a, b) => a.publish_at.localeCompare(b.publish_at))[0];
    },
...
// countdown finishes, the underlying data (is_published) flips via the
// Vercel cron job (api/auto-publish-results.js) — at that point
// useHasPublishedSchoolResults() becomes true on its next refetch and this
// card is replaced by the LIVE banner below automatically, no page reload
// needed. The full result set is already searchable at /results (indexed
// for SEO via ResultsSeoContent + RouteSEOInjector) the moment is_published
// flips, so "search any result" works immediately once the countdown ends.
function SchoolResultCountdownBanner({ schedule }: { schedule: ScheduledGroup }) {
  const classLabel = schedule.classes.length > 1
    ? `All Classes (${schedule.classes.sort((a, b) => Number(a) - Number(b)).join(", ")})`
...
function HomeResultsBanner() {
  ...
  const { data: schedule } = useSchoolScheduledPublish();
  ...
  if (schedule) return <SchoolResultCountdownBanner schedule={schedule} />;
