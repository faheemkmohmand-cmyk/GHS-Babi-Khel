        ...
        <m.div style={{ y: heroContentY, opacity: heroOpacity }} className="container mx-auto px-4 relative z-10 pt-0 pb-20 md:pt-2 md:pb-24">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <m.div initial="hidden" animate="visible" variants={stagger.parent} className="max-w-2xl relative">
              ...
              {/* ── Chevron banner — sits BEHIND the heading/description as a backdrop ──
                  Shape: top/bottom/right flush, LEFT edge has a subtle V-notch
                  cut INWARD (apex further right than the corners).
                    • top-left / bottom-left start at 70% across
                    • middle dips to 78% at 50% vertical (apex)
                    • right edge stays flush at 100%
                  Tweak the 78% to deepen (e.g. 82%) or soften (e.g. 74%) the V;
                  tweak the 70% to push the start point further right. */}
              {settings?.banner_url && !bannerFailed && (
                <m.div
                  variants={stagger.child}
                  className="absolute left-1/2 -translate-x-1/2 top-16 md:top-20 -z-10 flex justify-center pointer-events-none"
                >
                  <div
                    className="relative w-[27rem] h-[27rem] md:w-[31rem] md:h-[31rem] shrink-0 opacity-90"
                    style={{ clipPath: "polygon(70% 0, 100% 0, 100% 100%, 70% 100%, 78% 50%)" }}
                  >
