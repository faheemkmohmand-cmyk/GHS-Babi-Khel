...
/* ─── Word of the Day — 100% online, zero offline/local fallback ───────────
 * Every page load fetches a fresh candidate word list from the Datamuse API,
 * then looks up a real definition + example + audio from the Free Dictionary
 * API (api.dictionaryapi.dev). Both are free, keyless, CORS-open services —
 * verified directly (not assumed) to be reachable and returning valid data.
 *
 * There is NO built-in word list, NO localStorage cache, and NO client-side
 * substitute of any kind. If the online APIs cannot be reached, the section
 * shows the existing "Could not load today's word — Retry" UI — nothing else.
 * Every failure is also logged to the browser console with the real reason
 * (network error, CORS, bad HTTP status, empty response) so the exact cause
 * is visible instead of a silent guess.
 *
 * The word is deterministically chosen from today's date (so it's the same
 * word all day for everyone, and changes automatically at midnight), but the
 * lookup itself always happens live over the network on every page load.
 * ──────────────────────────────────────────────────────────────────────── */

/** Returns today's date key, e.g. "2026-08-03", used only to deterministically
 *  pick which candidate word is "today's word" — NOT used for any caching. */
function todayKey(): string {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

/** Seeded PRNG (mulberry32) driven by today's date key, so the same
 *  candidate word is picked all day, and a different one tomorrow. */
 ...
 *  Returns up to 25 candidate words, deterministically shuffled by today's
 *  date so the chosen word is stable within a day. Throws (with the real
 *  reason logged) if Datamuse is unreachable or returns bad data — the
 *  caller surfaces that as the connection-error UI. */
async function fetchCandidateWords(): Promise<string[]> {
  ...
  try {
    ...
    console.error("[WordOfDay] Datamuse fetch failed (network/CORS/blocked):", err);
    throw err;
  }
  if (!res.ok) {
    console.error(`[WordOfDay] Datamuse returned HTTP ${res.status}`);
    throw new Error(`Datamuse returned ${res.status}`);
  }
  const data = await res.json();
  if (!Array.isArray(data) || data.length === 0) {
    console.error("[WordOfDay] Datamuse returned no usable words:", data);
...
async function fetchDictEntry(word: string): Promise<DictEntry | null> {
  try {
    ...
    console.error(`[WordOfDay] Dictionary lookup failed for "${word}":`, err);
    return null;
  }
}

/** Fetch a fresh word-of-the-day entry live from the online APIs — always,
 *  on every call, no cache. Strategy: get a batch of candidate words from
 *  Datamuse, then look them up against the Free Dictionary API IN PARALLEL
 *  (not one-by-one, so a few missing entries don't stack up wait time).
 *  Prefers a candidate whose definition includes an example sentence, so
 *  the "Example" field is reliably filled. Returns null (surfaced as the
 *  Retry UI) only if the online APIs are genuinely unreachable or none of
 *  the candidates resolve to any definition at all. */
async function getTodayEntry(): Promise<DictEntry | null> {
  const candidates = await fetchCandidateWords(); // throws on network failure

  const lookups = candidates.map((raw) => {
    const word = raw.toLowerCase().trim();
    return word ? fetchDictEntry(word) : Promise.resolve(null);
  });
  const settled = await Promise.all(lookups);
  const valid = settled.filter((e): e is DictEntry => !!e && !!e.meanings?.length);

  if (valid.length === 0) {
    console.error("[WordOfDay] None of the candidate words returned a definition.");
    return null;
  }

  const hasExample = (e: DictEntry) =>
    e.meanings.some((m) => m.definitions.some((d) => !!d.example));

  return valid.find(hasExample) || valid[0];
}
