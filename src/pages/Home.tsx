...
/* ─── Dictionary data — all words are fetched live from the Free Dictionary
 * API. No offline/local dictionary is used; the feature always loads a
 * fresh word from the online API each day.
 */
...
/** Fetch a genuinely new word from the live API each day.
 *  Strategy: request multiple random words at once, then try each one
 *  against the Free Dictionary API until we find one that has a real
 *  definition. This avoids the problem where the random-word API returns
 *  obscure words that don't exist in the dictionary.
 *  Result is cached in localStorage per calendar day so it doesn't refetch
 *  on every visit and still works if the user goes offline later that day.
 *  No offline fallback is used — the feature always loads from online. */
async function getTodayEntry(): Promise<DictEntry | null> {
  const key = todayKey();

  // 1) Already fetched today? Use the cached entry.
  try {
  ...
  } catch { /* localStorage unavailable — ignore, fall through to fetch */ }

  // 2) Try to get a fresh word from the live APIs.
  //    Fetch multiple random words at once, then try each one against the
  //    dictionary API until one works. Repeat up to 3 rounds if needed.
  const MAX_ROUNDS = 3;
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    try {
      const rw = await fetch(
        `https://random-word-api.herokuapp.com/word?number=${WORDS_PER_ROUND}`,
        { signal: AbortSignal.timeout(8000) }
      );
      if (!rw.ok) continue;
      const candidates: string[] = await rw.json();
      if (!Array.isArray(candidates) || candidates.length === 0) continue;

      // Try each candidate word against the dictionary API
      for (const raw of candidates) {
        ...
        try {
          const defRes = await fetch(
            ...
            { signal: AbortSignal.timeout(DICT_TIMEOUT) }
          );
          if (!defRes.ok) continue;
          const data = await defRes.json();
          const entry: DictEntry | undefined = Array.isArray(data) ? data[0] : undefined;
          if (entry && entry.meanings?.length) {
            try {
              localStorage.setItem(WOTD_CACHE_KEY, JSON.stringify({ date: key, entry }));
            } catch { /* ignore */ }
            return entry;
          }
        } catch {
          // Dictionary lookup failed for this word — try the next candidate
          continue;
        }
      }
    } catch {
      // Random-word API failed for this round — try the next round
      continue;
    }
  }

  // 3) All online attempts failed — return null (no offline fallback).
  //    The UI will simply not render the section if no entry is available.
  return null;
}
