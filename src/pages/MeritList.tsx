...
const MeritListPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    const hasHidden =
      entries.some((e) => e.hidden) || total > visibleEntries.length;
    const passing =
    ...
    const highest = hasHidden
      ...
      : selectedList?.highest_percentage != null
        ...
        : visibleEntries.length ? Math.max(...visibleEntries.map((e) => e.percentage)) : 0;
    const avg =
                            ...
                            <table className="w-full text-sm">
                              ...
                              <tbody>
                                {filteredEntries.map((e) => {
                                  ...
                                  const hasPlaceholderRows = entries.some((x) => x.hidden);
                                  // Withheld students sit in the snapshot as anonymous
                                  // placeholder rows — render them as an all-dash row at
                                  // their official rank. On legacy rows (no placeholders
                                  // stored) the first VISIBLE rows get reserved-dash rank
                                  // badges instead. Medals only when nobody is hidden.
                                  if (e.hidden) {
                                    return (
                                      <tr
                                  ...
                                  const topSpotReserved = stats.hasHidden && !hasPlaceholderRows && idx < 3;
                                  return (
                                    <tr
                                    ...
                                    >
                                      <td className="p-3 text-center">
                                        ...
                                        >
                                          {isTop3 ? ["🥇", "🥈", "🥉"][idx] : topSpotReserved ? "—" : e.position || idx + 1}
                                        </span>
                                      </td>
                                      {isMultiClassScope(selectedList.scope) && (
                                        <td className="p-3 text-center">
