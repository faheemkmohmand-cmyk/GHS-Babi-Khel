...
const MeritListPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    // a stage reveal), while `entries` is the visible-only snapshot. If those
    // two disagree, some students — possibly the real top achiever(s) — are
    // hidden, so "Highest %" can't be trusted from the visible list alone.
    const hasHidden = total > entries.length;
    const highest = hasHidden
      ? null
                            ...
                            <table className="w-full text-sm">
                              ...
                              <tbody>
                                {filteredEntries.map((e) => {
                                  ...
                                  // While toppers are withheld, the first visible
                                  // rows are NOT the real achievers: no medals, no
                                  // medal row colours, and the reserved top-3 spots
                                  // show a dash — matching the Highest % stat card.
                                  const isTop3 = idx < 3 && !stats.hasHidden;
                                  const topSpotReserved = stats.hasHidden && idx < 3;
                                  return (
                                    <tr
                                    ...
                                    >
                                      <td className="p-3 text-center">
                                        ...
                                        >
                                          {isTop3 ? ["🥇", "🥈", "🥉"][idx] : topSpotReserved ? "—" : idx + 1}
                                        </span>
                                      </td>
                                      {isMultiClassScope(selectedList.scope) && (
                                        <td className="p-3 text-center">
