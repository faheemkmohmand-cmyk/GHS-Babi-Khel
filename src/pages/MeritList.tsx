  ...
  return (
    ...
    >
      ...
      <div className="relative mb-2 sm:mb-3">
        {isFirst && (
          <Crown className="absolute -top-5 sm:-top-7 left-1/2 -translate-x-1/2 w-4 h-4 sm:w-6 sm:h-6 text-gold" fill="currentColor" />
        )}
      ...
      {/* Name + class */}
      <p className={`text-[11px] sm:text-sm font-bold leading-tight w-full truncate px-0.5 ${style.nameColor}`}>{entry.full_name}</p>
      ...
      {onShare && (
        <button
          ...
          className="mt-1.5 sm:mt-2.5 inline-flex items-center gap-1 sm:gap-1.5 rounded-full border border-gold/40 bg-gold/10 px-2 sm:px-3.5 py-1 sm:py-1.5 text-[9px] sm:text-[11px] font-bold text-amber-700 dark:text-amber-300 hover:bg-gold/20 active:scale-95 transition-all disabled:opacity-60 cursor-pointer"
        >
          {sharing ? <Loader2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-spin" /> : <Share2 className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
          Share
        </button>
      )}
    ...
    <div className="relative overflow-hidden rounded-3xl border border-gold/30 bg-gradient-to-br from-gold/10 via-card to-gold/5 p-3 sm:p-7 shadow-elevated">
      ...
      <div className="relative flex items-center gap-2 mb-4 sm:mb-6">
        ...
        <div>
          ...
          <p className="text-[9px] sm:text-[11px] text-muted-foreground mt-1">Hall of fame — this exam's champions</p>
        </div>
        <Medal className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gold ml-auto" />
      </div>
