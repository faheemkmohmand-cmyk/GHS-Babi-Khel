...
import {
  Trophy, Crown, Medal, Sparkles, Search, Users, Target,
...
import { shareMeritCard, toastShareOutcome } from "@/utils/shareResultCard";
import { Skeleton } from "@/components/ui/skeleton";
import PageLayout from "@/components/layout/PageLayout";
import ConstellationOfToppers from "@/components/merit/ConstellationOfToppers";
...
function PodiumItem({ entry, rank, style, heightClass, showClass, delay, onShare, sharing }: {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center text-center w-full max-w-[220px] sm:max-w-[150px] sm:flex-1"
    >
      ...
      {/* Share this position — school-branded merit card + website link */}
      {onShare && (
        <button
          type="button"
          onClick={onShare}
          disabled={sharing}
          aria-label={`Share ${entry.full_name}'s merit position`}
          title="Share this position — beautiful card + website link"
          className="mt-2.5 inline-flex items-center gap-1.5 rounded-full border border-gold/40 bg-gold/10 px-3.5 py-1.5 text-[11px] font-bold text-amber-700 dark:text-amber-300 hover:bg-gold/20 active:scale-95 transition-all disabled:opacity-60 cursor-pointer"
        >
...
function Top3Podium({ entries, showClass, onShareEntry, sharingKey }: {
  entries: MeritStudentEntry[];
  showClass: boolean;
  onShareEntry?: (e: MeritStudentEntry) => void;
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-gold/30 bg-gradient-to-br from-gold/10 via-card to-gold/5 p-5 sm:p-7 shadow-elevated">
      ...
      {/* Mobile: stacked, rank-ordered (1st, 2nd, 3rd) — three side-by-side
          cards don't fit a ~380px screen without clipping. From sm: up,
          switch to the classic podium row with 2nd–1st–3rd ordering. */}
      <div className="relative flex flex-col items-center gap-5 sm:hidden">
        {top3.map((entry, i) => {
          const rank = i + 1;
          const heights = ["h-20", "h-16", "h-14"];
          return (
            <PodiumItem
              key={`${entry.student_id}-${rank}`}
              entry={entry}
              rank={rank}
              style={PODIUM_STYLES[rank - 1]}
              heightClass={heights[i]}
              showClass={showClass}
              delay={i * 0.15}
              onShare={onShareEntry ? () => onShareEntry(entry) : undefined}
              sharing={sharingKey != null && sharingKey === entry.student_id}
            />
          );
        })}
      </div>
      <div className="relative hidden sm:flex items-end justify-center gap-8">
        {order.map((o) => (
          <PodiumItem
            ...
            showClass={showClass}
            delay={o.delay}
            onShare={onShareEntry ? () => onShareEntry(o.entry) : undefined}
            sharing={sharingKey != null && sharingKey === o.entry.student_id}
          />
        ))}
      </div>
    </div>
...
const MeritListPage = () => {
  ...
  /* ── Share a student's merit position — renders a school-branded merit
        card IMAGE (gold position medallion, marks, %, grade) and sends it
        with the /merit-list link via the Web Share API. Fallbacks: text
        share → download + clipboard (same chain as the Results page). ── */
  const [sharingMerit, setSharingMerit] = useState<string | null>(null);
  const handleShareEntry = async (e: MeritStudentEntry) => {
    if (!selectedList || sharingMerit) return;
    setSharingMerit(e.student_id);
    try {
      const outcome = await shareMeritCard({
              ...
              {selectedList && (
                isSelectedLive ? (
                  <div className="space-y-6">
                    ...
                    {/* Constellation of Toppers — the night sky showpiece */}
                    {!entriesLoading && entries.length > 0 && (
                      <ConstellationOfToppers
                        entries={entries}
                        showClass={isMultiClassScope(selectedList.scope)}
                        onShareEntry={handleShareEntry}
                        sharingKey={sharingMerit}
                      />
                    )}
                            ...
                            <table className="w-full text-sm">
                              <thead>
                                <tr className="bg-muted/50 border-b border-border">
                                  ...
                                  <th className="p-3 text-center font-bold text-xs uppercase tracking-wide text-muted-foreground">Share</th>
                                </tr>
                              </thead>
                              <tbody>
                                    ...
                                    >
                                      ...
                                      <td className="p-3 text-center">
                                        <button
                                          ...
                                          disabled={sharingMerit === e.student_id}
                                          aria-label={`Share ${e.full_name}'s merit position`}
                                          title="Share this position — beautiful card + website link"
                                          className="inline-flex items-center justify-center w-9 h-9 rounded-full border border-gold/40 bg-gold/10 text-amber-700 dark:text-amber-300 hover:bg-gold/20 active:scale-95 transition-all disabled:opacity-60 cursor-pointer"
                                        >
                                          {sharingMerit === e.student_id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Share2 className="w-4 h-4" />}
                                        </button>
                                      </td>
                                    </tr>
                                  );
                                ...
                                {filteredEntries.length === 0 && (
                                  <tr>
                                    <td colSpan={isMultiClassScope(selectedList.scope) ? 8 : 7} className="p-8 text-center text-sm text-muted-foreground">
                    ...
                    >
                      ...
                      <div className="relative">
                        {(() => {
                          ...
                          return (
                            <>
                              <span className="w-16 h-16 mx-auto mb-5 flex items-center justify-center"><RealisticClockIcon className="w-14 h-14" hours={hours} minutes={mins} seconds={secs} /></span>
