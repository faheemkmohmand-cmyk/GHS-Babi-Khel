 ...
 * Mobile-first Online Classes page — clean, centered, professional layout.
 *
 * Supports real-time live classes via Jitsi Meet (free, no Zoom license).
...
import {
  Wifi, Calendar, Clock, TrendingUp,
...
export default function OnlineClasses() {
  ...
  const buildRoomName = (cls: OnlineClass) => {
    const slug = `ghs-${cls.class_name.replace(/\s+/g, "-")}-${cls.subject.replace(/\s+/g, "-")}-${cls.id.slice(-6)}`.toLowerCase();
  ...
  return (
    <PageLayout>
      <div className="min-h-screen bg-background">
        ...
        {/* ── Stats — professional card below hero ─────────────────────────── */}
          ...
          <div className="max-w-3xl mx-auto grid grid-cols-4 divide-x divide-border rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            {[
              ...
              <div key={s.label} className="py-4 text-center">
                ...
                <p className="font-heading font-extrabold text-lg text-foreground leading-none">{s.value}</p>
        ...
        {/* ── Footer banner ────────────────────────────────────────────────── */}
        <section className="border-t border-border py-6 px-4 text-center">
              ...
              <div className="p-3">
                ...
                {!user && (
                  <div className="mt-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-xl p-3 text-xs text-amber-700 dark:text-amber-300">
                    ...
                    <Link to="/auth/signin" className="ml-1 underline font-semibold">Sign in →</Link>
                  </div>
                )}
              </div>
            </motion.div>
