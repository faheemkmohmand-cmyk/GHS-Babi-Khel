...
const Results = () => {
  ...
  // ── Result Card toggle ────────────────────────────────────────────────────
  // Previously the homepage Results page had a separate "Result Card / DMC"
  // section at the bottom. It's now accessed via an orange "Result Card"
  // button next to the exam-type tabs (Semester 1/2, Annual-I/II). Clicking
  // it swaps the class-wise results table for the Result Card search
  // interface (ResultCardSearch). Clicking "Back to Results" returns to the
  // normal results view.
    ...
    <PageLayout>
      ...
      <section className="py-8 sm:py-16">
        <div className="container mx-auto px-3 sm:px-4">
          ...
          {/* Sub-tabs: exam type + orange Result Card button */}
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-4 sm:mb-6">
            ...
            {/* Orange Result Card button — sits next to the exam-type tabs
                (Semester 1/2, Annual-I/II). Clicking it swaps the class-wise
                results table for the Result Card search interface. Mobile-
                friendly: ml-auto pushes it to the right on desktop, and it
                wraps to its own row on very narrow screens. */}
            <button
              onClick={() => setShowResultCard(true)}
              className="ml-auto inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold bg-orange-500 hover:bg-orange-600 text-white shadow-card transition-colors"
            >
          ...
          {/* ── Result Card search view ───────────────────────────────────────
              When the orange "Result Card" button is clicked, render the
              ResultCardSearch interface instead of the class-wise results
              table/filters. A "Back to Results" button (also orange) returns
              to the normal results view. */ }
          {showResultCard ? (
