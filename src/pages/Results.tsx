...
// `publish_at` value itself (combined with exam_type+year for the label),
// and aggregate every class that shares that publish_at into ONE card. The
// card then reads "All Classes (6, 7, 8, 9, 10) — <exam_type> <year>"
// instead of repeating the countdown 5 times.
function useScheduledPublishes() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Sort each group's classes numerically and return groups in publish_at
      // ascending order so the soonest-publishing card appears first.
