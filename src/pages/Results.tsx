...
const BiseResultSearch = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const data: BiseProxyResponse = await resp.json().catch(() => ({
        found: false,
      ...
      if (data.found) {
        ...
        // BiseResult interface for clean rendering.
        setResult({
          roll_no: data.roll_no || roll,
          name: data.name || "",
          father_name: data.father_name || "",
          marks: data.marks || "",
          grade: data.grade || "",
          remarks: data.remarks || "",
          collect_dmc_from: data.collect_dmc_from || "",
        ...
        setErrorMsg(data.error || "Could not fetch result from BISE Peshawar.");
      }
    } catch {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-muted-foreground text-center mb-6">
        Powered by BISE Peshawar · No school result has been published yet
      </p>
      ...
      <div className="bg-card rounded-2xl shadow-elevated p-5 mb-6 border border-border">
        <div className="space-y-3">
          ...
          <a href={BISEP_PORTAL_URL} target="_blank" rel="noopener noreferrer"
            className="w-full font-medium py-2.5 rounded-xl flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-all">
            <ExternalLink className="w-3.5 h-3.5" />
            Open BISE Peshawar Portal
          </a>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            {errorMsg ? (
              ...
              <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <div className="px-5 py-3 flex items-center justify-center gap-2 text-xs text-muted-foreground">
                  <ExternalLink className="w-3 h-3" />
                  Source: BISE Peshawar (cloud.bisep.edu.pk)
                </div>
              </motion.div>
            ) : null}
          </motion.div>
