...
const BiseResultSearch = () => {
  ...
  if (result) {
    ...
    bisepRevealData = {
      ...
      isPass,
      subjects: (() => {
        // Same accurate rule as the table: obtained (theory + practical,
        // numbers only) relative to the top subject. No guessed maximums.
        const obt = result.subjects.map(biseSubjectObtained);
        const top = Math.max(0, ...obt.filter((n): n is number => n !== null));
        return result.subjects.map((s, i) => ({
          ...
          theory: s.theory,
          practical: s.practical,
          barPct: obt[i] === null || top === 0 ? null : ((obt[i] as number) / top) * 100,
          barFail: s.theory_fail === true || s.practical_fail === true,
        }));
      })(),
      photoUrl: null,
