...
const BiseResultSearch = () => {
  ...
  const handleShareBiseResult = async () => {
    ...
    const outcome = await shareBiseResultCard({
      ...
      subjects: (() => {
        // Same fixed-maximum rule as the on-screen table + reveal.
  ...
  if (result) {
    ...
    bisepRevealData = {
      ...
      isPass,
      subjects: (() => {
        // Same rule as the table: (theory + practical) / the board's fixed
        // maximum for that subject at this exam level.
        const level = biseLevel(liveTitle, result.subjects);
        return result.subjects.map((s) => {
          const ob = biseSubjectObtained(s);
          const mx = biseSubjectMax(s.subject, level);
