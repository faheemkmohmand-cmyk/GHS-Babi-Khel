...
function CountdownCard({ item }: { item: { publish_at: string; exam_type: string; year: number; classes: string[] } }) {
  ...
  // Inside the final 15 minutes, the countdown gets a slow, cinematic
  // breathing zoom — subtle in/out scale, not a hard blink — so the last
  // stretch reads as building anticipation instead of static digits.
  const totalSecondsLeft = timeLeft.d * 86400 + timeLeft.h * 3600 + timeLeft.m * 60 + timeLeft.s;
  const isFinalStretch = !timeLeft.isDue && timeLeft.d === 0 && totalSecondsLeft <= 900;
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl p-8 sm:p-12 text-center border border-gray-200 dark:border-gray-700 overflow-hidden relative">
      ...
      {/* BIG RED COUNTDOWN NUMBERS — slow cinematic breathing zoom in the
          final 15 minutes; a calm, professional pulse, not a jarring blink. */}
      <motion.div
        className="flex items-center justify-center gap-3 sm:gap-4 mb-5"
        animate={isFinalStretch ? { scale: [1, 1.055, 1] } : { scale: 1 }}
        transition={isFinalStretch ? { duration: 2.2, repeat: Infinity, ease: "easeInOut" } : { duration: 0.3 }}
      >
        {timeLeft.d > 0 && (
                ...
                {rcResults.map(r => (
                  <motion.div key={r.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <div className="px-5 py-3 flex items-center justify-between gap-2 flex-wrap border-b border-border">
                      <span className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200 text-amber-900 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold shrink-0">
...
function BisepBigCountdown({ targetDate }: { targetDate: string }) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      if (diff <= 0) { setTimeLeft("00h 00m 00s"); setIsFinalStretch(false); return; }
      const d = Math.floor(diff / 86400000);
      const h = Math.floor((diff % 86400000) / 3600000);
      const m = Math.floor((diff % 3600000) / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      ...
      // Same "final 15 minutes" cinematic pulse gate used by the school
      // countdown card, so both look and feel consistent.
      setIsFinalStretch(d === 0 && h === 0 && m < 15);
    };
    calc();
    const t = setInterval(calc, 1000);
    return () => clearInterval(t);
