...
function RealisticClockIcon({ className = "w-8 h-8", hours = 0, minutes = 0, seconds = 0 }: { className?: string; hours?: number; minutes?: number; seconds?: number }) {
  // Hands point toward the remaining time itself (a literal countdown dial):
  // 12 o'clock = 0, sweeping clockwise as h/m/s tick down toward zero.
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl p-8 sm:p-12 text-center border border-gray-200 dark:border-gray-700 overflow-hidden relative">
      ...
      <div className="flex justify-center mb-4">
        <RealisticClockIcon className="w-16 h-16 sm:w-20 sm:h-20" hours={timeLeft.h} minutes={timeLeft.m} seconds={timeLeft.s} />
      </div>
...
// ── Realistic analog countdown clock ────────────────────────────────────
// A proper clock face (numerals, tick marks, hour/minute/second hands)
// whose hands sweep BACKWARDS — driven by the remaining countdown time
// itself, not the real wall clock. At the moment the countdown started,
// the hands read like a normal 12-hour clock showing the remaining
// duration (e.g. 19h 38m 30s → hour hand near 7/8, minute hand near 38,
// second hand at 30); every second the hands step one second in reverse,
// so the whole face visibly "unwinds" toward 12:00:00 as time runs out.
// Pure SVG + CSS transforms, no external libs, updates every second in
// step with the existing digital readout below it.
function AnalogCountdownClock({ targetDate }: { targetDate: string }) {
  ...
  // Remaining time mapped onto a 12-hour analog face.
  const totalSecOnFace = diffSeconds % (12 * 3600);
  const secondsPart = totalSecOnFace % 60;
  const minutesPart = Math.floor(totalSecOnFace / 60) % 60;
  const hoursPart = Math.floor(totalSecOnFace / 3600) % 12;

  // Hands point at the CURRENT remaining reading, same geometry as a real
  // clock (hour hand creeps between numerals). Nothing here depends on
  // real-world wall-clock time — only on how much countdown time is left —
  // so as the countdown ticks down, every hand visibly ticks backwards.
  const secondDeg = secondsPart * 6;
  const minuteDeg = minutesPart * 6 + secondsPart * 0.1;
  const hourDeg = hoursPart * 30 + minutesPart * 0.5;
  ...
  const ticks = Array.from({ length: 60 }, (_, i) => {
    ...
    return { x1, y1, x2, y2, isHour, key: i };
  });
  ...
  return (
    <div className="flex justify-center mb-3">
      <svg
        ...
        role="img"
        aria-label="Countdown clock ticking backwards toward result announcement"
      >
        {/* Face */}
        <circle cx={cx} cy={cy} r={r} className="fill-background stroke-border" strokeWidth={3} />
        <circle cx={cx} cy={cy} r={r - 1.5} fill="none" className="stroke-red-200 dark:stroke-red-900/40" strokeWidth={1} />
        ...
        {ticks.map(t => (
          <line
            ...
            className={t.isHour ? "stroke-foreground" : "stroke-muted-foreground"}
        ...
        <line
          ...
          strokeWidth={5}
          strokeLinecap="round"
          style={{ transformOrigin: `${cx}px ${cy}px`, transform: `rotate(${hourDeg}deg)` }}
        />
        {/* Minute hand */}
        <line
          x1={cx} y1={cy} x2={cx} y2={cy - 65}
          className="stroke-foreground"
          strokeWidth={3.5}
          strokeLinecap="round"
          style={{ transformOrigin: `${cx}px ${cy}px`, transform: `rotate(${minuteDeg}deg)` }}
        />
        {/* Second hand — ticks BACKWARDS every second, following the
            countdown itself rather than the real clock */}
        <line
          x1={cx} y1={cy + 14} x2={cx} y2={cy - 72}
          ...
          style={{
            ...
            transition: "transform 0.2s cubic-bezier(0.4, 2.3, 0.5, 1)",
          }}
        />
        <circle cx={cx} cy={cy} r={4.5} className="fill-red-600 dark:fill-red-500" />
        <circle cx={cx} cy={cy} r={1.8} className="fill-background" />
      </svg>
    </div>
  );
}
...
function BisepBigCountdown({ targetDate }: { targetDate: string }) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      if (diff <= 0) { setTimeLeft("00h 00m 00s"); setIsRunning(false); return; }
      const d = Math.floor(diff / 86400000);
      const h = Math.floor((diff % 86400000) / 3600000);
      const m = Math.floor((diff % 3600000) / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      ...
      // Cinematic breathing pulse runs for the whole countdown, not just
      // the final stretch — same treatment as the school countdown card.
      setIsRunning(true);
    };
    calc();
    const t = setInterval(calc, 1000);
    ...
    <div>
      ...
      {bisepLoading ? (
        ...
        <div className="bg-card rounded-2xl shadow-elevated p-8 mb-6 border border-border text-center">
          <AnalogCountdownClock targetDate={countdownDate} />
