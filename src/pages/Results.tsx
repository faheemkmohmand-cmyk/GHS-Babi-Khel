...
const ComparisonModal = ({ open, onClose, examTitle }: { open: boolean; onClose: () => void; examTitle: string }) => {
  ...
  const handleShareComparison = async () => {
    ...
    const outcome = await shareComparisonNode(comparisonCardRef.current, {
      examTitle,
      nameA,
      rollA: a.roll,
      pctA,
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data && data.found === true) {
        ...
        const anyPartFailed = safeSubjects.some(s => s.theory_fail || s.practical_fail);
        const isPass = !anyPartFailed && !/fail/i.test(
