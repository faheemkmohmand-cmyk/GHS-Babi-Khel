  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl p-8 sm:p-12 text-center border border-gray-200 dark:border-gray-700 overflow-hidden relative">
      ...
      <div className="flex justify-center mb-4">
        <RealisticClockIcon className="w-16 h-16 sm:w-20 sm:h-20" />
      </div>
              ...
              <>
                ...
                {rcResults.map(r => (
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <div className="result-hero-blue px-5 py-4 text-white relative overflow-hidden">
                      {/* subtle decorative glow for extra polish */}
                      <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full bg-white/10 blur-2xl pointer-events-none" />
                      <div className="flex items-center justify-between gap-4 relative z-10">
                        <div className="flex items-center gap-4">
                          {r.students?.photo_url
                            ? <img src={r.students.photo_url} alt="" className="w-12 h-12 rounded-full object-cover border-2 border-white/50 shadow-lg" />
                            : <div className="w-12 h-12 rounded-full bg-white/15 backdrop-blur-sm flex items-center justify-center text-xl font-bold border-2 border-white/40 shadow-lg">{(r.students?.full_name || "S").charAt(0)}</div>
                          }
                          <div>
                            <p className="text-xs opacity-80">{examTypeLabel(r.exam_type)} {r.year} — Class {r.class}</p>
                            <h3 className="font-heading font-bold text-lg drop-shadow-sm">{r.students?.full_name}</h3>
                          </div>
                        </div>
                        {r.exam_roll_no && (
                          <div className="text-right shrink-0">
                            <p className="text-xs opacity-75">Exam Roll No</p>
                            <p className="font-mono font-bold text-xl tracking-wider drop-shadow-sm">{r.exam_roll_no}</p>
                          </div>
                        )}
                      </div>
                    </div>
