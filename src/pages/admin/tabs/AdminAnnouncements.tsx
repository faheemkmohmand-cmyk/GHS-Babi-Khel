...
import { Plus, Pencil, Trash2, Loader2, CalendarIcon, Upload, Image as ImageIcon, Trophy, Bell, Newspaper, TrendingUp, Sparkles, Pin } from "lucide-react";
...
const NoticesSection = () => {
  ...
  const [form, setForm] = useState({ title: "", content: "", category: "general", is_urgent: false, is_published: true, is_pinned: false, expires_at: null as Date | null });
  ...
  const openAdd = () => { setEditing(null); setForm({ title: "", content: "", category: "general", is_urgent: false, is_published: true, is_pinned: false, expires_at: null }); setModalOpen(true); };
  const openEdit = (n: Notice) => {
    setEditing(n); setForm({ title: n.title, content: n.content || "", category: n.category, is_urgent: n.is_urgent, is_published: n.is_published, is_pinned: n.is_pinned ?? false, expires_at: n.expires_at ? new Date(n.expires_at) : null }); setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      is_urgent: form.is_urgent, is_published: form.is_published, is_pinned: form.is_pinned,
    ...
    if (error) toast.error("Save failed");
      ...
      toast.success(editing ? "Updated" : "Notice published! 📋");
    ...
    <div className="space-y-4">
      ...
      <Card><CardContent className="p-0 overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {notices.length === 0 && (
              <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No notices yet</TableCell></TableRow>
            )}
            {notices.map(n => (
...
const AnnouncementTabs = () => {
  ...
  return (
    <div className="w-full" style={{ contain: "layout style" }}>
      {/* Tab bar — horizontally scrollable on small screens so all 4 tabs
          stay on one line instead of cramming into a 2x2 grid */}
      <div className="w-full overflow-x-auto scrollbar-hide -mx-1 px-1 mb-4">
        <div className="flex items-center gap-1 bg-muted rounded-md p-1 w-max min-w-full sm:w-full">
          {tabs.map(t => (
            <button
              key={t.id}
              onClick={() => setActive(t.id)}
              className={`inline-flex items-center justify-center gap-1.5 text-xs py-2 px-3.5 rounded-sm font-medium whitespace-nowrap transition-colors shrink-0 sm:flex-1 ${
                active === t.id
