...
const AdminAttendance = () => {
  ...
  const handleSave = async () => {
    ...
    // Try upsert first (requires UNIQUE(student_id, date) constraint).
    // If that fails, fall back to delete+insert which always works.
    const { error: upsertErr } = await supabase
      .from("attendance")
      .upsert(rows, { onConflict: "student_id,date", ignoreDuplicates: false });

    if (upsertErr) {
      console.warn("[Attendance] upsert failed, trying delete+insert:", upsertErr.message);
      // Fallback: delete existing rows for this class+date, then insert fresh
      const studentIds = students.map((s) => s.id);
      ...
      const { error: insErr } = await supabase
        ...
        .insert(rows);
      if (insErr) {
        console.error("[Attendance] insert failed:", insErr.message);
        toast.error(`Save failed: ${insErr.message}`, { duration: 6000 });
        setSaving(false);
        return;
      }
    }
