...
function drawDMC(
  ...
  // ── Header banner (blue, matches .header gradient) ─────────────────────────
  const headerH = 26;
  doc.setFillColor(3, 105, 161);
  doc.roundedRect(cardX, y, cardW, headerH, 3, 3, "F");
  // square off the bottom corners of the rounded header so it reads as one
  // continuous banner rather than a floating rounded box
  doc.rect(cardX, y + headerH - 4, cardW, 4, "F");

  // Logo circle (left)
  const logoR = 8.5;
  const logoCx = cardX + 16;
  ...
  if (logoDataUrl) {
    try {
      ...
      doc.addImage(logoDataUrl, logoCx - size / 2, logoCy - size / 2, size, size);
    } catch { /* fall through to text mark */ }
  }
  if (!logoDataUrl) {
    doc.setTextColor(3, 105, 161);
    doc.setFontSize(8.5);
    doc.setFont("helvetica", "bold");
    doc.text("GHS", logoCx, logoCy + 1.3, { align: "center" });
  }
  ...
  // (centered on the full card width so it stays centered whether or not a
  // photo box is present, mirroring the .photo-spacer fix from the HTML)
  ...
  // ── Exam info bar (5 columns, matches .exam-bar) ────────────────────────────
  const examBarH = 9;
  ...
  // Rebuilt to properly center the whole group (PASS/FAIL + Position + Rank)
  // as one block, rather than using fixed pixel offsets that broke alignment
  // when the text lengths differed (e.g. "Position: #1 in Class").
  const statusH = r.position || schoolRank ? 13 : 10;
  ...
  const passLineY = (r.position || schoolRank) ? y + 5.5 : y + statusH / 2 + 1.5;
  doc.setTextColor(...(r.is_pass ? [22, 163, 74] as [number, number, number] : [220, 38, 38] as [number, number, number]));
  doc.setFontSize(12);
  doc.setFont("helvetica", "bold");
  doc.text(statusText, tableX + tableW / 2, passLineY, { align: "center" });

  if (r.position || schoolRank) {
    const parts: string[] = [];
    if (r.position) parts.push(`Position #${r.position} in Class`);
    if (schoolRank) parts.push(`Rank #${schoolRank} in School`);
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(7.5);
    doc.setFont("helvetica", "normal");
    doc.text(parts.join("   \u2022   "), tableX + tableW / 2, y + statusH - 3, { align: "center" });
  }
  y += statusH + 4;
...
function AdminDMCs({ cls, examType, year }: AdminDMCsProps) {
  ...
  const { data: results = [], isLoading, error: fetchError } = useQuery<ResultRecord[]>({
    queryKey: ["admin-dmcs", scope, cls, examType, year],
    queryFn: async () => {
      let query = supabase
        .from("results")
        .select("id, student_id, class, exam_type, year, total_marks, obtained_marks, percentage, grade, position, is_pass, remarks, subject_marks, exam_roll_no, students(full_name, roll_number, photo_url, father_name)")
  ...
  // "Rank #X in whole school" alongside "Position #Y in Class".
  const { data: schoolWideResults = [] } = useQuery<ResultRecord[]>({
    queryKey: ["admin-dmcs-schoolwide-rank", examType, year],
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .eq("exam_type", examType)
        .eq("year", year)
        .in("class", ALL_CLASSES);
      if (error) throw error;
      return (data ?? []) as unknown as ResultRecord[];
    },
    enabled: !!examType && !!year,
    staleTime: 10 * 60 * 1000,
  });
  ...
  const handleGenerateSingle = async () => {
    ...
    try {
      ...
      const fileName = `DMC_Class${target.class}_${sanitizeFileName(target.students?.full_name || "Student")}_${examType.replace(/\s/g, "")}_${year}.pdf`;
      doc.save(fileName);
  ...
  const handleGenerateBatch = async () => {
    ...
    try {
      ...
      const rootFolder = zip.folder(`DMCs_${zipLabel}_${examType.replace(/\s/g, "")}_${year}`);
      ...
      a.download = `DMCs_${zipLabel}_${examType.replace(/\s/g, "")}_${year}.zip`;
      a.href = url;
      a.click();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Generate official Detail Marks Certificates (DMCs) &middot; {examType} {year}
          </p>
        </div>
        ...
        <CardContent className="p-4 sm:p-5 space-y-4">
          ...
          <div>
            ...
            {scope === "school" && (
              <p className="text-[11px] text-muted-foreground mt-1.5">
                Generates DMCs for every class ({ALL_CLASSES.join(", ")}) &middot; {examType} {year}. The ZIP will contain one subfolder per class.
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              {selectedStudentId === "all" ? "Generate First Student's DMC" : "Generate DMC"}
            </Button>
