...
function drawDMC(
  ...
  // (Watermark logo is drawn later, once the empty space below the
  // Rank/PASS/Position bar is known — see below, so it never overlaps
  // the header or the subject table.)
  ...
  // School logo box (left) — mirrors the photo box on the right for a
  // balanced header. Square, no border (logo usually has its own shape).
  const logoSize = 18;
  const logoX = cardX + 16 - logoSize / 2;
  ...
  // — centered on the full card width, so it stays balanced whether or not
  // the logo/photo boxes are present on either side.
  ...
  drawInfoRow("SESSION", `${examTypeLabel(r.exam_type)} ${r.year}`, cardX + gridColW + gridPad, y + 12);
  y += gridH + 4;
  ...
  headers.forEach((hLabel, i) => {
    ...
    doc.text(hLabel.toUpperCase(), tx, y + rowH / 2 + 1.2, { align: "center" });
    cx += colWidths[i];
  });
  y += rowH;
  ...
  if (subjects.length === 0) {
    ...
    subjects.forEach(([name, m], idx) => {
      ...
      doc.setFontSize(7.5);
      doc.setFont("helvetica", "normal");
      doc.text(name, cx + colWidths[0] / 2, y + rowH / 2 + 1, { align: "center" });
      cx += colWidths[0];
      // Total
    ...
    doc.setFontSize(8);
    doc.setFont("helvetica", "bold");
    doc.text("GRAND TOTAL", cx + colWidths[0] / 2, y + rowH / 2 + 1, { align: "center" });
    cx += colWidths[0];
  ...
  // ── Centered watermark logo (faint, in the empty space below the rank
  // bar and above the signature section — never overlaps the header or
  // the subject table, which sit above this point) ───────────────────────
  if (logoDataUrl) {
    try {
      const freeTop = y;
      const freeBottom = (cardBottom - 14) - 4; // matches sigY - 4 below
      const freeH = freeBottom - freeTop;
      if (freeH > 10) {
        const wmSize = Math.min(cardW * 0.55, freeH * 0.95);
        const wmX = cardX + cardW / 2 - wmSize / 2;
        const wmY = freeTop + freeH / 2 - wmSize / 2;
        doc.saveGraphicsState();
        // @ts-ignore — GState is available at runtime on jsPDF v4 even if types lag behind
        doc.setGState(new doc.GState({ opacity: 0.12 }));
        doc.addImage(logoDataUrl, wmX, wmY, wmSize, wmSize);
  ...
  doc.setTextColor(3, 105, 161);
  doc.setFont("helvetica", "bold");
  doc.text(`Official DMC  |  ${examTypeLabel(r.exam_type)} ${r.year}`, cardX + cardW - 5, footY + 2, { align: "right" });
}
...
function AdminDMCs({ cls, examType, year }: AdminDMCsProps) {
  ...
  const school: SchoolInfo = {
    ...
    // DMC-specific logo: a dedicated emblem used ONLY on the DMC
    // certificate (header + watermark). This is intentionally separate
    // from School Settings' logo_url, which is the site's regular logo
    // shown elsewhere (navbar, etc.) — changing this file never affects
    // that.
    logo_url: "/dmc-logo.png",
