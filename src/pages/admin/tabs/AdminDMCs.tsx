...
function AdminDMCs({ cls, examType, year }: AdminDMCsProps) {
  ...
  const { data: results = [], isLoading, error: fetchError } = useQuery<ResultRecord[]>({
    queryKey: ["admin-dmcs", scope, selectedClass, examType, year],
    queryFn: async () => {
      let query = supabase
        .from("results")
      ...
      query = scope === "school" ? query.in("class", ALL_CLASSES) : query.eq("class", selectedClass);

      const { data, error } = await query;
      if (error) throw error;
      return (data ?? []) as unknown as ResultRecord[];
    },
    enabled: !!examType && !!year && (scope === "school" || !!selectedClass),
          ...
          <div>
            ...
            {scope === "school" && (
              <p className="text-[11px] text-muted-foreground mt-1.5">
                Generates DMCs for every class ({ALL_CLASSES.join(", ")}) &middot; {examType} {year}. The ZIP will contain one subfolder per class \u2014 6, 7, 8, 9, and 10 (any class with no published results for this exam/year will simply be empty).
              </p>
            )}
        ...
        <Card>
          <CardContent className="p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-sm mx-auto">
              No results are available for {scope === "school" ? "any class in the school" : `Class ${selectedClass}`} &middot; {examType} &middot; {year}.
