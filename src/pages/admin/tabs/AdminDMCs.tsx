...
function drawDMC(
  ...
  // ── Centered watermark logo (faint, behind all content) ────────────────────
  // Drawn large and low-opacity in the middle of the card, before anything
  // else so all text/table content renders on top of it.
  if (logoDataUrl) {
    try {
      const wmSize = Math.min(cardW, cardBottom - cardTop) * 0.62;
      const wmX = cardX + cardW / 2 - wmSize / 2;
      const wmY = cardTop + (cardBottom - cardTop) / 2 - wmSize / 2;
