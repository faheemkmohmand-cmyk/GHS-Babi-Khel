...
function drawDMC(
  ...
  // ── Header banner (neutral dark ink — matches the professional style
  //    used on Merit List PDFs, replacing the previous blue banner) ───────────
  const headerH = 26;
  ...
  // Logo circle (left) — FIX: logo is now drawn preserving its real aspect
  // ratio (contained within the circle) instead of being stretched into a
  // square, which distorted non-square school logos.
  const logoR = 8.5;
  ...
  if (logoDataUrl) {
    try {
      const dims = (doc as any).getImageProperties(logoDataUrl);
      const naturalRatio = dims?.width && dims?.height ? dims.width / dims.height : 1;
      const maxSize = logoR * 1.7;
      let drawW = maxSize, drawH = maxSize;
      if (naturalRatio > 1) drawH = maxSize / naturalRatio;
      else if (naturalRatio < 1) drawW = maxSize * naturalRatio;
      doc.addImage(logoDataUrl, logoCx - drawW / 2, logoCy - drawH / 2, drawW, drawH);
  ...
  // ── Exam info bar (5 columns, neutral gray — was blue) ──────────────────────
  const examBarH = 9;
