...
function drawDMC(
  ...
  if (r.position) {
    doc.text(statusText, tableX + tableW / 2 - 18, y + statusH / 2 + 1.5, { align: "center" });
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.text(`Position: #${r.position} in Class`, tableX + tableW / 2 + 22, y + statusH / 2 + 1.5, { align: "center" });
  } else {
    doc.text(statusText, tableX + tableW / 2, y + statusH / 2 + 1.5, { align: "center" });
  }
  y += statusH + 4;
  ...
  doc.setTextColor(3, 105, 161);
  doc.setFont("helvetica", "bold");
  doc.text(`\u2713 Official DMC  |  ${r.exam_type} ${r.year}`, cardX + cardW - 4, footY + 2, { align: "right" });
}
...
async function generateDMCPDF(
  ...
  drawDMC(doc, r, school, logoDataUrl, photoDataUrl);
  return doc;
}
...
function AdminDMCs({ cls, examType, year }: AdminDMCsProps) {
  ...
  // re-fetching per student for a whole-school batch would be slow).
  ...
  const handleGenerateSingle = async () => {
    ...
    try {
      const doc = await generateDMCPDF(target, school, logoDataUrl);
      setProgress(90);
  ...
  const handleGenerateBatch = async () => {
    ...
    try {
      ...
      for (let i = 0; i < rankedResults.length; i++) {
        ...
        const doc = await generateDMCPDF(r, school, logoDataUrl);
