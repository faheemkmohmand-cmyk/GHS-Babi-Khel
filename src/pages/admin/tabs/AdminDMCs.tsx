...
function drawDMC(
  ...
  const LABEL_H = 5.2;
  const bottomLimit = H - 36 - 5; // remarks must end ≥5 mm above the signature lines
  ...
  // (74 mm conservatively covers masthead + title + particulars + labels.)
  const stackBottom = () =>
    17 + 74
    + (nRows === 0 ? 13 : 6.4 + nRows * rowH + 7)
  ...
  const nameSize = fitFont(doc, school.school_name, "bold", 14, CW - 8);
  doc.setFontSize(nameSize);
  doc.setTextColor(...INK);
  doc.text(school.school_name, W / 2, y, { align: "center" });
  y += nameSize * 0.42 + 1.6;

  const addressLine = school.address
    .split(",").map((s) => s.trim()).filter(Boolean).join("  ·  ").toUpperCase();
  ...
  // Double rule closing the masthead (thick + thin — classic editorial)
  ...
  // ── STUDENT PARTICULARS — 4×2 grid, everything centred ───────────────────
  sectionLabel("STUDENT PARTICULARS");
  const gridCols = 4;
  const gridRows = 2;
  ...
  if (nRows === 0) {
    ...
    subjects.forEach(([name, m], idx) => {
      ...
      // PERFORMANCE — slim progress bar (green on warm hairline track)
      ...
      if (fillPct > 0) {
        ...
        doc.roundedRect(barX, barY, Math.max(barH, (barW * fillPct) / 100), barH, 1.2, 1.2, "F");
      }
    ...
    // Grand total row — green tint, strong rule above
    doc.setFillColor(...GREEN_TINT);
    doc.rect(MX, y, CW, totalRowH, "F");
    doc.setDrawColor(...INK);
    doc.setLineWidth(0.5);
    doc.line(MX, y, MX + CW, y);
    const tCy = y + totalRowH / 2;
    doc.setFont("times", "bold");
    doc.setFontSize(7.8);
    doc.setTextColor(...INK);
    doc.text("GRAND TOTAL", colX(0) + (colW[0] + colW[1]) / 2, tCy + baseOffset(7.8), { align: "center", charSpace: 0.5 });
    doc.text(String(r.total_marks), colCx(2), tCy + baseOffset(7.8), { align: "center" });
    doc.setTextColor(...GREEN);
    doc.setFontSize(8.4);
    doc.text(String(r.obtained_marks), colCx(3), tCy + baseOffset(8.4), { align: "center" });
    doc.setFont("times", "normal");
    doc.setFontSize(7.8);
    doc.setTextColor(...BODY);
    doc.text(overallPct.replace("%", ""), colCx(5), tCy + baseOffset(7.8), { align: "center" });
    doc.setFont("times", "bold");
    doc.setFontSize(8.4);
    doc.setTextColor(...(fail ? FAIL_RED : GREEN));
    doc.text(overallGrade, colCx(6), tCy + baseOffset(8.4), { align: "center" });
    y += totalRowH;
  }
  y += gPostTable;
  ...
  // ── Signatures — anchored near the page foot ─────────────────────────────
  const sigY = H - 36;
  const sigColCx = [MX + CW * 0.27, MX + CW * 0.73];
  const sigLineW = 44;
  doc.setDrawColor(...INK);
  doc.setLineWidth(0.3);
  sigColCx.forEach((cx) => doc.line(cx - sigLineW / 2, sigY, cx + sigLineW / 2, sigY));
  doc.setFont("times", "bold");
  doc.setFontSize(6.5);
  doc.setTextColor(...INK);
  doc.text("CLASS TEACHER", sigColCx[0], sigY + 3.8, { align: "center", charSpace: 0.9 });
  doc.text("PRINCIPAL", sigColCx[1], sigY + 3.8, { align: "center", charSpace: 0.9 });
  doc.setFont("times", "italic");
  doc.setFontSize(5.7);
  doc.setTextColor(...MUTED);
  doc.text("Signature & School Stamp", sigColCx[1], sigY + 6.9, { align: "center" });

  // ── Footer ───────────────────────────────────────────────────────────────
  const footRuleY = H - 21;
  doc.setDrawColor(...HAIR2);
  doc.setLineWidth(0.3);
  doc.line(MX, footRuleY, MX + CW, footRuleY);
  const footText = `${school.school_name}  ·  ${school.address}  ·  EMIS ${school.emis_code}`;
  doc.setFont("times", "normal");
  const footSize = fitFont(doc, footText, "normal", 5.9, CW - 4, 0.2);
  doc.setFontSize(footSize);
  doc.setTextColor(...MUTED);
  doc.text(footText, W / 2, footRuleY + 3.6, { align: "center", charSpace: 0.2 });
