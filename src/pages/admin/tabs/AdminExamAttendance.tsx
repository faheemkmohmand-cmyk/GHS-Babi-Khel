...
const AdminExamAttendance = () => {
  ...
  const handleAllClassScan = (qrData: string) => {
    ...
    if (!seatParsed) {
      toast.error("Invalid QR — please scan a desk/seat QR code");
      return;
    }
    scanSeatingAttendance.mutate(
      { decoded: seatParsed, subject: allScanSubject, examDate: selectedDate, scannedBy: scannerId },
      {
        onSuccess: (result) => {
