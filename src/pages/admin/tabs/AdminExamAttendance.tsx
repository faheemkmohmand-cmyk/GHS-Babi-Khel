...
import {
  ...
  Edit3, UserX, ScanLine, Camera, AlertCircle, CheckCircle2, History,
} from "lucide-react";
...
import {
  ...
  useScanSeatingAttendance, useScanExamAttendance,
  decodeExamQRData,
...
// Same proven implementation used in TeacherExamScanTab.tsx — kept local to
// this file so nothing shared is modified.
function QRScanner({ onScan, enabled }: { onScan: (data: string) => void; enabled: boolean }) {
  const [active, setActive] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const scannerRef = useRef<Html5Qrcode | null>(null);
  const containerIdRef = useRef(`aqr-reader-${Math.random().toString(36).slice(2)}`);
  const observerRef = useRef<MutationObserver | null>(null);
  // ── SCAN COOLDOWN (fixes "scanner closes after every scan") ──
  // Html5Qrcode's decode callback fires on EVERY camera frame (~10/sec)
  // that a QR is still in view — without a guard, the very first scan
  // immediately re-fires dozens of times a second while the code is still
  // in frame, flooding onScan/mutate calls back-to-back. That flood is what
  // was destabilizing the scanner and making it seem like it "closed" —
  // really it was choking on repeated calls for the same code. The fix:
  // ignore repeat scans of the SAME code within a short cooldown window,
  // and always require the code to actually change (or the cooldown to
  // fully elapse) before firing onScan again. The camera itself is never
  // stopped, so it's ready for the NEXT different student's QR the instant
  // they move it into view — no manual reopen needed.
  ...
  const stop = useCallback(async () => {
    ...
    if (container) {
      const resizeObs = (container as any)._resizeObserver as ResizeObserver | undefined;
      if (resizeObs) {
        resizeObs.disconnect();
        delete (container as any)._resizeObserver;
      }
      const interval = (container as any)._safetyInterval as ReturnType<typeof setInterval> | undefined;
      if (interval) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // getUserMedia actually returns — not from our container. It does
      // this asynchronously, inside a promise chain that resolves only
      // after the camera stream is granted, which on a weak connection
      // (the screenshot showed ~109 B/s signal) can take well over 500ms.
      // The previous fix used flat setTimeout(fn, 200/500) calls, so on a
      // slow phone the video element didn't exist yet when they fired —
      // they silently did nothing. A MutationObserver reacts the instant
      // the library inserts or restyles the <video>, no matter how long
      // getUserMedia takes, and keeps re-applying if it touches it again.
      const container = document.getElementById(containerIdRef.current);
      const containerWidth = container?.clientWidth || 320;
      // qrbox smaller than the container's shorter side so small/desk-sticker
      // QR codes fill more of the scan box (bigger in-frame = faster decode),
      // while still leaving margin so the box fits inside the video.
      const boxSize = Math.round(Math.min(containerWidth, 320) * 0.7);
      ...
      const forceVideoFill = () => {
        ...
        const containerHeight = containerRect.height || container.clientHeight || 240;

        // Fix VIDEO element - this is what shows the camera feed
        ...
        // Fix ALL internal div elements created by html5-qrcode
        ...
        // Also fix any canvas or other elements
        const canvases = container.querySelectorAll('canvas');
        canvases.forEach(canvas => {
          canvas.style.setProperty('width', `${containerWidth}px`, 'important');
          canvas.style.setProperty('height', `${containerHeight}px`, 'important');
          canvas.style.setProperty('position', 'absolute', 'important');
          canvas.style.setProperty('top', '0', 'important');
          canvas.style.setProperty('left', '0', 'important');
        });

        // Fix any img elements (sometimes used for QR overlay)
      ...
      if (container) {
        ...
        // Resize observer - handles screen rotation / container size changes
        resizeObserver = new ResizeObserver(() => {
          // Debounce slightly to avoid excessive calls
          requestAnimationFrame(forceVideoFill);
        });
      ...
      // No `aspectRatio` here on purpose: html5-qrcode uses that value to
      // build its own square wrapper div around the <video>, which is what
      // was clipping the feed into the small square instead of filling the
      // rectangular container. Omitting it lets the library size the video
      ...
      if (errMsg.includes("Permission") || errMsg.includes("NotAllowed") || errMsg.includes("PermissionDenied")) {
        setError("Camera permission denied. Please allow camera access in your browser settings and try again.");
      } else if (errMsg.includes("NotFound") || errMsg.includes("DevicesNotFound")) {
        setError("No camera found on this device.");
      } else if (errMsg.includes("NotReadable") || errMsg.includes("TrackStart") || errMsg.includes("in use")) {
        setError("Camera is already in use by another app. Close other apps using the camera and try again.");
      } else {
        setError(`Camera failed to start: ${errMsg}`);
      }
      setActive(false);
    }
  }, [handleDecoded]);
  ...
  // Clicking the outer "Open QR Scanner" button is already a user gesture
  // (satisfies mobile Chrome's getUserMedia-requires-gesture rule), so we
  // can start the camera immediately when this component mounts instead of
  // requiring a second click on an inner "Scan QR Code" button.
  useEffect(() => {
    if (enabled) start();
...
const AdminExamAttendance = () => {
  ...
  const scannerId = profile?.id || null;
  ...
  const [tab, setTab] = useState<"scan" | "allscan" | "overview">("scan");
  const [search, setSearch] = useState("");
  const [confirmDelete, setConfirmDelete] = useState(false);
  // All-Class Scan mode: no class picker — class is resolved per-scan from
  // the seat QR (see useScanSeatingAttendance). Session/Subject/Date only.
  const [allScanSubject, setAllScanSubject] = useState<string>("");
  const [showAllScanner, setShowAllScanner] = useState(false);
  const [allScanLog, setAllScanLog] = useState<{ name: string; roll: string; cls: string; seat: string; room: string; time: string }[]>([]);
  const scanSeatingAttendance = useScanSeatingAttendance();
  const scanExamAttendance = useScanExamAttendance();
  // NOTE: "All-Class Scan" mode has no class picker — the class is only
  // known AFTER a QR is scanned (see useScanSeatingAttendance). So this
  // lookup can't pass a classFilter and matches by session+subject+date
  // only. If multiple plans with different classes/end-times share the
  // same date+subject, this mode may not distinguish between them — use
  // the per-class "Scan" tab (which IS class-filtered, see below) when
  // classes have different paper end-times on the same day.
  const { data: allScanPaperTimes, isLoading: loadingAllScanPaperTimes } = usePaperTimesFromSeatingPlan(
    selectedSession || undefined,
    allScanSubject || undefined,
    selectedDate || undefined
  );
  const allScanWindowStatus = useMemo(
    () => getPaperWindowStatus(selectedDate, null, null, allScanPaperTimes, nowTick),
    [selectedDate, allScanPaperTimes, nowTick]
  );
  const canAllScan = canMarkExamAttendance(allScanWindowStatus);
  const handleAllClassScan = (qrData: string) => {
    if (!canAllScan) {
      toast.error(paperWindowMessage(allScanWindowStatus, allScanPaperTimes?.start, allScanPaperTimes?.end));
      return;
    }

    // ── Try desk-QR format first ({t:"seat",...}) ──
    // Desk QRs carry the student's class via the seat assignment. The scan
    // mutation (useScanSeatingAttendance) has a multi-fallback lookup that
    // handles stale stickers: desk-location → student_id → qr_token →
    // exam_roll_no. So OLD desk stickers (printed before "Update Seating")
    // still scan correctly — they mark whoever is CURRENTLY assigned to
    // that physical desk.
    const seatParsed = decodeSeatingQRData(qrData);
    if (seatParsed) {
      scanSeatingAttendance.mutate(
        { decoded: seatParsed, scannedQrToken: qrData, subject: allScanSubject, examDate: selectedDate, scannedBy: scannerId },
        {
          onSuccess: (result) => {
            if (result.status === "already") {
              toast(`${result.studentName} (Class ${result.class}) already marked Present`, { icon: "✅" });
            } else {
              toast.success(`${result.studentName} · Class ${result.class} marked Present`);
              setAllScanLog(prev => [
                { name: result.studentName, roll: result.examRollNo, cls: result.class, seat: result.seatLabel, room: result.roomName, time: new Date().toLocaleTimeString() },
                ...prev,
              ]);
            }
            if (result.sessionId === selectedSession) {
              qc.invalidateQueries({ queryKey: ["exam-attendance", selectedSession, result.class, allScanSubject, selectedDate] });
              qc.invalidateQueries({ queryKey: ["exam-attendance-overview", selectedSession, result.class] });
            }
          },
          onError: (err: any) => {
    ...
    // Also accept OLD admit-card QRs. The scan mutation
    // (useScanExamAttendance) has a fallback: student_id → exam_roll_no.
    // So OLD admit cards (printed before "Update Exam Roll Numbers") still
    // scan correctly — student_id is a stable UUID and always matches.
    const examParsed = decodeExamQRData(qrData);
    if (examParsed) {
      // Don't reject cross-session QRs — the student_id might be valid in
      // the current session even if the QR was printed for a different
      // session. The mutation's fallback (exam_roll_no) will handle it.
      scanExamAttendance.mutate(
        {
          sessionId: selectedSession || examParsed.sessionId,
          studentId: examParsed.studentId,
          examRollNo: examParsed.examRollNo,
          subject: allScanSubject,
        ...
        },
        {
          onSuccess: (result) => {
            if (result.status === "already") {
              ...
              toast.success(`Marked Present · Class ${result.class}`);
              setAllScanLog(prev => [
                { name: "—", roll: examParsed.examRollNo, cls: result.class, seat: "—", room: "—", time: new Date().toLocaleTimeString() },
                ...prev,
              ]);
            }
            if (selectedSession) {
          ...
          onError: (err: any) => {
            toast.error(err?.message || "Failed to mark attendance from admit-card QR");
          },
        }
      );
      return;
    ...
    toast.error("Invalid QR — please scan a desk sticker or exam roll number QR");
  };
  ...
  // classFilter (selectedClass) ensures we only match a seating plan that
  // actually covers this class — critical when two plans run the same day
  // with different end times (e.g. 6th/7th ends 11 AM, 8th/9th ends 12 PM).
  // Without this, a class-6/7 lookup could accidentally match the 8th/9th
  // plan's times or vice versa, locking/unlocking at the wrong moment.
  ...
  // Overview: pivot — subject as columns, students as rows
  // ── SUBJECT-ONLY GROUPING (rev. 13 — Problem 2 fix) ─────────────────────
  // Columns are keyed by SUBJECT ONLY (no date). For each (student, subject)
  // we keep the row with the LATEST timestamp so the displayed status is
  // always the most recent one. This mirrors the fix in
  // AdminExamRollNumbers.tsx and prevents the "Mathematics appearing twice"
  // bug when a subject is re-initialized on a different date.
  //
  // Combined with useInitExamAttendance now REPLACING (not appending) rows
  // for the same subject, this guarantees one column per subject forever.
  //
  // The cell also stores the record ID + exam_date for per-cell deletion.
  const overviewPivot = useMemo(() => {
    ...
    // Column key = SUBJECT ONLY. Track latest date per subject for header display.
    const subjectLatestDate = new Map<string, string>();
    ...
    // Grid: student_id -> subject -> { status, recordId, examDate, subject }
    // For each (student, subject), keep the row with the LATEST timestamp
    // (scanned_at || created_at) so the cell shows the most recent status.
    const cellKeep = new Map<string, { status: Status; recordId: string; examDate: string; subject: string; ts: string }>();
    overviewData.forEach(r => {
      const key = `${r.student_id}__${r.subject}`;
      const ts = r.scanned_at || r.created_at || "";
      const cur = { status: r.status, recordId: r.id!, examDate: r.exam_date, subject: r.subject, ts };
      const prev = cellKeep.get(key);
      if (!prev || cur.ts >= prev.ts) cellKeep.set(key, cur);
    });
    ...
    cellKeep.forEach((val, key) => {
      ...
      const subject = parts.slice(1).join("__");
      if (!grid[studentId]) grid[studentId] = {};
      grid[studentId][subject] = {
        status: val.status,
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex gap-1 bg-secondary/50 rounded-xl p-1">
            ...
            <button onClick={() => setTab("allscan")} className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${tab === "allscan" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>
              <ScanLine className="w-3.5 h-3.5 inline mr-1" />All-Class Scan
            </button>
      ...
      {tab === "allscan" && (
        <>
          <Card>
            <CardContent className="p-4 space-y-3">
              <p className="text-xs text-muted-foreground">
                Scan desk/seat QR codes for <strong>any class</strong> without switching — each scan
                reads the student's class straight from the seat QR and saves it to the right sheet.
                Re-scanning the same student just updates their record, it never duplicates.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">Exam Session</label>
                  {loadingSessions ? <Skeleton className="h-10 rounded-lg" /> : (
                    <select value={selectedSession} onChange={e => { setSelectedSession(e.target.value); setAllScanSubject(""); }}
                      className="w-full px-3 py-2.5 rounded-xl bg-secondary/50 border border-border text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30">
                      <option value="">Select Session</option>
                      {sessions.map(s => <option key={s.id} value={s.id}>{s.title} ({s.exam_term} {s.exam_year})</option>)}
                    </select>
                ...
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">Subject / Paper</label>
                  <select value={allScanSubject} onChange={e => setAllScanSubject(e.target.value)}
                    disabled={!selectedSession}
                    className="w-full px-3 py-2.5 rounded-xl bg-secondary/50 border border-border text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50">
                    <option value="">Select Subject</option>
                    {EXAM_SUBJECTS.map(s => <option key={s} value={s}>{s}</option>)}
          ...
          {(!selectedSession || !allScanSubject) && (
            <Card className="border-dashed border-2">
              <CardContent className="py-14 text-center">
                ...
                <p className="font-heading font-semibold text-foreground">Select Session & Subject</p>
                <p className="text-xs text-muted-foreground mt-1">No class selection needed — each scan auto-detects the student's class</p>
              </CardContent>
            </Card>
          )}
        </>
