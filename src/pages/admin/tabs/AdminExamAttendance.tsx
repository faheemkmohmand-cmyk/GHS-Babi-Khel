...
function QRScanner({ onScan, enabled }: { onScan: (data: string) => void; enabled: boolean }) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // Force video element to fill container after library initializes
      const forceVideoFill = () => {
        ...
        if (container) {
          ...
          // html5-qrcode wraps the <video> in its own div(s) with inline
          // width/height it computes from the (now-removed) aspectRatio —
          // those are what actually clip the visible box, so every
          // descendant div must be forced to fill, not just top-level ones.
      ...
      // was clipping the feed into the small square seen in the screenshot
      // instead of filling the rectangular container. Omitting it lets the
      // library size the video to the container's actual width/height.
      const container = document.getElementById(containerIdRef.current);
      ...
      // Apply fix after scanner starts
      setTimeout(forceVideoFill, 200);
      setTimeout(forceVideoFill, 500);
