...
import {
  ...
  CalendarDays, BookOpen, Users, FileSpreadsheet, Trash2, Clock, Lock,
} from "lucide-react";
...
const AdminExamAttendance = () => {
  ...
  const [paperStartTime, setPaperStartTime] = useState<string>("");
  const [paperEndTime, setPaperEndTime] = useState<string>("");
  // Re-check the paper window every 15s so the UI locks/unlocks live as time passes
  ...
  // Paper timing window — derived from the first attendance record's stored
  // paper_start_time/paper_end_time (all rows in a sheet share the same times).
  const firstRecord = attendance[0];
  const windowStatus = useMemo(
    () => getPaperWindowStatus(selectedDate, firstRecord?.paper_start_time, firstRecord?.paper_end_time, nowTick),
    [selectedDate, firstRecord?.paper_start_time, firstRecord?.paper_end_time, nowTick]
  );
  ...
  const handleInitSheet = async () => {
    ...
    if (selectedDate !== todayStr()) {
      toast.error("You can only initialize attendance for today's date");
      return;
    }
    if (!paperStartTime || !paperEndTime) {
      toast.error("Enter the paper's start and end time first");
      return;
    }
    if (paperEndTime <= paperStartTime) {
      toast.error("Paper end time must be after start time");
      return;
    }
  ...
  const handleStatusChange = (record: ExamAttendanceRecord, newStatus: Status) => {
    if (!canMark) {
      toast.error(paperWindowMessage(windowStatus, record.paper_start_time, record.paper_end_time));
      return;
    }
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {/* Date — locked to today: attendance can never be taken for past or future exam dates */}
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">Exam Date</label>
              <input type="date" value={selectedDate} min={todayStr()} max={todayStr()}
                onChange={e => setSelectedDate(e.target.value)}
                disabled={!selectedSubject}
                className="w-full px-3 py-2.5 rounded-xl bg-secondary/50 border border-border text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50" />
              <p className="text-[10px] text-muted-foreground mt-1">Locked to today — attendance can't be taken for past or future dates.</p>
            </div>
          </div>

          {/* Paper start/end time — required before initializing a sheet; controls when marking is allowed */}
          {selectedSubject && !isInitialized && (
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">Paper Start Time</label>
                <input type="time" value={paperStartTime} onChange={e => setPaperStartTime(e.target.value)}
                  className="w-full px-3 py-2.5 rounded-xl bg-secondary/50 border border-border text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">Paper End Time</label>
                <input type="time" value={paperEndTime} onChange={e => setPaperEndTime(e.target.value)}
                  className="w-full px-3 py-2.5 rounded-xl bg-secondary/50 border border-border text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
            </div>
          )}

          {/* Live paper window banner once a sheet exists */}
          {isInitialized && firstRecord?.paper_start_time && (
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold ${
              ...
              {windowStatus === "in_progress" ? <Clock className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
              {paperWindowMessage(windowStatus, firstRecord.paper_start_time, firstRecord.paper_end_time)}
            </div>
          )}
            ...
            <Card className="border-dashed border-2">
              <CardContent className="py-10 text-center space-y-3">
                ...
                <Button onClick={handleInitSheet}
                  disabled={initAttendance.isPending || rollNumbers.length === 0 || !paperStartTime || !paperEndTime || selectedDate !== todayStr()}
                  className="gap-2">
                ...
                {(!paperStartTime || !paperEndTime) && (
                  <p className="text-[10px] text-muted-foreground">Set the paper start and end time above first.</p>
                )}
              </CardContent>
            </Card>
          ) : (
