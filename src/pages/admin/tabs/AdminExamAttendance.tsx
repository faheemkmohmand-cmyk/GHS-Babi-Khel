...
const AdminExamAttendance = () => {
  ...
  // CORRECTION MODE (rev. 8): status updates are ALWAYS allowed once the sheet
  // exists. The window check only applies to INIT (creating new sheets) and
  // scanning. This lets admins fix mistakes after the paper has ended.
        ...
        <>
          ...
          {!isInitialized ? (
            ...
            <>
              ...
              {/* Correction mode banner when sheet exists but paper window is closed */}
              {!canMark && (
                <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/50 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div className="text-xs">
                    <p className="font-semibold text-blue-700 dark:text-blue-400">Correction Mode — paper is not in progress.</p>
                    <p className="text-blue-600 dark:text-blue-500 mt-0.5">
                      {paperWindowMessage(windowStatus, displayPaperStart, displayPaperEnd)}
                    </p>
                    <p className="text-blue-600/70 dark:text-blue-500/70 mt-1">
                      You can still edit existing records for corrections. Changes will reflect in the Live Console instantly. New sheets and QR scanning are locked until the paper window opens.
                    </p>
                  </div>
                </div>
              )}
