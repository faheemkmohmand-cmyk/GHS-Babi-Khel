      ...
      <CardContent className="p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs sm:text-sm text-muted-foreground mb-1 flex items-center gap-2 uppercase tracking-wider font-semibold">
              ...
              MCQs Timer{totalMin > 0 ? ` · ${totalMin} min` : ""}
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isRunning && (
              <Button variant="outline" size="sm" onClick={() => { stop(); toast("MCQs timer stopped"); }}>
                <Square className="w-3.5 h-3.5" /> Stop
              </Button>
            )}
            <Button variant="outline" size="sm" onClick={() => { reset(); toast.success("MCQs timer reset"); }}>
        ...
        <p className="text-[11px] text-muted-foreground mt-2">
          {isFinished
            ...
            : "Timer is paused. Tap Reset to clear, or set a new timer from the MCQs Timer button."}
        </p>
      </CardContent>
    </Card>
  );
