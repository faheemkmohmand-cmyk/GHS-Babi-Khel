...
const ConsoleView = ({ planId, onBack }: { planId: string; onBack: () => void }) => {
  ...
  //
  // For a recurring "All Papers" plan, plan.exam_date is fixed to whatever
  // day the plan was first created and never advances — but the SAME plan
  // is reused every day via "Update Seating". So for recurring plans we
  // derive the live date from paper_start_at (which the admin sets fresh
  // for each new day's paper) instead of the stale exam_date field. This
  // is what makes Console Live show TODAY's tally instead of getting stuck
  // showing (or querying for) the very first day's attendance forever.
  const examDate = plan?.is_recurring && plan?.paper_start_at
    ? plan.paper_start_at.slice(0, 10)
    : plan?.exam_date;
