...
import {
  useAllSeatingPlans, useSeatingPlan, useLiveAttendance,
  getEffectiveExamDate, listPlanPaperTimes,
  listLivePlanPaperTimesToday, planHasLivePaperToday,
...
// ── Live Date Sheet helpers (bug fix — see comments near their call sites) ──
...
function useMcqTimer(now: number) {
  ...
  // ── AUTO-RESET after the siren finishes (Bug 3 fix) ───────────────────────
  // When the timer becomes "finished", arm a 16-second timeout (15s siren +
  // 1s buffer) to DELETE the timer from localStorage. This ensures:
  //   • Reopening the 3D Hall does NOT re-trigger the siren (no "finished"
  //     timer left in localStorage to read).
  //   • The Console's timer card disappears after the siren finishes.
  //   • The admin must start a NEW timer to get another siren.
  //
  // This runs in BOTH the Console tab AND the 3D Hall tab (whichever is
  // open). If both are open, both arm the same timeout — the first one to
  // fire deletes the timer, the second one's removeItem is a no-op. Safe.
  useEffect(() => {
    if (!state || state.status !== "finished") return;
    const finishedAt = state.finishedAt ?? state.endTime;
    const msSinceFinish = Date.now() - finishedAt;
    const SIREN_DURATION_MS = 15 * 1000;
    const RESET_DELAY = SIREN_DURATION_MS + 1000; // 16s total
    // If we're already past the reset delay (e.g. tab was backgrounded),
    // reset immediately.
    if (msSinceFinish >= RESET_DELAY) {
      writeMcqTimer(null);
      setState(null);
      return;
    }
    const remaining = RESET_DELAY - msSinceFinish;
    const t = setTimeout(() => {
      // Double-check the timer hasn't been restarted by the user in the
      // meantime. Only delete if it's still the SAME finished timer.
      const cur = readMcqTimer();
      if (cur && cur.status === "finished" && (cur.finishedAt ?? cur.endTime) === finishedAt) {
        writeMcqTimer(null);
        setState(null);
      }
    }, remaining);
                ...
                >
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    ...
                    <div className="flex flex-wrap gap-1 pt-0.5">
                      {listPlanPaperTimes(p).map((t, i) => (
                        <span key={i} className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-secondary/60">
                          <span className="font-semibold">{t.classes.map(c => `Cls ${c}`).join(", ")}</span>
...
const ConsoleView = ({ planId, onBack }: { planId: string; onBack: () => void }) => {
  ...
  // ── LIVE DATE SHEET (bug fix) ─────────────────────────────────────────────
  // The Console previously trusted the seating plan's frozen `class_paper_times`
  // snapshot (copied once at plan creation) and a date-RANGE check
  // (isPlanActiveToday) that only asked "is today between exam_date_from and
  // exam_date_to" — with no check that today actually has a paper in the
  // Exam Date Sheet. That caused two bugs: (1) the console showing a "paper
  // in progress" countdown on days the Date Sheet has no paper at all (a
  // free day), and (2) per-class subject/date pills staying stuck on
  // whatever paper the plan was created with, even after the Date Sheet was
  // edited later. Both are fixed by reading the Date Sheet LIVE, every
  // render, instead of the frozen snapshot.
  ...
  const todayShiftedTimes = useMemo(() => {
    ...
    // ── LIVE FIX (rev. 5 — "shows paper running on a free day" bug) ──────
    // Previously: if today fell anywhere inside [exam_date_from,
    // exam_date_to], this unconditionally shifted the plan's ORIGINAL
    // stored start/end TIME-OF-DAY onto today and called it "today's
    // paper" — even on a day the Exam Date Sheet has NO paper at all for
    // any class in this plan (a free/gap day), and even after the Date
    // Sheet was edited to move papers to different dates/times. That's
    // exactly the bug: it showed "Paper started today, 5h 11m remaining"
    // while the Date Sheet said today was free and tomorrow's papers
    // (Urdu / Mutalia Quran) hadn't started yet.
    //
    // FIX: only shift to today if the LIVE Date Sheet confirms at least one
    // class in this plan actually has a paper today. `paperTimeline` below
    // is already built from listLivePlanPaperTimesToday (live Date Sheet),
    // so we defer to it: when it's empty, there is genuinely no paper
    // today and this shifted single-clock fallback must not fabricate one.
    if (!planHasLivePaperToday(plan, examSession, dateSheetEntries)) {
      // No live paper today for anything in this plan — return the raw
      // stored timestamps unshifted (correctly resolves to "ended" or
      // "not started", never a phantom "in progress today").
  ...
  // When classes mixed into this plan sit DIFFERENT papers at DIFFERENT
  // times on the same day (e.g. Class 8 Islamiyat 9-12, Class 10 Pak-Study
  // 9-11), the console shouldn't show one shared clock. Instead: build a
  // timeline of every distinct paper time today (sorted by end time), and
  // count down whichever paper is CURRENTLY ACTIVE with the SOONEST end —
  // when it finishes, automatically switch to counting down the next one.
  // If every class shares the same time, this collapses to a single entry
  // and behaves exactly like the old single-clock countdown.
  const paperTimeline = useMemo(() => {
    if (!plan) return [] as { classes: string[]; subject: string | null; startMs: number; endMs: number }[];
    // LIVE fix: only classes with an ACTUAL Date Sheet paper today produce an
    // entry here. On a free day (no Date Sheet paper for any class in this
    // plan), this returns [] — the console will correctly show "no paper
    // today" / countdown-to-next-paper instead of a phantom "in progress".
    ...
    // Sort by end time ascending — the paper ending soonest is handled first.
    out.sort((a, b) => a.endMs - b.endMs);
    return out;
  }, [plan, examSession, dateSheetEntries]);

  // The active timeline entry: prefer one currently in progress (now inside
  // its window); if none is in progress, the next one that hasn't started
  // yet; otherwise the last one (everything today has ended).
  ...
  // ── NEXT PAPER DAY (LIVE fix) ─────────────────────────────────────────────
  // PREVIOUSLY this guessed "tomorrow, same time-of-day as paper_start_at" —
  // wrong whenever the next paper is on a different date or at a different
  // time than the plan was originally created with (exactly the case in the
  // bug report: plan created around the 3 Aug G.Science/Physics papers, but
  // the actual next paper is 5 Aug Urdu/Mutalia Quran at different times).
  // FIX: scan the LIVE Date Sheet for the nearest future date (strictly
  // after today) that has an actual entry for any class in this plan, and
  // use THAT date's real start time.
  const nextPaperInfo = useMemo(() => {
    if (!plan || !examSession) return null;
    const today = todayLocalDateStr();
    // Find the nearest future date with a paper, then collect ALL classes/
    // subjects/times on THAT exact date (not just the single earliest one) —
    // needed so the Start/End card and banner can show the correct time for
    // whichever class's paper is actually next, even when different classes
    // in the plan sit different subjects/times on that date.
    let bestDate: string | null = null;
    for (const cls of plan.classes || []) {
      const entries = dateSheetEntries.filter(
        (e: any) => e.class === cls && e.year === examSession.exam_year && examTermMatchesConsole(e.exam_type, examSession.exam_term) && e.exam_date > today && e.start_time
      );
    ...
    // Collect every class's paper on bestDate.
    type Row = { cls: string; subject: string; startTime: string; endTime: string | null };
    const rows: Row[] = [];
    for (const cls of plan.classes || []) {
      const entry = dateSheetEntries.find(
        (e: any) => e.class === cls && e.year === examSession.exam_year && examTermMatchesConsole(e.exam_type, examSession.exam_term) && e.exam_date === bestDate && e.start_time
      );
    ...
    return {
      dateStr: bestDate,
      startTime: earliest.startTime,   // earliest start across classes — drives the countdown
      endTime: earliest.endTime,       // that same class's end time, shown alongside it
      subject: earliest.subject,
      rows,                            // full per-class breakdown, for the pills/labels
    };
  }, [plan, examSession, dateSheetEntries]);
  ...
  // The start time of the NEXT actual paper (from the live Date Sheet) —
  // used for the "Time remaining to next paper" countdown.
  ...
  const countdown = useMemo(() => {
    // ── Multi-paper timeline path (2+ distinct paper times today) ──
    // Counts down the CURRENTLY ACTIVE paper (soonest-ending one in
    // progress); once it ends, automatically advances to the next paper's
    // countdown — exactly like a real exam day where Islamiyat (9-11) ends
    // before Urdu (9-12), and the console should then count down the
    // remaining ~1 hour of Urdu, not just say "ended".
    if (paperTimeline.length >= 2 && activeTimelineEntry) {
      const e = activeTimelineEntry;
      const label = e.classes.length > 0 || e.subject
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {/* Row 1: Back + Title + LIVE + Paper Times + MCQs Timer */}
        <div className="flex items-start gap-2 flex-wrap">
          ...
          {/* LIVE fix: this badge previously showed "LIVE" unconditionally,
              even on a free day with no paper actually running (matching
              the screenshot bug). It now reflects the real countdown phase
          ...
          {countdown.phase === "during" ? (
            ...
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium shrink-0 mt-1">
              <span className="inline-flex rounded-full h-2.5 w-2.5 bg-muted-foreground/40"></span>
              {countdown.phase === "before-next-paper" || countdown.phase === "before-start" ? "UPCOMING" : "NOT LIVE"}
            </div>
          )}
        ...
        <div className="flex items-center gap-2 flex-wrap text-xs text-muted-foreground pl-10">
          {/* LIVE fix: was plan.paper_subject — the frozen field that kept
              showing "G.Science" here even after today had no G.Science
              paper (Date Sheet moved on). Now shows the live paper that's
              actually today, and falls back to the upcoming paper's
              subject (labelled "Upcoming") when nothing is on today. */}
        ...
        {/* Row 3: compact class + subject chips (timing lives in the
            countdown card below — see paperTimeline). Classes mixed into
            this plan can be sitting different papers on the same day (e.g.
            Class 8 Islamiyat, Class 10 Pak-Study) — this just labels which
            class sits which subject, small and mobile-friendly. */}
        <div className="flex items-center gap-1 flex-wrap pl-10">
          {/* LIVE fix: was listPlanPaperTimes(plan) — the frozen snapshot,
              which is why these pills stayed stuck on "Cls 8 · G.Science /
              Cls 9 · Physics" (3 Aug's papers) even after the Date Sheet
              moved on. Now reads the Date Sheet live, so pills always show
              TODAY's actual paper per class, and simply don't render when
              a class has no paper today. */}
          {listLivePlanPaperTimesToday(plan, examSession, dateSheetEntries).map((t, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-secondary/60 text-foreground border border-border"
            >
          ...
          {listLivePlanPaperTimesToday(plan, examSession, dateSheetEntries).length === 0 && (
            <span className="text-[10px] text-muted-foreground italic">No paper scheduled today</span>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-[200px]">
              <p className="text-sm text-muted-foreground mb-1 flex items-center gap-2">
                ...
                {countdown.label}
                {countdown.sublabel && <span className="text-[11px] font-medium text-foreground/70">({countdown.sublabel})</span>}
              </p>
            ...
            <div className="flex flex-col gap-2 text-xs">
              {(paperTimeline.length >= 2 && activeTimelineEntry) ? (
                <>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="font-semibold uppercase tracking-wider text-[10px] w-10">Start</span>
                    <span className="text-foreground font-medium">
                      {new Date(activeTimelineEntry.startMs).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="font-semibold uppercase tracking-wider text-[10px] w-10">End</span>
                    <span className="text-foreground font-medium">
                      {new Date(activeTimelineEntry.endMs).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
                </>
              ) : (paperTimeline.length === 1 && activeTimelineEntry) ? (
                <>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="font-semibold uppercase tracking-wider text-[10px] w-10">Start</span>
                    <span className="text-foreground font-medium">
              ...
              ) : (countdown.phase === "before-next-paper" && nextPaperInfo) ? (
                // LIVE fix (Problem 2, rev. 2): previously this showed only
                // ONE class's start/end (the "earliest" one), so when Cls 9's
                // Mutalia Quran ends at 7PM but Cls 8's Urdu (picked as
                // "earliest") had no end_time recorded, the End side showed
                // blank. Now shows EVERY distinct start/end pairing among
                // the upcoming papers — so Urdu's 12PM-9PM and Mutalia
                // Quran's 12PM-7PM both appear, each labelled by class.
                (() => {
                  // Group nextPaperInfo.rows by identical start+end so
                  // classes sharing a time show once, same as the pills.
                  const groups = new Map<string, { classes: string[]; startTime: string; endTime: string | null }>();
                  for (const r of nextPaperInfo.rows) {
                    const key = `${r.startTime}|${r.endTime ?? ""}`;
                    const g = groups.get(key);
                    if (g) g.classes.push(r.cls);
                    else groups.set(key, { classes: [r.cls], startTime: r.startTime, endTime: r.endTime });
                  }
                  const fmt = (t: string) => {
                    const [hh, mm] = t.split(":").map(Number);
                    return new Date(2000, 0, 1, hh, mm).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
                  };
                  return (
                    <>
                      {Array.from(groups.values()).map((g, i) => (
                        <div key={i} className="flex flex-col gap-0.5 pb-1 border-b border-border/50 last:border-0 last:pb-0">
                          <span className="text-[9px] font-semibold text-muted-foreground/80">
                            {g.classes.map(c => `Cls ${c}`).join(", ")}
                          </span>
                          <div className="flex items-center gap-2 text-muted-foreground">
                            <span className="font-semibold uppercase tracking-wider text-[10px] w-10">Start</span>
                            <span className="text-foreground font-medium">{fmt(g.startTime)}</span>
                          </div>
                          {g.endTime && (
                            <div className="flex items-center gap-2 text-muted-foreground">
                              <span className="font-semibold uppercase tracking-wider text-[10px] w-10">End</span>
                              <span className="text-foreground font-medium">{fmt(g.endTime)}</span>
                            </div>
                          )}
                        </div>
                      ))}
                ...
                <>
                  ...
                  {!plan.paper_start_at && !plan.paper_end_at && (
                    <p className="text-muted-foreground">Set paper timing in Exam Date Sheet</p>
                  )}
                </>
              )}
            </div>
      ...
      {/* MCQs Timer now lives as a small control in Row 1, next to Paper Times —
          see McqTimerButton usage above. The big banner card was removed per
          request. */}
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-300 dark:border-blue-700/50 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-blue-700 dark:text-blue-400 mt-0.5">
              ...
              <strong>
                {/* LIVE fix: was plan.paper_start_at (static) — now the real
                    next paper's start time from the Date Sheet. */}
                {nextPaperInfo
                  ? (() => {
                      const [hh, mm] = nextPaperInfo.startTime.split(":").map(Number);
                      return new Date(2000, 0, 1, hh, mm).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          ...
          size="sm"
          onClick={() => {
            // Soft refresh — refetch only data queries without page reload
            const promises = [];
            try {
              promises.push(queryClient.invalidateQueries({ queryKey: ['seating-plan'] }));
              promises.push(queryClient.invalidateQueries({ queryKey: ['seating-plans'] }));
            } catch(e) { /* ignore */ }
            try {
              promises.push(queryClient.invalidateQueries({ queryKey: ['live-attendance'] }));
              promises.push(queryClient.invalidateQueries({ queryKey: ['exam-attendance'] }));
            } catch(e) { /* ignore */ }
            try {
              promises.push(queryClient.invalidateQueries({ queryKey: ['exam-sessions'] }));
            } catch(e) { /* ignore */ }
            Promise.allSettled(promises).then(() => {
              toast.success('Data refreshed', { duration: 2000, icon: '✓' });
            });
          }}
...
function McqTimerButton({ timer }: { timer: McqTimerApi }) {
  ...
  const handleStart = () => {
    ...
    start(m);
    toast.success(`MCQs timer started — ${m} min`);
    setOpen(false);
  };
      ...
      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogTrigger asChild>
          <Button variant="outline" size="sm">
            <Timer className="w-3.5 h-3.5" /> MCQs Timer
          </Button>
        </AlertDialogTrigger>
        <AlertDialogContent className="max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <Timer className="w-4 h-4 text-primary" /> Start MCQs Timer
            </AlertDialogTitle>
    ...
    <div className="inline-flex items-center gap-1 rounded-md border border-border bg-secondary/40 p-0.5">
      ...
      {state.status === "running" && (
        <Button
          ...
          onClick={() => { stop(); toast("MCQs timer stopped"); }}
          title="Stop timer"
        >
      ...
      <Button
        ...
        size="sm"
        className="h-7 px-2 text-xs"
        onClick={() => { reset(); toast.success("MCQs timer reset"); }}
        title="Reset timer"
      >
