    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Real-time exam hall dashboard — countdowns, live tally, threshold alerts, and absentee PDF
        </p>
      </div>
...
const ConsoleView = ({ planId, onBack }: { planId: string; onBack: () => void }) => {
  ...
  // Live attendance — only fetches if plan has session_id + paper_subject + exam_date.
  const sessionId = plan?.session_id;
  const subject = plan?.paper_subject;
  const examDate = plan?.exam_date;
  const { data: attendance = [], isLoading: loadingAtt } = useLiveAttendance(sessionId, subject, examDate);
  ...
  const tally = useMemo(() => {
    ...
    for (const a of attendance) attMap.set(a.student_id, a);
  ...
  const missingPaperInfo = !plan.paper_subject || !plan.exam_date;
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-xl font-heading font-bold text-foreground truncate flex items-center gap-2">
            <Monitor className="w-5 h-5 text-primary" /> {plan.title}
          </h2>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
              {!plan.paper_subject ? "Paper subject is not set" : ""}{!plan.paper_subject && !plan.exam_date ? " and " : ""}{!plan.exam_date ? "Exam date is not set" : ""}
            </p>
            <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
              Live attendance tracking requires both paper subject and exam date on the plan. Edit the plan to add them.
            </p>
          </div>
        </div>
      )}
