    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Monitor live attendance as invigilators scan QR codes
        </p>
      </div>
...
const ConsoleView = ({ planId, onBack }: { planId: string; onBack: () => void }) => {
  ...
  // Live attendance — queries by session_id + exam_date (no subject filter,
  // so attendance taken in the Exam Attendance tab is detected regardless
  // of which subject name was selected there).
  const sessionId = plan?.session_id;
  const examDate = plan?.exam_date;
  const { data: attendance = [], isLoading: loadingAtt } = useLiveAttendance(sessionId, examDate);

  // ── Compute tally ──
  const tally = useMemo(() => {
    if (!plan) return { present: 0, late: 0, absent: 0, leave: 0, total: 0, rooms: [] as any[] };

    // Build a per-student attendance map. Since we don't filter by subject,
    // a student might have multiple rows (different papers on the same day).
    // We prefer 'present' > 'leave' > 'absent', and for same status, the most
    // recent scanned_at wins. This ensures the Live Console reflects the
    // best-known status for each student.
    ...
    for (const a of attendance) {
      ...
      if (!existing) {
        ...
        if (aP > eP) {
          attMap.set(a.student_id, a);
        } else if (aP === eP && a.scanned_at && (!existing.scanned_at || new Date(a.scanned_at) > new Date(existing.scanned_at))) {
          attMap.set(a.student_id, a);
        }
      }
    }
    ...
    const perRoom: Array<{ roomId: string; roomName: string; invigilator: string | null; present: number; late: number; absent: number; leave: number; total: number }> = [];

    for (const room of plan.rooms) {
      let rp = 0, rl = 0, ra = 0, rv = 0;
      for (const assign of room.assignments) {
        const att = attMap.get(assign.student_id);
        if (att?.status === "present") {
          if (startTime && att.scanned_at && new Date(att.scanned_at).getTime() > startTime) {
            late++; rl++;
          } else {
            present++; rp++;
          }
      ...
      perRoom.push({
        ...
        present: rp, late: rl, absent: ra, leave: rv, total: room.assignments.length,
      });
    }

    const total = present + late + absent + leave;
    return { present, late, absent, leave, total, rooms: perRoom };
  }, [plan, attendance]);

  const absentPct = tally.total > 0 ? Math.round((tally.absent / tally.total) * 100) : 0;
  const presentPct = tally.total > 0 ? Math.round(((tally.present + tally.late) / tally.total) * 100) : 0;
  ...
  // Only exam_date is required for live attendance (subject is no longer
  // filtered — see useLiveAttendance).
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5 flex-wrap text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1"><Building2 className="w-3 h-3" /> {plan.rooms.length} rooms · {tally.total} students</span>
          </div>
        </div>
        {/* Realtime indicator */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className={countdown.phase === "before-start" ? "ring-2 ring-blue-500/50" : countdown.phase === "during" ? "ring-2 ring-emerald-500/50" : ""}>
          <CardContent className="p-5">
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
              <Clock className="w-4 h-4" /> {countdown.label}
            </div>
            <p className={`text-4xl font-bold font-mono ${countdown.color}`}>
              {countdown.value || "—"}
            </p>
            {plan.paper_start_at && (
              <p className="text-xs text-muted-foreground mt-2">
                Start: {new Date(plan.paper_start_at).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}
              </p>
            )}
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
              <Timer className="w-4 h-4" /> Paper End
            </div>
            <p className={`text-4xl font-bold font-mono ${
              countdown.phase === "during" ? "text-emerald-600 dark:text-emerald-400" :
              countdown.phase === "ended" ? "text-muted-foreground" :
              "text-muted-foreground"
            }`}>
              {plan.paper_end_at
                ? (countdown.phase === "ended"
                    ? formatDuration(now - (endTime ?? now)) + " ago"
                    : countdown.phase === "during"
                      ? formatDuration((endTime ?? 0) - now)
                      : "Not started")
                : "Not set"}
            </p>
            {plan.paper_end_at && (
              <p className="text-xs text-muted-foreground mt-2">
                End: {new Date(plan.paper_end_at).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <TallyCard icon={<Clock className="w-5 h-5" />} label="Late" value={tally.late} total={tally.total} color="text-amber-600" bg="bg-amber-500/10" />
        ...
        <CardContent>
          {tally.rooms.length === 0 ? (
            ...
            <div className="space-y-2">
              {tally.rooms.map(r => {
                const roomPresent = r.present + r.late;
                const roomPct = r.total > 0 ? Math.round((roomPresent / r.total) * 100) : 0;
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 flex-wrap mb-2">
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        ...
                        <span className="text-amber-600 dark:text-amber-400 font-semibold">L: {r.late}</span>
                    ...
                    <div className="w-full h-2 rounded-full bg-secondary overflow-hidden flex">
                      ...
                      <div className="bg-amber-500" style={{ width: `${r.total > 0 ? (r.late / r.total) * 100 : 0}%` }} title={`Late: ${r.late}`} />
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Used by the console for countdowns and late-detection (scans after start time = "Late").
          </AlertDialogDescription>
        </AlertDialogHeader>
...
const AbsenteePdfButton = ({ plan, attendance, tally }: {
  ...
  const handle = async () => {
    setBusy(true);
    try {
      // Build the absentee list: any student with no attendance row, OR status='absent'/'leave'.
      ...
      for (const a of attendance) {
        ...
        if (!existing) {
          ...
          const aP = priority[a.status] ?? 0;
          const eP = priority[existing.status] ?? 0;
          if (aP > eP || (aP === eP && a.scanned_at && (!existing.scanned_at || new Date(a.scanned_at) > new Date(existing.scanned_at)))) {
            attMap.set(a.student_id, a);
          }
        }
      }
      ...
      const absentPct = tally.total > 0 ? Math.round((tally.absent / tally.total) * 100) : 0;
      doc.text(
        `Total: ${tally.total}  ·  Present: ${tally.present}  ·  Late: ${tally.late}  ·  Absent: ${tally.absent} (${absentPct}%)  ·  Leave: ${tally.leave}`,
