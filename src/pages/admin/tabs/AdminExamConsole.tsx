...
const ConsoleView = ({ planId, onBack }: { planId: string; onBack: () => void }) => {
  ...
  // Live attendance — queries by exam_date ONLY (no session_id, no subject).
  // This way, ANY attendance taken on that day is detected, regardless of
  // which session or subject the admin selected in the Exam Attendance tab.
  const examDate = plan?.exam_date;
  const { data: attendance = [], isLoading: loadingAtt } = useLiveAttendance(examDate);
