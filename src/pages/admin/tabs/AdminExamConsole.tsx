            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {plans.map(p => (
                <button
                  ...
                  className="w-full text-left p-4 rounded-xl border border-border hover:border-primary/40 hover:shadow-md transition-all"
                >
                  <div className="flex items-center justify-between gap-2 mb-2.5">
                    <h3 className="font-semibold text-sm text-foreground truncate flex-1 min-w-0">{p.title}</h3>
                  ...
                  <div className="space-y-1.5 text-xs text-muted-foreground">
                    {p.paper_subject && <p className="flex items-center gap-1.5"><BookOpen className="w-3 h-3 shrink-0" /> {p.paper_subject}</p>}
                    {p.exam_date && <p className="flex items-center gap-1.5"><CalendarDays className="w-3 h-3 shrink-0" /> {new Date(p.exam_date).toLocaleDateString()}</p>}
                    <p className="flex items-center gap-1.5"><Building2 className="w-3 h-3 shrink-0" /> {p.total_seated} students seated</p>
                    {p.paper_start_at && (
                      <p className="flex items-center gap-1.5"><Clock className="w-3 h-3 shrink-0" /> Starts {new Date(p.paper_start_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                    )}
                  </div>
                </button>
              ))}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {/* Row 1: Back + Title + LIVE (title row stays clean on mobile) */}
        ...
        {/* Row 1b: Paper Times + MCQs Timer — own row so it wraps cleanly on mobile */}
