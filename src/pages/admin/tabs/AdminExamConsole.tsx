  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        {/* Row 3: PER-CLASS paper timing — classes mixed into this plan can be
            sitting different papers at different times on the same day (e.g.
            Class 6 Urdu 9-12 vs Class 10 Mutalia Quran 9-11). Each chip below
            shows one class group's own subject/time, synced from the Exam
            Date Sheet — never a single shared clock for the whole plan. */}
        <div className="flex items-center gap-1.5 flex-wrap pl-10">
          {listPlanPaperTimes(plan).map((t, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-1 rounded-full bg-secondary/60 text-foreground border border-border"
              title="Synced from Exam Date Sheet"
            >
              ...
              {t.startTime && t.endTime ? (
                <span className="flex items-center gap-0.5 text-muted-foreground">
                  <Clock className="w-3 h-3" /> {t.startTime}–{t.endTime}
                </span>
              ) : (
                <span className="text-amber-600 dark:text-amber-400">no time set</span>
              )}
            </span>
          ))}
        </div>
      ...
      <Card className={
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            {/* Start + End times (plan reference — see per-class chips above for exact per-class timing) */}
