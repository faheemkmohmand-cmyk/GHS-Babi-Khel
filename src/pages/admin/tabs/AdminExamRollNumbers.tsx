...
const AdminExamRollNumbers = () => {
  ...
  const downloadPrint = async () => {
    ...
    // Exam Date Sheet (exam_schedule table), matched on class + exam_type + year.
    const scheduleByClass = new Map<string, ExamScheduleEntry[]>();
    for (const cls of selectedSession.class_order) {
      const rows = allExamSchedule
    ...
    const drawSlip = (slip: ExamRollEntry, x: number, y: number) => {
      ...
      doc.text(`${selectedSession.title.toUpperCase()} — ${selectedSession.exam_term.toUpperCase()} ${selectedSession.exam_year}`, x + w / 2, y + 12, { align: "center" });
      ...
      if (schedule.length > 0) {
        autoTable(doc, {
          startY: tableStartY,
          margin: { left: x + 4, right: pageW - (x + w) + 4 },
          tableWidth: w - 8,
          ...
          styles: { fontSize: 6.5, cellPadding: 1.3, textColor: [40, 40, 40], lineColor: [180, 180, 180], lineWidth: 0.15 },
          headStyles: { fillColor: [235, 235, 235], textColor: [40, 40, 40], fontStyle: "bold", fontSize: 6.5 },
      ...
      doc.text("Deputy Controller of Exams", x + w - 30.5, sigY + 4, { align: "center" });

      // ── FOOTER INSTRUCTIONS ──
      doc.setDrawColor(150, 150, 150);
      doc.setLineWidth(0.2);
      doc.line(x + 4, y + h - 9, x + w - 4, y + h - 9);
      doc.setFontSize(5.2);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(120, 120, 120);
      doc.text("1. Bring this Roll No Slip to every paper. 2. Errors/omissions must be reported before the exam begins.", x + 4, y + h - 6);
      doc.text("3. No slip, no entry to the examination hall.", x + 4, y + h - 3);
    };
