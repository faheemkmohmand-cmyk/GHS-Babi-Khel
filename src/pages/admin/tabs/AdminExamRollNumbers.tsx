  ...
  const start = useCallback(async () => {
    ...
    const attemptStart = async (): Promise<void> => {
      ...
      await qr.start(
        ...
        // Faster + more forgiving decode: higher fps polls the camera more
        // often (each poll is cheap — it's just a decode attempt on the
        // already-open camera stream), and a bigger qrbox relative to a
        // typical phone viewport means the admin doesn't have to frame the
        // code as precisely, so the FIRST attempt is more likely to hit
        // instead of needing several seconds of re-aiming.
        { fps: 24, qrbox: { width: 280, height: 280 }, disableFlip: true },
        handleDecoded,
        () => {}
      );
    };
...
const AdminExamRollNumbers = () => {
  ...
  const AUTO_CONFIRM_MS = 1100;
  useEffect(() => {
    return () => { if (scanPreviewTimerRef.current) clearTimeout(scanPreviewTimerRef.current); };
  }, []);
  ...
  const handleQRScan = async (qrData: string) => {
    ...
    if (previewStudent && previewClass) {
      ...
      // Auto-confirm shortly after showing, like a board scanner beep —
      // admin can tap "Not them?" to cancel within the window if the
      // wrong student's sticker was scanned.
                    ...
                    <Card className="border-emerald-200 dark:border-emerald-800/50" style={{ contain: "layout style" }}>
                      ...
                      <CardContent className="space-y-3">
                        {/* ── Instant scan preview / confirm card ──────────
                            ...
                            actual student before it's marked. Auto-confirms
                            shortly after; tap "Not them?" to cancel within
                            the window if the wrong sticker was scanned. */}
                        {scanPreview && (
                          <div className="rounded-xl border-2 border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 p-4 space-y-3 animate-in fade-in zoom-in-95 duration-150">
                            <div className="flex items-center gap-3">
                              <div className="w-11 h-11 rounded-full bg-emerald-500 text-white flex items-center justify-center shrink-0">
                                <QrCode className="w-5 h-5" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <p className="font-bold text-lg leading-tight truncate">{scanPreview.studentName}</p>
                                <p className="text-sm text-muted-foreground">
                                  Class {scanPreview.class} · Roll #{scanPreview.classRollNo} · Exam Roll {scanPreview.examRollNo}
                                  {scanPreview.subject ? ` · ${scanPreview.subject}` : ""}
                                </p>
                              </div>
                            </div>
                            <div className="flex gap-2">
                              <Button size="sm" className="flex-1 bg-emerald-500 hover:bg-emerald-600 gap-1.5" onClick={confirmScanNow}>
                                <CheckCircle2 className="w-4 h-4" /> Confirm Present
                              </Button>
                              <Button size="sm" variant="outline" className="gap-1.5" onClick={cancelScanPreview}>
                                <XCircle className="w-4 h-4" /> Not them?
                              </Button>
                            </div>
                          </div>
                        )}
