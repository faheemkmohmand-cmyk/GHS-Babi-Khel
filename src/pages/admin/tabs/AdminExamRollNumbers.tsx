  ...
  const downloadPrint = async () => {
    ...
    const drawSlip = (slip: ExamRollEntry, x: number, y: number) => {
      ...
      if (qrImg) {
        ...
        doc.roundedRect(qrX - 1, infoY - 4, qrSize + 2, qrSize + 2, 1, 1, "S");
        doc.addImage(qrImg, "PNG", qrX, infoY - 3, qrSize, qrSize);
      }

      // ── DATE SHEET TABLE (center, spans full slip width) ──
      // Bottom block is FIXED at the slip bottom: signature + 6 instructions
      // (with divider). Everything is anchored from the bottom edge so the
      // instructions always sit at the very bottom of the slip regardless
      // of how many date-sheet rows the class has. The table is allowed to
      // grow downward but must stop above the signature line.
      //
      // Bottom block layout (from slip bottom = y + h upward):
      //   instruction line 1-6  : 6 × 3mm = 18mm  (lines at -3, -6, -9, -12, -15, -18)
      //   divider              : at y + h - 20
      //   "Deputy Controller"  : 4mm tall, sits at y + h - 23
      //   signature line       : at y + h - 26
      //   ── 2mm gap then date sheet table fills the remaining space ──
      const tableStartY = Math.max(rowY + 3, infoY - 4 + qrSize + 6);
      const tableBottomLimit = y + h - 28; // date sheet must stop above signature line (+2mm gap)
      if (schedule.length > 0) {
        autoTable(doc, {
          startY: tableStartY,
          margin: { left: x + 4, right: pageW - (x + w) + 4, bottom: pageH - tableBottomLimit },
          tableWidth: w - 8,
      ...
      // ── SIGNATURE LINE (sits just above the instruction block) ──
      // Anchored to slip bottom so it never floats away from the bottom
      // even when the date sheet has very few rows.
      const sigY = y + h - 26;
      ...
      // ── FOOTER INSTRUCTIONS (6 numbered lines, pinned to slip bottom) ──
      // Divider line sits 20mm above slip bottom, then 6 lines spaced 3mm
      // apart so the LAST instruction line is exactly 3mm above the bottom
      // border. No empty gap between signature and instructions; no empty
      // gap between the date sheet and the signature (table fills the gap).
      const instrTopY = y + h - 20;
      doc.setDrawColor(150, 150, 150);
      doc.setLineWidth(0.2);
      doc.line(x + 4, instrTopY, x + w - 4, instrTopY);
      doc.setFontSize(5.2);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(120, 120, 120);
      const instructions = [
      ...
      instructions.forEach((line, i) => {
        // First line at instrTopY + 2.5, subsequent lines 3mm below, so
        // the 6th line ends at instrTopY + 2.5 + 5*3 + ~2 (text height)
        // ≈ instrTopY + 19.5 ≈ slip_bottom - 0.5 (right at the bottom border).
        doc.text(line, x + 4, instrTopY + 2.5 + i * 3);
      });
    };
