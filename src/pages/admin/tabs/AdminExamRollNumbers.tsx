...
import { decodeSeatingQRData, resolveLiveClassPaperTime } from "@/hooks/useExamSeating";
...
const AdminExamRollNumbers = () => {
  ...
  const downloadPrint = async () => {
    ...
    const drawSlip = (slip: ExamRollEntry, x: number, y: number) => {
      ...
      doc.text(`${classExamType.toUpperCase()} ${selectedSession.exam_year}`, x + w / 2, y + 12, { align: "center" });
      ...
      // Tightened vertical spacing (4.2mm per row instead of 5.5mm) so the
      // 4 fields + QR code fit in roughly one printed line less, leaving
      // more room for the date sheet + instructions below. infoY is set
      // ~6.5mm below the header divider so the "Student Name:" row has
      // clear breathing room from the horizontal line above.
      const infoY = y + 25;
      const qrSize = 18;
      ...
      let rowY = infoY;
      rowY = drawRow("Student Name:", slip.student_name, rowY);
      rowY = drawRow("Father Name:", slip.father_name || "—", rowY);
      rowY = drawRow("Exam Roll No:", slip.exam_roll_no, rowY);
      rowY = drawRow("Class:", `Class ${slip.class}`, rowY);
      ...
      // ── DATE SHEET TABLE (center, spans full slip width) ──
      // The date sheet is followed (one line below) by a small "Instructions:"
      // heading and the 6 numbered instructions — that whole block flows
      // directly under the table. The Deputy Controller signature is the
      // ONLY thing anchored at the slip bottom.
      const tableStartY = Math.max(rowY + 3, infoY - 4 + qrSize + 6);
      // Reserve 18mm at the bottom of the slip for the signature block.
      // The date sheet + instructions block is laid out above that.
      const sigBlockTopY = y + h - 18; // signature line sits just below this
      ...
      // ── INSTRUCTIONS (directly below the date sheet, with a heading) ──
      // The instructions block is laid out immediately under the date sheet
      // (one small gap), with a bold "Instructions:" heading on its own line
      // followed by the 6 numbered rules. This is NOT pinned to the bottom —
      // it sits right under the table so the eye reads top-to-bottom.
      // The Deputy Controller signature is the only thing at the bottom.
      const tableEndY = (doc as any).lastAutoTable?.finalY ?? (tableStartY + 30);
      const instrStartY = tableEndY + 4; // one line gap below the date sheet
      ...
      // ── SIGNATURE LINE (the only thing anchored at the bottom) ──
      const sigY = y + h - 10;
  ...
  const dateSheetLookup = useMemo<DateSheetLookup | null>(() => {
    if (!attSession || !attDate) return null;
    if (!attSelectedSession) return null;
    // ── SINGLE SOURCE OF TRUTH: resolveLiveClassPaperTime ──────────────
    // This is the EXACT SAME function the Live Console (AdminExamConsole)
    // and 3D Hall use to decide, per class, whether a paper is live right
    // now. Attendance now calls it directly instead of re-deriving its
    // own date-sheet match — so "Console shows Class 9 Chemistry live"
    // and "Attendance is unlocked for Class 9 Chemistry" can never
    // disagree again. It also naturally self-corrects at the exact
    // moment Console would: the instant a paper's end_time passes,
    // isToday flips false-for-window and this locks; the instant a new
    // paper's start_time is reached, it unlocks — with no dependency on
    // which seating plan exists, only the live Exam Date Sheet.
    const session = { exam_year: attSelectedSession.exam_year, exam_term: attSelectedSession.exam_term };
    const minimalEntries = allExamSchedule.map(r => ({
      class: r.class, exam_type: r.exam_type, year: r.year, subject: r.subject,
      exam_date: r.exam_date, start_time: r.start_time, end_time: r.end_time,
    }));

    if (isAllClassesMode) {
      // Window is OPEN if ANY class with a selected subject has a LIVE
      // (isToday) Date Sheet paper right now — mirrors Console's
      // "any class in this plan has a live paper today" check.
      const classesWithSubject = availableClasses
        .map(cls => ({ cls, subj: allClassSubjects[cls] }))
        .filter((c): c is { cls: string; subj: string } => !!c.subj);
      if (classesWithSubject.length === 0) return null;
      const live = classesWithSubject
        .map(c => resolveLiveClassPaperTime(c.cls, session, minimalEntries))
        .find(t => t.isToday && t.startTime && t.endTime);
      if (live) {
        return { exists: true, start: live.startTime, end: live.endTime, examDate: live.examDate };
      }
      // No class has a live paper today. Distinguish "date sheet has rows
      // for today but window closed" (handled by time check below) from
      // "genuinely nothing scheduled" — either way it must lock, so just
      // report not-exists; the time-window guard in getPaperWindowStatus
      // then locks it regardless of reason.
      return { exists: false, start: null, end: null, examDate: null };
    }

    // Single-class mode
    if (!attClass || !attSubject) return null;
    const live = resolveLiveClassPaperTime(attClass, session, minimalEntries);
    if (live.isToday && live.startTime && live.endTime) {
      return { exists: true, start: live.startTime, end: live.endTime, examDate: live.examDate };
    }
    return { exists: false, start: null, end: null, examDate: null };
  }, [
  ...
  // Display priority: date-sheet times > row-snapshot times > seating plan
  // times. The date sheet is the authoritative source for the "official"
  // paper window, so we surface it in the lock banner so the admin sees
  // the correct time even if the seating plan has stale or different
  // times.
  const attDisplayPaperStart = dateSheetLookup?.start || firstAttRecord?.paper_start_time || seatingPaperTimes?.start || null;
  const attDisplayPaperEnd = dateSheetLookup?.end || firstAttRecord?.paper_end_time || seatingPaperTimes?.end || null;
