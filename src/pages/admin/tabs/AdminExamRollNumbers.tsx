...
const AdminExamRollNumbers = () => {
  ...
  const { data: seatingPaperTimes } = usePaperTimesFromSeatingPlan(attSession, attSubject, attDate);
  ...
  const overviewPivot = useMemo(() => {
    ...
    // Column key = subject + date, so the same subject on two different dates
    // (e.g. a retaken paper) gets its own column instead of overwriting.
    const columnMap = new Map<string, { key: string; subject: string; date: string }>();
    overviewData.forEach(r => {
      const key = `${r.subject}__${r.exam_date}`;
      if (!columnMap.has(key)) columnMap.set(key, { key, subject: r.subject, date: r.exam_date });
    });
    const columns = Array.from(columnMap.values()).sort((a, b) => a.date.localeCompare(b.date) || a.subject.localeCompare(b.subject));
    ...
    // GRID DEDUPE (rev. 10): if the same (student, subject, date) appears
    // more than once in overviewData (pre-migration duplicates, or a
    // student scanned twice), keep the most recent scan — prefer non-null
    // scanned_at, then latest scanned_at, then latest created_at. This is
    // a UI-level safety net; the hook-level dedupeAttendanceRows helper
    // already collapses duplicates at the query layer, but this ensures
    // the grid never shows a stale status even if some duplicates slip
    // through.
    const cellKeep = new Map<string, { status: Status; scanned_at: string | null; created_at?: string }>();
    overviewData.forEach(r => {
      const key = `${r.student_id}__${r.subject}__${r.exam_date}`;
      const prev = cellKeep.get(key);
      const cur = { status: r.status, scanned_at: r.scanned_at, created_at: r.created_at };
      if (!prev) { cellKeep.set(key, cur); return; }
      const aHas = !!cur.scanned_at, bHas = !!prev.scanned_at;
      let keep: typeof cur;
      if (aHas && !bHas) keep = cur;
      else if (!aHas && bHas) keep = prev;
      else if (aHas && bHas) keep = (cur.scanned_at! >= prev.scanned_at!) ? cur : prev;
      else keep = ((cur.created_at || "") >= (prev.created_at || "")) ? cur : prev;
      cellKeep.set(key, keep);
    });
    const grid: Record<string, Record<string, Status>> = {};
    cellKeep.forEach((val, key) => {
      const [studentId, subject, date] = key.split("__");
      const colKey = `${subject}__${date}`;
      if (!grid[studentId]) grid[studentId] = {};
      grid[studentId][colKey] = val.status;
    });
  ...
  // ── CROSS-CLASS SCAN HANDLERS (rev. 10) ───────────────────────────────
  // The exam hall seats classes in arrangement order (e.g. 8th, then 7th,
  // then 6th). An invigilator walking the aisles needs to scan student QR
  // codes from ANY class without flipping the dropdown each time. These
  // handlers:
  //   1. Try the SELECTED class first (fast path — student is in the loaded
  //      attRollNumbers list).
  //   2. Fall back to a session-wide lookup in exam_roll_numbers if the
  //      student isn't in the selected class.
  //   3. Pass the student's ACTUAL class to the mutation hook, which uses
  //      it for the saved attendance row's `class` field. So a class-7
  //      student scanned while class 8 is selected gets saved to the
  //      class-7 sheet — never to a random class.
  //   4. Re-scans of the same student just UPDATE the existing row (atomic
  //      upsert in the hook — no duplicates).
  //
  ...
  const handleQRScan = async (qrData: string) => {
    ...
    // Desk QRs carry the student's class via the seat assignment, so they
    // work cross-class out of the box. No dropdown lookup needed.
        ...
        },
        {
          onSuccess: (result) => {
            if (result.status === "already") {
              ...
              toast.success(`${result.studentName} · Class ${result.class} marked Present · ${result.seatLabel} · ${result.roomName}`);
              setScanLog(prev => [
                { name: result.studentName, roll: result.examRollNo, time: new Date().toLocaleTimeString(), status: "present" },
                ...prev,
              ]);
            }
            // Invalidate the student's ACTUAL class queries so the Live
            // Console and Class Overview update regardless of which class
            // the admin had selected in the dropdown.
            if (result.sessionId && result.class) {
              qc.invalidateQueries({ queryKey: ["exam-attendance", result.sessionId, result.class, attSubject, attDate] });
              qc.invalidateQueries({ queryKey: ["exam-attendance-overview", result.sessionId, result.class] });
            }
            // Also invalidate the selected class (in case it matches).
            if (attSession && attClass && result.class !== attClass) {
              qc.invalidateQueries({ queryKey: ["exam-attendance", attSession, attClass, attSubject, attDate] });
            }
    ...
    // Fast path: student is in the SELECTED class's loaded list.
    let student = attRollNumbers.find(r => r.student_id === parsed.studentId);
    let studentClass = attClass;

    // Slow path: student isn't in the selected class. Look up across ALL
    // classes in the session via exam_roll_numbers. This is a single
    ...
    // info-toast the admin but still proceed — the saved row goes to the
    // student's real class sheet (handled by the mutation hook).
    if (studentClass !== attClass) {
  ...
  const handleManualRoll = async () => {
    ...
    // Fast path: roll number is in the SELECTED class.
    let student = rollMap.get(roll);
    let studentClass = attClass;
    // Slow path: session-wide lookup so the admin can type a roll number
    // from any class without switching the dropdown.
    if (!student) {
      try {
        const { data: crossStudent, error } = await supabase
          .from("exam_roll_numbers")
  ...
  const doScan = (studentId: string, studentName: string, examRoll: string, studentClass?: string) => {
    ...
    // `cls` here is the student's ACTUAL class (resolved by handleQRScan /
    // handleManualRoll from exam_roll_numbers). The mutation hook uses
    // this to invalidate the right queries AND (via its own rollEntry
    // lookup) to set the saved row's `class` field. So even if the admin
    // picked class 8 but scanned a class-7 student, the row lands in
    // class 7's sheet.
    scanAttendance.mutate({
      sessionId: attSession, studentId, subject: attSubject,
      examDate: attDate, cls: studentClass || attClass, scannedBy: null,
    }, {
      onSuccess: (result) => {
        if (result.status === "already") {
          toast(`${studentName} already marked Present`, { icon: "✅" });
  ...
  const exportOverviewPDF = () => {
    if (!overviewPivot.students.length) { toast.error("No data to export"); return; }
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text(`CLASS ${attClass} — EXAM ATTENDANCE OVERVIEW`, w / 2, 25.5, { align: "center" });

    // ── Table ──
    const head = [["#", "Exam Roll", "Student Name", ...overviewPivot.columns.map(c => `${c.subject}\n${c.date}`), "Present", "Absent"]];
    const body = overviewPivot.students.map((s, idx) => {
      ...
      const cells = overviewPivot.columns.map(col => {
    ...
    doc.save(`ClassOverview-Class${attClass}-${attSession}.pdf`);
            ...
            <CardContent className="p-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="date" value={attDate} readOnly disabled={!attSubject}
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button onClick={handleManualRoll} disabled={!manualRoll.trim() || !canMarkAtt} className="bg-emerald-500 hover:bg-emerald-600 text-white gap-1.5 shrink-0"><Check className="w-4 h-4" /> Present</Button>
                          </div>
                        </div>
                        {/* Recent scans */}
                        {scanLog.length > 0 && (
