...
const AdminExamRollNumbers = () => {
  ...
  const handleUpdateStudents = async (session: ExamSession) => {
    ...
    try {
      // Re-pull each class's CURRENT active students, in class-roll-number
      // order — same as Generate. The session's saved class_order and
      // starting_number are reused as-is, so the exam-roll sequence
      // (which class comes first/second/etc.) never changes; only which
      // students land in which slot can change.
      const studentsPerClass: Record<string, Student[]> = {};
      ...
      const orderedStudents: Student[] = [];
      for (const cls of session.class_order) { if (studentsPerClass[cls]) orderedStudents.push(...studentsPerClass[cls]); }
