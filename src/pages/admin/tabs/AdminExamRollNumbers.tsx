...
import {
  ...
  findDateSheetEntry, type DateSheetLookup, examTermMatches,
...
const AdminExamRollNumbers = () => {
  ...
  const dateSheetLookup = useMemo<DateSheetLookup | null>(() => {
    ...
    // Filter the pre-fetched exam_schedule rows to this session's
    // exam_term + exam_year + the picked date. The date sheet is keyed
    // by (class, exam_type, year, exam_date) — exam_type stores the
    // term ("1st Semester", "Annual-I", etc.) so we match against the
    // session's exam_term loosely (bidirectional substring, same way
    // AdminExamSeating.tsx and AdminExamConsole.tsx do).
    if (!attSelectedSession) return null;
    const sessionTerm = String(attSelectedSession.exam_term ?? "").trim();
    const sessionYear = attSelectedSession.exam_year;
    // Ordinal-aware term match (shared with AdminExamSeating.tsx /
    // AdminExamConsole.tsx) — Class 9/10 date-sheet rows use "Annual-I" /
    // "Annual-II" while the session's exam_term may be phrased "1st
    // Semester 2026" etc. A plain substring match misses these rows,
    // which would wrongly keep attendance locked for a paper the Console
    // already shows as live. See examTermMatches() for details.
    const matchingRows = allExamSchedule.filter(r => {
      if (r.year !== sessionYear) return false;
      if (r.exam_date !== attDate) return false;
      if (!sessionTerm || !r.exam_type) return false;
      return examTermMatches(r.exam_type, sessionTerm);
    });

    if (isAllClassesMode) {
      // Window OPEN if any class with a selected subject has a date-sheet
      // row. Return the FIRST matching row's times as the representative
      // for display purposes (the actual lock guard is "does ANY class
      // have a row").
      ...
      const firstMatch = classesWithSubject
        .map(c => ({ c, entry: findDateSheetEntry(matchingRows, c.cls, c.subj, attDate) }))
        .find(x => !!x.entry);
      return {
        exists: !!firstMatch,
        start: firstMatch?.entry?.start_time ?? null,
        end: firstMatch?.entry?.end_time ?? null,
        examDate: firstMatch?.entry?.exam_date ?? null,
      };
    }

    // Single-class mode
    if (!attClass || !attSubject) return null;
    const entry = findDateSheetEntry(matchingRows, attClass, attSubject, attDate);
    return {
      exists: !!entry,
