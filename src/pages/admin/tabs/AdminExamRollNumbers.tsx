...
const AdminExamRollNumbers = () => {
  ...
  const overviewPivot = useMemo(() => {
    ...
    // ── DATE NORMALIZATION (rev. 11) ──────────────────────────────────────
    // The exam_attendance table stores `exam_date` inconsistently — some rows
    // have "2026-07-06" (date picker path), others have "2026-07-06T00:00:00.000Z"
    // (legacy insert path). Using the raw string as the column key creates
    // DUPLICATE columns labeled "Mathematics" with slightly different dates
    // underneath. This was the root cause of the "Mathematics appearing 3
    // times" bug in the Class Overview screenshot.
    //
    // Fix: normalize every exam_date to "YYYY-MM-DD" before using it as a
    // key. Also normalize the cellKeep dedupe key so duplicate rows (same
    // student + subject + normalized date) collapse to ONE cell.
    const normalizeDate = (raw: string | null | undefined): string => {
      if (!raw) return "—";
      const s = String(raw).trim();
      if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;            // already YYYY-MM-DD
      if (/^\d{4}-\d{2}-\d{2}[T ]/.test(s)) return s.slice(0, 10);  // ISO/timestamptz
      const d = new Date(s);
      if (!isNaN(d.getTime())) {
    ...
    // Column key = subject + NORMALIZED date, so the same subject on two
    // different real dates (e.g. a retaken paper) gets its own column, but
    // the same subject on the same date with different string formats
    ...
    overviewData.forEach(r => {
      ...
      const key = `${r.subject}__${normDate}`;
      if (!columnMap.has(key)) columnMap.set(key, { key, subject: r.subject, date: normDate });
    });
    ...
    // GRID DEDUPE (rev. 11): if the same (student, subject, NORMALIZED date)
    // appears more than once in overviewData (pre-migration duplicates, or a
    // student scanned twice, or rows with different exam_date string formats
    // for the same actual date), keep the most recent scan — prefer non-null
    // scanned_at, then latest scanned_at, then latest created_at.
    const cellKeep = new Map<string, { status: Status; scanned_at: string | null; created_at?: string }>();
    overviewData.forEach(r => {
      const normDate = normalizeDate(r.exam_date);
      const key = `${r.student_id}__${r.subject}__${normDate}`;
    ...
    cellKeep.forEach((val, key) => {
      ...
      const normDate = parts.slice(2).join("__");  // safe even if date has __
      const colKey = `${subject}__${normDate}`;
  ...
  const handleInitSheet = () => {
    ...
    if (isAllClassesMode) {
      ...
      // already has attendance recorded on a DIFFERENT date — proceeding
      // would create a new duplicate paper column instead of updating the
      // existing one.
      ...
      if (conflicts.length > 0) {
        const lines = conflicts.map(c => `Class ${c.cls} · ${c.subject}: already has attendance on ${c.dates.join(", ")}`).join("\n");
        const ok = window.confirm(
          `This will create a NEW attendance sheet dated ${attDate}, separate from existing attendance for the same paper:\n\n${lines}\n\n` +
          `If this was meant to be the SAME paper, cancel and change the date above to match. Continue anyway?`
        );
        if (!ok) return;
      }
      (async () => {
    ...
    }
    {
      ...
      if (conflictDates.length > 0) {
        const ok = window.confirm(
          ...
          `This will create a NEW sheet dated ${attDate} instead of updating that one. ` +
          `If this is the same paper, cancel and change the date above to match. Continue anyway?`
        );
        if (!ok) return;
      }
    }
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>This deletes the entire attendance sheet for this paper ({col.date}) for all Class {section.cls} students. This cannot be undone.</AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                                    <AlertDialogAction
                                      onClick={() => deleteAttendance.mutate({ sessionId: attSession, cls: section.cls, subject: col.subject, examDate: col.date })}
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>This deletes the entire attendance sheet for this paper ({col.date}) for Class {attClass}. This cannot be undone.</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                                  <AlertDialogAction
                                    onClick={() => deleteAttendance.mutate({ sessionId: attSession, cls: attClass, subject: col.subject, examDate: col.date })}
                                    className="bg-destructive text-destructive-foreground"
                                  >
                                    Delete
                                  </AlertDialogAction>
