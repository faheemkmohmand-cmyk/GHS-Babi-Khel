...
import {
  ...
  findDateSheetEntry, type DateSheetLookup,
...
const AdminExamRollNumbers = () => {
  ...
  const dateSheetLookup = useMemo<DateSheetLookup | null>(() => {
    ...
    const sessionTerm = String(attSelectedSession.exam_term ?? "").trim().toLowerCase();
    ...
    const matchingRows = allExamSchedule.filter(r => {
      ...
      const sheetTerm = String(r.exam_type ?? "").trim().toLowerCase();
      if (!sessionTerm || !sheetTerm) return false;
      return sheetTerm === sessionTerm || sheetTerm.includes(sessionTerm) || sessionTerm.includes(sheetTerm);
    });
