...
function QRScanner({ onScan, enabled }: { onScan: (data: string) => void; enabled: boolean }) {
  ...
  const start = useCallback(async () => {
    ...
    // Adaptive wait time based on device
    const waitTime = isMobile ? 600 : 300;
    ...
    try {
      ...
      const retryDelay = isMobile ? 1200 : 800;
      try {
        ...
        } else if (retryMsg.includes("NotAllowedError")) {
          setError("Camera blocked. Please enable camera permissions in your browser.");
        } else if (retryMsg.includes("NotFound") || retryMsg.includes("DevicesNotFound")) {
          setError("No camera detected. Please connect a webcam and refresh the page.");
        } else {
          setError(`Scanner error: ${retryMsg}`);
        }
        setActive(false);
      }
    }
...
const AdminExamRollNumbers = () => {
  ...
  const downloadPrint = async () => {
    ...
    const drawSlip = (slip: ExamRollEntry, x: number, y: number) => {
      ...
      doc.text(`${examTypeLabel(classExamType).toUpperCase()} ${selectedSession.exam_year}`, x + w / 2, y + 12, { align: "center" });
