...
export default function AdminExamSchedule() {
  ...
  const buildPdfForClass = (doc: jsPDF, cls: string, examType: string, isFirstSection: boolean) => {
    ...
    const tableBody = entries.map((e, idx) => [
      String(idx + 1),
      e.subject,
      format(new Date(e.exam_date), "EEE, dd MMM yyyy"),
  ...
  const runExport = () => {
    ...
    if (exportScope === "single") {
      wroteAny = buildPdfForClass(doc, exportCls, exportExam, true);
      if (!wroteAny) { toast.error(`No schedule found for Class ${exportCls} · ${exportExam} · ${bulkYear}`); return; }
    } else {
      for (const cls of classes) {
        for (const ex of getExamTypes(cls)) {
          const wrote = buildPdfForClass(doc, cls, ex, !wroteAny);
          if (wrote) wroteAny = true;
        }
      }
      if (!wroteAny) { toast.error(`No exam schedule found for ${bulkYear} yet`); return; }
    }
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      doc.text(`Page ${p} of ${totalPages}`, w - 12, h - 8, { align: "right" });
      doc.text("Generated by GHS Babi Khel Admin Panel", 12, h - 8);
    }

    const filename = exportScope === "single"
      ? `Exam-Date-Sheet-Class-${exportCls}-${exportExam.replace(/\s+/g, "-")}-${bulkYear}.pdf`
      : `Exam-Date-Sheet-All-Classes-${bulkYear}.pdf`;
    doc.save(filename);
    setExportOpen(false);
