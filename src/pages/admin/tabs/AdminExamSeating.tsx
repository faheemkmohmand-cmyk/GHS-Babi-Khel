...
const DutiesPdfButton = ({ plan }: { plan: SeatingPlanFull }) => {
  ...
  const handle = async () => {
    ...
    try {
      ...
      const examDateText = plan.exam_date
        ? new Date(plan.exam_date).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })
        : "—";
      const paperSubjectText = plan.paper_subject || "—";
      const classesText = plan.classes.map(c => `Class ${c}`).join(", ");

      // Info table (2 columns x 3 rows)
      const infoColW = contentW / 2;
      const infoRowH = 6;
      const infoItems = [
        { label: "Exam Date:", value: examDateText },
        { label: "Paper Subject:", value: paperSubjectText },
        { label: "Classes:", value: classesText },
