...
const PlanStaffEditor = ({ plan }: { plan: SeatingPlanFull }) => {
  ...
  const save = async (next: { superintendent?: string; deputy?: string }) => {
    const s = next.superintendent ?? superintendent;
    const d = next.deputy ?? deputy;
    if (s === ((plan as any).superintendent ?? "") && d === ((plan as any).deputy_superintendent ?? "")) return;
...
const RoomDeskMap = ({ room, plan }: { room: RoomWithAssignments; plan: SeatingPlanFull }) => {
  ...
  const saveInvigilators = async (next: string[]) => {
    try {
      await upsert.mutateAsync({
        planId: plan.id,
        room: { id: room.id, name: room.name, rows: room.rows, cols: room.cols, block_layout: room.block_layout, invigilators: next.map(s => s.trim()).filter(Boolean), notes: room.notes },
      });
    } catch { /* handled */ }
  };
  ...
  const toggleBlocked = async (r: number, c: number) => {
    ...
    try {
      await upsert.mutateAsync({
        planId: plan.id,
        room: { id: room.id, name: room.name, rows: room.rows, cols: room.cols, block_layout: next, invigilators: room.invigilators, notes: room.notes },
      });
    } catch { /* handled */ }
  };
              ...
              <Input
                ...
                onBlur={async (e) => {
                  if (e.target.value !== (room.notes ?? "")) {
                    await upsert.mutateAsync({
                      planId: plan.id,
                      room: { id: room.id, name: room.name, rows: room.rows, cols: room.cols, block_layout: room.block_layout, invigilators: room.invigilators, notes: e.target.value || null },
                    });
                  }
                }}
...
const PrintDeskMapButton = ({ room, plan }: { room: SeatingRoom; plan: SeatingPlanFull }) => {
  ...
  const handle = async () => {
    ...
    try {
      ...
      const invigilatorNames = (room as any).invigilators?.length ? (room as any).invigilators.join(", ") : room.invigilator;
      if (invigilatorNames) {
        doc.setFontSize(9); doc.setTextColor(110);
        const label = (room as any).invigilators?.length > 1 ? "Invigilators" : "Invigilator";
        doc.text(`${label}: ${invigilatorNames}`, pageW / 2, 25.5, { align: "center" });
        headerBottom = 28;
      }
