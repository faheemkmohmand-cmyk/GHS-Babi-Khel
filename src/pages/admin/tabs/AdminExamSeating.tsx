...
// Lazy-loaded to keep the seating-tab bundle lean — only loads when admin
// actually enters the War Room. All three top-level tabs stay independent.
const LiveSeatingWarRoom = React.lazy(() => import("@/components/admin/warroom/LiveSeatingWarRoom"));
import {
  useExamSessions,
...
import {
  ...
  encodeSeatingQRData, autoSplitRowDuties, resolveRowDuties,
...
const AdminExamSeating = () => {
  const [topTab, setTopTab] = useState<"seating" | "console" | "warroom">("seating");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-secondary/50 rounded-xl p-1">
        ...
        <button onClick={() => setTopTab("warroom")}
          className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 ${
            topTab === "warroom" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
          }`}>
      ...
      {topTab === "console" ? (
        ...
        <React.Suspense fallback={<div className="flex items-center justify-center py-12"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>}>
          <LiveSeatingWarRoom />
        </React.Suspense>
      ) : selectedPlanId ? (
...
const RoomDeskMap = ({ room, plan }: { room: RoomWithAssignments; plan: SeatingPlanFull }) => {
  ...
  // Row duty range per invigilator (index-aligned with invigilatorsInput).
  ...
  const [dutiesInput, setDutiesInput] = useState<{ row_start: number; row_end: number }[]>(
    resolveRowDuties(room)
  );
  useEffect(() => {
    setInvigilatorsInput(room.invigilators?.length ? room.invigilators : [""]);
    setNotesInput(room.notes ?? "");
    setDutiesInput(resolveRowDuties(room));
  }, [room.id]);
  ...
  const saveInvigilators = async (next: string[]) => {
    try {
      ...
      // added/removed invigilator immediately gets a sensible row range
      // instead of an empty one.
      const nextDuties = autoSplitRowDuties(room.rows, cleanNext.length);
  ...
  const saveDuties = async (next: { row_start: number; row_end: number }[]) => {
    try {
      await upsert.mutateAsync({
        planId: plan.id,
  ...
  // Which invigilator (by index) is responsible for a given 0-indexed row,
  // used to color-band the grid so duty coverage is visible at a glance.
  const invigilatorForRow = (rowIdx0: number) => {
    const rowNum = rowIdx0 + 1; // duties are 1-indexed
    const idx = dutiesInput.findIndex(d => rowNum >= d.row_start && rowNum <= d.row_end);
    return idx;
  };
  const DUTY_BAND_COLORS = [
    "border-l-4 border-l-blue-400", "border-l-4 border-l-emerald-400",
    "border-l-4 border-l-amber-400", "border-l-4 border-l-rose-400",
    "border-l-4 border-l-violet-400", "border-l-4 border-l-cyan-400",
  ];
            ...
            <div className="flex flex-wrap items-center gap-3 mb-2 text-[11px] text-muted-foreground">
              ...
              {invigilatorsInput.map((name, i) => name.trim() && (
                <span key={i} className="flex items-center gap-1">
                  <span className={`inline-block w-2.5 h-2.5 rounded-sm ${DUTY_BAND_COLORS[i % DUTY_BAND_COLORS.length].replace("border-l-4 border-l-", "bg-")}`} />
                  {name.trim()} (Rows {dutiesInput[i]?.row_start ?? "—"}–{dutiesInput[i]?.row_end ?? "—"})
                </span>
              ))}
            </div>
          )}
          <div className="overflow-x-auto">
            ...
            >
              {Array.from({ length: room.rows * room.cols }).map((_, idx) => {
                ...
                const bandIdx = invigilatorForRow(r);
                const bandClass = multiInvigilator && c === 0 && bandIdx >= 0
          ...
          <div className="mt-4 pt-3 border-t border-border grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5 mb-1">
                {INVIGILATOR_DUTY} With more than one, each is assigned a row range below.
              </p>
              <div className="mt-1 space-y-1.5">
                {invigilatorsInput.map((val, idx) => (
                  <div key={idx} className="flex items-center gap-1.5">
                    ...
                    {invigilatorsInput.filter(Boolean).length > 1 && dutiesInput[idx] && (
                      <div className="flex items-center gap-1 shrink-0" title="Rows this invigilator is responsible for">
                        <span className="text-[10px] text-muted-foreground">Rows</span>
                        <Input
                          type="number" min={1} max={room.rows}
                          value={dutiesInput[idx].row_start}
                          onChange={e => {
                            const v = Math.max(1, Math.min(room.rows, +e.target.value || 1));
                            setDutiesInput(list => list.map((d, i) => i === idx ? { ...d, row_start: v } : d));
                          }}
                          onBlur={() => saveDuties(dutiesInput)}
                          className="h-8 w-12 text-xs px-1 text-center"
                        />
                        <span className="text-[10px] text-muted-foreground">–</span>
                        <Input
                          type="number" min={1} max={room.rows}
                          value={dutiesInput[idx].row_end}
                          onChange={e => {
                            const v = Math.max(1, Math.min(room.rows, +e.target.value || 1));
                            setDutiesInput(list => list.map((d, i) => i === idx ? { ...d, row_end: v } : d));
                          }}
                          onBlur={() => saveDuties(dutiesInput)}
                          className="h-8 w-12 text-xs px-1 text-center"
                        />
  ...
  const handle = async () => {
    ...
    try {
      ...
      if (invigilatorList.length) {
        const duties = resolveRowDuties(room as any);
        const label = invigilatorList.length > 1 ? "Invigilators" : "Invigilator";
        const text = invigilatorList.length > 1
          ? invigilatorList.map((name, i) => duties[i] ? `${name} (Rows ${duties[i].row_start}–${duties[i].row_end})` : name).join(", ")
          : invigilatorList[0];
