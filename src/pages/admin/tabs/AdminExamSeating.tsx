...
const CreatePlanForm = ({ sessionId, onCreated }: { sessionId: string; onCreated: (id: string) => void }) => {
  ...
  // times apply to every day in [from, to] — the Live Console + Exam
  // Attendance automatically use today's date when it's in the range.
  ...
  const handleCreate = async () => {
    ...
    try {
      const plan = await createMut.mutateAsync({
        ...
        isRecurring,
        // Pass the date range (null if empty). The hook only includes these
        // in the INSERT if they're not undefined, so old DBs without the
        // columns still work.
  ...
  return (
    <div className="border-t border-border pt-4 space-y-3">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div>
          <Label className="text-xs">Exam Date (single day — optional)</Label>
          <Input type="date" value={examDate} onChange={e => setExamDate(e.target.value)} className="mt-1" />
          <p className="text-[10px] text-muted-foreground mt-1">For a one-day paper. For multi-day papers, use the range below.</p>
        </div>
        <div>
          <Label className="text-xs">Paper Start (optional)</Label>
          <Input type="datetime-local" value={paperStartAt} onChange={e => setPaperStartAt(e.target.value)} className="mt-1" />
          <p className="text-[10px] text-muted-foreground mt-1">Used by the Live Exam Console for countdowns</p>
        </div>
        <div>
          <Label className="text-xs">Paper End (optional)</Label>
          <Input type="datetime-local" value={paperEndAt} onChange={e => setPaperEndAt(e.target.value)} className="mt-1" />
          <p className="text-[10px] text-muted-foreground mt-1">When the paper ends — countdown turns red after this</p>
        </div>
        {/* ── Recurring date range (Problem 2 fix) ── */}
        <div>
          <Label className="text-xs">Exam Date From (optional, first paper day)</Label>
          <Input type="date" value={examDateFrom} onChange={e => setExamDateFrom(e.target.value)} className="mt-1" />
          <p className="text-[10px] text-muted-foreground mt-1">Start of the multi-day exam window</p>
        </div>
        <div>
          <Label className="text-xs">Exam Date To (optional, last paper day)</Label>
          <Input type="date" value={examDateTo} onChange={e => setExamDateTo(e.target.value)} className="mt-1" />
          <p className="text-[10px] text-muted-foreground mt-1">End of the multi-day exam window</p>
        </div>
        <div className="sm:col-span-2">
          {examDateFrom && examDateTo ? (
            <div className="rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-900/20 p-2.5 text-[11px] text-emerald-700 dark:text-emerald-400">
              ✓ Multi-day mode: the paper will run every day from <strong>{examDateFrom}</strong> to <strong>{examDateTo}</strong> at the same start/end time. The Live Console and Exam Attendance will automatically use today's date within this range.
            </div>
          ) : examDateFrom || examDateTo ? (
            <div className="rounded-lg border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-900/20 p-2.5 text-[11px] text-amber-700 dark:text-amber-400">
              ⚠ Set BOTH "From" and "To" dates to enable multi-day mode. Currently only one is set.
            </div>
          ) : null}
        </div>
        <div>
          <Label className="text-xs">Classes to mix</Label>
      ...
      <label className="flex items-start gap-2.5 p-3 rounded-xl border border-border bg-secondary/30 cursor-pointer">
        ...
        />
        <span>
          <span className="block text-sm font-semibold text-foreground">All Papers (recurring plan)</span>
          <span className="block text-xs text-muted-foreground mt-0.5">
            Use this one plan for every paper — no need to create a new plan each time.
            Before each paper, open it and click <strong>Update Seating</strong> to get a
            fresh anti-cheat arrangement (a different desk layout than last time, same rooms/classes).
          </span>
        </span>
      </label>
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-xl font-heading font-bold text-foreground break-words">{plan.title}</h2>
...
const GenerateSeatingButton = ({ plan }: { plan: SeatingPlanFull }) => {
  ...
  const isUpdate = plan.is_recurring && plan.total_seated > 0;
  ...
  // Recurring "All Papers" plans stay published across every paper — the
  // admin re-runs seating via Update instead of the status gate that blocks
  // a normal one-off plan from being regenerated after publish.
  const disabled = genMut.isPending || (!plan.is_recurring && plan.status === "published");
