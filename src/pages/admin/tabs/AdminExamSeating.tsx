...
import {
  ...
  Building2, Users, AlertTriangle, CheckCircle2, Grid3x3, QrCode, FileText, Search,
} from "lucide-react";
...
const AdminExamSeating = () => {
  ...
  return <PlansList selectedSessionId={selectedSessionId} setSelectedSessionId={setSelectedSessionId} onOpenPlan={setSelectedPlanId} />;
};
  ...
  return (
    <div className="border-t border-border pt-4 space-y-3">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div>
          <Label className="text-xs">Exam Date (optional)</Label>
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-xl font-heading font-bold text-foreground truncate">{plan.title}</h2>
      ...
      <div className="grid lg:grid-cols-[280px,1fr] gap-5">
        ...
        <div className="space-y-5">
          {activeRoom ? (
            <RoomDeskMap room={activeRoom} plan={plan} />
          ) : (
            <Card>
...
const GenerateSeatingButton = ({ plan }: { plan: SeatingPlanFull }) => {
  ...
  return (
    <Button onClick={handleGenerate} disabled={genMut.isPending || plan.status === "published"}>
      {genMut.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
...
const PublishButton = ({ plan }: { plan: SeatingPlanFull }) => {
  ...
  return (
    <Button
      onClick={() => pubMut.mutate({ planId: plan.id, sessionId: plan.session_id })}
      disabled={disabled || pubMut.isPending}
      variant={plan.status === "published" ? "secondary" : "default"}
    >
      {pubMut.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> :
       plan.status === "published" ? <CheckCircle2 className="w-4 h-4" /> : <Send className="w-4 h-4" />}
...
const PrintDeskMapButton = ({ room, plan }: { room: SeatingRoom; plan: SeatingPlanFull }) => {
  ...
  const handle = async () => {
    ...
    try {
      ...
      doc.text(`${room.name}  ·  ${room.rows}×${room.cols} grid  ·  ${room.assignments.length} seated`, margin, 20);
      if (room.invigilator) {
        doc.text(`Invigilator: ${room.invigilator}`, margin, 25);
      }
  ...
  const run = async (m: "pdf" | "csv") => {
    ...
    try {
      if (m === "pdf") {
        ...
        doc.setFontSize(14); doc.setFont("helvetica", "bold");
        doc.text(`${plan.title} — Seat Lookup`, 10, 14);
        doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(80);
        doc.text(`Generated ${new Date().toLocaleString()}  ·  ${all.length} students seated`, 10, 19);
        autoTable(doc, {
          startY: 24,
          ...
          didDrawPage: (data) => {
            ...
            doc.text("GHS Babi Khel · Exam Seating Plan Engine", doc.internal.pageSize.getWidth() / 2, doc.internal.pageSize.getHeight() - 6, { align: "center" });
          },
        });
...
const QrStickerSheetButton = ({ plan }: { plan: SeatingPlanFull }) => {
  ...
  const handle = async () => {
    ...
    try {
      ...
      const stickerSize = Math.min(cellW - 8, 50);
      ...
      for (const a of all) {
        ...
        doc.roundedRect(x, y, cellW - 4, cellH - 4, 2, 2, "S");
        ...
        const qrX = x + (cellW - 4 - stickerSize) / 2;
        const qrY = y + 4;
        doc.addImage(qrDataURL, "PNG", qrX, qrY, stickerSize, stickerSize);

        // Labels below QR
        const labelY = qrY + stickerSize + 4;
        doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(40);
        doc.text(a.seat_label, x + (cellW - 4) / 2, labelY, { align: "center" });
        doc.setFont("helvetica", "normal"); doc.setFontSize(7); doc.setTextColor(80);
        const nm = a.student_name.length > 24 ? a.student_name.slice(0, 23) + "…" : a.student_name;
        doc.text(nm, x + (cellW - 4) / 2, labelY + 4, { align: "center" });
        doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(40);
        doc.text(a.exam_roll_no, x + (cellW - 4) / 2, labelY + 9, { align: "center" });
        doc.setFont("helvetica", "normal"); doc.setFontSize(6); doc.setTextColor(120);
        doc.text(`Class ${a.class} · ${a.room_name}`, x + (cellW - 4) / 2, labelY + 13, { align: "center" });
        doc.text("Scan to mark attendance", x + (cellW - 4) / 2, labelY + 17, { align: "center" });
