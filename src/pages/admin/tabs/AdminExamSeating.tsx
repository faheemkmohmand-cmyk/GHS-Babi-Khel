...
const CreatePlanForm = ({ sessionId, onCreated }: { sessionId: string; onCreated: (id: string) => void }) => {
  ...
  // time entry here. Classes mixed into the same hall/plan can be sitting
  // DIFFERENT papers at DIFFERENT times on the same day (e.g. Class 6 Urdu
  // 9:00–12:00 while Class 10 Mutalia Quran runs 9:00–11:00) — so timing is
  // picked PER CLASS, not once for the whole plan.
  const { data: dateSheetEntries = [] } = useAllExamSchedule();

  // ── MODE TOGGLE (Problem 1 fix): Single Day vs All Papers ──────────────
  //   "single"  → Single Day Exam: one specific paper per class, all on the
  //               same exam date. is_recurring = false, no date range.
  //   "all"     → All Papers (Multi-Day): every date-sheet paper for each
  //               selected class (given its own exam-type/year) is
  //               included; the plan spans the earliest → latest date
  //               across all selected classes. Each class keeps its own
  //               per-paper timing on Console/3D Hall.
  type PlanMode = "single" | "all";
  const [mode, setMode] = useState<PlanMode>("single");

  const [selectedClasses, setSelectedClasses] = useState<string[]>([]);
  // Per-class timing source selections: which exam type / year / (single
  // mode only) specific date-sheet paper to use for THIS class.
  const [classExamType, setClassExamType] = useState<Record<string, string>>({});
  const [classYear, setClassYear] = useState<Record<string, number>>({});
  const [classEntryId, setClassEntryId] = useState<Record<string, string>>({});
  ...
  const toggleClass = (c: string) => {
    setSelectedClasses(prev => {
      ...
      // Seed sensible defaults for a newly-added class.
      setClassYear(y => (y[c] !== undefined ? y : { ...y, [c]: new Date().getFullYear() }));
      return [...prev, c];
    });
  };

  // Exam types available in the Date Sheet for a given class.
  const examTypesFor = (cls: string) =>
    Array.from(new Set(dateSheetEntries.filter(e => e.class === cls).map(e => e.exam_type)));

  // Date-sheet entries available for a given class, given its chosen
  // exam-type/year.
  const entriesFor = (cls: string) => {
    const et = classExamType[cls];
    const yr = classYear[cls] ?? new Date().getFullYear();
    if (!et) return [] as ExamScheduleEntry[];
    return dateSheetEntries.filter(e => e.class === cls && e.exam_type === et && e.year === yr);
  };

  const allSelectedReady = selectedClasses.every(cls => {
    if (mode === "single") {
      const entry = entriesFor(cls).find(e => e.id === classEntryId[cls]);
  ...
  const handleCreate = async () => {
    ...
    if (!allSelectedReady) { toast.error("Pick a paper from the Exam Date Sheet for every selected class"); return; }
    ...
    // Build the per-class timing map from the Date Sheet.
    ...
    for (const cls of selectedClasses) {
      const entries = entriesFor(cls);
      if (mode === "single") {
        const entry = entries.find(e => e.id === classEntryId[cls])!;
        classPaperTimes[cls] = {
        ...
        // All Papers mode: use this class's FIRST date-sheet paper as its
        // representative timing on the plan-wide fallback fields; Console/
        // 3D Hall re-derive per-day timing from the Date Sheet directly.
    ...
    try {
      const plan = await createMut.mutateAsync({
        ...
        // Plan-wide fields kept as a fallback reference (first class's first
        // paper) — Console/3D Hall prefer class_paper_times per student.
  ...
  return (
    <div className="border-t border-border pt-4 space-y-4">
      ...
      {/* ── PER-CLASS TIMING SOURCE: Exam Date Sheet (no manual time entry) ──
          Each selected class picks its OWN paper — classes mixed into the
          same rooms can be sitting different papers at different times on
          the same day (e.g. Class 6 Urdu 9-12, Class 10 Mutalia Quran 9-11). */}
      {selectedClasses.length > 0 && (
        <div className="space-y-3">
          <p className={`text-xs font-semibold flex items-center gap-1.5 ${
            ...
            Paper Timing (per class, from Exam Date Sheet)
          </p>
          <p className="text-[11px] text-muted-foreground -mt-2">
            Each class below has its own subject/date/time — pulled directly from the <strong>Exam Date Sheet</strong>,
            not typed here. Set the date sheet first if a paper isn't listed.
          </p>
          {selectedClasses.map(cls => {
            const types = examTypesFor(cls);
            const entries = entriesFor(cls);
            const entry = entries.find(e => e.id === classEntryId[cls]);
            const sortedEntries = [...entries].sort((a, b) => a.exam_date.localeCompare(b.exam_date));
            return (
              <div
                key={cls}
              ...
              >
                <p className={`text-xs font-bold px-2 py-0.5 rounded-full inline-block ${colorFor(cls).bg} ${colorFor(cls).text}`}>
                  Class {cls}
                </p>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      value={classExamType[cls] ?? ""}
                      onChange={e => {
                        setClassExamType(m => ({ ...m, [cls]: e.target.value }));
                        setClassEntryId(m => ({ ...m, [cls]: "" }));
                      }}
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={classYear[cls] ?? new Date().getFullYear()}
                      onChange={e => {
                        setClassYear(m => ({ ...m, [cls]: Number(e.target.value) || new Date().getFullYear() }));
                        setClassEntryId(m => ({ ...m, [cls]: "" }));
                      }}
                      className="mt-1"
                    />
                ...
                {mode === "single" ? (
                  <div>
                    ...
                    <select
                      value={classEntryId[cls] ?? ""}
                      onChange={e => setClassEntryId(m => ({ ...m, [cls]: e.target.value }))}
                      disabled={!classExamType[cls]}
                    ...
                    {classExamType[cls] && entries.length === 0 && (
                      <p className="text-[11px] text-red-500 font-semibold mt-1">
                        ⚠ No papers found in the Date Sheet for Class {cls}. Add them in Exam Date Sheet first.
                      </p>
                    )}
                    {entry && (
                      <div className="mt-2 rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-900/20 p-2.5 text-[11px] text-emerald-700 dark:text-emerald-400">
                        ✓ {entry.subject} on <strong>{entry.exam_date}</strong>, {entry.start_time || "—"}–{entry.end_time || "—"} (from Exam Date Sheet)
                      </div>
                    )}
                  </div>
                ) : (
                  <div>
                    {classExamType[cls] && entries.length > 0 ? (
                      <div className="rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-900/20 p-2.5 text-[11px] text-emerald-700 dark:text-emerald-400 space-y-1">
                        <p>✓ Found <strong>{entries.length}</strong> paper(s) for Class {cls}.</p>
                        <p>Spans <strong>{sortedEntries[0].exam_date}</strong> to <strong>{sortedEntries[sortedEntries.length - 1].exam_date}</strong>. Each paper keeps its own Date Sheet timing on the Live Console / 3D Hall.</p>
                      </div>
                    ) : classExamType[cls] ? (
                      ...
                      <p className="text-[11px] text-muted-foreground">Pick an exam type and year above to pull papers from the Date Sheet.</p>
                    )}
                  </div>
                )}
              </div>
