...
const CreatePlanForm = ({ sessionId, onCreated }: { sessionId: string; onCreated: (id: string) => void }) => {
  ...
  // ── Paper timing now comes ONLY from the Exam Date Sheet — no manual
  // time entry here. The admin picks a class + a date-sheet entry (which
  // already carries subject, date, start_time, end_time) and that entry's
  // timing is what gets stored on the plan. This keeps Exam Date Sheet as
  // the single source of truth; Live Console / 3D Hall read paper_start_at /
  // paper_end_at exactly as before — only where those values come from has
  // changed.
  const { data: dateSheetEntries = [] } = useAllExamSchedule();

  // ── MODE TOGGLE (Problem 1 fix): Single Day vs All Papers ──────────────
  //   "single"  → Single Day Exam: one paper, pulled from the Date Sheet.
  //               is_recurring = false, no date range.
  //   "all"     → All Papers (Multi-Day): every date-sheet paper for the
  //               chosen class/exam-type/year is included; the plan simply
  //               spans the earliest → latest date-sheet date. Per-paper
  //               timing is still read from each entry's own start/end
  //               time in the Date Sheet by Console/3D Hall — nothing is
  //               flattened to one shared time here.
  type PlanMode = "single" | "all";
  const [mode, setMode] = useState<PlanMode>("single");

  // Class used to pick date-sheet entries for the "reference paper" that
  // drives this plan's stored paper_start_at/paper_end_at (used by Console
  // countdowns). All classes selected for seating still get mixed below —
  // this is only which class's date sheet row to read timing from.
  const [timingClass, setTimingClass] = useState<string>("");
  const [timingExamType, setTimingExamType] = useState<string>("");
  const [timingYear, setTimingYear] = useState<number>(new Date().getFullYear());
  ...
  // Date-sheet entries available for the chosen class/exam-type/year.
  const availableEntries = useMemo(
    () => dateSheetEntries.filter(
      e => e.class === timingClass && e.exam_type === timingExamType && e.year === timingYear
    ),
    [dateSheetEntries, timingClass, timingExamType, timingYear]
  );
  const selectedEntry = availableEntries.find(e => e.id === selectedEntryId);

  // Distinct exam types available for the chosen class (from the date sheet).
  const examTypesForClass = useMemo(
    () => Array.from(new Set(dateSheetEntries.filter(e => e.class === timingClass).map(e => e.exam_type))),
    [dateSheetEntries, timingClass]
  );
  ...
  const handleCreate = async () => {
    ...
    if (mode === "single") {
      if (!selectedEntry) { toast.error("Pick a paper from the Exam Date Sheet for its date/timing"); return; }
      if (!selectedEntry.start_time || !selectedEntry.end_time) {
        toast.error("That paper has no start/end time set in the Exam Date Sheet yet — set it there first");
        return;
      }
    }
    if (mode === "all") {
      if (availableEntries.length === 0) {
        toast.error("No papers found in the Exam Date Sheet for that class/exam-type/year"); return;
      }
    }

    // Build full ISO datetimes straight from the date sheet's own date +
    // start_time/end_time — no manual time input involved.
    const toIso = (date: string, time: string | null): string | null => {
      if (!date || !time) return null;
      return new Date(`${date}T${time}:00`).toISOString();
    };

    // For "all" mode, span the plan across the earliest → latest date-sheet
    // date for this class/exam-type/year, and use the FIRST paper's time as
    // the reference paper_start_at/paper_end_at (Console/3D Hall re-derive
    // per-day timing; this reference keeps the initial countdown accurate).
    const sortedByDate = [...availableEntries].sort((a, b) => a.exam_date.localeCompare(b.exam_date));
    ...
    try {
      const plan = await createMut.mutateAsync({
        ...
        paperSubject: mode === "single" ? (selectedEntry!.subject || null) : null,
        examDate: mode === "single" ? selectedEntry!.exam_date : null,
        paperStartAt: mode === "single"
          ? toIso(selectedEntry!.exam_date, selectedEntry!.start_time)
          : toIso(firstEntry.exam_date, firstEntry.start_time),
        paperEndAt: mode === "single"
          ? toIso(selectedEntry!.exam_date, selectedEntry!.end_time)
          : toIso(firstEntry.exam_date, firstEntry.end_time),
        isRecurring: mode === "all",
        examDateFrom: mode === "all" ? firstEntry.exam_date : null,
        examDateTo:   mode === "all" ? lastEntry.exam_date : null,
      });
      onCreated(plan.id);
  ...
  return (
    <div className="border-t border-border pt-4 space-y-4">
      ...
      {/* ── TIMING SOURCE: Exam Date Sheet (no manual time entry) ── */}
      <div className={`rounded-xl border p-3 space-y-3 ${
        ...
        <p className={`text-xs font-semibold flex items-center gap-1.5 ${
          ...
          {mode === "single" ? "Single Day Exam" : "All Papers (Multi-Day)"}
        </p>
        <p className="text-[11px] text-muted-foreground">
          Paper date and timing are pulled directly from the <strong>Exam Date Sheet</strong> — nothing
          is typed here. Set the date sheet first if the paper you need isn't listed below.
        </p>
        <div className="grid sm:grid-cols-3 gap-3">
          <div>
            <Label className="text-xs">Class (for timing)</Label>
            <select
              value={timingClass}
              onChange={e => { setTimingClass(e.target.value); setTimingExamType(""); setSelectedEntryId(""); }}
          ...
          <div>
            ...
            <select
              value={timingExamType}
              onChange={e => { setTimingExamType(e.target.value); setSelectedEntryId(""); }}
              disabled={!timingClass}
              className="mt-1 w-full px-3 py-2 rounded-lg border border-input bg-background text-sm disabled:opacity-50"
            >
              <option value="">— Select —</option>
              {examTypesForClass.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            ...
            <Input
              type="number"
              value={timingYear}
              onChange={e => { setTimingYear(Number(e.target.value) || new Date().getFullYear()); setSelectedEntryId(""); }}
              className="mt-1"
            />
          </div>
        </div>
        ...
        {mode === "single" ? (
          <div>
            <Label className="text-xs">Paper (from Date Sheet)</Label>
            <select
              value={selectedEntryId}
              onChange={e => setSelectedEntryId(e.target.value)}
              disabled={!timingExamType}
              className="mt-1 w-full px-3 py-2 rounded-lg border border-input bg-background text-sm disabled:opacity-50"
            >
              <option value="">— Select a paper —</option>
              {availableEntries.map(e => (
                <option key={e.id} value={e.id}>
                  {e.subject} — {e.exam_date} ({e.start_time || "no start time"}–{e.end_time || "no end time"})
                </option>
              ))}
            </select>
            {timingExamType && availableEntries.length === 0 && (
              <p className="text-[11px] text-red-500 font-semibold mt-1">
                ⚠ No papers found in the Date Sheet for this class/exam-type/year. Add them in Exam Date Sheet first.
              </p>
            )}
            {selectedEntry && (
              <div className="mt-2 rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-900/20 p-2.5 text-[11px] text-emerald-700 dark:text-emerald-400">
                ✓ {selectedEntry.subject} on <strong>{selectedEntry.exam_date}</strong>, {selectedEntry.start_time || "—"}–{selectedEntry.end_time || "—"} (from Exam Date Sheet)
              </div>
            )}
          </div>
        ) : (
          <div>
            {timingExamType && availableEntries.length > 0 ? (
              <div className="rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-900/20 p-2.5 text-[11px] text-emerald-700 dark:text-emerald-400 space-y-1">
                <p>✓ Found <strong>{availableEntries.length}</strong> paper(s) in the Date Sheet for Class {timingClass} · {timingExamType} · {timingYear}.</p>
                <p>Plan will span <strong>{[...availableEntries].sort((a,b)=>a.exam_date.localeCompare(b.exam_date))[0].exam_date}</strong> to <strong>{[...availableEntries].sort((a,b)=>a.exam_date.localeCompare(b.exam_date))[availableEntries.length-1].exam_date}</strong>. Each paper keeps its own Date Sheet timing on the Live Console / 3D Hall.</p>
              </div>
            ) : timingExamType ? (
              ...
              <p className="text-[11px] text-muted-foreground">Pick a class, exam type, and year above to pull papers from the Date Sheet.</p>
            )}
          </div>
        )}
      </div>
