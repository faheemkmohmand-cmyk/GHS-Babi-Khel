...
function QuotesManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("[AdminExtras] Quote save failed:", err?.message || err);
      toast.error(err?.message?.includes("relation") 
        ? "Database table missing. Please run migration 014 in Supabase." 
        : err?.message || "Failed. Check console for details.");
    }
    setSaving(false);
  };
...
function HonorRollManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("[AdminExtras] Honor Roll save failed:", err?.message || err);
      toast.error(err?.message?.includes("relation") 
        ? "Database table missing. Please run migration 014 in Supabase." 
        : err?.message || "Failed. Check console for details.");
    }
