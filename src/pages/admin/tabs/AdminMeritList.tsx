...
import {
  ...
  TrendingUp, Target, Sparkles, Crown, Medal, ShieldCheck, ListFilter,
} from "lucide-react";
...
function useBiseClassMerit(year: number, examType: string) {
  return useQuery<Record<string, BiseMeritEntry[]>>({
    queryKey: ["bise-class-merit", year, examType],
    queryFn: async () => {
      if (!year || year < 2000 || !examType) return {};
      ...
      const out: Record<string, BiseMeritEntry[]> = {};
      ...
      if (entries9.length) out["9"] = entries9;
      if (entries10.length) out["10"] = entries10;
      return out;
    },
...
function useSaveMeritList() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const passingCount = params.entries.filter(e => e.is_pass).length;
...
function PublishDialog({
  ...
  // ── Student visibility ──────────────────────────────────────────────────
  // A student excluded here is dropped from the SNAPSHOT entirely — not
  // just hidden with CSS. This is what lets an admin publish a merit list
  // before a program without revealing who the toppers are: uncheck the
  // top performers, publish, and only the remaining (unchecked = hidden)
  // students' data ever leaves the browser. Re-published later with
  ...
  // For school scope, the full pool is ALL classes combined, ranked together.
  ...
  // What actually gets snapshotted — hidden students removed, ranks
  // renumbered sequentially so the published list has no gaps.
  const snapshotEntries: MeritEntry[] = useMemo(() => {
    return fullPool
      .filter(e => !hiddenIds.has(e.student_id))
  ...
  const hideAll = () => setHiddenIds(new Set(fullPool.map(e => e.student_id)));

  // "Top N only" — show only the first N by rank (overall, or per class for
  // the school-combined view), hide everyone else. This is the one-tap
  // version of the exact scenario the admin described: reveal only
  // positions 1–3 per class ahead of a prize-giving program.
  const showTopNPerClass = (n: number) => {
    if (scope !== "school" || !byClass) {
      const keepIds = new Set(fullPool.slice(0, n).map(e => e.student_id));
      setHiddenIds(new Set(fullPool.filter(e => !keepIds.has(e.student_id)).map(e => e.student_id)));
      return;
    }
    const keepIds = new Set<string>();
    for (const c of Object.keys(byClass)) {
      const sorted = [...(byClass[c] || [])].sort((a, b) => b.percentage - a.percentage);
      for (const e of sorted.slice(0, n)) keepIds.add(e.student_id);
    }
    setHiddenIds(new Set(fullPool.filter(e => !keepIds.has(e.student_id)).map(e => e.student_id)));
  };

  const showTopNOverall = (n: number) => {
    const keepIds = new Set(fullPool.slice(0, n).map(e => e.student_id));
    setHiddenIds(new Set(fullPool.filter(e => !keepIds.has(e.student_id)).map(e => e.student_id)));
  };
  ...
  const handleSave = async () => {
    ...
    if (!snapshotEntries.length) { toast.error("Everyone is hidden — at least one student must be visible to publish"); return; }
    ...
    try {
      await saveMut.mutateAsync({
        ...
        examType: scope === "school" ? "ALL" : examType,
        year,
        title,
        notes: notes || null,
        publishAt,
      ...
      >
        ...
        <div className={`p-6 sm:p-8 border-b border-border bg-gradient-to-br ${selectedTheme.from} ${selectedTheme.to} text-white shrink-0`}>
          <h3 className="font-heading font-bold text-xl sm:text-2xl flex items-center gap-3">
            ...
            {scope === "school" ? "Publish School Merit List" : `Publish Class ${cls} Merit List`}
          </h3>
          <p className="text-sm sm:text-base text-white/85 mt-2.5 leading-relaxed">
            ...
            {" "}{scope === "school" ? "All classes combined." : `${examTypeLabel(examType)} · ${year}`}
            {hiddenIds.size > 0 && (
          ...
          <div className="rounded-2xl border-2 border-amber-200 dark:border-amber-800/60 bg-gradient-to-br from-amber-50/70 via-card to-yellow-50/40 dark:from-amber-950/20 dark:via-card dark:to-yellow-950/10 overflow-hidden">
            <div className="p-4 sm:p-5 pb-3 sm:pb-4 border-b border-amber-200/70 dark:border-amber-800/40">
              ...
              <div className="flex flex-wrap gap-1.5 mt-3.5">
                <button
                  onClick={showAll}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-300 hover:brightness-105 transition-all"
                >
                ...
                >
                  <Crown className="w-3.5 h-3.5" /> Top 3 overall only
                </button>
                {scope === "school" && (
                  <button
                    onClick={() => showTopNPerClass(3)}
                    className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300 hover:brightness-105 transition-all"
                  >
                    <ListFilter className="w-3.5 h-3.5" /> Top 3 per class only
                  </button>
                )}
              </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {scope === "school" && <>Cls {e.class} · </>}Roll {e.roll_number} · {Number(e.percentage).toFixed(1)}%
                      </p>
                    </div>
                    {isHidden ? (
          ...
          <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 p-4">
            <p className="text-xs text-emerald-700 dark:text-emerald-400 flex items-start gap-2 leading-relaxed">
              ...
              <span>
                ...
                {hiddenIds.size > 0 && (
                  <> Hidden students are <strong>not included</strong> in the snapshot at all —
                  you can publish an updated list later with everyone visible.</>
                )}
              </span>
            </p>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-foreground leading-tight">
                    {r.title || (r.scope === "school"
                      ? `School Merit List ${r.year}`
                      : `Class ${r.class} — ${r.exam_type || "Exam"} ${r.year}`)}
                  </p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {r.scope === "school" ? "School" : `Class ${r.class}`}
...
function SchoolMeritTab() {
  ...
  const [publishOpen, setPublishOpen] = useState(false);
  const [biseExamType, setBiseExamType] = useState("Annual-I");
  const year = parseInt(yearInput, 10);
  const validYear = !isNaN(year) && year >= 2000 && year <= 2099;

  const { data: schoolByClass = {}, isLoading } = useAllClassesMerit(validYear ? year : 0);
  const { data: biseByClass = {}, isLoading: biseLoading, isFetching: biseFetching } =
    useBiseClassMerit(validYear ? year : 0, biseExamType);
  const { data: records = [] } = useMeritListRecords();

  // Merge BISE 9th/10th (percentage-ranked, 600/1200 totals) with the
  // school's own class 9/10 results (if both exist for the same class —
  // e.g. a school-administered mid-term alongside BISE's board exam).
  // Everything else (classes 6-8) comes from school results only, untouched.
  const byClass = useMemo(() => {
    const merged: Record<string, MeritEntry[]> = {};
    const allClassKeys = new Set([...Object.keys(schoolByClass), ...Object.keys(biseByClass)]);
    for (const cls of allClassKeys) {
      const combined = [...(schoolByClass[cls] || []), ...(biseByClass[cls] || [])];
      merged[cls] = combined
        .sort((a, b) => b.percentage - a.percentage)
        .map((e, i) => ({ ...e, position: i + 1 }));
    }
    return merged;
  }, [schoolByClass, biseByClass]);
  ...
  const biseCount = (biseByClass["9"]?.length || 0) + (biseByClass["10"]?.length || 0);
  ...
  return (
    <div className="space-y-4">
      <Card><CardContent className="p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex-1 min-w-[140px]">
            <Label className="text-xs text-muted-foreground mb-1 block">
              BISE Exam Type <span className="text-muted-foreground/60">(Class 9th/10th)</span>
            </Label>
            <Select value={biseExamType} onValueChange={setBiseExamType}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        {!biseLoading && !biseFetching && biseCount > 0 && (
          <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-2.5 flex items-center gap-1.5">
            <CheckCircle2 className="w-3 h-3" />
            {biseCount} BISE Peshawar result{biseCount === 1 ? "" : "s"} included (Class 9th: 600 total, Class 10th: 1200 total — ranked by percentage).
          </p>
        )}
      </CardContent></Card>
    ...
    <div className="space-y-5">
      ...
      <Tabs defaultValue="class">
        <TabsList className="grid grid-cols-3 w-full sm:w-auto sm:inline-grid">
          ...
          <TabsTrigger value="school" className="gap-1.5"><School className="w-4 h-4" /> School Merit</TabsTrigger>
