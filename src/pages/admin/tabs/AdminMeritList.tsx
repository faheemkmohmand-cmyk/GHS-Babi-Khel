...
function useSaveMeritList() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      // not just the visible snapshot — hiding a topper for a stage reveal
      // should not shrink the published pass rate or average. Only the
      // ranking/entries list itself (and highest_percentage, which is
      // necessarily the top achiever) should honor the hide list.
      ...
      const payload = {
        ...
        entries: params.entries as unknown as any,        // JSONB — array of MeritEntry (hidden removed)
        total_students: pool.length,                      // ← counts hidden students too
        passing_count: passingCount,                       // ← counts hidden students too
        highest_percentage: params.entries.length ? highest : 0, // 0/dash when everyone relevant is hidden
