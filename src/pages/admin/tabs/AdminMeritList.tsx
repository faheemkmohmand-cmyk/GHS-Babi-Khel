...
function PublishDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-foreground/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-2xl sm:rounded-3xl shadow-elevated max-w-lg w-full max-h-[92vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header with gradient accent */}
        <div className={`p-6 sm:p-7 border-b border-border bg-gradient-to-br ${selectedTheme.from} ${selectedTheme.to} text-white`}>
          <h3 className="font-heading font-bold text-lg sm:text-xl flex items-center gap-2.5">
          ...
          <p className="text-sm text-white/85 mt-2 leading-relaxed">
            {totalStudents} students will be snapshotted. {scope === "school" ? "All classes combined." : `${examTypeLabel(examType)} · ${year}`}
          </p>
        </div>
        ...
        <div className="p-6 sm:p-7 border-t border-border flex gap-2.5 justify-end">
    ...
    <div className="space-y-4">
      ...
      {!isLoading && totalStudents > 0 && (
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <Button onClick={() => setPublishOpen(true)} className="gap-1.5">
            <Plus className="w-4 h-4" />
            {existing ? "Update Published Merit List" : "Publish School Merit List to Dashboard"}
          </Button>
          {existing && (
            <Badge className="self-center bg-emerald-100 text-emerald-700 gap-1">
              <CheckCircle2 className="w-3 h-3" />
