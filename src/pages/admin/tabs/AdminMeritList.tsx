...
function useBiseClassMerit(year: number, examType: string) {
  return useQuery<Record<string, BiseMeritEntry[]>>({
    ...
    queryFn: async () => {
      ...
      (out as any).__meta = { savedRolls, resolvedRolls, missingRolls };
      return out;
    },
...
function Top3Podium({ entries, showClass = false }: { entries: MeritEntry[]; showClass?: boolean }) {
  ...
  const heights = ["h-12 sm:h-24", "h-16 sm:h-32", "h-10 sm:h-20"]; // 2nd, 1st, 3rd — shorter on mobile so the card doesn't overflow
  const medalColors = [
  ...
  return (
    <div className="rounded-2xl border border-border bg-gradient-to-br from-amber-50/50 via-card to-yellow-50/30 dark:from-amber-950/20 dark:to-yellow-950/10 p-3.5 sm:p-5">
        ...
        {order.map((entry, idx) => {
          ...
          return (
            <div key={`${entry.student_id}-${rank}`} className="flex flex-col items-center text-center flex-1 max-w-[120px]">
              ...
              {/* Podium pillar */}
              <div className={`mt-2 w-full ${heights[idx]} rounded-t-lg bg-gradient-to-br ${medalColors[idx]} flex items-start justify-center pt-1`}>
...
function BiseMeritTab() {
  ...
  const { data: biseByClass = {}, isLoading, isFetching, refetch } =
    useBiseClassMerit(validYear ? year : 0, examType);
  const { data: records = [] } = useMeritListRecords();

  const meta = (biseByClass as any).__meta as
    | { savedRolls: number; resolvedRolls: number; missingRolls: { roll: string; className: "9th" | "10th" }[] }
    | undefined;

  const sortedClasses = Object.keys(biseByClass).filter(k => k !== "__meta").sort();
