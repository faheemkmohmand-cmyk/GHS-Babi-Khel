...
const AdminPendingRequests = () => {
  ...
  const rejectUser = useMutation({
    mutationFn: async (userId: string) => {
      // ── Step 1: Get user info before deletion for toast message ──
      ...
      // ── Step 2: PERMANENTLY DELETE the auth account ──
      // This cascades and removes:
      // - auth.users row (the actual login account)
      // - profiles row (the public profile)
      // - Any linked data via foreign keys
      // The user will need to sign up again from scratch!
      const { error: deleteError } = await supabase.rpc("admin_delete_user", { 
        target_user_id: userId 
      });
      
      if (deleteError) {
        console.error("[AdminPendingRequests] Delete failed:", deleteError);
        
        // Fallback: If RPC fails, try direct admin API delete
        try {
          const { error: adminDeleteError } = await supabase.auth.admin.deleteUser(userId);
          if (adminDeleteError) throw adminDeleteError;
        } catch (fallbackErr: any) {
          // Last resort: Just mark as rejected if we can't delete
          console.warn("[AdminPendingRequests] Using fallback status update");
          ...
          return {
            fallback: true,
            message: `Marked ${userInfo?.full_name || 'user'} as rejected (account deletion requires service_role)`
          };
        }
      }
    ...
    onSuccess: (data: any) => {
      if (data?.deleted) {
        toast.success(`🚫 ${data.userName} — Account permanently deleted & rejection recorded`);
    ...
    onError: (err: any) => {
      console.error("[AdminPendingRequests] Rejection error:", err);
