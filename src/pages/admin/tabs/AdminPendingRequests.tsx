    ...
    mutationFn: async (userId: string) => {
      ...
      if (freshUser.role === "student" && freshUser.full_name) {
        ...
        if (!matchByAll && !matchByClass) {
          // No matching student record found — create one
          ...
          // Try including user_id (column may or may not exist)
          ...
          if (insertError) {
            ...
            // (The column might not exist in older schemas)
            console.warn("Student auto-creation warning:", insertError.message);
          }
        }
        // If a match was found, the existing student record is already
        // linked by name/class — the resolveStudentRecord() function
        // in financeService.ts will find it.
      }
    },
    onSuccess: () => {
