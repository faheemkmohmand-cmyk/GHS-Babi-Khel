...
const AdminPendingRequests = () => {
  ...
  const approveUser = useMutation({
    mutationFn: async (userId: string) => {
      ...
      // ── Auto-create/link student record for student role ────────────────
      // CRITICAL FIX: When a student signs up, only a `profiles` record exists.
      // Admin assigns fees using `students.id`, but the user dashboard queries
      // with `profiles.id`. We must ensure a matching `students` record exists
      // so the finance system can link the two.
      if (freshUser.role === "student" && freshUser.full_name) {
        // First, try to find an existing student record matching this profile
        const { data: existingStudents } = await supabase
          .from("students")
          .select("id, full_name, class, roll_number")
          .ilike("full_name", freshUser.full_name);

        const matchByAll = (existingStudents ?? []).find(
          (s) =>
            s.class === freshUser.class &&
            s.roll_number === freshUser.roll_number
        );
        const matchByClass = !matchByAll && freshUser.class
          ? (existingStudents ?? []).find((s) => s.class === freshUser.class)
          : null;

        if (!matchByAll && !matchByClass) {
          // No matching student record found — create one WITH user_id
          // so the fee system can link fee_vouchers.student_id → students.id → auth.uid()
          const newStudent: Record<string, any> = {
            full_name: freshUser.full_name,
            class: freshUser.class || "6",
            roll_number: freshUser.roll_number || `AUTO-${Date.now().toString(36).toUpperCase()}`,
            father_name: null,
            is_active: true,
            user_id: userId,  // CRITICAL: links students.id to auth.users.id
          };
          ...
          if (insertError) {
            console.warn("Student auto-creation warning:", insertError.message);
          }
        } else {
          // A match was found — update the existing student record with user_id
          // so the fee system can link back to the auth user
          const matched = matchByAll || matchByClass;
