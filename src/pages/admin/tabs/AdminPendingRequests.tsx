  ...
  const rejectUser = useMutation({
    mutationFn: async (userId: string) => {
      const { data, error } = await supabase
        ...
        .single();
      if (error) throw error;
      // Belt-and-suspenders: confirm the write actually landed as
      // "rejected" rather than trusting a silent no-op update.
      if (!data || data.status !== "rejected") {
        throw new Error("Status did not update to rejected — please retry.");
      }
      return data;
    },
    onSuccess: () => {
      toast.success("🚫 User rejected — moved to the Rejected tab");
      // Switch to the Rejected tab so the change is immediately visible
      // instead of the user simply vanishing from the Pending list,
      // which can look like nothing happened (or like it got approved).
      setFilter("rejected");
      qc.invalidateQueries({ queryKey: ["admin-pending-users"] });
      qc.invalidateQueries({ queryKey: ["admin-users"] });
    },
