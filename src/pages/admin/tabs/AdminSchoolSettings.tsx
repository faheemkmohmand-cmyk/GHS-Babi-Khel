...
import { Save, Loader2, ImageIcon, CheckCircle, AlertTriangle, RefreshCw, MapPin, Navigation, ExternalLink, X } from "lucide-react";
import toast from "react-hot-toast";
import { useDropzone } from "react-dropzone";

const ImageUploader = ({
  label, currentUrl, folder, onUploaded, onRemove, onUploadingChange, onUploadComplete,
}: {
  label: string;
      ...
      <div {...getRootProps()}
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-1 text-primary">
            <Loader2 className={`w-5 h-5 ${uploadProgress?.phase === 'processing' ? 'animate-pulse' : 'animate-spin'}`} />
            <p className="text-xs">
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card>
        ...
        <CardContent className="grid sm:grid-cols-3 gap-4">
          ...
          <div><Label>Results</Label>
            <Input type="text" placeholder="e.g. A+, A1, A++" value={form.board_results} onChange={e => set("board_results", e.target.value)} /></div>
        </CardContent>
      </Card>
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 flex items-start gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          Images are auto-saved to the database after upload. You can also click <strong>"Save All Changes"</strong> to save all fields at once.
        </p>
      </div>

      <Button onClick={handleSave} disabled={saving || uploadsInFlight > 0} className="gap-2 min-w-[160px]">
        {uploadsInFlight > 0 ? <><Loader2 className="w-4 h-4 animate-spin" />Waiting for upload...</>
