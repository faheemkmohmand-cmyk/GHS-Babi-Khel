              ...
              <Card key={s.id} className="overflow-hidden">
                <CardContent className="p-3.5">
                  <div className="flex items-start gap-3">
                    {/* Photo / Avatar with error handling */}
                    {(() => {
                      const [imgError, setImgError] = useState(false);
                      return s.photo_url && !imgError ? (
                        <img src={s.photo_url} alt={`${s.full_name}'s photo`} className="w-11 h-11 rounded-full object-cover shrink-0 border-2 border-background shadow-sm" loading="lazy" decoding="async" onError={() => setImgError(true)} />
                      ) : (
                        <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shrink-0 border-2 border-background shadow-sm">
                    ...
                    {students.map((s) => (
                      <tr key={s.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                        <td className="px-4 py-2.5">
                          {(() => {
                            const [tableImgError, setTableImgError] = useState(false);
                            return s.photo_url && !tableImgError ? (
                              <img src={s.photo_url} alt={`${s.full_name}'s photo`} className="w-8 h-8 rounded-full object-cover" loading="lazy" decoding="async" onError={() => setTableImgError(true)} />
                            ) : (
                              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold">{s.full_name.charAt(0)}</div>
                            );
                          })()}
                        </td>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                ...
                <Select value={promotionExamType} onValueChange={v => { setPromotionExamType(v); setPromotionPreview(null); }}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {ALL_EXAM_TYPES.map(e => <SelectItem key={e} value={e}>{examTypeLabel(e)}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="flex-1">
