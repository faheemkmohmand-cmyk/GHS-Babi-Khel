...
function exportSubstitutionImage(
  ...
  ctx.fillText(`Absent: ${absentTeacher}`, W / 2, 116);
  ...
  // ── Download ──
  const link = document.createElement("a");
  link.download = `Substitutes_${absentTeacher.replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.png`;
  link.href = canvas.toDataURL("image/png");
  link.click();
}
...
function SubstituteTeacherDialog({
  ...
  const handleAssign = async () => {
    if (!selectedTeacher) {
      toast.error("Please select the absent teacher first.");
      return;
    }
    setResult(null);
  ...
  const handleDownloadImage = () => {
    ...
    exportSubstitutionImage(selectedTeacher, result);
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Pick the absent teacher. Free teachers will be auto-assigned to cover their classes <strong>today only</strong> ({todayName}, {todayDateStr}). A shareable image is generated for the teachers' WhatsApp group.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 mt-2">
          {/* ── Teacher picker ── */}
          <div>
            <Label className="text-xs font-semibold">Absent Teacher</Label>
            <Select value={selectedTeacher} onValueChange={(v) => { setSelectedTeacher(v); setResult(null); }}>
              <SelectTrigger className="mt-1">
                <SelectValue placeholder="Select the absent teacher..." />
              </SelectTrigger>
            ...
            <div className="mt-2 space-y-2">
              ...
              {result.assigned.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Assigned Classes</p>
                  {result.assigned.map((a, i) => (
                    <div key={i} className="rounded-md border border-border p-2 text-xs">
                      ...
                      <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                        <span className="text-muted-foreground line-through">{a.original_teacher || "—"}</span>
              ...
              {result.uncovered.length > 0 && (
                <div className="rounded-md border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/20 p-2">
                  <p className="text-xs font-semibold text-red-700 dark:text-red-400 mb-1">
                    ⚠ Could not cover {result.uncovered.length} period(s):
                  </p>
                  {result.uncovered.map((u, i) => (
                                      ...
                                      {teachers.map((t) => {
                                        ...
                                        return (
                                          <SelectItem key={t.id} value={t.full_name} disabled={!!busyClass && t.full_name !== cell.teacher}>
                                            {t.full_name}{t.subject ? ` (${t.subject})` : ""}{busyClass && t.full_name !== cell.teacher ? ` — busy: Class ${busyClass}` : ""}
                                          </SelectItem>
                                        );
                                      })}
                                    </SelectContent>
