...
function generateAllClassesCombinedPdf(
  ...
  const footerReserve = 12; // keep clear of the footer rule/text at page bottom
  ...
  if (y + subjectSectionHeight > h - footerReserve) {
    ...
    doc.rect(5, 5, w - 10, h - 10);
    y = my + 4;
    // Light continuation header so the new page doesn't start bare
    doc.setDrawColor(PDF.gold[0], PDF.gold[1], PDF.gold[2]);
    doc.setLineWidth(0.8);
    doc.line(mx, y, w - mx, y);
    y += 4;
    ...
    doc.setFontSize(9);
    doc.setTextColor(PDF.ink[0], PDF.ink[1], PDF.ink[2]);
    doc.text("GOVERNMENT HIGH SCHOOL BABI KHEL — RESULT ANALYTICS (continued)", mx, y + 3);
    y += 7;
  }
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.text(`Page ${p} / ${pageCount}`, w - mx, h - 4, { align: "right" });
  }
