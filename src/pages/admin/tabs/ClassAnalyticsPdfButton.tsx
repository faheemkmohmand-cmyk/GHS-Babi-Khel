...
function generateClassAnalyticsPdf(data: ReturnType<typeof computeAnalyticsPdfData>) {
  ...
  data.subjectAverages.forEach((s, i) => {
    ...
    doc.setFontSize(4.3);
    doc.setTextColor(PDF.sub[0], PDF.sub[1], PDF.sub[2]);
    // Full subject name — no truncation/abbreviation, so "Geography" always
    // reads as "Geography", never "Geo.". A steep -60° rotation plus the
    // generous vertical band reserved below (see chartBaseY + 22 further
    // down) gives every full name room to run diagonally without clipping
    // against the page edge or the next section.
  ...
  // Numbered spokes instead of full subject names crammed around a small
  // circle — with up to 11 subjects, full names always collided or clipped
  // off the page edge. A compact numbered legend underneath (see below)
  // maps each number back to its full subject name with no truncation.
  ...
  // Numbered legend — full subject names, wrapped into two columns so long
  // names (e.g. "Computer Science") never truncate or run off the page.
  ...
  data.topStudents.forEach((s, i) => {
    ...
    const midY = ry + rowH / 2 + 1.5; // vertical center of the row, baseline-adjusted
    if (i % 2 === 0) {
      doc.setFillColor(PDF.panel[0], PDF.panel[1], PDF.panel[2]);
      doc.rect(tableColX, ry, tableColW, rowH, "F");
    }

    // Top 3 get a hand-drawn vector medal (ribbon + circle) instead of a
    // plain numbered circle, for a proud, celebratory feel — real medal
    // colors (gold/silver/bronze), not an emoji glyph (jsPDF's built-in
    // fonts don't include emoji glyphs, so text "🥇" would render as a
    // blank box rather than an actual medal).
    ...
    if (s.rank <= 3) {
      ...
      // Small ribbon tails poking above the medal circle.
      ...
      doc.triangle(badgeCx + 1.6, badgeCy - 1.2, badgeCx + 0.2, badgeCy - 1.2, badgeCx + 1.6, badgeCy - 4.2, "F");
      // Medal disc with a thin darker rim for a coin-like edge.
