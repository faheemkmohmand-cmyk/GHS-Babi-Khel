...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  }, [iframeReady, plan, paperPhase, countdownText, tally, perRoomTally, sendMessage]);
    ...
    <div className="fixed inset-0 z-[100] bg-black overflow-hidden">
      ...
      {flythroughActive && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10">
          <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl px-4 py-2.5 flex items-center gap-3 text-white min-w-[260px]">
            <Plane className="w-4 h-4 text-violet-300 animate-pulse shrink-0" />
            ...
            <span className="text-[10px] text-white/40 shrink-0 whitespace-nowrap">
              ...
              {flythroughStep.loop > 1 && <span className="ml-1 text-violet-300">· Loop {flythroughStep.loop}</span>}
            </span>
            <button
              onClick={() => sendMessage({ type: "FLYTHROUGH_STOP" })}
              className="text-rose-300 hover:text-rose-200 shrink-0 bg-rose-500/20 rounded-md p-1"
              title="Stop flyover"
            >
