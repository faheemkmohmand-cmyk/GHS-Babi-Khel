...
// ─── MCQ Timer — read-only mirror of AdminExamConsole's timer state ──────────
// The admin starts/stops/resets the MCQ timer from the Live Exam Console
// (AdminExamConsole.tsx). That tab writes the state to localStorage under
// MCQ_TIMER_KEY. This 3D Hall view reads the SAME key, so:
//   • the big countdown shows up on the 3D Hall top bar in real time, AND
//   • the in-scene HD screen (Hall_3D.html) gets UPDATE_TIMER messages
//     every second so the timer is visible ON the screen inside the 3D
//     hall, AND
//   • when the timer hits zero, this view sends SIREN_ON to the iframe —
//     which fires a red lightning/siren overlay across the whole 3D scene
//     for 15 seconds (great for signalling the end of an MCQ paper).
//
// Why localStorage and not Supabase? The timer is a per-admin-session UI
// affordance, not persistent school data. localStorage is instant, works
// offline, doesn't need an RLS policy, and the browser fires a `storage`
// event in OTHER tabs for free — so the Console tab and the 3D Hall tab
// stay in sync without any network round-trip.
const MCQ_TIMER_KEY = "ghs-exam-mcq-timer";
const SIREN_DURATION_MS = 15 * 1000; // 15 seconds — matches the user's spec
...
/** Format ms as MM:SS for display in the 3D Hall top bar + on the screen. */
...
// ─── Siren Sound (Web Audio API — played from the PARENT document) ──────────
// The MCQ-timer-finished siren SOUND is generated HERE in the parent React
// document, NOT inside the iframe. Reason: the SIREN_ON trigger arrives from
// a timer (not a direct user gesture). Mobile browsers (Chrome/Safari) block
// AudioContext.resume() outside a user gesture, so an AudioContext created
// inside the iframe stays 'suspended' and produces NO sound — the red lights
// flash (CSS only) but the siren is silent.
//
// The PARENT document, however, has received plenty of user gestures (the
// admin logged in, navigated to the admin panel, opened the 3D Hall, started
// the timer). So an AudioContext created + resumed here on the first
// pointerdown/keydown is 'running' and the siren plays loudly. The iframe
// keeps handling the VISUAL (red lightning overlay) via SIREN_ON/SIREN_OFF;
// we only moved the SOUND to the parent.
//
...
function ensureSirenAudioCtx(): AudioContext | null {
...
/**
 * Unlock the AudioContext on first user interaction. Mobile autoplay policy
 * requires a user gesture to start audio. We resume the context here on the
 * first pointerdown/keydown in the 3D Hall view so it's 'running' before the
 * siren needs to fire.
 */
...
function startSirenSound() {
  const ctx = ensureSirenAudioCtx();
  if (!ctx) return;
  // Resume if suspended (best-effort — may fail outside a user gesture, but
  // the unlockSirenAudio() listener should have already resumed it on the
  // admin's first click/tap in the 3D Hall view).
  if (ctx.state === "suspended") {
    ctx.resume().catch(() => {});
  }
...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  // The siren sound + red flash must play for the FULL 15 seconds. The
  ...
  // the stopSirenSound() call in this effect's cleanup, it would run on
  // every re-render (~1s) and KILL the siren after ~1-2 seconds — which is
  // exactly the bug the user reported.
  //
  // Solution: manage the siren lifecycle via REFS, not effect cleanup.
  //   • sirenFiredForRef — guards against double-firing for the same finishedAt
  //   • sirenTimeoutRef  — stores the 15s timeout; survives effect re-runs
  // The effect itself has NO cleanup function, so it never stops the sound
  // prematurely. The sound is stopped ONLY by:
  //   1. The 15s timeout (normal completion), or
  //   2. The unmount-only cleanup effect (user closes the 3D Hall)
  //
  // AUTO-RESET (fixes "siren re-fires on reopen"):
  // When the siren fires, we IMMEDIATELY delete the timer from localStorage
  // (not after 15s). This way:
  //   • If the user closes the 3D Hall mid-siren and reopens it, the timer
  //     is already gone — no re-fire.
  //   • The Console tab (if open) gets the storage event and hides its card.
  //   • The admin must start a NEW timer to get another siren.
  // The siren + red flash continue playing for 15s in-memory (tracked by
  // refs), completely independent of localStorage.
  ...
  // Mobile autoplay policy requires a user gesture to start audio. The
  // SIREN_ON trigger arrives from a timer (not a gesture), so we must resume
  // the AudioContext BEFORE the siren needs to fire. We listen for the first
  // pointerdown/keydown in the document and resume the context — the admin's
  // clicks/taps/keystrokes while using the 3D Hall view unlock it.
  useEffect(() => {
    ...
    return () => {
      ...
      // Unmount-only cleanup: stop the siren sound + clear the timeout.
      // This runs ONLY when the 3D Hall component unmounts (user closes it),
      // NOT on every re-render. This is what prevents the siren from being
      // killed prematurely while allowing it to be stopped when the user
      // actually leaves.
  ...
  useEffect(() => {
    ...
    // Fire the siren when we observe a fresh "finished" state we haven't
    // yet sirened for. finishedAt is the unique key.
    if (mcqTimer?.status === "finished") {
      ...
      if (sirenFiredForRef.current !== finishedAt) {
        ...
        // IMMEDIATELY delete the timer from localStorage so reopening the
        // 3D Hall (or the Console tab re-detecting "finished") does NOT
        // re-fire the siren. The siren + red flash play for 15s in-memory
        // (tracked by refs), independent of localStorage.
        try { localStorage.removeItem(MCQ_TIMER_KEY); } catch { /* ignore */ }
        setMcqTimer(null);

        // Start the siren: visual (iframe) + sound (parent document).
        sendMessage({ type: "SIREN_ON", durationMs: SIREN_DURATION_MS });
        startSirenSound();

        // Arm the 15s auto-off. Stored in a ref so it survives effect re-runs
        // (the effect re-runs every second when mcqTimer changes, but we do
        // NOT stop the sound in a cleanup — only this timeout stops it).
        ...
        // NOTE: intentionally NO return cleanup here. The sound must keep
        // playing for the full 15s even as this effect re-runs. The unmount
        // cleanup (in the unlock effect above) handles stopping on close.
      }
    }
