...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  const [flythroughStep, setFlythroughStep] = useState({ step: 0, total: 0, label: "" });
  const [qrUrl, setQrUrl] = useState("");
  const [panelOpen, setPanelOpen] = useState(true); // left control panel visibility on mobile
    ...
    const handler = (e: MessageEvent) => {
      ...
      switch (msg.type) {
        ...
        case "FLYTHROUGH_STEP":
          setFlythroughStep({ step: msg.step, total: msg.total, label: msg.label });
          break;
        case "FLYTHROUGH_DONE":
          setFlythroughActive(false);
          setFlythroughStep({ step: 0, total: 0, label: "" });
  ...
  const handleFlythrough = () => {
    ...
    setFlythroughStep({ step: 1, total: 7, label: "Starting…" });
    sendMessage({ type: "FLYTHROUGH" });
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black overflow-hidden">
      ...
      {/* ── Left control panel ── */}
      <div className={`absolute top-20 sm:top-24 left-2 sm:left-4 z-10 transition-all duration-300 ${
        ...
        <div className="bg-black/70 backdrop-blur-md border border-white/10 rounded-2xl p-3 sm:p-4 w-[280px] sm:w-[300px] max-h-[calc(100vh-8rem)] overflow-y-auto space-y-3 text-white">
          ...
          {/* ── Auto camera flythrough (Feature 4) ── */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-white/50 mb-1.5">Camera</p>
            <div className="grid grid-cols-2 gap-1.5">
              <Button size="sm" variant="secondary" onClick={handleFlythrough} disabled={flythroughActive}
                className="h-8 text-xs bg-violet-500/30 hover:bg-violet-500/50 text-violet-100 border-violet-400/30">
                <Plane className="w-3 h-3" /> Flythrough
              </Button>
            ...
            {plan.status === "generated" && !flythroughActive && (
              <p className="text-[10px] text-violet-300/80 mt-1.5">✨ Tip: run the flythrough before publishing to preview the hall.</p>
            )}
          </div>
      ...
      {!panelOpen && (
        <button
          ...
          className="absolute top-20 sm:top-24 left-2 z-10 bg-black/70 backdrop-blur-md border border-white/10 rounded-xl p-2 text-white hover:bg-black/80"
        >
          <Layers className="w-4 h-4" />
        </button>
      )}

      {/* ── Collapse panel button (when open, mobile only) ── */}
      {panelOpen && (
        <button
          onClick={() => setPanelOpen(false)}
          className="absolute top-20 sm:top-24 right-2 z-10 sm:hidden bg-black/70 backdrop-blur-md border border-white/10 rounded-xl p-2 text-white hover:bg-black/80"
        >
          <ChevronDown className="w-4 h-4 rotate-90" />
        </button>
      )}

      {/* ── Right info panel (Feature 5: Seat Desk Sticker Visualization) ── */}
      {clickedAssignment && (
        <div className="absolute top-20 sm:top-24 right-2 sm:right-4 z-10 w-[260px] sm:w-[300px]">
          <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-3 sm:p-4 space-y-3 text-white">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-white/50">Seat Selected</p>
                <p className="text-lg font-bold font-mono truncate">{clickedAssignment.seatLabel}</p>
              </div>
              <button
            ...
            <div className="flex gap-3">
              {qrUrl && (
                <div className="shrink-0">
                  <div className="w-24 h-24 rounded-lg border border-white/20 p-1.5 bg-white">
                    <img src={qrUrl} alt="Desk QR" className="w-full h-full" />
                  </div>
                  <p className="text-[9px] text-white/40 text-center mt-1 flex items-center justify-center gap-0.5">
              ...
              <div className="flex-1 min-w-0 space-y-1.5 text-xs">
                ...
                <InfoRow icon={<Users className="w-3 h-3" />} label="Class" value={`Class ${clickedAssignment.class}`} />
      ...
      {/* ── Flythrough progress overlay (Feature 4) ── */}
      {flythroughActive && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10">
          <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl px-4 py-2.5 flex items-center gap-3 text-white min-w-[240px]">
            <Plane className="w-4 h-4 text-violet-300 animate-pulse" />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate">{flythroughStep.label || "Flying…"}</p>
              {flythroughStep.total > 0 && (
                <div className="h-1 bg-white/10 rounded-full overflow-hidden mt-1">
                  <div
                    className="h-full bg-violet-400 transition-all duration-500"
                    style={{ width: `${(flythroughStep.step / flythroughStep.total) * 100}%` }}
                  />
                </div>
              )}
            </div>
            <span className="text-[10px] text-white/40 shrink-0">
              {flythroughStep.step}/{flythroughStep.total}
            </span>
            <button
              onClick={() => { setFlythroughActive(false); sendMessage({ type: "CAMERA_RESET" }); }}
...
function InfoRow({ icon, label, value, mono, valueColor }: { icon: React.ReactNode; label: string; value: string; mono?: boolean; valueColor?: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className={`font-medium flex-1 truncate ${mono ? "font-mono" : ""} ${valueColor ?? "text-white"}`}>{value}</span>
    </div>
  );
}
