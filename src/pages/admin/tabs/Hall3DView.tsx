...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  //
  // Either way, the FIRST time this view observes the transition into
  // "finished", we send SIREN_ON to the iframe and arm a 15-second timer
  // to send SIREN_OFF. We track "have we already fired the siren for this
  // finishedAt?" in a ref so a re-render doesn't double-fire.
  //
  // AUTO-RESET (Bug 3 fix): after the siren finishes (15s + 1s buffer),
  // we DELETE the timer from localStorage. This ensures:
  //   • Reopening the 3D Hall does NOT re-trigger the siren (no "finished"
  //     timer left to read).
  //   • The Console tab's timer card disappears (it gets the storage event).
  //   • The admin must start a NEW timer to get another siren — exactly
  //     what the user wants.
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Also stop the siren sound if the component unmounts while it's playing
      // (e.g., the admin closes the 3D Hall mid-siren).
      stopSirenSound();
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (mcqTimer?.status === "finished") {
      ...
      if (sirenFiredForRef.current !== finishedAt) {
        ...
        // Play the air-raid siren SOUND from the parent document. The iframe
        // handles the VISUAL (red lightning); we handle the SOUND here because
        // the parent has user-gesture activation (the AudioContext was unlocked
        // on the admin's first click/tap above).
        startSirenSound();
        // Auto-off the siren sound + visual after 15s.
        const t = setTimeout(() => {
          sendMessage({ type: "SIREN_OFF" });
          stopSirenSound();
          // AUTO-RESET: delete the timer from localStorage so reopening the
          // 3D Hall doesn't re-trigger the siren. The Console tab picks
          // this up via the storage event and hides its timer card too.
          try { localStorage.removeItem(MCQ_TIMER_KEY); } catch { /* ignore */ }
          setMcqTimer(null);
        }, SIREN_DURATION_MS);
        // Best-effort cleanup on unmount or re-run: clear the timeout AND stop
        // the sound so it can't keep playing if the component unmounts mid-siren.
        return () => {
          clearTimeout(t);
          stopSirenSound();
        };
