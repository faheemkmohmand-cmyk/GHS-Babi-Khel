...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  // Send stickers to iframe when ALL QR codes are ready.
  // IMPORTANT: The QR generation effect above updates `stickerQrs` in batches of 15.
  // Previously this effect fired on EVERY batch, sending SET_STICKERS 8 times for a
  // 120-student plan. Each call made the iframe clearStickers() + recreate ALL sticker
  // meshes + textures from scratch — extremely wasteful on mobile Chrome (caused
  // jank and GPU memory churn). Now we wait until every QR is generated, then send
  // a single SET_STICKERS message. The iframe creates all plaques in one pass.
  useEffect(() => {
    if (!iframeReady || assignmentsWithChairs.length === 0) return;
    // Wait until every assigned chair has its QR code generated (or failed → "")
    const readyCount = Object.keys(stickerQrs).length;
    if (readyCount < assignmentsWithChairs.length) return;
    try {
      const stickers = assignmentsWithChairs
        .filter(a => stickerQrs[a.chairIdx]) // skip chairs whose QR generation failed (empty string)
        .map(a => ({
          chairIdx: a.chairIdx,
