...
export default function Hall3DView({ planId, onClose }: Props) {
  ...
  const mcqIsRunning = mcqTimer?.status === "running";
  const mcqIsFinished = mcqTimer?.status === "finished";
        ...
        <div className="bg-gradient-to-b from-black/80 to-transparent p-3 sm:p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 pointer-events-auto">
              {/* ── MCQ Timer pill ─────────────────────────────────────────────
                  Visible only when the admin has started an MCQ timer (from
                  the Live Exam Console). Mobile: shows just MM:SS. Desktop:
                  shows "MCQ MM:SS". Red-pulses when finished. */}
              {mcqDisplay && (
                <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs sm:text-sm font-bold font-mono backdrop-blur-md border ${
                  mcqIsFinished
                ...
                }`} title="MCQ Timer — started from the Live Exam Console">
                  <Timer className="w-3.5 h-3.5" />
                  <span className="hidden sm:inline">MCQ</span>
                  <span>{mcqIsFinished ? "00:00" : mcqDisplay}</span>
                </div>
              )}
              {countdownText && (
