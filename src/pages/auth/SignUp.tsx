...
// Teacher role removed — Teacher Dashboard no longer exists, so signup
// only creates student accounts now. Role is fixed to "student" below.
...
const SignUp = () => {
  ...
  const validateForm = (): boolean => {
    ...
    // Role is fixed to "student" (Teacher signup removed) — no validation needed.
  ...
  // A database trigger (handle_new_auth_user) fires automatically the
  // instant auth.users gets a new row and inserts the matching profiles
  // row (with role/phone/status pulled from the signUp() metadata) using
  // ON CONFLICT (id) DO NOTHING. That means by the time this client-side
  // code runs, the profile has very likely already been created server-side
  // — we just need to confirm it, not race to insert it ourselves.
  const checkProfileExists = async (userId: string): Promise<boolean> => {
    ...
    if (error) {
      console.warn('[SignUp] Profile existence check failed:', error.message);
      return false;
    }
    return !!data;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2: Confirm the profile record exists.
      //
      // A DB trigger (handle_new_auth_user, AFTER INSERT ON auth.users)
      // already creates the profiles row automatically using the metadata
      // passed to signUp() above — full_name, role, phone, status:'pending'.
      // So there's normally nothing left to do here. We just verify it,
      // with a short retry loop to allow for replication lag, and only
      // as a genuine last resort try to create it ourselves via the
      // create_my_profile RPC (which upserts safely with ON CONFLICT).
      //
      // NOTE: we deliberately do NOT do a raw `.insert()` fallback here.
      // profiles has no `updated_at` column, and an insert with no
      // ON CONFLICT clause throws a duplicate-key error against the row
      // the trigger already created — that combination was the actual
      // cause of the "Account created but profile setup needs attention"
      // false-positive error, even though signup had fully succeeded.
      if (sessionUser) {
        console.log('[SignUp] Verifying profile for:', sessionUser.id);

        let profileExists = false;

        // Trigger fires synchronously in the same transaction as the
        // auth.users insert, so it should already be visible — but allow
        // a couple of short retries in case of read-replica lag.
        for (let attempt = 0; attempt < 3 && !profileExists; attempt++) {
          if (attempt > 0) await new Promise(r => setTimeout(r, 500));
          profileExists = await checkProfileExists(sessionUser.id);
        }

        if (!profileExists) {
          console.log('[SignUp] Trigger row not found yet, trying create_my_profile RPC...');
          try {
            ...
            if (!rpcError) {
              profileExists = true;
              console.log('[SignUp] ✅ Profile created via RPC fallback!');
            } else {
              console.error('[SignUp] RPC fallback failed:', rpcError.message);
            }
          } catch (rpcErr: any) {
            console.error('[SignUp] RPC fallback exception:', rpcErr?.message);
          }
        }

        if (!profileExists) {
          console.error('[SignUp] ❌ Profile could not be confirmed after signup.');
          toast.error(
