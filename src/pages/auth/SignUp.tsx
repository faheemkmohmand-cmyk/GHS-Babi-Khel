...
const SignUp = () => {
  ...
  const validateForm = (): boolean => {
    ...
    // CAPTCHA verification (Problem 24)
    if (!captchaVerified) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create profile record — this MUST succeed for the account to be
      // usable. Previously this result was never checked: if it silently
      // failed (e.g. RLS blocking an unconfirmed/session-less insert while
      // "Confirm email" is still ON in Supabase), signup showed "success"
      // but no profiles row existed. SignIn's fallback then treated that
      // as "deleted by admin" and silently recreated a fresh pending
      // request on every login attempt — which is the bug you hit.
      if (sessionUser) {
        ...
        if (profileError) {
          ...
          // This is the case that matters most: if this fails AGAIN even
          // with an active session, it's not a session/RLS timing issue —
          // it's either a real RLS policy misconfiguration or a DB error,
          // and telling the user to "contact the admin" is the honest,
          // correct answer rather than a dead-end retry loop.
