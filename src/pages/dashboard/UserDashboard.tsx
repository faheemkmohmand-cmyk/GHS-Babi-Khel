...
// cannot be lazy-loaded because they're not separate modules — they stay
// eager, which is fine because they're already part of this file's bundle
// anyway.
//
// Timetable and Exam Schedule were previously combined into one "Schedule"
// tab (ScheduleHubTab, with an inner sub-tab switcher). Per site-owner
// request they are now two fully separate dashboard tabs/sidebar entries —
// TimetableTab is lazy-loaded below, ExamScheduleTab stays local/eager.
// ScheduleHubTab.tsx is no longer used anywhere.
//
// 9 previously-imported tabs (ResultsTab, NoticesTab, NewsTab, GalleryTab,
// VideosTab, AchievementsTab, RollNumbersTab, ResultCardTab) were imported
// but NEVER referenced anywhere in this file — they were dead code from a
// past refactor. They've been removed entirely, which also removes their
// (sometimes heavy) transitive deps from the User Dashboard's main bundle.
// MediaHighlightsTab (Achievements + Honor Roll) removed from User Dashboard per site-owner request.
const TimetableTab        = lazy(() => import("./tabs/TimetableTab"));
...
// Honor Roll Card/Tab removed from User Dashboard (Media tab was removed) — dead code cleaned up.

// (Schedule Hub wrapper removed — Timetable and Exam Schedule are now
// separate dashboard tabs instead of being combined under one "Schedule"
// tab with an inner sub-tab switcher.)
