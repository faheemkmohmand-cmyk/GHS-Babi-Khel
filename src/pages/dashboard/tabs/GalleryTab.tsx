...
const GalleryTab = () => {
  ...
  const activeEmbed =
    lightboxIndex !== null && photos[lightboxIndex]
      ? getEmbedInfo(photos[lightboxIndex].photo_url)
      : null;
            ...
            : photos.map((p, i) => {
                ...
                const ytThumb =
                  embed?.provider === "youtube"
                    ? youTubeThumbnail(p.photo_url)
                    : null;
                const videoItem = !embed && isVideo(p.photo_url, p.media_type);
                return (
                  <div key={p.id} onClick={() => setLightboxIndex(i)} className="aspect-square rounded-lg overflow-hidden cursor-pointer group relative bg-secondary">
                    ...
                    {(videoItem || embed) && (
                      <Badge className="absolute top-1 left-1 bg-foreground/70 text-white text-[9px] gap-0.5 z-30">
                        <Play className="w-2.5 h-2.5" />{embed ? embed.badge : "VIDEO"}
                      </Badge>
                    )}
                  </div>
                );
                ...
                {activeEmbed ? (
                  <div
                    ...
                    style={{
                      ...
                      width: `min(92vw, ${(70 * activeEmbed.aspect).toFixed(2)}vh)`,
                    }}
                  >
                    <EmbedFrame
                      url={photos[lightboxIndex].photo_url}
                      interactive
                  ...
                  <video src={photos[lightboxIndex].photo_url} controls autoPlay className="max-w-full max-h-[80vh] rounded-xl" preload="metadata" playsInline />
                ) : (
                  <img 
