...
async function tryFetch(url: string, timeoutMs = 15000): Promise<Fact | null> {
  try {
...
async function fetchFact(path: string): Promise<Fact | null> {
  // Strategy 1: same-origin Vercel proxy (PRIMARY — has local fallback built-in)
  ...
  // Strategy 2: direct numbersapi.com (may come back online in the future)
  const direct = await tryFetch(`https://numbersapi.com/${path}?json`, 8000);
...
export default function NumberFacts() {
  ...
  const go = async (pathOverride?: string, labelOverride?: string) => {
    ...
    if (result && result.text) {
      ...
      setError("Could not fetch fact. The service might be temporarily unavailable. Please try again.");
    }
    setLoading(false);
  };
