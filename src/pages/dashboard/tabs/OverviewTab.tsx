...
const quickActions = [
  ...
  { id: "results", label: "Roll No. Slip", emoji: "📝", desc: "Check roll number", href: "/roll-no-slip" },
      ...
      <div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
          {quickActions.map((a, i) => {
            ...
            const cardClassName = "relative overflow-hidden bg-card rounded-2xl p-4 shadow-card hover:shadow-elevated transition-shadow text-center group border border-border/40 hover:border-primary/30";

            // "href" quick actions (e.g. Roll No. Slip) navigate to a public
            // route outside the dashboard shell instead of an internal tab.
            if ("href" in a && a.href) {
              return (
                <motion.div
                  key={a.id}
                  ...
                  whileTap={{ scale: 0.97 }}
                >
                  <Link to={a.href} className={cardClassName + " block"}>
                    {cardInner}
                  </Link>
                </motion.div>
              );
                ...
                >
                  {item.image_url ? (
                    ...
                    <div className="w-28 gradient-hero flex items-center justify-center shrink-0">
                      <Newspaper className="w-6 h-6 text-white/40" />
                    </div>
                  )}
