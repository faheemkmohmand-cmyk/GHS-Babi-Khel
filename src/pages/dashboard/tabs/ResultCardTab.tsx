              ...
              <>
                ...
                {results.map(r => (
                  <motion.div key={r.id} initial={{opacity:0,scale:0.98}} animate={{opacity:1,scale:1}}
                    ...
                    {(() => {
                      const hasSubjects = r.subject_marks && Object.keys(r.subject_marks).length > 0;
                      const subjectSum = hasSubjects ? Object.values(r.subject_marks!).reduce((s, m) => s + m.obtained, 0) : 0;
                      // Guard against stale subject_marks: if every subject shows 0
                      // obtained but the recorded obtained_marks total is actually
                      // greater than 0, the per-subject breakdown is out of sync
                      // with the real total (e.g. left over from an unfinished
                      // Add Result draft or overwritten by a CSV/manual edit that
                      // only updated the top-level totals). Showing "0/75" for
                      // every subject in that case is misleading, so fall back to
                      // the "not entered" notice instead.
                      const isStale = hasSubjects && subjectSum === 0 && r.obtained_marks > 0;
                      return hasSubjects && !isStale ? (
