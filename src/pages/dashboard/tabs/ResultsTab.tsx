...
import { Search, GraduationCap, Trophy, Users, TrendingUp, Award, XCircle, FileText, ArrowLeft } from "lucide-react";
...
const ResultsTab = () => {
  ...
  // ── Result Card toggle ────────────────────────────────────────────────────
  // Previously "Result Card" was a third tab in the ResultsHubTab horizontal
  // scroll (Results / Roll Numbers / Result Card), which crowded the bar on
  // small screens. It now lives behind this orange button inside the Results
  // view: clicking it swaps the class-wise results table for the Result Card
  // search interface (ResultCardTab). Clicking again (or "Back to Results")
  // returns to the normal results view.
  ...
  // ── Result Card view ──────────────────────────────────────────────────────
  // When the orange "Result Card" button is clicked, render the Result Card
  // search interface (ResultCardTab) instead of the class-wise results view.
  // A "Back to Results" button (also orange) returns to the normal view.
  // Hooks above are always called so React's rules of hooks are respected.
  if (showResultCard) {
    return (
      <div className="space-y-4">
        <button
          onClick={() => setShowResultCard(false)}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-orange-500 hover:bg-orange-600 text-white shadow-card transition-colors"
        >
  ...
  return (
    <div className="space-y-5">
      {/* Class tabs + orange Result Card button */}
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/* Orange Result Card button — mobile-friendly, toggles the Result
            Card search interface. Sits at the end of the class tab row so
            it's always reachable without horizontal scrolling. */}
        <button
          onClick={() => setShowResultCard(true)}
          className="ml-auto inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold bg-orange-500 hover:bg-orange-600 text-white shadow-card transition-colors"
        >
