...
import {
  ...
  ScanLine, X, Hash, Shield, UserCheck, Award,
} from "lucide-react";
...
function SeatDetailDialog({
  ...
  const downloadSlip = async () => {
    ...
    try {
      ...
      // Defensive: slip is only shown when isMe (line ~282), so profileName
      // is the right value to print here. Fall back to assignment.student_name
      // if profileName is somehow missing.
      drawDetail("Student:", (isMe && profileName) ? profileName : assignment.student_name);
      ...
      const invigilatorText = room.invigilators?.length ? room.invigilators.join(", ") : room.invigilator;
      if (invigilatorText) drawDetail(room.invigilators?.length > 1 ? "Invigilators:" : "Invigilator:", invigilatorText);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* ── Layout fix (2026-07-16) ───────────────────────────────────────
          Previously: `max-w-md` (28rem ≈ 448px) with a horizontal flex of
          [128px QR | flex-1 details]. After dialog padding (48px) the
          details column got only ~208px, which squeezed the DetailRow
          label (w-20 = 80px) and value (~106px) into an ugly, "distracted"
          layout — names and roll numbers wrapped mid-word.

          Fix:
          • Widen to `max-w-lg` (32rem ≈ 512px) for breathing room.
          • On small screens (sm:), stack QR ABOVE details so each gets
            the full dialog width — no squeezing.
          • On large screens, keep the side-by-side layout but give the
            details column a `min-w-0` so flex children can shrink
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4">
          {/* QR + key fields — stacks vertically on mobile, side-by-side on sm+ */}
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 min-w-0 space-y-2 text-sm">
              {/* ── BUG FIX (2026-07-16) ─────────────────────────────────────
                  Previously: `value={profileName || assignment.student_name}`
                  This meant the dialog ALWAYS showed the logged-in student's
                  name (profileName) for EVERY seat — because profileName is
                  truthy for any signed-in user. So clicking Ahmad's seat
                  still showed "Faheem Khan" (the signed-in user).

                  Fix: only prefer profileName when this seat IS the signed-in
                  user's own seat (isMe === true). For everyone else's seat,
                  always use assignment.student_name from the database.
                  ────────────────────────────────────────────────────────── */}
              <DetailRow icon={<User className="w-3.5 h-3.5" />} label="Student" value={isMe ? (profileName || assignment.student_name) : assignment.student_name} />
              ...
              {(room.invigilators?.length ? room.invigilators.join(", ") : room.invigilator) && (
                <DetailRow
                  icon={<User className="w-3.5 h-3.5" />}
                  label={room.invigilators?.length > 1 ? "Invigilators" : "Invigilator"}
                  value={room.invigilators?.length ? room.invigilators.join(", ") : (room.invigilator as string)}
                />
              )}
            </div>
          </div>
...
const DetailRow = ({ icon, label, value, mono }: { icon: React.ReactNode; label: string; value: string; mono?: boolean }) => (
  // ── Layout fix (2026-07-16) ─────────────────────────────────────────
  // `min-w-0` on the value span lets long roll numbers / names truncate
  // with `truncate` instead of overflowing the dialog. Label keeps `w-20`
  // for alignment but is now `shrink` (not `shrink-0`) so on very narrow
  // screens it can compress before the value wraps.
  <div className="flex items-center gap-2 min-w-0">
    <span className="text-muted-foreground shrink-0">{icon}</span>
    <span className="text-muted-foreground text-xs w-20 shrink min-w-[4rem]">{label}</span>
    <span className={`font-medium text-foreground flex-1 min-w-0 truncate ${mono ? "font-mono" : ""}`} title={value}>{value}</span>
  </div>
);
          ...
          <div className="flex items-center gap-3">
            {(room.invigilators?.length ? room.invigilators.join(", ") : room.invigilator) && (
              <span className="text-xs text-muted-foreground flex items-center gap-1">
                <User className="w-3 h-3" /> {room.invigilators?.length ? room.invigilators.join(", ") : room.invigilator}
              </span>
            )}
                ...
                if (a) {
                  ...
                  return (
                    <button
                      ...
                      aria-label={`Seat ${a.seat_label}: ${a.student_name}, Class ${a.class}, Exam Roll ${a.exam_roll_no}`}
                    >
...
const SeatingTab = () => {
  ...
  const allSeats = selectedPlan
    ? selectedPlan.rooms.flatMap(r =>
        r.assignments.map(a => ({ ...a, room_name: r.name, invigilator: r.invigilators?.length ? r.invigilators.join(", ") : r.invigilator }))
      )
    : [];
            ...
            <>
              ...
              {selectedPlan.status === "published" && (
                <>
                  ...
                  {/* ── Exam Staff & Duties card ──
                      Shows the Superintendent, Deputy Superintendent, and
                      per-room Invigilators so students know who's in charge
                      during the exam. This is read-only — students can see
                      the names and duties but can't edit them (that's in
                      the admin panel). Mobile-friendly: cards stack
                      vertically on small screens, 2-col grid on sm+. */}
                  {(((selectedPlan as any).superintendent || (selectedPlan as any).deputy_superintendent) ||
                    selectedPlan.rooms.some(r => (r.invigilators?.length ?? 0) > 0 || r.invigilator)) && (
                    <Card>
                      <CardContent className="p-4 sm:p-5">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <div>
                            <h3 className="text-sm font-heading font-bold text-foreground">Exam Staff & Duties</h3>
                            <p className="text-[11px] text-muted-foreground">Officials supervising this examination</p>
                          </div>
                        </div>

                        {/* Superintendent + Deputy — 2-column grid on sm+ */}
                        <div className="grid sm:grid-cols-2 gap-3 mb-3">
                          ...
                          <div className="rounded-xl border border-blue-200 dark:border-blue-900/50 bg-blue-50 dark:bg-blue-950/20 p-3">
                            <div className="flex items-center gap-1.5 mb-1">
                              ...
                              <span className="text-[11px] font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wide">Superintendent</span>
                            </div>
                            <p className="text-sm font-bold text-foreground truncate">
                              {(selectedPlan as any).superintendent || "Not assigned"}
                            </p>
                            <p className="text-[10px] text-muted-foreground mt-1 line-clamp-2">
                              {(selectedPlan as any).superintendent_duty || SUPERINTENDENT_DUTY}
                            </p>
                          </div>

                          {/* Deputy Superintendent */}
                          <div className="rounded-xl border border-teal-200 dark:border-teal-900/50 bg-teal-50 dark:bg-teal-950/20 p-3">
                            <div className="flex items-center gap-1.5 mb-1">
                              <UserCheck className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />
                              <span className="text-[11px] font-bold text-teal-700 dark:text-teal-300 uppercase tracking-wide">Deputy Superintendent</span>
                            </div>
                            <p className="text-sm font-bold text-foreground truncate">
                              {(selectedPlan as any).deputy_superintendent || "Not assigned"}
                            </p>
                            <p className="text-[10px] text-muted-foreground mt-1 line-clamp-2">
                              {(selectedPlan as any).deputy_superintendent_duty || DEPUTY_SUPERINTENDENT_DUTY}
                            </p>
                          </div>
                        </div>

                        {/* Invigilators per room */}
                        <div className="rounded-xl border border-border bg-secondary/30 p-3">
                          <div className="flex items-center gap-1.5 mb-2">
                            <Users className="w-3.5 h-3.5 text-muted-foreground" />
                            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Invigilators (by room)</span>
                          </div>
                          <div className="space-y-2">
                            {selectedPlan.rooms.map((room, idx) => {
                              const invigilators = room.invigilators?.length
                                ? room.invigilators
                                : (room.invigilator ? [room.invigilator] : []);
                              const duties = resolveColDuties(room);
                              if (invigilators.length === 0) return null;
                              return (
                                <div key={room.id} className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 pb-2 border-b border-border/50 last:border-0 last:pb-0">
                                  ...
                                  <div className="flex-1 flex flex-wrap gap-1.5">
                                    {invigilators.map((name, i) => {
                                      const duty = duties[i];
                                      const dutyText = duty ? `Cols ${duty.col_start}–${duty.col_end}` : null;
                                      return (
                                        <span
                                          key={i}
                                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-card border border-border text-[11px] font-medium text-foreground"
                                        >
                                          <User className="w-2.5 h-2.5 text-muted-foreground" />
                                          {name}
                                          {dutyText && (
                                            <span className="text-[9px] text-muted-foreground font-mono">({dutyText})</span>
                                          )}
                                        </span>
                                      );
                                    })}
                            ...
                            {selectedPlan.rooms.every(r => !(r.invigilators?.length ?? 0) && !r.invigilator) && (
                              <p className="text-xs text-muted-foreground italic">No invigilators assigned yet.</p>
                            )}
                          </div>
                        </div>
                      </CardContent>
