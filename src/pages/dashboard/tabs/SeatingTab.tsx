...
import {
  ...
  ScanLine, X, Hash, AlertTriangle, RefreshCw,
} from "lucide-react";
...
const SeatingTab = () => {
  ...
  // isError is now surfaced by the hook so we can distinguish "no plans yet"
  // (legitimate empty state) from "couldn't load" (RLS / network / auth).
  // The default `data: []` is still kept so existing render branches don't
  // have to add null-guards everywhere.
  const { data: plans = [], isLoading, isError, error, refetch, isFetching } = usePublishedSeatingPlans();
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        // ── Error state ──
        // Previously this case was indistinguishable from "no plans yet"
        // because the hook swallowed errors and returned []. Now we render a
        // distinct card so the user knows it's a load failure, not an empty
        // school — and we give them a Retry button. The most common cause is
        // missing RLS policies on the exam_seating_* tables; the admin
        // should run migration 022_exam_seating_student_rls.sql.
        <div className="bg-card rounded-2xl p-8 text-center border border-destructive/30">
          <AlertTriangle className="w-10 h-10 text-destructive/60 mx-auto mb-3" />
          <p className="font-semibold text-foreground">Couldn't Load Seating Plans</p>
          <p className="text-sm text-muted-foreground mt-1 max-w-md mx-auto">
            We hit an error while fetching the seating plan from the school
            database. This is usually a permissions issue on the server —
            please ask the admin to run the seating RLS migration, then retry.
          </p>
