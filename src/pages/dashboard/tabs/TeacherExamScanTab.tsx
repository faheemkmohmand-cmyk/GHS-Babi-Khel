...
const TeacherExamScanTab = () => {
  ...
  // CORRECTION MODE (rev. 8): manual status updates are ALWAYS allowed once
  // the sheet exists. Only scanning and init are locked outside the window.
        ...
        <>
          {!isInitialized ? (
            ...
            <>
              ...
              {/* Correction mode banner — scanning locked, but manual edits allowed */}
              {!canMark && (
                <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/50 rounded-xl p-3 flex items-start gap-2">
                  <Edit3 className="w-4 h-4 text-blue-600 mt-0.5 shrink-0" />
                  <div className="text-xs">
                    <p className="font-semibold text-blue-700 dark:text-blue-400">Correction Mode — scanning is locked.</p>
                    <p className="text-blue-600 dark:text-blue-500 mt-0.5">
                      {paperWindowMessage(windowStatus, displayPaperStart, displayPaperEnd)}
                    </p>
                    <p className="text-blue-600/70 dark:text-blue-500/70 mt-1">
                      QR scanning is locked, but you can still manually change a student's status below for corrections.
                    </p>
                  </div>
                </div>
              )}
