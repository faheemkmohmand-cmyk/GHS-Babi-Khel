...
function QRScanner({ onScan, enabled }: { onScan: (data: string) => void; enabled: boolean }) {
  ...
  const start = useCallback(async () => {
    ...
    // ── Start scanner directly (no pre-flight getUserMedia call) ───────────
    // Previous implementation called getUserMedia() first to check permission,
    // then stopped the stream immediately. This caused issues on mobile browsers:
    //   - The user gesture was consumed by the first call
    //   - Camera resource hadn't reset when Html5Qrcode tried to open it
    //   - Result: "Permission denied" error even though permission was granted
    // Fix: Let Html5Qrcode handle camera initialization directly.
    setActive(true);
