...
const TeacherExamScanTab = () => {
  ...
  const handleQRScan = (qrData: string) => {
    ...
    if (seatParsed) {
      // Desk QR — route to the seating-aware scan path. The mutation looks
      // up the assignment by plan+room+seat+student, resolves the actual
      // session/class, and upserts exam_attendance with seat_id/room_id/
      // seat_label populated.
      scanSeatingAttendance.mutate(
        ...
        },
        {
          onSuccess: (result) => {
            ...
            // If the seat belongs to the teacher's currently-selected session,
            // invalidate so the in-class list updates live. If it belongs to a
            // different session, we still marked attendance — just don't refresh
            // the local list (the teacher would switch sessions to see it).
