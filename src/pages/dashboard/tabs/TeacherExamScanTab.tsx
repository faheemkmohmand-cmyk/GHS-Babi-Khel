 ...
 * Uses html5-qrcode for real QR scanning from camera.
 *
 * ─── STRICT PAPER-WINDOW ENFORCEMENT (rev. 2) ───────────────────────────────
 * Same rules as the admin tab:
 *   1. Date locked to TODAY (read-only).
 *   2. Paper times auto-fetched from the seating plan.
 *   3. Scan / init / manual-status all blocked outside the paper window.
 * The teacher sees a clear LOCKED banner when the paper isn't running.
 */
...
import {
  ...
  Clock, Lock, AlertTriangle, Edit3,
} from "lucide-react";
...
// FIXED: Resolved half-visible camera feed on mobile by:
// 1. Using aspect-ratio instead of fixed pixel heights
// 2. Comprehensive video fill logic with ResizeObserver + safety interval
// 3. CSS rules that override html5-qrcode's internal pixel-based sizing
function QRScanner({ onScan, enabled }: { onScan: (data: string) => void; enabled: boolean }) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // ── COMPREHENSIVE VIDEO FILL LOGIC ──
      // Fixes half-visible camera feed by forcing all internal elements
      // to properly fill the container regardless of screen size.
      const forceVideoFill = () => {
        if (!container) return;
        
        // Get actual container dimensions for responsive sizing
        const containerRect = container.getBoundingClientRect();
        const cWidth = containerRect.width || container.clientWidth || 320;
        const cHeight = containerRect.height || container.clientHeight || 240;

        // Fix VIDEO element - this is what shows the camera feed
        const video = container.querySelector('video') as HTMLVideoElement | null;
        if (video) {
          video.style.setProperty('width', `${cWidth}px`, 'important');
          video.style.setProperty('height', `${cHeight}px`, 'important');
        ...
        innerDivs.forEach(div => {
          ...
          if (el.id?.includes('scan-region') || el.className?.toString().includes('scan-region')) {
            el.style.setProperty('width', `${cWidth}px`, 'important');
            el.style.setProperty('height', `${cHeight}px`, 'important');
            el.style.setProperty('position', 'absolute', 'important');
            el.style.setProperty('top', '0', 'important');
            el.style.setProperty('left', '0', 'important');
            return;
        ...
        canvases.forEach(canvas => {
          canvas.style.setProperty('width', `${cWidth}px`, 'important');
          canvas.style.setProperty('height', `${cHeight}px`, 'important');
      ...
      if (container) {
        ...
        // Safety interval - catches late-loading or async style changes
      ...
      // No aspectRatio config - let forceVideoFill handle sizing instead
      await qr.start(
        { 
    ...
    <div className="space-y-3">
      {!active ? (
        ...
        <div className="space-y-3">
          ...
          />
          <p className="text-[11px] text-center text-muted-foreground">Point camera at student's QR code or desk sticker</p>
...
const TeacherExamScanTab = () => {
  ...
  // Re-check the paper window every 10s
  ...
  // Auto-fetch paper times from the seating plan.
  ...
  // Paper window — uses row times first, falls back to seating plan times.
  ...
  const handleInitSheet = async () => {
    ...
    if (selectedDate !== todayStr()) { toast.error("Attendance can only be initialized for today's date"); return; }
    if (!seatingPaperTimes) {
      toast.error("No seating plan with paper times found. Ask the admin to set paper start/end time in Exam Seating first.");
      return;
    }
    if (windowStatus !== "in_progress") {
  ...
  const handleQRScan = async (qrData: string) => {
    ...
    // ── Try the new desk-QR format first ({t:"seat",...}) ──
        ...
        },
        {
          onSuccess: (result) => {
            if (result.status === "already") {
              toast(`${result.studentName} already marked Present`, { icon: "✅" });
            } else {
              toast.success(`${result.studentName} marked Present · ${result.seatLabel} · ${result.roomName}`);
              setScanLog(prev => [
            ...
            if (result.sessionId === selectedSession) {
              qc.invalidateQueries({ queryKey: ["exam-attendance", selectedSession, result.class, selectedSubject, selectedDate] });
              qc.invalidateQueries({ queryKey: ["exam-attendance-overview", selectedSession, result.class] });
            }
          },
          onError: (err: any) => {
            toast.error(err?.message || "Failed to mark attendance from seat QR");
          },
        }
      );
      return;
    ...
    // CROSS-CLASS SCAN SUPPORT (rev. 10):
    // The admit-card QR carries (sessionId, studentId, examRollNo) but NOT
    // the student's class. Previously the teacher had to pick the matching
    // class in the dropdown first — scanning a class-7 student while class 8
    // was selected would say "Student not found in this class".
    //
    // Now: we look up the student ACROSS ALL CLASSES in the session. The
    // saved attendance row uses the student's ACTUAL class (resolved by the
    // mutation hook from exam_roll_numbers), never the dropdown class — so
    // the teacher can scan 8th, 7th, and 6th students back-to-back without
    // touching the dropdown. Re-scans of the same student just update the
    // existing row (atomic upsert in the hook — no duplicates).
    const parsed = decodeExamQRData(qrData);
    if (!parsed) { toast.error("Invalid QR code — not an exam roll number or desk QR"); return; }
    // STALE-QR TOLERANT: Do NOT reject cross-session QRs. The student_id
    // might be valid in the current session even if the QR was printed for
    // a different session. We try the current session first, then fall
    // back to exam_roll_no lookup.
    const effectiveSessionId = selectedSession || parsed.sessionId;

    // First try the selected class (fast path — student is in the loaded list).
    let student = rollNumbers.find(r => r.student_id === parsed.studentId);
    let studentClass = selectedClass;

    if (!student) {
      // Slow path: student isn't in the selected class. Look up across
      // ALL classes in the session via exam_roll_numbers. This is a single
      // indexed query — fast even for big sessions.
      try {
    ...
    // This handles OLD admit cards where "Update Exam Roll Numbers"
    // reshuffled the table — the exam_roll_no in the QR might now belong
    // to a different student, and we mark the CURRENT holder present.
    ...
    // If the scanned student is in a DIFFERENT class than the dropdown,
    // warn the teacher but still proceed — the saved row goes to the
    // student's real class sheet (handled by the mutation hook).
    if (studentClass !== selectedClass) {
      toast(`Cross-class scan: student is in Class ${studentClass}, not Class ${selectedClass}. Saving to Class ${studentClass}.`, { icon: "ℹ️" });
    }
    doScan(parsed.studentId, student.student_name, parsed.examRollNo, studentClass);
  };
  ...
  const handleManualRoll = async () => {
    ...
    if (!roll) return;
    // CROSS-CLASS MANUAL ENTRY (rev. 10):
    // Look up the roll number across the SELECTED class first (fast path),
    // then fall back to a session-wide lookup so the teacher can type a
    // roll number from any class without switching the dropdown. The saved
    // attendance row uses the student's ACTUAL class (resolved inside the
    // mutation hook from exam_roll_numbers).
    ...
    if (!student) { toast.error(`Roll number ${roll} not found in this exam session`); return; }
    if (studentClass !== selectedClass) {
      toast(`Cross-class entry: student is in Class ${studentClass}, not Class ${selectedClass}. Saving to Class ${studentClass}.`, { icon: "ℹ️" });
    }
    doScan(student.student_id, student.student_name, roll, studentClass);
    setManualRoll("");
  };

  const doScan = (studentId: string, studentName: string, examRoll: string, studentClass?: string) => {
    const existing = attMap.get(studentId);
    if (existing?.status === "present") {
      toast(`${studentName} already marked Present`, { icon: "✅" });
      return;
    }
    // `cls` here is the SELECTED class (used by the hook to invalidate the
    // teacher's current view). The mutation hook resolves the student's
    // ACTUAL class from exam_roll_numbers and uses THAT for the saved row
    // + for invalidating the student's class sheet. So even if the teacher
    // picked class 8 but scanned a class-7 student, the row lands in class 7.
    scanAttendance.mutate({
      sessionId: selectedSession, studentId, subject: selectedSubject,
      examDate: selectedDate, cls: studentClass || selectedClass, scannedBy: teacherId,
      examRollNo: examRoll,
    }, {
      onSuccess: (result) => {
        if (result.status === "already") {
          ...
          // Show the student's ACTUAL class in the toast (result.class is
          // returned by the hook from rollEntry.class).
          const clsLabel = result.class ? ` · Class ${result.class}` : "";
          toast.success(`${studentName}${clsLabel} marked Present!`);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Scan QR codes or enter roll numbers — locked to today & the paper time window.
          </p>
        </div>
        {isInitialized && (
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          {/* Auto-fetched paper times + window banner */}
          {selectedSubject && (
            <>
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">
                    Paper Start {isInitialized && firstRecord?.paper_start_time ? "(stored)" : "(from seating plan)"}
                  </label>
                  <div className="w-full px-3 py-2.5 rounded-xl bg-secondary/30 border border-border text-sm text-foreground flex items-center gap-2">
                    ...
                    {loadingPaperTimes ? (
                      ...
                      <span className="text-red-500 italic text-xs">Not set</span>
                    )}
                  </div>
                </div>
                <div>
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">
                    Paper End {isInitialized && firstRecord?.paper_end_time ? "(stored)" : "(from seating plan)"}
                  </label>
                  <div className="w-full px-3 py-2.5 rounded-xl bg-secondary/30 border border-border text-sm text-foreground flex items-center gap-2">
                    <Clock className="w-3.5 h-3.5 text-muted-foreground" />
                    {loadingPaperTimes ? (
        ...
        <>
          {!isInitialized ? (
              ...
              <CardContent className="py-10 text-center space-y-3">
                ...
                {!seatingPaperTimes && (
                  <p className="text-[10px] text-muted-foreground">
                    Ask the admin to set paper start & end time in <strong>Exam Seating</strong> for this subject & date first.
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <>
              ...
              {/* Locked banner — scanning AND manual edits both locked outside the paper window */}
              {!canMark && (
                <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div className="text-xs">
                    <p className="font-semibold text-amber-700 dark:text-amber-400">Attendance Locked.</p>
                    <p className="text-amber-600 dark:text-amber-500 mt-0.5">
                      {paperWindowMessage(windowStatus, displayPaperStart, displayPaperEnd)}
                    </p>
                    <p className="text-amber-600/70 dark:text-amber-500/70 mt-1">
                      QR scanning and manual status changes are both locked. Ask the admin to extend the paper end-time from the Live Console to re-open editing.
                    </p>
                  </div>
                </div>
              )}
              ...
              {/* Scan toggle button */}
              <Button onClick={() => setShowScanner(!showScanner)} disabled={!canMark}
                className={`w-full gap-2 ${showScanner ? "bg-red-500 hover:bg-red-600" : "bg-emerald-500 hover:bg-emerald-600"} text-white disabled:opacity-50 disabled:cursor-not-allowed`} size="lg">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button onClick={handleManualRoll} disabled={!manualRoll.trim() || !canMark}
