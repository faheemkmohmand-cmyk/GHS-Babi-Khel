...
export default function WorldExplorer() {
  ...
  useEffect(() => {
    ...
    fetch(PROXY_URL)
      ...
      .catch(() => {
        // Fallback: try the old API directly (in case proxy is unavailable)
        const BASE = "https://restcountries.com/v3.1/all?fields=cca2,cca3,name,capital,population,area,region,subregion,languages,currencies,borders,flags,coatOfArms,latlng,timezones,continents,fifa,tld,independent,unMember";
        fetch(BASE)
          .then((r) => { if (!r.ok) throw new Error("API error"); return r.json(); })
          .then((data: Country[]) => {
            const sorted = [...data].sort((a, b) =>
