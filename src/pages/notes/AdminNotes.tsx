      ...
      {tab==="content" && (
        <Card>
          <CardContent className="pt-5 space-y-4">
            ...
            {/* Content editor — HTML paste only. Chapter content is always
                authored as raw HTML and rendered live below using the same
                renderer students see. */}
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1.5">
                ⚡ Paste ready-made HTML and it renders live below, using the same renderer students see.
              </p>
            </div>
          </CardContent>
        </Card>
...
const AdminNotes = () => {
  ...
  if (view==="subjects" && !editingSubject) return (
    <div className="space-y-5">
      ...
      {(() => {
        const slugCounts = subjects.reduce<Record<string, number>>((acc, s) => { acc[s.slug] = (acc[s.slug] || 0) + 1; return acc; }, {});
        const duplicateSlugs = Object.keys(slugCounts).filter(sl => slugCounts[sl] > 1);
        if (duplicateSlugs.length === 0) return null;
        const dupSubjects = subjects.filter(s => duplicateSlugs.includes(s.slug));
        return (
          <div className="rounded-xl border border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-800/40 px-4 py-3 text-sm text-red-800 dark:text-red-300">
            ⚠️ <strong>Duplicate subject{duplicateSlugs.length===1?"":"s"} detected</strong> — {dupSubjects.map(s=>s.name).join(", ")} share the same URL slug (<code>{duplicateSlugs.join(", ")}</code>). This is why the public site can show "0 chapters" even after publishing — the homepage only opens the first matching subject, so chapters attached to the other copy stay hidden. Open each one below, confirm which copy actually has your chapters, and delete the empty duplicate.
          </div>
        );
      })()}
