...
const MistakeReportsPanel = ({ chapterId, canView }: { chapterId: string; canView: boolean }) => {
  ...
  const load = useCallback(async () => {
    if (!canView) return;
    const { data } = await supabase.from("mistake_reports")
  ...
  return (
    <div className="mt-10" id="mistake-reports">
      <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
        ...
        <span className="text-sm font-normal text-muted-foreground">({pending.length} pending)</span>
      </h3>
...
const ChapterPage = () => {
  ...
  // data asynchronously after the chapter itself loads — so we can't scroll
  // immediately on mount. We poll for up to ~5 seconds waiting for the element
  // to appear, then scroll it into view (smooth, with sticky-navbar offset
  // via the `scroll-mt-24` Tailwind class on each card) and flash a yellow
  // highlight ring for ~3 seconds so the admin's eye lands on it.
  useEffect(() => {
    if (!location.hash) return;
    const id = location.hash.replace(/^#/, "");  // e.g. "question-<uuid>"
    if (!id) return;
    ...
    const tryScroll = () => {
      ...
      if (el) {
        ...
        // Add a temporary highlight ring. We use a class so it can be styled
        // globally if needed; the inline animation is self-contained here so
        ...
        clearInterval(timer);
        // Element never appeared (e.g. report was resolved and is hidden,
        // or question was deleted). Silently give up — admin is still on
        // the right chapter page and can scan manually.
      }
    };

    timer = setInterval(tryScroll, 100);
    // Also try once immediately in case the element is already in the DOM
    // (e.g. user navigates within the same chapter).
    tryScroll();
