        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-12">
            {filtered.map((subject, i) => {
              const textColor = getReadableTextColor(subject.color);
              const mutedTextColor = getReadableTextColorMuted(subject.color);
              const isDarkText = textColor === "#111827";
              const chipBg = isDarkText ? "rgba(17,24,39,0.10)" : "rgba(255,255,255,0.2)";
              const chipBgHover = isDarkText ? "rgba(17,24,39,0.16)" : "rgba(255,255,255,0.3)";
              return (
                ...
                <Link to={`/notes/${subject.slug}`}>
                  <div className="relative overflow-hidden rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 h-36"
                    ...
                    <div className="absolute inset-0 opacity-10"
                      style={{ backgroundImage: `radial-gradient(circle at 80% 20%, ${textColor} 1px, transparent 1px)`, backgroundSize: "20px 20px" }} />
                    ...
                    <div className="relative p-3.5 flex flex-col h-full justify-between">
                      <div>
                        ...
                        <h3 className="text-base font-black mt-1" style={{ color: textColor }}>{subject.name}</h3>
                        <p className="text-xs mt-0.5 line-clamp-1" style={{ color: mutedTextColor }}>{subject.description}</p>
                      </div>

                      <div className="flex items-center justify-between">
                        <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: chipBg, color: textColor }}>
                          Class {subject.class_level}
                        </span>
                        <div className="flex items-center gap-1 font-semibold text-xs px-2.5 py-1 rounded-lg transition-colors"
                          style={{ backgroundColor: chipBg, color: textColor }}
                          onMouseEnter={e => e.currentTarget.style.backgroundColor = chipBgHover}
                          onMouseLeave={e => e.currentTarget.style.backgroundColor = chipBg}>
