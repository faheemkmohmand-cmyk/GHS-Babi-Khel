...
// ── Comparison share text ───────────────────────────────────────────────
// Same style/format as biseShareText above: emoji-led lines, ending with
// the school link, ready to drop straight into WhatsApp.
...
// Shares a captured comparison-card DOM node to WhatsApp/the OS share
// sheet, exactly like shareResultCardNode above (file share → text-only
// share → download+clipboard fallback), so "beautiful comparison send to
// WhatsApp" behaves identically to the existing result-card Share button.
export async function shareComparisonNode(
  node: HTMLElement | null,
  data: ComparisonShareData,
): Promise<ShareOutcome> {
  const fileName = `Comparison-${safeName(data.rollA)}-vs-${safeName(data.rollB)}-GHS-Babi-Khel.png`;
  return shareResultCardNode(node, {
    fileName,
    title: "Result Comparison — GHS Babi Khel",
    text: comparisonShareText(data),
  });
}
...
//   • pixelRatio capped at 2 — plenty sharp for sharing/saving, keeps the
//     PNG small and the capture fast on low-end phones.
