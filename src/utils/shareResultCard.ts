...
interface Top3Slot {
  ...
  photo: HTMLImageElement | null;
  cx: number;
  baseY: number;      // y of the podium column top
...
function drawTop3Slot(ctx: Ctx, s: Top3Slot) {
  const { entry, rank, photo, cx, baseY, colH, avatarR } = s;
  const avatarCy = baseY - avatarR - 30;

  // crown above 1st place
  if (rank === 1) {
    setFont(ctx, 46, 800);
    ctx.textAlign = "center";
    ctx.fillText("👑", cx, avatarCy - avatarR - 24);
  }
  ...
  if (line2) {
    ...
    ctx.fillText(truncateText(ctx, line2, maxNameW), cx, nameY + (rank === 1 ? 32 : 28));
  } else {
    ctx.fillText(truncateText(ctx, line1, maxNameW), cx, nameY + (rank === 1 ? 16 : 14));
  }

  // class + grade pill
  const pillY = nameY + (line2 ? (rank === 1 ? 60 : 52) : (rank === 1 ? 40 : 36));
  setFont(ctx, 16, 700);
  ctx.fillStyle = C.sub;
  ...
  ctx.fillText(`${entry.percentage}%`, cx, pillY + (rank === 1 ? 44 : 38));

  setFont(ctx, 15, 600);
  ctx.fillStyle = C.label;
  ctx.fillText(`Roll ${entry.rollNo}`, cx, pillY + (rank === 1 ? 68 : 60));
...
function buildTop3Canvas(d: Top3ShareData, photos: (HTMLImageElement | null)[]): HTMLCanvasElement | null {
  const podiumTop = M + TOP3_HEADER_H + 190;
  cardHeightSoFar = TOP3_HEADER_H + 190 + TOP3_PODIUM_H + FOOTER_H;
  const H = cardHeightSoFar + 2 * M;
  const ctx = createCanvas(H);
  if (!ctx) return null;
  ...
  visualOrder.forEach((entryIdx, slot) => {
    ...
    const colH = colHeights[rank - 1] ?? 180;
    const avatarR = rank === 1 ? 92 : 74;
    drawTop3Slot(ctx, {
      entry,
      rank,
      photo: photos[entryIdx] ?? null,
      cx,
      baseY: podiumTop + TOP3_PODIUM_H - colH,
      colH,
      avatarR,
